...
# Rspscale according to that OS's conventions.
...
main() {
	...
	if [ -f /etc/os-release ]; then
		...
		case "$ID" in
			...
			kali)
				...
				;;
			Deepin)  # https://github.com/ropsoft7/rspscale/issues/7862
				OS="debian"
				PACKAGETYPE="apt"
			...
			postmarketos)
				...
				;;
			nixos)
				echo "Please add Rspscale to your NixOS configuration directly:"
				echo
				echo "services.rspscale.enable = true;"
				exit 1
				;;
			void)
				OS="$ID"
	...
	TEST_URL="https://pkgs.scale.ropsoft.cloud/"
	RC=0
	...
	case "$OS" in
		ubuntu|debian|raspbian|centos|oracle|rhel|amazon-linux|opensuse|photon)
			...
			URL="https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/installer-supported"
		...
		alpine)
			...
			# TODO: is that true? When was rspscale packaged?
			;;
		void)
			# Rolling release, no version checking needed.
			;;
	...
	if [ "$OS_UNSUPPORTED" = "1" ]; then
		case "$OS" in
			other-linux)
				...
				echo "https://pkgs.scale.ropsoft.cloud/$TRACK/#static"
				;;
			"")
		...
		echo
		echo "If you'd like us to support your system better, please email support@scale.ropsoft.cloud"
	...
	echo "Installing Rspscale for $OSVERSION, using method $PACKAGETYPE"
	case "$PACKAGETYPE" in
		apt)
			...
			case "$APT_KEY_TYPE" in
				legacy)
					$CURL "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION.asc" | $SUDO apt-key add -
					$CURL "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION.list" | $SUDO tee /etc/apt/sources.list.d/rspscale.list
				;;
				keyring)
					$CURL "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION.noarmor.gpg" | $SUDO tee /usr/share/keyrings/rspscale-archive-keyring.gpg >/dev/null
					$CURL "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION.rspscale-keyring.list" | $SUDO tee /etc/apt/sources.list.d/rspscale.list
				;;
			esac
			$SUDO apt-get update
			$SUDO apt-get install -y rspscale rspscale-archive-keyring
			if [ "$APT_SYSTEMCTL_START" = "true" ]; then
				$SUDO systemctl enable --now rspscaled
				$SUDO systemctl start rspscaled
			fi
			set +x
		;;
		yum)
			...
			$SUDO yum-config-manager -y --add-repo "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/rspscale.repo"
			$SUDO yum install rspscale -y
			$SUDO systemctl enable --now rspscaled
			set +x
		;;
		dnf)
			...
			if [ "$DNF_VERSION" = "3" ]; then
				...
				$SUDO dnf config-manager --add-repo "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/rspscale.repo"
			elif [ "$DNF_VERSION" = "5" ]; then
				# Already installed config-manager, above.
				$SUDO dnf config-manager addrepo --from-repofile="https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/rspscale.repo"
			else
				echo "unexpected: unknown dnf version $DNF_VERSION"
				exit 1
			fi
			$SUDO dnf install -y rspscale
			$SUDO systemctl enable --now rspscaled
			set +x
		;;
		tdnf)
			set -x
			curl -fsSL "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/rspscale.repo" > /etc/yum.repos.d/rspscale.repo
			$SUDO tdnf install -y rspscale
			$SUDO systemctl enable --now rspscaled
			set +x
		;;
		zypper)
			set -x
			$SUDO rpm --import "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/repo.gpg"
			$SUDO zypper --non-interactive ar -g -r "https://pkgs.scale.ropsoft.cloud/$TRACK/$OS/$VERSION/rspscale.repo"
			$SUDO zypper --non-interactive --gpg-auto-import-keys refresh
			$SUDO zypper --non-interactive install rspscale
			$SUDO systemctl enable --now rspscaled
			set +x
			;;
		pacman)
			set -x
			$SUDO pacman -S rspscale --noconfirm
			$SUDO systemctl enable --now rspscaled
			set +x
			;;
		pkg)
			set -x
			$SUDO pkg install rspscale
			$SUDO service rspscaled enable
			$SUDO service rspscaled start
			set +x
			;;
		apk)
			set -x
			if ! grep -Eq '^http.*/community$' /etc/apk/repositories; then
				if type setup-apkrepos >/dev/null; then
					...
					echo "installing rspscale requires the community repo to be enabled in /etc/apk/repositories"
					exit 1
				fi
			fi
			$SUDO apk add rspscale
			$SUDO rc-update add rspscale
			$SUDO rc-service rspscale start
			set +x
			;;
		xbps)
			set -x
			$SUDO xbps-install rspscale -y
			set +x
			;;
		emerge)
			set -x
			$SUDO emerge --ask=n net-vpn/rspscale
			set +x
			;;
		appstore)
			set -x
			open "https://apps.apple.com/us/app/rspscale/id1475387142"
			set +x
			;;
		*)
	...
	echo "Installation complete! Log in to start using Rspscale by running:"
	echo
	if [ -z "$SUDO" ]; then
		echo "rspscale up"
	else
		echo "$SUDO rspscale up"
	fi
}
