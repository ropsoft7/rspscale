...
const (
	...
	bucketName     = "rspscale-integration-vm-images"
)
...
func (h *Harness) testDistro(t *testing.T, d Distro, ipm ipMapping) {
	...
	t.Run("start-rspscale", func(t *testing.T) {
		...
		case "openrc":
			...
			// of service readiness. If this sleep is removed then rspscale will not be
			// ready once the `rspscale up` command is sent. This is not ideal, but I
			// am not really sure there is a good way around this without a delay of
			// some kind.
			batch = append(batch, &expect.BSnd{S: "rc-service rspscaled start && sleep 2\n"})
		case "systemd":
			batch = append(batch, &expect.BSnd{S: "systemctl start rspscaled.service\n"})
		}
	...
	t.Run("login", func(t *testing.T) {
		runTestCommands(t, timeout, cli, []expect.Batcher{
			&expect.BSnd{S: fmt.Sprintf("rspscale up --login-server=%s\n", loginServer)},
			&expect.BSnd{S: "echo Success.\n"},
			&expect.BExp{R: `Success.`},
		})
	})

	t.Run("rspscale status", func(t *testing.T) {
		dur := 100 * time.Millisecond
		var outp []byte
		var err error

		// NOTE(Xe): retry `rspscale status` a few times until it works. When rspscaled
		// starts with testcontrol sometimes there can be up to a few seconds where
		// rspscaled is in an unknown state on these virtual machines. This exponential
		// delay loop should delay long enough for rspscaled to be ready.
		for count := 0; count < 10; count++ {
			sess := getSession(t, cli)

			outp, err = sess.CombinedOutput("rspscale status")
			if err == nil {
				t.Logf("rspscale status: %s", outp)
	...
	for _, tt := range []struct {
		...
		t.Run(tt.ipProto+"-address", func(t *testing.T) {
			...
			ipBytes, err := sess.Output("rspscale ip -" + string(tt.ipProto[len(tt.ipProto)-1]))
			if err != nil {
	...
	t.Run("incoming-ssh-ipv4", func(t *testing.T) {
		...
		defer sess.Close()
		ipBytes, err := sess.Output("rspscale ip -4")
		if err != nil {
			t.Fatalf("can't run `rspscale ip -4`: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("can't negotiate connection over rspscale: %v", err)
		}
		defer sshConn.Close()
		...
		testIPBytes, err := sess.Output("rspscale ip -4")
		if err != nil {
	...
	t.Run("incoming-udp-ipv4", func(t *testing.T) {
		...
		ip, err := sess.Output("rspscale ip -4")
		if err != nil {
...
func runTestCommands(t *testing.T, timeout time.Duration, cli *ssh.Client, batch []expect.Batcher) {
	...
	if err != nil {
		...
		if terr != nil {
			t.Fatalf("can't dump rspscaled logs on failed test: %v", terr)
		}
		sess.Stdout = logger.FuncWriter(t.Logf)
		sess.Stderr = logger.FuncWriter(t.Logf)
		terr = sess.Run("journalctl -u rspscaled")
		if terr != nil {
			t.Fatalf("can't dump rspscaled logs on failed test: %v", terr)
		}
