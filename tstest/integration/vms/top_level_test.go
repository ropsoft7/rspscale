...
//
// We invoke rspscale netcheck because the networking check is done
// by rspscale rather than rspscaled, making it easier to configure
// the proxy.
//
...
func TestMITMProxy(t *testing.T) {
	...
	runTestCommands(t, 30*time.Second, cli, []expect.Batcher{
		&expect.BSnd{S: "SSL_CERT_FILE=/tmp/squid/myca-mitm.pem HTTPS_PROXY=http://127.0.0.1:3128 rspscale netcheck\n"},
