...
/*
   ...
   3. run `nixos-generators -f qcow -o $CACHE_DIR/rspscale/nixos/version -c generated-config.nix`
...
let
  # Define the package (derivation) for Rspscale based on the binaries we
  # just built for this test:
  testRspscale = pkgs.stdenv.mkDerivation {
    ...
    # We need to wrap rspscaled such that it has iptables in its $PATH.
    ...
    installPhase = with pkgs; ''
      ...
      # Install rspscale{,d}
      cp $src/rspscale $out/bin/rspscale
      cp $src/rspscaled $out/bin/rspscaled

      # Wrap rspscaled with the ip and iptables commands.
      wrapProgram $out/bin/rspscaled --prefix PATH : ${
        lib.makeBinPath [ iproute iptables ]
      }

      # Install systemd unit.
      cp $src/systemd/rspscaled.service .
      sed -i -e "s#/usr/sbin#$out/bin#" -e "/^EnvironmentFile/d" ./rspscaled.service
      install -D -m0444 -t $out/lib/systemd/system ./rspscaled.service
    '';
  };
in {
  ...
  # Rspscale settings:
  services.rspscale = {
    # We want Rspscale to start at boot.
    enable = true;

    # Use the Rspscale package we just assembled.
    package = testRspscale;
  };

  # Override TS_LOG_TARGET to our private logcatcher.
  systemd.services.rspscaled.environment."TS_LOG_TARGET" = "{{.LogTarget}}";
}`

func (h *Harness) copyUnit(t *testing.T) {
	t.Helper()

	data, err := os.ReadFile("../../../cmd/rspscaled/rspscaled.service")
	if err != nil {
		t.Fatal(err)
	}
	os.MkdirAll(filepath.Join(h.binaryDir, "systemd"), 0755)
	err = os.WriteFile(filepath.Join(h.binaryDir, "systemd", "rspscaled.service"), data, 0666)
	if err != nil {
		t.Fatal(err)
	}
}
