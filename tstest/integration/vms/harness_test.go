...
func newHarness(t *testing.T) *Harness {
	...
	h := &Harness{
		...
		cli:            integration.RspscaleBinary(t),
		daemon:         integration.RspscaledBinary(t),
		signer:         signer,
...
func (h *Harness) Rspscale(t *testing.T, args ...string) []byte {
	t.Helper()
...
// makeTestNode creates a userspace rspscaled running in netstack mode that
// enables us to make connections to and from the rspscale network being
// tested. This mutates the Harness to allow tests to dial into the rspscale
// network as well as control the tester's rspscaled.
func (h *Harness) makeTestNode(t *testing.T, controlURL string) {
	...
	err = cmd.Start()
	if err != nil {
		t.Fatalf("can't start rspscaled: %v", err)
	}
...
outer:
	for {
		select {
		case <-ctx.Done():
			t.Fatal("timed out waiting for rspscaled to come up")
			return
		case <-ticker.C:
	...
	h.testerV4 = bytes2Netaddr(h.Rspscale(t, "ip", "-4"))
}
