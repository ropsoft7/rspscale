...
import (
	...
	"scale.ropsoft.cloud/cmd/testwrapper/flakytest"
	"scale.ropsoft.cloud/ipn"
	"scale.ropsoft.cloud/ipn/ipnlocal"
	"scale.ropsoft.cloud/ipn/ipnstate"
	"scale.ropsoft.cloud/ipn/store"
	"scale.ropsoft.cloud/net/tsaddr"
	"scale.ropsoft.cloud/net/tstun"
	"scale.ropsoft.cloud/safesocket"
	"scale.ropsoft.cloud/syncs"
	"scale.ropsoft.cloud/tailcfg"
	"scale.ropsoft.cloud/tstest"
	"scale.ropsoft.cloud/tstest/integration/testcontrol"
...
var (
	verboseRspscaled = flag.Bool("verbose-rspscaled", false, "verbose rspscaled logging")
	verboseRspscale  = flag.Bool("verbose-rspscale", false, "verbose rspscale CLI logging")
)
...
// Tests that rspscaled starts up in TUN mode, and also without data races:
// https://github.com/ropsoft7/rspscale/issues/7894
...
func TestControlKnobs(t *testing.T) {
	...
	cmd := n1.Rspscale("debug", "control-knobs")
	cmd.Stdout = nil // in case --verbose-rspscale was set
	cmd.Stderr = nil // in case --verbose-rspscale was set
...
func TestStateSavedOnStart(t *testing.T) {
	...
	// cmd/rspscale implements "up" via LocalBackend.Start.
	n1.MustDown()

	// And change the hostname to something:
	if err := n1.Rspscale("up", "--login-server="+n1.env.controlURL(), "--hostname=foo").Run(); err != nil {
		t.Fatalf("up: %v", err)
	}
...
func TestOneNodeUpAuth(t *testing.T) {
	...
	cmd := n1.Rspscale("up", "--login-server="+env.controlURL())
...
func TestTwoNodes(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		cleanLog := func(n *testNode) []byte {
			b := n.rspscaledParser.allBuf.Bytes()
			b = rxNoDates.ReplaceAll(b, nil)
			return b
		}

		t.Logf("writing rspscaled logs to n1.log and n2.log")
	...
	if err := tstest.WaitFor(2*time.Second, func() error {
		...
		if len(st.RspscaleIPs) == 0 {
			return errors.New("no Rspscale IPs")
		}
...
func TestNodeAddressIPFields(t *testing.T) {
	tstest.Shard(t)
	flakytest.Mark(t, "https://github.com/ropsoft7/rspscale/issues/7008")
	tstest.Parallel(t)
	env := newTestEnv(t)
	n1 := newTestNode(t, env)
	d1 := n1.StartDaemon()
...
// Issue 2434: when "down" (WantRunning false), rspscaled shouldn't
...
// Issue 2137: make sure Windows rspscaled works with the CLI alone,
...
func TestClientSideJailing(t *testing.T) {
	...
	lc1 := &rspscale.LocalClient{
		Socket:        n1.sockFile,
		UseSocketOnly: true,
	}
	lc2 := &rspscale.LocalClient{
		Socket:        n2.sockFile,
		UseSocketOnly: true,
	}
	...
	testDial := func(t *testing.T, lc *rspscale.LocalClient, ip netip.Addr, port uint16, shouldFail bool) {
		t.Helper()
	...
	waitPeerIsJailed := func(t *testing.T, b *rspscale.IPNBusWatcher, jailed bool) {
		t.Helper()
		for {
			n, err := b.Next()
			if err != nil {
...
func TestNATPing(t *testing.T) {
	flakytest.Mark(t, "https://github.com/ropsoft7/rspscale/issues/12169")
	tstest.Shard(t)
	tstest.Parallel(t)
	for _, v6 := range []bool{false, true} {
		...
		for _, tc := range tests {
			t.Run(fmt.Sprintf("v6=%t/%v", v6, tc.name), func(t *testing.T) {
				...
				if got := n2AsN1Peer.RspscaleIPs[ipIdx]; got != tc.n1SeesN2IP {
					t.Fatalf("n1 sees n2 as %v; want %v", got, tc.n1SeesN2IP)
				}

				s2 := n2.MustStatus()
				n1AsN2Peer := s2.Peer[k1]
				if got := n1AsN2Peer.RspscaleIPs[ipIdx]; got != tc.n2SeesN1IP {
					t.Fatalf("n2 sees n1 as %v; want %v", got, tc.n2SeesN1IP)
				}

				if err := n1.Rspscale("ping", tc.n1SeesN2IP.String()).Run(); err != nil {
					t.Fatal(err)
				}

				if err := n1.Rspscale("ping", "-peerapi", tc.n1SeesN2IP.String()).Run(); err != nil {
					t.Fatal(err)
				}

				if err := n2.Rspscale("ping", tc.n2SeesN1IP.String()).Run(); err != nil {
					t.Fatal(err)
				}

				if err := n2.Rspscale("ping", "-peerapi", tc.n2SeesN1IP.String()).Run(); err != nil {
					t.Fatal(err)
				}
			})
		}
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(t *testing.T, n *testNode) {
				...
				if out, err := n.RspscaleForOutput("set", "--auto-update").CombinedOutput(); err != nil {
		...
		},
		{
			...
			run: func(t *testing.T, n *testNode) {
				...
				sendAndCheckDefault(t, n, false, true)
				// But can be changed explicitly by the user.
				if out, err := n.RspscaleForOutput("set", "--auto-update=false").CombinedOutput(); err != nil {
...
func TestDNSOverTCPIntervalResolver(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"tcp4",
			tsaddr.RspscaleServiceIP(),
		},
		{
			"tcp6",
			tsaddr.RspscaleServiceIPv6(),
		},
	}
	for _, c := range cases {
...
func TestNetstackTCPLoopback(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			network:  "tcp4",
			dialAddr: net.JoinHostPort(tsaddr.RspscaleServiceIPString, loopbackPortStr),
		},
		{
			lisAddr:  net.JoinHostPort("::1", loopbackPortStr),
			network:  "tcp6",
			dialAddr: net.JoinHostPort(tsaddr.RspscaleServiceIPv6String, loopbackPortStr),
		},
	}
...
func TestNetstackUDPLoopback(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			network:     "udp4",
			dialAddr:    &net.UDPAddr{IP: tsaddr.RspscaleServiceIP().AsSlice(), Port: loopbackPort},
		},
		{
			pingerLAddr: &net.UDPAddr{IP: ip6.AsSlice(), Port: loopbackPort + 1},
			pongerLAddr: &net.UDPAddr{IP: net.ParseIP("::1"), Port: loopbackPort},
			network:     "udp6",
			dialAddr:    &net.UDPAddr{IP: tsaddr.RspscaleServiceIPv6().AsSlice(), Port: loopbackPort},
		},
	}
...
func newTestEnv(t testing.TB, opts ...testEnvOpt) *testEnv {
	...
	e := &testEnv{
		t:                 t,
		cli:               RspscaleBinary(t),
		daemon:            RspscaledBinary(t),
		LogCatcher:        logc,
...
// testNode is a machine with a rspscale & rspscaled.
// Currently, the test is simplistic and user==node==machine.
// That may grow complexity later to test more.
type testNode struct {
	env              *testEnv
	rspscaledParser *nodeOutputParser

	dir        string // temp dir for sock & state
	configFile string // or empty for none
	sockFile   string
	stateFile  string
	upFlagGOOS string // if non-empty, sets TS_DEBUG_UP_FLAG_GOOS for cmd/rspscale CLI
...
func newTestNode(t *testing.T, env *testEnv) *testNode {
	dir := t.TempDir()
	sockFile := filepath.Join(dir, "rspscale.sock")
	if len(sockFile) >= 104 {
	...
	n := &testNode{
		...
		dir:       dir,
		sockFile:  sockFile,
		stateFile: filepath.Join(dir, "rspscale.state"),
	}
...
// AwaitResponding waits for n's rspscaled to be up enough to be
...
// addLogLineHook registers a hook f to be called on each rspscaled
// log line output.
...
// nodeOutputParser parses stderr of rspscaled processes, calling the
...
func (d *Daemon) MustCleanShutdown(t testing.TB) {
	...
	if err != nil {
		t.Fatalf("rspscaled Wait: %v", err)
	}
	if ps.ExitCode() != 0 {
		t.Errorf("rspscaled ExitCode = %d; want 0", ps.ExitCode())
	}
}

// StartDaemon starts the node's rspscaled, failing if it fails to start.
...
func (n *testNode) StartDaemonAsIPNGOOS(ipnGOOS string) *Daemon {
	...
	n.rspscaledParser = &nodeOutputParser{n: n}
	cmd.Stderr = n.rspscaledParser
	if *verboseRspscaled {
	...
	if err := cmd.Start(); err != nil {
		t.Fatalf("starting rspscaled: %v", err)
	}
...
func (n *testNode) MustDown() {
	t := n.env.t
	t.Logf("Running down ...")
	if err := n.Rspscale("down", "--accept-risk=all").Run(); err != nil {
		t.Fatalf("down: %v", err)
	}
}
...
func (n *testNode) MustLogOut() {
	...
	if err := n.Rspscale("logout").Run(); err != nil {
...
func (n *testNode) Ping(otherNode *testNode) error {
	...
	return n.Rspscale("ping", ip).Run()
}

// AwaitListening waits for the rspscaled to be serving local clients
...
func (n *testNode) RspscaleForOutput(arg ...string) *exec.Cmd {
	cmd := n.Rspscale(arg...)
	cmd.Stdout = nil
	cmd.Stderr = nil
	return cmd
...
// Rspscale returns a command that runs the rspscale CLI with the provided arguments.
// It does not start the process.
func (n *testNode) Rspscale(arg ...string) *exec.Cmd {
...
func (n *testNode) Status() (*ipnstate.Status, error) {
	cmd := n.Rspscale("status", "--json")
	cmd.Stdout = nil // in case --verbose-rspscale was set
	cmd.Stderr = nil // in case --verbose-rspscale was set
	out, err := cmd.CombinedOutput()
	if err != nil {
		return nil, fmt.Errorf("running rspscale status: %v, %s", err, out)
	}
	st := new(ipnstate.Status)
	if err := json.Unmarshal(out, st); err != nil {
		return nil, fmt.Errorf("decoding rspscale status JSON: %w", err)
	}
	return st, nil
}
...
// HTTP traffic tries to leave localhost from rspscaled. We don't
