...
import (
	...
	"scale.ropsoft.cloud/tstest/natlab/vnet"
)

var (
	logRspscaled = flag.Bool("log-rspscaled", false, "log rspscaled output")
...
func (nt *natTest) runTest(addNode ...addNodeFunc) pingRoute {
	...
	for i, node := range nodes {
		...
		for _, e := range node.Env() {
			fmt.Fprintf(&envBuf, " rspscaled.env=%s=%s", e.Key, e.Value)
		}
		...
		cmd := exec.Command("qemu-system-x86_64",
			...
			"-kernel", nt.kernel,
			"-append", "console=hvc0 root=PARTUUID=60c24cc1-f3f9-427a-8199-76baa2d60001/PARTNROFF=1 ro init=/gokrazy/init panic=10 oops=panic pci=off nousb tsc=unstable clocksource=hpet gokrazy.remote_syslog.target="+sysLogAddr+" rspscale-tta=1"+envStr,
	...
	for i, c := range clients {
		...
		eg.Go(func() error {
			...
			t.Logf("%v up with %v", node, sts[i].Self.RspscaleIPs)
			return nil
		})
	}
	...
	pingRes, err := ping(ctx, clients[0], sts[1].Self.RspscaleIPs[0])
	if err != nil {
...
func ping(ctx context.Context, c *vnet.NodeAgentClient, target netip.Addr) (*ipnstate.PingResult, error) {
	...
	for n < 10 {
		n++
		pr, err := c.PingWithOpts(ctx, target, tailcfg.PingDisco, rspscale.PingOpts{})
		if err != nil {
			if anyPong {
				return res, nil
			}
...
// (blackholed) IPv4.
//
// See https://github.com/ropsoft7/rspscale/issues/13346
func TestSingleDualBrokenIPv4(t *testing.T) {
	if !*knownBroken {
		t.Skip("skipping known-broken test; set --known-broken to run; see https://github.com/ropsoft7/rspscale/issues/13346")
	}
	nt := newNatTest(t)
...
// TestBPFDisco tests https://github.com/ropsoft7/rspscale/issues/3824 ...
