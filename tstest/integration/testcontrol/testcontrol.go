...
import (
	...
	"scale.ropsoft.cloud/control/controlhttp/controlhttpserver"
...
func (s *Server) getUser(nodeKey key.NodePublic) (*tailcfg.User, *tailcfg.Login) {
	...
	login := &tailcfg.Login{
		...
		ProfilePicURL: "https://scale.ropsoft.cloud/static/images/marketing/team-carney.jpg",
	}
	user := &tailcfg.User{
		ID:          id,
		LoginName:   loginName,
...
func (s *Server) serveRegister(w http.ResponseWriter, r *http.Request, mkey key.MachinePublic) {
	...
	v6Prefix := netip.PrefixFrom(tsaddr.Rspscale4To6(v4Prefix.Addr()), 128)
...
func keepClientEndpoint(ipp netip.AddrPort) bool {
	...
	if ip.Is6() && ip.IsLinkLocalUnicast() {
		...
		// rspscaled doesn't know how to use them yet
