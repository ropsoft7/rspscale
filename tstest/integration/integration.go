...
// Package integration contains Rspscale integration tests.
//
...
// CleanupBinaries cleans up any resources created by calls to BinaryDir, RspscaleBinary, or RspscaledBinary.
...
// BinaryDir returns a directory containing test rspscale and rspscaled binaries.
...
// RspscaleBinary returns the path to the test rspscale binary.
// If any test calls RspscaleBinary, there must be a TestMain function that calls
// CleanupBinaries after all tests are complete.
func RspscaleBinary(tb testing.TB) string {
	return filepath.Join(BinaryDir(tb), "rspscale"+exe())
}

// RspscaledBinary returns the path to the test rspscaled binary.
// If any test calls RspscaleBinary, there must be a TestMain function that calls
// CleanupBinaries after all tests are complete.
func RspscaledBinary(tb testing.TB) string {
	return filepath.Join(BinaryDir(tb), "rspscaled"+exe())
}
...
// buildTestBinaries builds rspscale and rspscaled.
...
func buildTestBinaries() (string, error) {
	...
	err = build(bindir, "scale.ropsoft.cloud/cmd/rspscaled", "scale.ropsoft.cloud/cmd/rspscale")
	if err != nil {
		os.RemoveAll(bindir)
		return "", err
	}
...
func findGo() (string, error) {
	...
	if os.Getuid() == 0 && os.Getenv("GITHUB_ACTIONS") == "true" {
		const sudoGithubGo = "/home/runner/.cache/rspscale-go/bin/go"
