...
// to test Rspscale working end-to-end in various network conditions.
//
// See https://github.com/ropsoft7/rspscale/issues/13038
package vnet
...
func (s *Server) PopulateDERPMapIPs() error {
	out, err := exec.Command("rspscale", "debug", "derp-map").Output()
	if err != nil {
		return fmt.Errorf("rspscale debug derp-map: %v", err)
	}
...
func (n *network) acceptTCP(r *tcp.ForwarderRequest) {
	...
	if n.s.derpIPs.Contains(destIP) {
		...
		targetDial = "controlplane.scale.ropsoft.cloud:" + strconv.Itoa(int(destPort))
	}
	if targetDial != "" {
...
// serveLogCatchConn serves a TCP connection to "log.rspscale.io", speaking the
...
func New(c *Config) (*Server, error) {
	...
	s := &Server{
		...
		control: &testcontrol.Server{
			DERPMap:         derpMap,
			ExplicitBaseURL: "http://control.rspscale",
		},
...
// as that's all that Rspscale uses.
