...
var (
	...
	fakeProxyControlplane = newVIP("controlplane.scale.ropsoft.cloud", 1)
	fakeTestAgent         = newVIP("test-driver.rspscale", 2)
	fakeControl           = newVIP("control.rspscale", 3)
	fakeDERP1             = newVIP("derp1.rspscale", "33.4.0.1") // 3340=DERP; 1=derp 1
	fakeDERP2             = newVIP("derp2.rspscale", "33.4.0.2") // 3340=DERP; 2=derp 2
	fakeLogCatcher        = newVIP("log.rspscale.io", 4)
	fakeSyslog            = newVIP("syslog.rspscale", 9)
)
