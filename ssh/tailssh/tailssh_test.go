...
import (
	...
	"scale.ropsoft.cloud/util/lineiter"
...
func (ts *localState) RspscaleVarRoot() string {
	return ""
}
...
func TestSSHAuthFlow(t *testing.T) {
	...
	acceptRule := newSSHRule(&tailcfg.SSHAction{
		Accept:  true,
		Message: "Welcome to Rspscale SSH!",
	})
	...
	tests := []struct {
		...
		},
		{
			...
			wantBanners: []string{"Welcome to Rspscale SSH!"},
		},
		{
			name: "reject",
			state: &localState{
		...
		},
		{
			...
			wantBanners: []string{"First", "url-here", "Welcome to Rspscale SSH!"},
		},
		{
			name: "check-reject",
			state: &localState{
...
func TestSSH(t *testing.T) {
	...
	sc.insecureSkipRspscaleAuth = true
	...
	t.Run("env", func(t *testing.T) {
		if cibuild.On() {
			t.Skip("Skipping for now; see https://github.com/ropsoft7/rspscale/issues/4051")
		}
