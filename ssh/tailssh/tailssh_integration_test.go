...
import (
	...
	glider "scale.ropsoft.cloud/tempfork/gliderlabs/ssh"
...
// - RSPSCALED_PATH environment variable points at rspscaled binary
// - User "testuser" exists
// - "testuser" is in groups "groupone" and "grouptwo"

func TestMain(m *testing.M) {
	// Create our log file.
	file, err := os.OpenFile("/tmp/rspscalessh.log", os.O_CREATE|os.O_WRONLY, 0666)
	if err != nil {
		log.Fatal(err)
	}
	file.Close()
	...
	// Tail our log file.
	cmd := exec.Command("tail", "-F", "/tmp/rspscalessh.log")
...
func TestSSHAgentForwarding(t *testing.T) {
	...
	// Run rspscale SSH server and connect to it
	username := "testuser"
	rspscaleAddr := testServer(t, username, false, false)
	tcl, err := ssh.Dial("tcp", rspscaleAddr, &ssh.ClientConfig{
...
func testServer(t *testing.T, username string, forceV1Behavior bool, allowSendEnv bool) string {
	srv := &server{
		...
		logf:           log.Printf,
		rspscaledPath: os.Getenv("RSPSCALED_PATH"),
		timeNow:        time.Now,
	}
...
func (tb *testBackend) RspscaleVarRoot() string {
	return ""
}
