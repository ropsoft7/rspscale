...
// This file contains the code for the incubator process.  Rspscaled
...
// `rspscaled be-child ssh` as the entrypoint.
//
// If ss.srv.rspscaledPath is empty, this method is equivalent to
// exec.CommandContext.
//
...
func (ss *sshSession) newIncubatorCommand(logf logger.Logf) (cmd *exec.Cmd, err error) {
	...
	if ss.conn.srv.rspscaledPath == "" {
		if isSFTP {
			// SFTP relies on the embedded Go-based SFTP server in rspscaled,
			// so without rspscaled, we can't serve SFTP.
			return nil, errors.New("no rspscaled found on path, can't serve SFTP")
		}
	...
	case isSFTP:
		...
		// rspscaled as `be-child sftp`. If login or su is available, and
		...
		incubatorArgs = append(incubatorArgs, "--sftp", fmt.Sprintf("--cmd=%s be-child sftp", ss.conn.srv.rspscaledPath))
	case isShell:
	...
	return exec.CommandContext(ss.ctx, ss.conn.srv.rspscaledPath, incubatorArgs...), nil
}
...
func parseIncubatorArgs(args []string) (incubatorArgs, error) {
	...
	flags.StringVar(&ia.remoteIP, "remote-ip", "", "the remote Rspscale IP")
...
// beIncubator is the entrypoint to the `rspscaled be-child ssh` subcommand.
...
//
// Rspscaled launches the incubator as the same user as it was launched as.
func beIncubator(args []string) error {
	...
	if debugIncubator {
		...
		if sl, err := syslog.New(syslog.LOG_INFO|syslog.LOG_DAEMON, "rspscaled-ssh"); err == nil {
			dlogf = log.New(sl, "", 0).Printf
		}
	} else if ia.debugTest {
		// In testing, we don't always have syslog, so log to a temp file.
		if logFile, err := os.OpenFile("/tmp/rspscalessh.log", os.O_APPEND|os.O_WRONLY, 0666); err == nil {
...
// See http://github.com/ropsoft7/rspscale/issues/4908.
...
func handleSSHInProcess(dlogf logger.Logf, ia incubatorArgs) error {
	...
	if ee, ok := err.(*exec.ExitError); ok {
		...
		if code < 0 {
			...
			fmt.Fprintf(os.Stderr, "[rspscale-ssh: process died: %v]\n", ps.String())
...
func (ia *incubatorArgs) loginArgs(loginCmdPath string) []string {
	...
	case "linux":
		if distro.Get() == distro.Arch && !fileExists("/etc/pam.d/remote") {
			// See https://github.com/ropsoft7/rspscale/issues/4924
			//
