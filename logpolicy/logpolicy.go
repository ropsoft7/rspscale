...
import (
	...
	"scale.ropsoft.cloud/util/racebuild"
...
func LogsDir(logf logger.Logf) string {
	...
	switch runtime.GOOS {
	case "windows":
		if version.CmdName() == "rspscaled" {
			// In the common case, when rspscaled is run as the Local System (as a service),
			// we want to use %ProgramData% (C:\ProgramData\Rspscale), aside the
			// system state config with the machine key, etc. But if that directory's
			// not accessible, then it's probably because the user is running rspscaled
			...
			dir := filepath.Join(os.Getenv("ProgramData"), "Rspscale")
			if winProgramDataAccessible(dir) {
				logf("logpolicy: using dir %v", dir)
				return dir
			}
		}
		dir := filepath.Join(os.Getenv("LocalAppData"), "Rspscale")
	...
	// Default to e.g. /var/lib/rspscale or /var/db/rspscale on Unix.
	if d := paths.DefaultRspscaledStateFile(); d != "" {
		d = filepath.Dir(d) // directory of e.g. "/var/lib/ropsoft7/rspscaled.state"
	...
	if err == nil {
		d := filepath.Join(cacheDir, "Rspscale")
	...
	// it's something.
	tmp, err := os.MkdirTemp("", "rspscaled-log-*")
	if err != nil {
...
func winProgramDataAccessible(dir string) bool {
	...
	// The C:\ProgramData\Rspscale directory should be locked down
...
// https://github.com/ropsoft7/rspscale/issues/247 . We accidentally
...
func tryFixLogStateLocation(dir, cmdname string, logf logger.Logf) {
	...
	if cmdname == "" {
		logf("[unexpected] no cmdname given to tryFixLogStateLocation, please file a bug at https://github.com/ropsoft7/rspscale")
		return
	}
	if dir == "/" {
		// Trying to store things in / still. That's a bug, but don't
		// abort hard.
		logf("[unexpected] storing logging config in /, please file a bug at https://github.com/ropsoft7/rspscale")
		return
	}
	if os.Getuid() != 0 {
	...
	// (aka /var/cache/rspscale). We want to move files into the
	...
	if cacheDir != "" {
		existsInCache, err = checkExists("/var/cache/rspscale")
		if err != nil {
...
type Options struct {
	...
	// For example, logs for a node use "tailnode.log.rspscale.io".
	Collection string
...
func (opts Options) New() *Policy {
	if hostinfo.IsNATLabGuestVM() {
		// In NATLab Gokrazy instances, rspscaled comes up concurently with
	...
	if runtime.GOOS == "windows" {
		switch opts.CmdName {
		case "rspscaled":
			// Rspscale 1.14 and before stored state under %LocalAppData%
			// (usually "C:\WINDOWS\system32\config\systemprofile\AppData\Local"
			// when rspscaled.exe is running as a non-user system service).
			...
			//
			// The Windows service previously ran as rspscale-ipn.exe, so
			// machines which ran very old versions might still have their
			// log conf named %LocalAppData%\rspscale-ipn.log.conf
			//
			// Machines which started using Rspscale more recently will have
			// %LocalAppData%\rspscaled.log.conf
			//
			// Attempt to migrate the log conf to C:\ProgramData\Rspscale
			oldDir := filepath.Join(os.Getenv("LocalAppData"), "Rspscale")

			oldPath := filepath.Join(oldDir, "rspscaled.log.conf")
			if fi, err := os.Stat(oldPath); err != nil || !fi.Mode().IsRegular() {
				// *Only* if rspscaled.log.conf does not exist,
				// check for rspscale-ipn.log.conf
				oldPathOldCmd := filepath.Join(oldDir, "rspscale-ipn.log.conf")
	...
	if envknob.NoLogsNoSupport() || testenv.InTest() {
		opts.Logf("You have disabled logging. Rspscale will not be able to provide support.")
		...
		if conf.HTTPC == nil {
			...
			if val := getLogTarget(); val != "" {
				opts.Logf("You have enabled a non-default log target. Doing without being told to by Rspscale staff or your network administrator will make getting support difficult.")
				conf.BaseURL = val
				u, _ := url.Parse(val)
				logHost = u.Host
			}
	...
	if runtime.GOOS == "windows" && conf.Collection == logtail.CollectionNode {
		...
		exe, _ := os.Executable()
		if strings.EqualFold(filepath.Base(exe), "rspscaled.exe") {
			diskLogf := filelogger.New("rspscale-service", logID, lw.Logf)
...
func attachFilchBuffer(conf *logtail.Config, dir, cmdName string, logf logger.Logf) {
	...
	// https://github.com/ropsoft7/rspscale/issues/3551
	if runtime.GOOS == "linux" && (distro.Get() == distro.Synology || distro.Get() == distro.QNAP) {
		tmpfsLogs := "/tmp/rspscale-logs"
...
//   - If DNS lookup fails, consults the bootstrap DNS list of Rspscale hostnames.
...
// Rspscaled is manipulating the routing table.
...
func dialContext(ctx context.Context, netw, addr string, netMon *netmon.Monitor, logf logger.Logf) (net.Conn, error) {
	...
	if version.IsWindowsGUI() && strings.HasPrefix(netw, "tcp") {
		if c, err := safesocket.ConnectContext(ctx, ""); err == nil {
			...
			if err != nil {
				logf("logtail: CONNECT response error from rspscaled: %v", err)
				c.Close()
			} else {
				dialLog.Printf("connected via rspscaled")
				return c, nil
			}
		}
	}
...
func (opts TransportOptions) New() http.RoundTripper {
	...
	// Force TLS 1.3 since we know log.rspscale.io supports it.
