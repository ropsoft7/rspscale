...
import (
	...
	rspscaleroot "scale.ropsoft.cloud"
...
var (
	...
	// gitCommitStamp is the git commit of the github.com/ropsoft7/rspscale
	// repository at which Rspscale was built. Its format is the one returned
	...
	// imports scale.ropsoft.cloud, the Go tool doesn't stamp any dirtiness info into
	...
	// Rspscale was built. Its format is the same as gitCommit.
	//
	// extraGitCommit is used to track the source revision when the main
	// Rspscale repository is integrated into and built from another repository
	// (for example, Rspscale's proprietary code, or the Android OSS
...
//
//   - "x.y.z-commithash-otherhash" for release builds distributed by Rspscale
...
//     builds by Rspscale (these are not distributed).
...
func Long() string {
	return long.Get(func() string {
		...
		bi := getEmbeddedInfo()
		if !bi.valid {
			return strings.TrimSpace(rspscaleroot.VersionDotTxt) + "-ERR-BuildInfo"
		}
		return fmt.Sprintf("%s-dev%s-t%s%s", strings.TrimSpace(rspscaleroot.VersionDotTxt), bi.commitDate, bi.commitAbbrev(), dirtyString())
	})
}
...
//
//   - "x.y.z" for builds distributed by Rspscale or built with build_dist.sh
...
func Short() string {
	return short.Get(func() string {
		...
		return strings.TrimSpace(rspscaleroot.VersionDotTxt) + "-dev" + bi.commitDate
	})
}
