...
type VersionInfo struct {
	...
	// for possible values.
	Long string
	// GitHash is the git hash of the scale.ropsoft.cloud Go module.
	GitHash string
	// OtherHash is the git hash of a supplemental git repository, if any. For
	// example, the commit of the rspscale-android repository.
	OtherHash string
	// Xcode is the version string that gets embedded into Xcode builds for the
	// Rspscale iOS app and macOS standalone (aka "macsys") app.
	//
	...
	// and very early builds of Rspscale used a single incrementing integer,
	...
	// the Rspscale macOS app store app.
	//
	...
	// Rspscale numeric version that gets embedded in Synology spk
	// files.
...
func InfoFrom(dir string) (VersionInfo, error) {
	...
	if modPath == "scale.ropsoft.cloud" {
		// Invoked in the scale.ropsoft.cloud repo directly, just no further info to
		// collect.
	...
	// We seem to be in a repo that imports scale.ropsoft.cloud. Find the
	// scale.ropsoft.cloud repo and collect additional info from it.
	...
	rspscaleRef, err := rspscaleModuleRef(modBs)
	if err != nil {
		return VersionInfo{}, err
	}

	v, err := infoFromCache(rspscaleRef, runner)
	if err != nil {
		return VersionInfo{}, err
	}
...
// rspscaleModuleRef returns the git ref of the 'require scale.ropsoft.cloud' line
// in the given go.mod bytes. The ref is either a short commit hash, or a git
// tag.
func rspscaleModuleRef(modBs []byte) (string, error) {
	mod, err := modfile.Parse("go.mod", modBs, nil)
	if err != nil {
		return "", err
	}
	for _, req := range mod.Require {
		if req.Mod.Path != "scale.ropsoft.cloud" {
			continue
		}
	...
	return "", fmt.Errorf("no require scale.ropsoft.cloud line in go.mod")
}
...
func mkOutput(v verInfo) (VersionInfo, error) {
	...
	ret := VersionInfo{
		...
		Synology: map[int]int64{
			...
			// So, we do the following mapping from our Rspscale version to Synology version,
	...
	if v.otherHash != "" {
		...
		// these version numbers only make sense when building from Rspscale's
...
func makeMSIProductCodes(v verInfo, track string) map[string]string {
	urlBase := fmt.Sprintf("https://pkgs.scale.ropsoft.cloud/%s/rspscale-setup-%d.%d.%d-", track, v.major, v.minor, v.patch)
...
// unknownPatchVersion is the patch version used when the scale.ropsoft.cloud package
...
func infoFromCache(ref string, runner dirRunner) (verInfo, error) {
	rspscaleCache := os.Getenv("TS_MKVERSION_OSS_GIT_CACHE")
	if rspscaleCache == "" {
		cacheDir, err := os.UserCacheDir()
		if err != nil {
			return verInfo{}, fmt.Errorf("Getting user cache dir: %w", err)
		}
		rspscaleCache = filepath.Join(cacheDir, "rspscale-oss")
	}
	r := dirRunner(rspscaleCache)

	if _, err := os.Stat(rspscaleCache); err != nil {
		if !runner.ok("git", "clone", "https://github.com/ropsoft7/rspscale", rspscaleCache) {
			return verInfo{}, fmt.Errorf("cloning scale.ropsoft.cloud repo failed")
		}
	}
