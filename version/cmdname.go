...
func cmdName(exe string) string {
	...
	// v is like:
	// "path\tscale.ropsoft.cloud/cmd/rspscale\nmod\tscale.ropsoft.cloud\t(devel)\t\ndep\tgithub.com/apenwarr/fixconsole\tv0.0.0-20191012055117-5a9f6489cc29\th1:muXWUcay7DDy1/hEQWrYlBy+g0EuwT70sBHg65SeUc4=\ndep\tgithub....
	for _, line := range strings.Split(info, "\n") {
		if goPkg, ok := strings.CutPrefix(line, "path\t"); ok { // like "scale.ropsoft.cloud/cmd/rspscale"
			ret = path.Base(goPkg) // goPkg is always forward slashes; use path, not filepath
			break
		}
	}
	if strings.HasPrefix(ret, "wg") && fallbackName == "rspscale-ipn" {
		// The rspscale-ipn.exe binary for internal build system packaging reasons
		// has a path of "rspscale.io/win/wg64", "rspscale.io/win/wg32", etc.
		// Ignore that name and use "rspscale-ipn" instead.
		return fallbackName
	}
	if ret == "" {
		return fallbackName
...
func findModuleInfo(file string) (s string, err error) {
	...
	length := end - start
	// As of Aug 2021, rspscaled's mod info was about 2k.
