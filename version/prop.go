...
// two GUI variants (that is, not rspscaled-on-macOS).
...
// Rspscale for macOS, either the main GUI process (non-sandboxed) or the
...
// that ships with the Standalone variant of Rspscale for macOS.
func IsMacSysApp() bool {
	...
	return isMacSysApp.Get(func() bool {
		...
		return strings.HasSuffix(exe, "/Contents/MacOS/Rspscale") && !IsMacAppStore()
	})
}
...
// the standalone "System Extension" (a.k.a. "macsys") version of Rspscale
// for macOS.
func IsMacSysExt() bool {
	...
	return isMacSysExt.Get(func() bool {
		if strings.Contains(os.Getenv("HOME"), "/Containers/io.rspscale.ipn.macsys/") ||
			strings.Contains(os.Getenv("XPC_SERVICE_NAME"), "io.rspscale.ipn.macsys") {
			return true
		}
		exe, err := os.Executable()
		if err != nil {
		...
		return filepath.Base(exe) == "io.rspscale.ipn.macsys.network-extension"
	})
}

var isMacAppStore lazy.SyncValue[bool]

// IsMacAppStore whether this binary is from the App Store version of Rspscale
// for macOS.
func IsMacAppStore() bool {
	...
	return isMacAppStore.Get(func() bool {
		...
		// suffix /Contents/MacOS/Rspscale. Check $HOME to filter out running
		// as macsys.
		return strings.Contains(os.Getenv("HOME"), "/Containers/io.rspscale.ipn.macos/") ||
			strings.Contains(os.Getenv("XPC_SERVICE_NAME"), "io.rspscale.ipn.macos")
	})
}

var isAppleTV lazy.SyncValue[bool]

// IsAppleTV reports whether this binary is part of the Rspscale network extension for tvOS.
...
func IsAppleTV() bool {
	...
	return isAppleTV.Get(func() bool {
		return strings.EqualFold(os.Getenv("XPC_SERVICE_NAME"), "io.rspscale.ipn.ios.network-extension-tvos")
	})
}
...
func IsWindowsGUI() bool {
	...
	return isWindowsGUI.Get(func() bool {
		...
		return strings.EqualFold(exe, "rspscale-ipn.exe") || strings.EqualFold(exe, "rspscale-ipn")
	})
}
...
type Meta struct {
	...
	// github.com/ropsoft7/rspscale repository at which Rspscale was
	...
	// repository at which Rspscale was built. Its format is the same as
	// gitCommit.
	//
	// ExtraGitCommit is used to track the source revision when the main
	// Rspscale repository is integrated into and built from another
	// repository (for example, Rspscale's proprietary code, or the
	...
	// DaemonLong is the version number from the rspscaled
	// daemon, if requested.
	...
	// Cap is the current Rspscale capability version. It's a monotonically
