...
// version.
//
// Version comparison in Rspscale is a little complex, because we
...
// minimum
//
// version and minimum can both be either an official Rspscale
// version numbers (major.minor.patch-extracommits-extrastring), or an
// OSS build datestamp (date.YYYYMMDD). For Rspscale version numbers,
...
func AtLeast(version string, minimum string) bool {
	...
	case v.Datestamp != 0 && m.Datestamp == 0:
		// OSS version vs. Rspscale version
		return false
	case v.Datestamp == 0 && m.Datestamp != 0:
		// Rspscale version vs. OSS version
		return false
	case v.Datestamp != 0:
	...
	case v.Major == m.Major && v.Minor == m.Minor && v.Patch == m.Patch && v.ExtraCommits == m.ExtraCommits:
		// Exactly equal Rspscale versions
		return true
...
type parsed struct {
	Major, Minor, Patch, ExtraCommits int // for Rspscale version e.g. e.g. "0.99.1-20"
