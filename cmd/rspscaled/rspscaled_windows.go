...
//
// TODO: check if Rspscale service is already running, and fail early
//       like tswin does.
//
...
// one that you see in the Windows Event Viewer). rspscaled may optionally
// generate diagnostic messages in the same event timeline as the Windows
// Service Control Manager to assist with diagnosing issues with rspscaled's
// lifetime (such as slow shutdowns).
var syslogf logger.Logf = logger.Discard

// runWindowsService starts running Rspscale under the Windows
// Service environment.
//
	...
	for {
		...
		case cmd := <-r:
			...
			case cmdUninstallWinTun:
				syslogf("Stopping rspscaled child process and uninstalling WinTun")
				// At this point, doneCh is the channel which will be closed when the
				// rspscaled subprocess exits. We save that to childDoneCh.
				childDoneCh := doneCh
...
func beFirewallKillswitch() bool {
	...
	log.SetFlags(0)
	log.Printf("killswitch subprocess starting, rspscale GUID is %s", os.Args[2])
	...
	// Note(maisem): when local lan access toggled, rspscaled needs to
