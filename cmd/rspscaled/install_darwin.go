...
// /Library/LaunchDaemons/com.rspscale.rspscaled.plist or (in the
...
<dict>
  ...
  <key>Label</key>
  <string>com.rspscale.rspscaled</string>

  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/rspscaled</string>
  </array>
...
const sysPlist = "/Library/LaunchDaemons/com.rspscale.rspscaled.plist"
const targetBin = "/usr/local/bin/rspscaled"
const service = "com.rspscale.rspscaled"
...
func uninstallSystemDaemonDarwin(args []string) (ret error) {
	...
	if running {
		out, err := exec.Command("launchctl", "stop", "com.rspscale.rspscaled").CombinedOutput()
		if err != nil {
			fmt.Printf("launchctl stop com.rspscale.rspscaled: %v, %s\n", err, out)
			ret = err
		}
...
func installSystemDaemonDarwin(args []string) (err error) {
	...
	defer func() {
		if err != nil && os.Getuid() != 0 {
			err = fmt.Errorf("%w; try running rspscaled with sudo", err)
		}
	}()
	...
	// /usr/local/bin/rspscaled is a symlink to the actual binary.
	if !same {
