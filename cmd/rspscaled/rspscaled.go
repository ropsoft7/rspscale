...
// The rspscaled program is the Rspscale client daemon. It's configured
// and controlled via the rspscale CLI program.
//
...
import (
	...
	"scale.ropsoft.cloud/cmd/rspscaled/childproc"
	"scale.ropsoft.cloud/control/controlclient"
	"scale.ropsoft.cloud/drive/driveimpl"
	...
	"scale.ropsoft.cloud/ipn/ipnserver"
	"scale.ropsoft.cloud/ipn/store"
	"scale.ropsoft.cloud/logpolicy"
	"scale.ropsoft.cloud/logtail"
	...
	"scale.ropsoft.cloud/net/proxymux"
	"scale.ropsoft.cloud/net/socks5"
	...
	"scale.ropsoft.cloud/tsweb/varz"
	"scale.ropsoft.cloud/types/flagtype"
	...
	"scale.ropsoft.cloud/wgengine/netstack"
...
func defaultTunName() string {
	...
	case "linux":
		...
		case distro.Synology:
			...
			// See https://github.com/ropsoft7/rspscale-synology/issues/35
			return "rspscale0,userspace-networking"
		}
...
var args struct {
	// tunname is a /dev/net/tun tunnel name ("rspscale0"), the
...
func main() {
	...
	flag.StringVar(&args.statepath, "state", "", "absolute path of state file; use 'kube:<secret-name>' to use Kubernetes secrets or 'arn:aws:ssm:...' to store in AWS SSM; use 'mem:' to not store state and register as an ephemeral node. If empty and --statedir is provided, the default is <statedir>/rspscaled.state. Default: "+paths.DefaultRspscaledStateFile())
	flag.StringVar(&args.statedir, "statedir", "", "path to directory for storage of config state, TLS certs, temporary incoming Taildrop files, etc. If empty, it's derived from --state when possible.")
	flag.StringVar(&args.socketpath, "socket", paths.DefaultRspscaledSocket(), "path of the service unix socket")
	flag.StringVar(&args.birdSocketPath, "bird-socket", "", "path of the bird unix socket")
	flag.BoolVar(&printVersion, "version", false, "print version information and exit")
	flag.BoolVar(&args.disableLogs, "no-logs-no-support", false, "disable log uploads; this also disables any technical support")
	flag.StringVar(&args.confFile, "config", "", "path to config file, or 'vm:user-data' to use the VM's user-data (EC2)")

	if len(os.Args) > 0 && filepath.Base(os.Args[0]) == "rspscale" && beCLI != nil {
		beCLI()
		return
	}
	...
	if flag.NArg() > 0 {
		...
		if runtime.GOOS != "windows" || (flag.Arg(0) != "/subproc" && flag.Arg(0) != "/firewall") {
			log.Fatalf("rspscaled does not take non-flag arguments: %q", flag.Args())
		}
	}
	...
	if runtime.GOOS == "darwin" && os.Getuid() != 0 && !strings.Contains(args.tunname, "userspace-networking") && !args.cleanUp {
		log.SetFlags(0)
		log.Fatalf("rspscaled requires root; use sudo rspscaled (or use --tun=userspace-networking)")
	}
	...
	if args.statepath == "" && args.statedir == "" {
		args.statepath = paths.DefaultRspscaledStateFile()
	}
...
func trySynologyMigration(p string) error {
	...
	const oldPath = "/var/packages/Rspscale/etc/rspscaled.state"
...
func statePathOrDefault() string {
	...
	if args.statedir != "" {
		return filepath.Join(args.statedir, "rspscaled.state")
	}
	return ""
}

// serverOptions is the configuration of the Rspscale node agent.
type serverOptions struct {
	// VarRoot is the Rspscale daemon's private writable
	// directory (usually "/var/lib/rspscale" on Linux) that
	// contains the "rspscaled.state" file, the "certs" directory
...
func ipnServerOpts() (o serverOptions) {
	...
	if o.VarRoot == "" && filepath.IsAbs(args.statepath) {
		if dir := filepath.Dir(args.statepath); strings.EqualFold(filepath.Base(dir), "rspscale") {
			o.VarRoot = dir
		}
	}
...
func run() (err error) {
	...
	// such as when a system was rebooted without shutting down, or rspscaled
...
func startIPNServer(ctx context.Context, logf logger.Logf, logID logid.PublicID, sys *tsd.System) error {
	...
	// rspscaled. The default action is to terminate the process, we
	...
	go func() {
		...
		for {
			select {
			case s := <-interrupt:
				logf("rspscaled got signal %v; shutting down", s)
				cancel()
				return
			case <-wgEngineClosed:
...
func getLocalBackend(ctx context.Context, logf logger.Logf, logID logid.PublicID, sys *tsd.System) (_ *ipnlocal.LocalBackend, retErr error) {
	...
	if root := lb.RspscaleVarRoot(); root != "" {
		dnsfallback.SetCachePath(filepath.Join(root, "derpmap.cached.json"), logf)
	}
	lb.ConfigureWebClient(&rspscale.LocalClient{
		Socket:        args.socketpath,
		UseSocketOnly: args.socketpath != paths.DefaultRspscaledSocket(),
	})
...
func handleSubnetsInNetstack() bool {
	...
	case "windows", "darwin", "freebsd", "openbsd":
		// Enable on Windows and rspscaled-on-macOS (this doesn't
...
func tryEngine(logf logger.Logf, sys *tsd.System, name string) (onlyNetstack bool, err error) {
	...
	if onlyNetstack {
		if runtime.GOOS == "linux" && distro.Get() == distro.Synology {
			...
			// warnings in 'rspscale status' about DNS base
...
// protocol. On UNIX and MacOS rspscaled environment, Taildrive spawns child
// rspscaled processes in serve-taildrive mode in order to access the fliesystem
...
// macOS. This helps us clean up straggler rspscaled processes when the parent
...
// applyIntegrationTestEnvKnob applies the rspscaled.env=... environment
// variables specified on the Linux kernel command line, if the VM is being
// run in NATLab integration tests.
//
// They're specified as: rspscaled.env=FOO=bar rspscaled.env=BAR=baz
func applyIntegrationTestEnvKnob() {
	...
	for _, s := range strings.Fields(string(cmdLine)) {
		suf, ok := strings.CutPrefix(s, "rspscaled.env=")
		if !ok {
			continue
		}
