...
// formats the network traffic logs produced by "scale.ropsoft.cloud/wgengine/netlog"
// according to the schema in "scale.ropsoft.cloud/types/netlogtype.Message"
// in a more humanly readable format.
//
// Example usage:
//
//	$ cat netlog.json | go run scale.ropsoft.cloud/cmd/netlogfmt
//	=========================================================================================
//	NodeID: n123456CNTRL
...
var (
	resolveNames = flag.Bool("resolve-names", false, "convert rspscale IP addresses to hostnames; must also specify --api-key and --tailnet-id")
	apiKey       = flag.String("api-key", "", "API key to query the Rspscale API with; see https://login.scale.ropsoft.cloud/admin/settings/keys")
	tailnetName  = flag.String("tailnet-name", "", "tailnet domain name to lookup devices in; see https://login.scale.ropsoft.cloud/admin/settings/general")
)
...
func mustMakeNamesByAddr() map[netip.Addr]string {
	...
	// Query the Rspscale API for a list of devices in the tailnet.
	const apiURL = "https://api.scale.ropsoft.cloud/api/v2"
	...
	// Construct a unique mapping of Rspscale IP addresses to hostnames.
