...
func (i *jsIPN) run(jsCallbacks js.Value) {
	...
	i.lb.SetNotifyCallback(func(n ipn.Notify) {
		...
		// this bridging module (as opposed to actual bugs in Rspscale) and
			...
			jsNetMap := jsNetMap{
				...
				Peers: mapSlice(nm.Peers, func(p tailcfg.NodeView) jsNetMapPeerNode {
					...
					return jsNetMapPeerNode{
						...
						Online:              p.Online(),
						RspscaleSSHEnabled: p.Hostinfo().RspscaleSSHEnabled(),
					}
				}),
...
func (s *jsSSHSession) Run() {
	...
	config := &ssh.ClientConfig{
		HostKeyCallback: func(hostname string, remote net.Addr, key ssh.PublicKey) error {
			// Host keys are not used with Rspscale SSH, but we can use this
...
type jsNetMapPeerNode struct {
	...
	RspscaleSSHEnabled bool  `json:"rspscaleSSHEnabled"`
}
