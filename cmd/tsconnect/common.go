...
func commonSetup(dev bool) (*esbuild.BuildOptions, error) {
	...
	return &esbuild.BuildOptions{
		...
		Plugins: []esbuild.Plugin{
			{
				Name: "rspscale-tailwind",
				Setup: func(build esbuild.PluginBuild) {
					setupEsbuildTailwind(build, dev)
				},
			},
			{
				Name:  "rspscale-go-wasm-exec-js",
...
// setupEsbuildWasm generates an esbuild plugin that builds the Rspscale wasm
...
func buildWasm(dev bool) ([]byte, error) {
	...
	args := []string{"build", "-tags", "rspscale_go,osusergo,netgo,nethttpomithttp2,omitidna,omitpemdecrypt"}
	if !dev {
		if *devControl != "" {
