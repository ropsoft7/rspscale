    ...
    if (lockedOut) {
      lockedOutInstructions = (
        <div class="container mx-auto px-4 text-center space-y-4">
          <p>This instance of Rspscale Connect needs to be signed, due to
            {" "}<a href="https://scale.ropsoft.cloud/kb/1226/tailnet-lock/" class="link">tailnet lock</a>{" "}
            being enabled on this domain.
          </p>

          <p>
            Run the following command on a device with a trusted tailnet lock key:
            <pre>rspscale lock sign {netMap.self.nodeKey}</pre>
          </p>
        </div>
      )
    }
