...
import (
	...
	"scale.ropsoft.cloud/util/codegen"
...
func genView(buf *bytes.Buffer, it *codegen.ImportTracker, typ *types.Named, thisPkg *types.Package) {
	...
	for i := range t.NumFields() {
		...
		case *types.Slice:
			...
			case "byte":
				args.FieldType = it.QualifiedName(fieldType)
				it.Import("scale.ropsoft.cloud/types/views")
...
func main() {
	...
	fmt.Fprintf(buf, "//go:generate go run scale.ropsoft.cloud/cmd/cloner  %s\n\n", strings.Join(flagArgs, " "))
	runCloner := false
	...
	out += ".go"
	if err := codegen.WritePackageFile("rspscale/cmd/viewer", pkg, out, it, buf); err != nil {
		log.Fatal(err)
	}
	if runCloner {
		// When a new package is added or when existing generated files have
		// been deleted, we might run into a case where scale.ropsoft.cloud/cmd/cloner
