...
//
// See https://github.com/ropsoft7/rspscale/issues/10263 for background.
package main
...
import (
	...
	"scale.ropsoft.cloud/types/lazy"
...
var (
	...
	flagUseLocalRspscaled = flag.Bool("use-local-rspscaled", false, "use local rspscaled instead of tsnet")
	flagFunnel             = flag.Bool("funnel", false, "use Rspscale Funnel to make tsidp available on the public internet")
...
func main() {
	...
	if *flagUseLocalRspscaled {
		...
		if !anySuccess {
			log.Fatalf("failed to listen on any of %v", st.RspscaleIPs)
		}

		// rspscaled needs to be setting an HTTP header for funneled requests
		// that older versions don't provide.
		// TODO(naman): is this the correct check?
		if *flagFunnel && !version.AtLeast(st.Version, "1.71.0") {
			log.Fatalf("Local rspscaled not new enough to support -funnel. Update Rspscale or use tsnet mode.")
		}
		cleanup, watcherChan, err = serveOnLocalRspscaled(ctx, lc, st, uint16(*flagPort), *flagFunnel)
		if err != nil {
			log.Fatalf("could not serve on local rspscaled: %v", err)
		}
		defer cleanup()
	} else {
		ts := &tsnet.Server{
	...
	srv := &idpServer{
		lc:          lc,
		funnel:      *flagFunnel,
		localTSMode: *flagUseLocalRspscaled,
	}
	if *flagPort != 443 {
...
// serveOnLocalRspscaled starts a serve session using an already-running
// rspscaled instead of starting a fresh tsnet server, making something
// listening on clientDNSName:dstPort accessible over serve/funnel.
func serveOnLocalRspscaled(ctx context.Context, lc *rspscale.LocalClient, st *ipnstate.Status, dstPort uint16, shouldFunnel bool) (cleanup func(), watcherChan chan error, err error) {
	// In order to support funneling out in local rspscaled mode, we need
...
type authRequest struct {
	...
	// tailnet, via Rspscale Funnel. It is mutually exclusive with rpNodeID
	// and localRP.
...
func (ar *authRequest) allowRelyingParty(r *http.Request, lc *rspscale.LocalClient) error {
	if ar.localRP {
...
func (s *idpServer) authorize(w http.ResponseWriter, r *http.Request) {
	...
	if s.localTSMode {
		// in local rspscaled mode, the local rspscaled is forwarding us
...
func (s *idpServer) newMux() *http.ServeMux {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/" {
			io.WriteString(w, "<html><body><h1>Rspscale OIDC IdP</h1>")
			return
		}
...
var (
	openIDSupportedClaims = views.SliceOf([]string{
		...
		// Rspscale claims, these correspond to fields in rspscaleClaims.
...
// isFunnelRequest checks if an HTTP request is coming over Rspscale Funnel.
func isFunnelRequest(r *http.Request) bool {
	// If we're funneling through the local rspscaled, it will set this HTTP
	// header.
	if r.Header.Get("Rspscale-Funnel-Request") != "" {
		return true
	}
