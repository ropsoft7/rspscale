...
var versionCmd = &ffcli.Command{
	Name:       "version",
	ShortUsage: "rspscale version [flags]",
	ShortHelp:  "Print Rspscale version",
	FlagSet: (func() *flag.FlagSet {
		fs := newFlagSet("version")
		fs.BoolVar(&versionArgs.daemon, "daemon", false, "also print local node's daemon version")
		fs.BoolVar(&versionArgs.json, "json", false, "output in JSON format")
		fs.BoolVar(&versionArgs.upstream, "upstream", false, "fetch and print the latest upstream release version from pkgs.scale.ropsoft.cloud")
		return fs
	})(),
	Exec: runVersion,
}
...
func runVersion(ctx context.Context, args []string) error {
	...
	if versionArgs.upstream {
		upstreamVer, err = clientupdate.LatestRspscaleVersion(clientupdate.CurrentTrack)
		if err != nil {
			return err
		}
	}
