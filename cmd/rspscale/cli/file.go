...
import (
	...
	tsrate "scale.ropsoft.cloud/tstime/rate"
	"scale.ropsoft.cloud/util/quarantine"
...
var fileCmd = &ffcli.Command{
	Name:       "file",
	ShortUsage: "rspscale file <cp|get> ...",
...
var fileCpCmd = &ffcli.Command{
	Name:       "cp",
	ShortUsage: "rspscale file cp <files...> <target>:",
...
func runCp(ctx context.Context, args []string) error {
	...
	if len(args) < 2 {
		return errors.New("usage: rspscale file cp <files...> <target>:")
	}
	files, target := args[:len(args)-1], args[len(args)-1]
	target, ok := strings.CutSuffix(target, ":")
	if !ok {
		return fmt.Errorf("final argument to 'rspscale file cp' must end in colon")
	}
	hadBrackets := false
	...
	ip, _, err := rspscaleIPFromArg(ctx, target)
	if err != nil {
		return err
	}
		...
		if fileArg == "-" {
			...
			if err != nil {
				if version.IsSandboxedMacOS() {
					return errors.New("the GUI version of Rspscale on macOS runs in a macOS sandbox that can't read files")
				}
				return err
			}
			defer f.Close()
...
func fileTargetErrorDetail(ctx context.Context, ip netip.Addr) error {
	...
	if st, err := localClient.Status(ctx); err == nil && st.Self != nil {
		for _, peer := range st.Peer {
			for _, pip := range peer.RspscaleIPs {
				if pip == ip {
					found = true
	...
	if found {
		return errors.New("target seems to be running an old Rspscale version")
	}
	if !tsaddr.IsRspscaleIP(ip) {
		return fmt.Errorf("unknown target; %v is not a Rspscale IP address", ip)
	}
...
var fileGetCmd = &ffcli.Command{
	Name:       "get",
	ShortUsage: "rspscale file get [--wait] [--verbose] [--conflict=(skip|overwrite|rename)] <target-directory>",
	ShortHelp:  "Move files out of the Rspscale file inbox",
	Exec:       runFileGet,
	FlagSet: (func() *flag.FlagSet {
		fs := newFlagSet("get")
...
func runFileGet(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: rspscale file get <target-directory>")
	}
	log.SetFlags(0)
	...
	if getArgs.loop {
		for {
			...
			if len(errs) > 0 {
				...
				//    rspscale file get --conflict=skip ~/Downloads
