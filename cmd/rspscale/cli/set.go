...
import (
	...
	"scale.ropsoft.cloud/cmd/rspscale/cli/ffcomplete"
...
var setCmd = &ffcli.Command{
	Name:       "set",
	ShortUsage: "rspscale set [flags]",
	ShortHelp:  "Change specified preferences",
	LongHelp: `"rspscale set" allows changing specific preferences.

Unlike "rspscale up", this command does not require the complete set of desired settings.
...
func newSetFlagSet(goos string, setArgs *setArgsT) *flag.FlagSet {
	...
	setf.BoolVar(&setArgs.acceptRoutes, "accept-routes", false, "accept routes advertised by other Rspscale nodes")
	setf.BoolVar(&setArgs.acceptDNS, "accept-dns", false, "accept DNS configuration from the admin panel")
	setf.StringVar(&setArgs.exitNodeIP, "exit-node", "", "Rspscale exit node (IP or base name) for internet traffic, or empty string to not use an exit node")
	...
	setf.BoolVar(&setArgs.updateCheck, "update-check", true, "notify about available Rspscale updates")
	setf.BoolVar(&setArgs.updateApply, "auto-update", false, "automatically update to the latest available version")
	setf.BoolVar(&setArgs.postureChecking, "posture-checking", false, hidden+"allow management plane to gather device posture information")
	setf.BoolVar(&setArgs.runWebClient, "webclient", false, "expose the web interface for managing this node over Rspscale at port 5252")
	...
	if safesocket.GOOSUsesPeerCreds(goos) {
		setf.StringVar(&setArgs.opUser, "operator", "", "Unix username to allow to operate on rspscaled without sudo")
	}
	switch goos {
	case "linux":
	...
	case "windows":
		setf.BoolVar(&setArgs.forceDaemon, "unattended", false, "run in \"Unattended Mode\" where Rspscale keeps running even after the current GUI user logs out (Windows-only)")
	}
...
func runSet(ctx context.Context, args []string) (retErr error) {
	...
	if maskedPrefs.AutoUpdateSet.ApplySet {
		...
		// On macsys, rspscaled will set the Sparkle auto-update setting. It
		...
		if version.IsMacSysExt() {
			...
			out, err := exec.Command("defaults", "write", "io.rspscale.ipn.macsys", "SUAutomaticallyUpdate", apply).CombinedOutput()
			if err != nil {
	...
	if setArgs.runWebClient && len(st.RspscaleIPs) > 0 {
		printf("\nWeb interface now running at %s:%d", st.RspscaleIPs[0], web.ListenPort)
	}
...
// current value, the flags passed to "rspscale set".
