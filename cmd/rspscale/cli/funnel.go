...
// Funnel allows you to publish a 'rspscale serve' server publicly, open to the
// entire internet.
...
func newFunnelCommand(e *serveEnv) *ffcli.Command {
	return &ffcli.Command{
		...
		ShortUsage: strings.Join([]string{
			"rspscale funnel <serve-port> {on|off}",
			"rspscale funnel status [--json]",
		}, "\n"),
		LongHelp: strings.Join([]string{
			"Funnel allows you to publish a 'rspscale serve'",
		...
		Subcommands: []*ffcli.Command{
			{
				...
				ShortUsage: "rspscale funnel status [--json]",
...
// runFunnel is the entry point for the "rspscale funnel" subcommand and
...
func (e *serveEnv) verifyFunnelEnabled(ctx context.Context, port uint16) error {
	...
	case enableErr != nil:
		...
		// the feature flag on.
		// TODO(sonia,rspscale/corp#10577): Remove this fallback once the
...
func printFunnelWarning(sc *ipn.ServeConfig) {
	...
	if warn {
		fmt.Fprintf(Stderr, "         run: `rspscale serve --help` to see how to configure handlers\n")
	}
}
