...
// Package cli contains the cmd/rspscale CLI code in a package that can be included
...
var localClient = rspscale.LocalClient{
	Socket: paths.DefaultRspscaledSocket(),
}
...
func Run(args []string) (err error) {
	if runtime.GOOS == "linux" && os.Getenv("GOKRAZY_FIRST_START") == "1" && distro.Get() == distro.Gokrazy && os.Getppid() == 1 {
		...
		// Don't run the rspscale CLI as a service; just exit.
	...
	rspscale.SetVersionMismatchHandler(func(clientVer, serverVer string) {
		warnOnce.Do(func() {
			fmt.Fprintf(Stderr, "Warning: client version %q != rspscaled server version %q\n", clientVer, serverVer)
		})
	})
	...
	if rspscale.IsAccessDeniedError(err) && os.Getuid() != 0 && runtime.GOOS != "windows" {
		return fmt.Errorf("%v\n\nUse 'sudo rspscale %s' or 'rspscale up --operator=$USER' to not require root.", err, strings.Join(args, " "))
	}
	if errors.Is(err, flag.ErrHelp) {
		return nil
	}
...
func newRootCmd() *ffcli.Command {
	rootfs := newFlagSet("rspscale")
	rootfs.Func("socket", "path to rspscaled socket", func(s string) error {
	...
	rootCmd := &ffcli.Command{
		Name:       "rspscale",
		ShortUsage: "rspscale [flags] <subcommand> [command flags]",
		ShortHelp:  "The easiest, most secure way to use WireGuard.",
		LongHelp: strings.TrimSpace(`
For help on subcommands, add --help after: "rspscale status --help".
...
`),
		...
		Exec: func(ctx context.Context, args []string) error {
			if len(args) > 0 {
				return fmt.Errorf("rspscale: unknown subcommand: %s", args[0])
			}
			return flag.ErrHelp
		},
	}
...
// fullCmd returns the full "rspscale ... cmd" invocation for a subcommand.
