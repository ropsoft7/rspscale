...
func runDNSStatus(ctx context.Context, args []string) error {
	...
	enabledStr := "disabled.\n\n(Run 'rspscale set --accept-dns=true' to start sending DNS queries to the Rspscale DNS resolver)"
	if prefs.CorpDNS {
		enabledStr = "enabled.\n\nRspscale is configured to handle DNS queries on this device.\nRun 'rspscale set --accept-dns=false' to revert to your system default DNS resolver."
	}
	fmt.Print("\n")
	fmt.Println("=== 'Use Rspscale DNS' status ===")
	fmt.Print("\n")
	fmt.Printf("Rspscale DNS: %s\n", enabledStr)
	fmt.Print("\n")
	...
	fmt.Print("\n")
	fmt.Println("This is the DNS configuration that Rspscale believes your operating system is using.\nRspscale may use this configuration if 'Override Local DNS' is disabled in the admin console,\nor if no resolvers are provided by the coordination server.")
	fmt.Print("\n")
...
func dnsStatusLongHelp() string {
	return `The 'rspscale dns status' subcommand prints the current DNS status and configuration, including:
	
- Whether the built-in DNS forwarder is enabled.
- The MagicDNS configuration provided by the coordination server.
- Details on which resolver(s) Rspscale believes the system is using by default.
