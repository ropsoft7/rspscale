...
var ipCmd = &ffcli.Command{
	Name:       "ip",
	ShortUsage: "rspscale ip [-1] [-4] [-6] [peer hostname or ip address]",
	ShortHelp:  "Show Rspscale IP addresses",
	LongHelp:   "Show Rspscale IP addresses for peer. Peer defaults to the current machine.",
	Exec:       runIP,
...
func runIP(ctx context.Context, args []string) error {
	...
	if nflags > 1 {
		return errors.New("rspscale ip -1, -4, and -6 are mutually exclusive")
	}
	if !v4 && !v6 {
		v4, v6 = true, true
	}
	...
	ips := st.RspscaleIPs
	if of != "" {
		ip, _, err := rspscaleIPFromArg(ctx, of)
		if err != nil {
			return err
		}
		...
		ips = peer.RspscaleIPs
	}
	if len(ips) == 0 {
		return fmt.Errorf("no current Rspscale IPs; state: %v", st.BackendState)
	}
	...
	if !match {
		if ipArgs.want4 {
			return errors.New("no Rspscale IPv4 address")
		}
		if ipArgs.want6 {
			return errors.New("no Rspscale IPv6 address")
		}
	}
	return nil
}
...
func peerMatchingIP(st *ipnstate.Status, ipStr string) (ps *ipnstate.PeerStatus, ok bool) {
	...
	for _, ps = range st.Peer {
		for _, pip := range ps.RspscaleIPs {
			if ip == pip {
				return ps, true
			}
		}
