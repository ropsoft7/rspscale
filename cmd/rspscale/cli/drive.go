...
const (
	driveShareUsage   = "rspscale drive share <name> <path>"
	driveRenameUsage  = "rspscale drive rename <oldname> <newname>"
	driveUnshareUsage = "rspscale drive unshare <name>"
	driveListUsage    = "rspscale drive list"
)
...
// runDriveShare is the entry point for the "rspscale drive share" command.
...
// runDriveUnshare is the entry point for the "rspscale drive unshare" command.
...
// runDriveRename is the entry point for the "rspscale drive rename" command.
...
// runDriveList is the entry point for the "rspscale drive list" command.
...
Each share is identified by a name and points to a directory at a specific path. For example, to share the path /Users/me/Documents under the name "docs", you would run:
  ...
  $ rspscale drive share docs /Users/me/Documents
...
All Rspscale shares have a globally unique path consisting of the tailnet, the machine name and the share name. For example, if the above share was created on the machine "mylaptop" on the tailnet "mydomain.com", the share's path would be:
...
If you want a share to be accessed as a different user, you can use sudo to accomplish this. For example, to create the aforementioned share as "theuser", you could run:
  ...
  $ sudo -u theuser rspscale drive share docs /Users/theuser/Documents`
