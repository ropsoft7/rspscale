...
func TestShortUsage(t *testing.T) {
	t.Setenv("RSPSCALE_USE_WIP_CODE", "1")
...
// cutWord("rspscale debug scale 123", "scale") returns (" 123", true).
...
func TestCutWord(t *testing.T) {
	tests := []struct {
		...
		{"rspscale debug", "debug", "", true},
		{"rspscale debug", "bug", "", false},
		{"rspscale debug", "tail", "", false},
		{"rspscale debug scaley scale 123", "scale", " 123", true},
	}
...
func TestCheckForAccidentalSettingReverts(t *testing.T) {
	tests := []struct {
		...
		{
			// Issue 1725: "rspscale up --authkey=..." (or other non-empty flags) works from
		...
		{
			// Test that a pre-1.8 version of Rspscale with bogus NoSNAT pref
		...
		},
		{
			...
			flags: []string{"--login-server=https://controlplane.scale.ropsoft.cloud"},
			curPrefs: &ipn.Prefs{
				ControlURL:          "https://login.scale.ropsoft.cloud",
				CorpDNS:             true,
				NetfilterMode:       preftype.NetfilterOn,
				NoStatefulFiltering: opt.NewBool(true),
			},
...
func TestPrefsFromUpArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			st: &ipnstate.Status{
				RspscaleIPs: []netip.Addr{netip.MustParseAddr("100.105.106.107")},
			},
...
func TestPrefFlagMapping(t *testing.T) {
	...
	for i := range prefType.NumField() {
		...
		case "OSVersion", "DeviceModel":
			...
			continue
		case "NotepadURLs":
			// TODO(bradfitz): https://github.com/ropsoft7/rspscale/issues/1830
			continue
		case "Egg":
			// Not applicable.
			continue
		case "RunWebClient":
			// TODO(rspscale/corp#14335): Currently behind a feature flag.
			continue
		case "NetfilterKind":
			...
			continue
		case "DriveShares":
			// Handled by the rspscale share subcommand, we don't want a CLI
			// flag for this.
			continue
		case "AdvertiseServices":
			// Handled by the rspscale advertise subcommand, we don't want a
			// CLI flag for this.
			continue
...
func TestUpdatePrefs(t *testing.T) {
	tests := []struct {
		...
		// sshOverRspscale specifies if the cmd being run over SSH over Rspscale.
		// It is used to test the --accept-risks flag.
		sshOverRspscale bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.sshOverRspscale {
				tstest.Replace(t, &getSSHClientEnvVar, func() string { return "100.100.100.100 1 1" })
			} else if isSSHOverRspscale() {
				// The test is being executed over a "real" rspscale SSH
				// session, but sshOverRspscale is unset. Make the test appear
				// as if it's not over rspscale SSH.
...
func TestUpWorthWarning(t *testing.T) {
	...
	if !upWorthyWarning(healthmsg.RspscaleSSHOnBut + "some problem") {
