...
func newServeLegacyCommand(e *serveEnv) *ffcli.Command {
	return &ffcli.Command{
		...
		ShortUsage: strings.Join([]string{
			"rspscale serve http:<port> <mount-point> <source> [off]",
			"rspscale serve https:<port> <mount-point> <source> [off]",
			"rspscale serve tcp:<port> tcp://localhost:<local-port> [off]",
			"rspscale serve tls-terminated-tcp:<port> tcp://localhost:<local-port> [off]",
			"rspscale serve status [--json]",
			"rspscale serve reset",
		}, "\n"),
		LongHelp: strings.TrimSpace(`
*** BETA; all of this is subject to change ***

The 'rspscale serve' set of commands allows you to serve
content and local servers from your Rspscale node to
your tailnet.

You can also choose to enable the Rspscale Funnel with:
'rspscale funnel on'. Funnel allows you to publish
a 'rspscale serve' server publicly, open to the entire
internet. See https://scale.ropsoft.cloud/funnel.

EXAMPLES
  - To proxy requests to a web server at 127.0.0.1:3000:
    $ rspscale serve https:443 / http://127.0.0.1:3000

    Or, using the default port (443):
    $ rspscale serve https / http://127.0.0.1:3000

  - To serve a single file or a directory of files:
    $ rspscale serve https / /home/alice/blog/index.html
    $ rspscale serve https /images/ /home/alice/blog/images

  - To serve simple static text:
    $ rspscale serve https:8080 / text:"Hello, world!"

  - To serve over HTTP (tailnet only):
    $ rspscale serve http:80 / http://127.0.0.1:3000

    Or, using the default port (80):
    $ rspscale serve http / http://127.0.0.1:3000

  - To forward incoming TCP connections on port 2222 to a local TCP server on
    port 22 (e.g. to run OpenSSH in parallel with Rspscale SSH):
    $ rspscale serve tcp:2222 tcp://localhost:22

  - To accept TCP TLS connections (terminated within rspscaled) proxied to a
    local plaintext server on port 80:
    $ rspscale serve tls-terminated-tcp:443 tcp://localhost:80
`),
		Exec: e.runServe,
...
var errHelp = errors.New("try `rspscale serve --help` for usage info")
...
// rspscale.LocalClient. It includes only the methods used by the
// serve command.
//
...
type localServeClient interface {
	...
	WatchIPNBus(ctx context.Context, mask ipn.NotifyWatchOpt) (*rspscale.IPNBusWatcher, error)
...
//
// Returns error if unable to reach rspscaled or if self node is nil.
//
...
func (e *serveEnv) getLocalClientStatusWithoutPeers(ctx context.Context) (*ipnstate.Status, error) {
	...
	if err != nil {
		return nil, fixRspscaledConnectError(err)
	}
...
//
// Examples:
// - rspscale serve http / http://localhost:3000
// - rspscale serve https / http://localhost:3000
// - rspscale serve https /images/ /var/www/images/
// - rspscale serve https:10000 /motd.txt text:"Hello, world!"
// - rspscale serve tcp:2222 tcp://localhost:22
// - rspscale serve tls-terminated-tcp:443 tcp://localhost:80
func (e *serveEnv) runServe(ctx context.Context, args []string) error {
	...
	if srcType == "https" && !turnOff {
		...
		//
		// TODO(sonia,rspscale/corp#10577): The interactive feature flow
...
// handleWebServe handles the "rspscale serve (http/https):..." subcommand. It
// configures the serve config to forward HTTPS connections to the given source.
//
// Examples:
//   - rspscale serve http / http://localhost:3000
//   - rspscale serve https / http://localhost:3000
//   - rspscale serve https:8443 /files/ /home/alice/shared-files/
//   - rspscale serve https:10000 /motd.txt text:"Hello, world!"
...
// handleTCPServe handles the "rspscale serve tls-terminated-tcp:..." subcommand.
// It configures the serve config to forward TCP connections to the
// given source.
//
// Examples:
//   - rspscale serve tcp:2222 tcp://localhost:22
//   - rspscale serve tls-terminated-tcp:8443 tcp://localhost:8080
...
func (e *serveEnv) enableFeatureInteractive(ctx context.Context, feature string, caps ...tailcfg.NodeCapability) (err error) {
	...
	if err != nil {
		...
		log.Fatalf("lost connection to rspscaled: %v", err)
