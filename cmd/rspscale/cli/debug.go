...
import (
	...
	"scale.ropsoft.cloud/control/controlhttp"
	"scale.ropsoft.cloud/hostinfo"
	"scale.ropsoft.cloud/internal/noiseconn"
	"scale.ropsoft.cloud/ipn"
	"scale.ropsoft.cloud/net/netmon"
	"scale.ropsoft.cloud/net/tsaddr"
	"scale.ropsoft.cloud/net/tshttpproxy"
...
var debugCmd = &ffcli.Command{
	Name:       "debug",
	Exec:       runDebug,
	ShortUsage: "rspscale debug <debug-flags | subcommand>",
	ShortHelp:  "Debug commands",
	LongHelp:   hidden + `"rspscale debug" contains misc debug facilities; it is not a stable interface.`,
	...
	Subcommands: []*ffcli.Command{
		{
			Name:       "derp-map",
			ShortUsage: "rspscale debug derp-map",
			Exec:       runDERPMap,
			ShortHelp:  "Print DERP map",
		},
		{
			Name:       "component-logs",
			ShortUsage: "rspscale debug component-logs [" + strings.Join(ipn.DebuggableComponents, "|") + "]",
		...
		},
		{
			...
			ShortUsage: "rspscale debug daemon-goroutines",
			Exec:       runDaemonGoroutines,
			ShortHelp:  "Print rspscaled's goroutines",
		},
		{
			Name:       "daemon-logs",
			ShortUsage: "rspscale debug daemon-logs",
			Exec:       runDaemonLogs,
			ShortHelp:  "Watch rspscaled's server logs",
			FlagSet: (func() *flag.FlagSet {
		...
		{
			Name:       "metrics",
			ShortUsage: "rspscale debug metrics",
			Exec:       runDaemonMetrics,
			ShortHelp:  "Print rspscaled's metrics",
			FlagSet: (func() *flag.FlagSet {
		...
		{
			Name:       "env",
			ShortUsage: "rspscale debug env",
			Exec:       runEnv,
			ShortHelp:  "Print cmd/rspscale environment",
		},
		{
			Name:       "stat",
			ShortUsage: "rspscale debug stat <files...>",
			Exec:       runStat,
			ShortHelp:  "Stat a file",
		},
		{
			Name:       "hostinfo",
			ShortUsage: "rspscale debug hostinfo",
			Exec:       runHostinfo,
			ShortHelp:  "Print hostinfo",
		},
		{
			Name:       "local-creds",
			ShortUsage: "rspscale debug local-creds",
			Exec:       runLocalCreds,
			ShortHelp:  "Print how to access Rspscale LocalAPI",
		},
		{
			Name:       "restun",
			ShortUsage: "rspscale debug restun",
			Exec:       localAPIAction("restun"),
			ShortHelp:  "Force a magicsock restun",
		},
		{
			Name:       "rebind",
			ShortUsage: "rspscale debug rebind",
			Exec:       localAPIAction("rebind"),
			ShortHelp:  "Force a magicsock rebind",
		},
		{
			Name:       "derp-set-on-demand",
			ShortUsage: "rspscale debug derp-set-on-demand",
			Exec:       localAPIAction("derp-set-homeless"),
			ShortHelp:  "Enable DERP on-demand mode (breaks reachability)",
		},
		{
			Name:       "derp-unset-on-demand",
			ShortUsage: "rspscale debug derp-unset-on-demand",
			Exec:       localAPIAction("derp-unset-homeless"),
			ShortHelp:  "Disable DERP on-demand mode",
		},
		{
			Name:       "break-tcp-conns",
			ShortUsage: "rspscale debug break-tcp-conns",
			Exec:       localAPIAction("break-tcp-conns"),
			ShortHelp:  "Break any open TCP connections from the daemon",
		},
		{
			Name:       "break-derp-conns",
			ShortUsage: "rspscale debug break-derp-conns",
			Exec:       localAPIAction("break-derp-conns"),
			ShortHelp:  "Break any open DERP connections from the daemon",
		},
		{
			Name:       "pick-new-derp",
			ShortUsage: "rspscale debug pick-new-derp",
		...
		},
		{
			...
			Name:       "reload-config",
			ShortUsage: "rspscale debug reload-config",
			Exec:       reloadConfig,
			ShortHelp:  "Reload config",
		},
		{
			Name:       "control-knobs",
			ShortUsage: "rspscale debug control-knobs",
			Exec:       debugControlKnobs,
			ShortHelp:  "See current control knobs",
		},
		{
			Name:       "prefs",
			ShortUsage: "rspscale debug prefs",
			Exec:       runPrefs,
		...
		{
			Name:       "watch-ipn",
			ShortUsage: "rspscale debug watch-ipn",
			Exec:       runWatchIPN,
		...
		{
			Name:       "netmap",
			ShortUsage: "rspscale debug netmap",
			Exec:       runNetmap,
		...
		{
			Name: "via",
			ShortUsage: "rspscale debug via <site-id> <v4-cidr>\n" +
				"rspscale debug via <v6-route>",
			Exec:      runVia,
			ShortHelp: "Convert between site-specific IPv4 CIDRs and IPv6 'via' routes",
		},
		{
			Name:       "ts2021",
			ShortUsage: "rspscale debug ts2021",
			Exec:       runTS2021,
			ShortHelp:  "Debug ts2021 protocol connectivity",
			FlagSet: (func() *flag.FlagSet {
				fs := newFlagSet("ts2021")
				fs.StringVar(&ts2021Args.host, "host", "controlplane.scale.ropsoft.cloud", "hostname of control plane")
		...
		{
			Name:       "set-expire",
			ShortUsage: "rspscale debug set-expire --in=1m",
			Exec:       runSetExpire,
		...
		{
			Name:       "dev-store-set",
			ShortUsage: "rspscale debug dev-store-set",
			Exec:       runDevStoreSet,
		...
		{
			Name:       "derp",
			ShortUsage: "rspscale debug derp",
			Exec:       runDebugDERP,
			ShortHelp:  "Test a DERP configuration",
		},
		{
			Name:       "capture",
			ShortUsage: "rspscale debug capture",
			Exec:       runCapture,
		...
		{
			Name:       "portmap",
			ShortUsage: "rspscale debug portmap",
			Exec:       debugPortmap,
		...
		},
		{
			...
			ShortUsage: "rspscale debug peer-endpoint-changes <hostname-or-IP>",
			Exec:       runPeerEndpointChanges,
			ShortHelp:  "Prints debug information about a peer's endpoint changes",
		},
		{
			Name:       "dial-types",
			ShortUsage: "rspscale debug dial-types <hostname-or-IP> <port>",
		...
		{
			Name:       "resolve",
			ShortUsage: "rspscale debug resolve <hostname>",
		...
		{
			Name:       "go-buildinfo",
			ShortUsage: "rspscale debug go-buildinfo",
...
func runDebug(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return fmt.Errorf("rspscale debug: unknown subcommand: %s", args[0])
	}
	var usedFlag bool
	...
	return errors.New("rspscale debug: subcommand or flag required")
}
...
func runLocalCreds(ctx context.Context, args []string) error {
	...
	printf("curl --unix-socket %s http://local-rspscaled.sock/localapi/v0/status\n", paths.DefaultRspscaledSocket())
	return nil
}
...
func runVia(ctx context.Context, args []string) error {
	...
	case 1:
		...
		if !tsaddr.RspscaleViaRange().Contains(ipp.Addr()) {
...
var ts2021Args struct {
	host    string // "controlplane.scale.ropsoft.cloud"
...
func runDebugComponentLogs(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: rspscale debug component-logs [" + strings.Join(ipn.DebuggableComponents, "|") + "]")
	}
	component := args[0]
...
func runDevStoreSet(ctx context.Context, args []string) error {
	if len(args) != 2 {
		return errors.New("usage: rspscale debug dev-store-set --danger <key> <value>")
	}
...
func runDebugDERP(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: rspscale debug derp <region>")
	}
...
func runSetExpire(ctx context.Context, args []string) error {
	if len(args) != 0 || setExpireArgs.in == 0 {
		return errors.New("usage: rspscale debug set-expire --in=<duration>")
	}
...
func debugPortmap(ctx context.Context, args []string) error {
	opts := &rspscale.DebugPortmapOpts{
...
func runPeerEndpointChanges(ctx context.Context, args []string) error {
	...
	if len(args) != 1 || args[0] == "" {
		return errors.New("usage: rspscale debug peer-endpoint-changes <hostname-or-IP>")
	}
	var ip string

	hostOrIP := args[0]
	ip, self, err := rspscaleIPFromArg(ctx, hostOrIP)
	if err != nil {
		return err
	}
	if self {
		printf("%v is local Rspscale IP\n", ip)
		return nil
	}
	...
	req, err := http.NewRequestWithContext(ctx, "GET", "http://local-rspscaled.sock/localapi/v0/debug-peer-endpoint-changes?ip="+ip, nil)
	if err != nil {
		return err
	}
...
func runDebugDialTypes(ctx context.Context, args []string) error {
	...
	if len(args) != 2 || args[0] == "" || args[1] == "" {
		return errors.New("usage: rspscale debug dial-types <hostname-or-IP> <port>")
	}
	...
	hostOrIP := args[0]
	ip, _, err := rspscaleIPFromArg(ctx, hostOrIP)
	if err != nil {
		return err
	}
	if ip != hostOrIP {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", "http://local-rspscaled.sock/localapi/v0/debug-dial-types?"+qparams.Encode(), nil)
	if err != nil {
		return err
	}
...
func runDebugResolve(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: rspscale debug resolve <hostname>")
	}
