...
import (
	...
	"scale.ropsoft.cloud/safesocket"
...
var upCmd = &ffcli.Command{
	Name:       "up",
	ShortUsage: "rspscale up [flags]",
	ShortHelp:  "Connect to Rspscale, logging in if needed",

	LongHelp: strings.TrimSpace(`
"rspscale up" connects this machine to your Rspscale network,
triggering authentication if necessary.

With no flags, "rspscale up" brings the network online without
changing any settings. (That is, it's the opposite of "rspscale
down").
...
func newUpFlagSet(goos string, upArgs *upArgsT, cmd string) *flag.FlagSet {
	...
	// When adding new flags, prefer to put them under "rspscale set" instead
	// of here. Setting preferences via "rspscale up" is deprecated.
	...
	upf.BoolVar(&upArgs.acceptRoutes, "accept-routes", acceptRouteDefault(goos), "accept routes advertised by other Rspscale nodes")
	upf.BoolVar(&upArgs.acceptDNS, "accept-dns", true, "accept DNS configuration from the admin panel")
	upf.Var(notFalseVar{}, "host-routes", hidden+"install host routes to other Rspscale nodes (must be true as of Rspscale 1.67+)")
	upf.StringVar(&upArgs.exitNodeIP, "exit-node", "", "Rspscale exit node (IP or base name) for internet traffic, or empty string to not use an exit node")
	...
	if safesocket.GOOSUsesPeerCreds(goos) {
		upf.StringVar(&upArgs.opUser, "operator", "", "Unix username to allow to operate on rspscaled without sudo")
	}
	switch goos {
	case "linux":
	...
	case "windows":
		upf.BoolVar(&upArgs.forceDaemon, "unattended", false, "run in \"Unattended Mode\" where Rspscale keeps running even after the current GUI user logs out (Windows-only)")
	}
	upf.DurationVar(&upArgs.timeout, "timeout", 0, "maximum amount of time to wait for rspscaled to enter a Running state; default (0s) blocks forever")
...
// upArgsT is the type of upArgs, the argument struct for `rspscale up`.
...
// Fields output when `rspscale up --json` is used. Two JSON blocks will be output.
//
// When "rspscale up" is run it first outputs a block with AuthURL and QR populated,
...
//
//	{
//	   "AuthURL": "https://login.scale.ropsoft.cloud/a/0123456789abcdef",
...
type upOutputJSON struct {
	AuthURL      string `json:",omitempty"` // Authentication URL of the form https://login.scale.ropsoft.cloud/a/0123456789
...
// outside interactions (e.g. no making Rspscale LocalAPI calls).
...
//
// It returns simpleUp if we're running a simple "rspscale up" to
...
func updatePrefs(prefs, curPrefs *ipn.Prefs, env upCheckEnv) (simpleUp bool, justEditMP *ipn.MaskedPrefs, err error) {
	...
	if env.upArgs.forceReauth && isSSHOverRspscale() {
		if err := presentRiskToUser(riskLoseSSH, `You are connected over Rspscale; this action will result in your SSH session disconnecting.`, env.upArgs.acceptedRisks); err != nil {
			return false, nil, err
		}
	}
...
func presentSSHToggleRisk(wantSSH, haveSSH bool, acceptedRisks string) error {
	if !isSSHOverRspscale() || wantSSH == haveSSH {
		return nil
	}
	if wantSSH {
		return presentRiskToUser(riskLoseSSH, `You are connected over Rspscale; this action will reroute SSH traffic to Rspscale SSH and will result in your session disconnecting.`, acceptedRisks)
	}
	return presentRiskToUser(riskLoseSSH, `You are connected using Rspscale SSH; this action will result in your session disconnecting.`, acceptedRisks)
}
...
func runUp(ctx context.Context, cmd string, args []string, upArgs upArgsT) (retErr error) {
	...
	if err != nil {
		return fixRspscaledConnectError(err)
	}
	...
	printAuthURL := func(url string) bool {
		if url == "" {
			// Probably unnecessary but we used to have a bug where rspscaled
	...
	if distro.Get() == distro.Synology {
		notSupported := "not supported on Synology; see https://github.com/ropsoft7/rspscale/issues/1995"
	...
	if cmd == "up" {
		// "rspscale up" should not be able to change the
		// profile name.
	...
	// Special case: bare "rspscale up" means to just start
	...
	case err := <-watchErr:
		...
		return err
	case <-timeoutCh:
		return errors.New(`timeout waiting for Rspscale service to enter a Running state; check health with "rspscale status"`)
	}
}

// upWorthWarning reports whether the health check message s is worth warning
// about during "rspscale up". Many of the health checks are noisy or confusing
...
func upWorthyWarning(s string) bool {
	return strings.Contains(s, healthmsg.RspscaleSSHOnBut) ||
...
const accidentalUpPrefix = "Error: changing settings via 'rspscale up' requires mentioning all\n" +
...
// people running "rspscale up" with a subset of the flags they
// originally ran it with.
//
// For example, in Rspscale 1.6 and prior, a user might've advertised
...
func checkForAccidentalSettingReverts(newPrefs, curPrefs *ipn.Prefs, env upCheckEnv) error {
	...
	if len(flagIsSet) == 0 {
		// A bare "rspscale up" is a special case to just
	...
	for flagName := range flagsCur {
		...
		if flagName == "stateful-filtering" && valCur == true && valNew == false && env.goos == "linux" {
			// See https://github.com/ropsoft7/rspscale/issues/12307
			// Stateful filtering was on by default in rspscale 1.66.0-1.66.3, then off in 1.66.4.
			// This broke Rspscale installations in containerized
			// environments that use the default containerboot
			// configuration that configures rspscale using
			// 'rspscale up' command, which requires that all
	...
	// Some previously provided flags are missing. This run of 'rspscale
	// up' will error out.
...
func exitNodeIP(p *ipn.Prefs, st *ipnstate.Status) (ip netip.Addr) {
	...
	for _, p := range st.Peer {
		if p.ID == id {
			if len(p.RspscaleIPs) > 0 {
				return p.RspscaleIPs[0]
			}
			break
		}
	}
...
// starts with "tskey-client-" (as Rspscale OAuth secrets do) parses it like
//
...
// false. The "baseURL" defaults to https://api.scale.ropsoft.cloud.
...
func resolveAuthKey(ctx context.Context, v, tags string) (string, error) {
	...
	baseURL := "https://api.scale.ropsoft.cloud"
	...
	tsClient.UserAgent = "rspscale-cli"
