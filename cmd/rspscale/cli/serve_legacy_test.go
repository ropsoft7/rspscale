...
func TestCleanMountPoint(t *testing.T) {
	tests := []struct {
		...
		{"/foo//", "", true},                // too many slashes
		{"", "", true},                      // empty
		{"https://scale.ropsoft.cloud", "", true}, // not a path
	}
...
func TestVerifyFunnelEnabled(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr:              "Funnel not available; HTTPS must be enabled. See https://scale.ropsoft.cloud/s/https.",
		},
		{
			name:                 "fallback-flow-missing-acl-rule",
			queryFeatureResponse: mockQueryFeatureResponse{resp: nil, err: errors.New("not-allowed")},
			caps:                 []tailcfg.NodeCapability{tailcfg.CapabilityHTTPS},
			wantErr:              `Funnel not available; "funnel" node attribute not set. See https://scale.ropsoft.cloud/s/no-funnel.`,
		},
		{
			name:                 "fallback-flow-enabled",
			queryFeatureResponse: mockQueryFeatureResponse{resp: nil, err: errors.New("not-allowed")},
			caps:                 []tailcfg.NodeCapability{tailcfg.CapabilityHTTPS, tailcfg.NodeAttrFunnel, "https://scale.ropsoft.cloud/cap/funnel-ports?ports=80,443,8080-8090"},
			wantErr:              "", // no error, success
		},
		{
...
// fakeLocalServeClient is a fake rspscale.LocalClient for tests.
...
func (lc *fakeLocalServeClient) WatchIPNBus(ctx context.Context, mask ipn.NotifyWatchOpt) (*rspscale.IPNBusWatcher, error) {
