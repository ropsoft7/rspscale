...
var netlockCmd = &ffcli.Command{
	Name:       "lock",
	ShortUsage: "rspscale lock <subcommand> [arguments...]",
...
func runNetworkLockNoSubcommand(ctx context.Context, args []string) error {
	...
	if len(args) > 0 {
		return fmt.Errorf("rspscale lock: unknown subcommand: %s", args[0])
	}
...
var nlInitCmd = &ffcli.Command{
	Name:       "init",
	ShortUsage: "rspscale lock init [--gen-disablement-for-support] --gen-disablements N <trusted-key>...",
	ShortHelp:  "Initialize tailnet lock",
	LongHelp: strings.TrimSpace(`

The 'rspscale lock init' command initializes tailnet lock for the
...
running 'rspscale lock' on that node, and copying the node's tailnet
lock key.

To disable tailnet lock, use the 'rspscale lock disable' command
...
will be generated and transmitted to Rspscale, which support can use to disable
...
`),
	...
	FlagSet: (func() *flag.FlagSet {
		...
		fs.BoolVar(&nlInitArgs.disablementForSupport, "gen-disablement-for-support", false, "generates and transmits a disablement secret for Rspscale support")
...
func runNetworkLockInit(ctx context.Context, args []string) error {
	...
	if !nlInitArgs.confirm {
		...
		if nlInitArgs.disablementForSupport {
			fmt.Println("A disablement secret will be generated and transmitted to Rspscale support.")
		}
	...
	if nlInitArgs.disablementForSupport {
		...
		fmt.Fprintln(&successMsg, "A disablement secret for Rspscale support has been generated and transmitted to Rspscale.")
	}
...
var nlStatusCmd = &ffcli.Command{
	Name:       "status",
	ShortUsage: "rspscale lock status",
...
func runNetworkLockStatus(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return fmt.Errorf("rspscale lock status: unexpected argument")
	}
	...
	if st.Enabled && len(st.FilteredPeers) > 0 {
		...
		for _, p := range st.FilteredPeers {
			...
			line.WriteString("\t")
			for i, addr := range p.RspscaleIPs {
				line.WriteString(addr.String())
				if i < len(p.RspscaleIPs)-1 {
					line.WriteString(",")
				}
			}
			line.WriteString("\t")
...
var nlAddCmd = &ffcli.Command{
	Name:       "add",
	ShortUsage: "rspscale lock add <public-key>...",
...
var nlRemoveCmd = &ffcli.Command{
	Name:       "remove",
	ShortUsage: "rspscale lock remove [--re-sign=false] <public-key>...",
...
var nlSignCmd = &ffcli.Command{
	Name:       "sign",
	ShortUsage: "rspscale lock sign <node-key> [<rotation-key>]\nrspscale lock sign <auth-key>",
...
func runNetworkLockSign(ctx context.Context, args []string) error {
	...
	if len(args) == 0 || len(args) > 2 {
		return errors.New("usage: rspscale lock sign <node-key> [<rotation-key>]")
	}
...
var nlDisableCmd = &ffcli.Command{
	Name:       "disable",
	ShortUsage: "rspscale lock disable <disablement-secret>",
	ShortHelp:  "Consumes a disablement secret to shut down tailnet lock for the tailnet",
	LongHelp: strings.TrimSpace(`

The 'rspscale lock disable' command uses the specified disablement
...
func runNetworkLockDisable(ctx context.Context, args []string) error {
	...
	if len(secrets) != 1 {
		return errors.New("usage: rspscale lock disable <disablement-secret>")
	}
...
var nlLocalDisableCmd = &ffcli.Command{
	Name:       "local-disable",
	ShortUsage: "rspscale lock local-disable",
	ShortHelp:  "Disables tailnet lock for this node only",
	LongHelp: strings.TrimSpace(`

The 'rspscale lock local-disable' command disables tailnet lock for only
the current node.
...
var nlDisablementKDFCmd = &ffcli.Command{
	...
	ShortUsage: "rspscale lock disablement-kdf <hex-encoded-disablement-secret>",
...
func runNetworkLockDisablementKDF(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: rspscale lock disablement-kdf <hex-encoded-disablement-secret>")
	}
...
var nlLogCmd = &ffcli.Command{
	Name:       "log",
	ShortUsage: "rspscale lock log [--limit N]",
...
func runTskeyWrapCmd(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: lock tskey-wrap <rspscale pre-auth key>")
	}
...
var nlRevokeKeysCmd = &ffcli.Command{
	Name:       "revoke-keys",
	ShortUsage: "rspscale lock revoke-keys <tailnet-lock-key>...\n  revoke-keys [--cosign] [--finish] <recovery-blob>",
...
Revocation is a multi-step process that requires several signing nodes to ` + "`--cosign`" + ` the revocation. Use ` + "`rspscale lock remove`" + ` instead if the key has not been compromised.

1. To start, run ` + "`rspscale revoke-keys <tlpub-keys>`" + ` with the tailnet lock keys to revoke.
