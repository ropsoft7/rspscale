...
var sshCmd = &ffcli.Command{
	Name:       "ssh",
	ShortUsage: "rspscale ssh [user@]<host> [args...]",
	ShortHelp:  "SSH to a Rspscale machine",
	LongHelp: strings.TrimSpace(`

The 'rspscale ssh' command is an optional wrapper around the system 'ssh'
command that's useful in some cases. Rspscale SSH does not require its use;
most users running the Rspscale SSH server will prefer to just use the normal
'ssh' command or their normal SSH client.

The 'rspscale ssh' wrapper adds a few things:
...
* It works in userspace-networking mode, by supplying a ProxyCommand to the
  system 'ssh' command that connects via a pipe through rspscaled.
* It automatically checks the destination server's SSH host key against the
  node's SSH host key as advertised via the Rspscale coordination server.
`),
	Exec: runSSH,
}
...
func runSSH(ctx context.Context, args []string) error {
	if runtime.GOOS == "darwin" && version.IsMacAppStore() && !envknob.UseWIPCode() {
		return errors.New("The 'rspscale ssh' subcommand is not available on macOS builds distributed through the App Store or TestFlight.\nInstall the Standalone variant of Rspscale (download it from https://pkgs.scale.ropsoft.cloud), or use the regular 'ssh' client instead.")
	}
	if len(args) == 0 {
		return errors.New("usage: rspscale ssh [user@]<host>")
	}
	...
	argv = append(argv,
		...
		"-o", "CanonicalizeHostname no", // https://github.com/ropsoft7/rspscale/issues/10348
	)
	...
	if runtime.GOOS != "darwin" {
		socketArg := ""
		if localClient.Socket != "" && localClient.Socket != paths.DefaultRspscaledSocket() {
	...
	// be compatible, but only a subset. The "rspscale ssh"
...
func writeKnownHosts(st *ipnstate.Status) (knownHostsFile string, err error) {
	...
	tsConfDir := filepath.Join(confDir, "rspscale")
...
// isSSHOverRspscale checks if the invocation is in a SSH session over Rspscale.
