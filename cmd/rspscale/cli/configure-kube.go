...
var configureKubeconfigCmd = &ffcli.Command{
	Name:       "kubeconfig",
	ShortHelp:  "[ALPHA] Connect to a Kubernetes cluster using a Rspscale Auth Proxy",
	ShortUsage: "rspscale configure kubeconfig <hostname-or-fqdn>",
	LongHelp: strings.TrimSpace(`
Run this command to configure kubectl to connect to a Kubernetes cluster over Rspscale.

The hostname argument should be set to the Rspscale hostname of the peer running as an auth proxy in the cluster.

See: https://scale.ropsoft.cloud/s/k8s-auth-proxy
`),
...
func runConfigureKubeconfig(ctx context.Context, args []string) error {
	...
	if st.BackendState != "Running" {
		return errors.New("Rspscale is not running")
	}
...
func updateKubeconfig(cfgYaml []byte, fqdn string) ([]byte, error) {
	...
	cfg["users"] = appendOrSetNamed(users, "rspscale-auth", map[string]any{
