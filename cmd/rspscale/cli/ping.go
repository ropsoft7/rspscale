...
var pingCmd = &ffcli.Command{
	Name:       "ping",
	ShortUsage: "rspscale ping <hostname-or-IP>",
	ShortHelp:  "Ping a host at the Rspscale layer, see how it routed",
	LongHelp: strings.TrimSpace(`

The 'rspscale ping' command pings a peer node from the Rspscale layer
and reports which route it took for each response. The first ping or
so will likely go over DERP (Rspscale's TCP relay protocol) while NAT
traversal finds a direct path through.

If 'rspscale ping' works but a normal ping does not, that means one
side's operating system firewall is blocking packets; 'rspscale ping'
does not inject packets into either side's TUN devices.

By default, 'rspscale ping' stops after 10 pings or once a direct
(non-DERP) path has been established, whichever comes first.

The provided hostname must resolve to or be a Rspscale IP
(e.g. 100.x.y.z) or a subnet IP advertised by a Rspscale
relay node.
...
func runPing(ctx context.Context, args []string) error {
	...
	if len(args) != 1 || args[0] == "" {
		return errors.New("usage: rspscale ping <hostname-or-IP>")
	}
	var ip string
	...
	for {
		...
		pr, err := localClient.PingWithOpts(ctx, netip.MustParseAddr(ip), pingType(), rspscale.PingOpts{Size: pingArgs.size})
		cancel()
		if err != nil {
...
func rspscaleIPFromArg(ctx context.Context, hostOrIP string) (ip string, self bool, err error) {
	...
	for _, ps := range st.Peer {
		if match(ps) {
			...
			return ps.RspscaleIPs[0].String(), false, nil
		}
	}
	if match(st.Self) && len(st.Self.RspscaleIPs) > 0 {
		return st.Self.RspscaleIPs[0].String(), true, nil
	}
