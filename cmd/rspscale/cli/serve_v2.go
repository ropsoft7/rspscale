...
EXAMPLES
  ...
  - Expose an HTTP server running at 127.0.0.1:3000 in the background:
    $ rspscale %[1]s --bg 3000

  - Expose an HTTPS server with invalid or self-signed certificates at https://localhost:8443
    $ rspscale %[1]s https+insecure://localhost:8443

For more examples and use cases visit our docs site https://scale.ropsoft.cloud/kb/1247/funnel-serve-use-cases
`)
...
var infoMap = map[serveMode]commandInfo{
	serve: {
		...
		LongHelp: strings.Join([]string{
			"Rspscale Serve enables you to share a local server securely within your tailnet.\n",
			"To share a local server on the internet, use `rspscale funnel`\n\n",
		}, "\n"),
	},
	funnel: {
		Name:      "funnel",
		...
		LongHelp: strings.Join([]string{
			"Funnel enables you to share a local server on the internet using Rspscale.\n",
			"To share only within your tailnet, use `rspscale serve`\n\n",
		}, "\n"),
	},
}
...
var errHelpFunc = func(m serveMode) error {
	return fmt.Errorf("try `rspscale %s --help` for usage info", infoMap[m].Name)
}
...
func newServeV2Command(e *serveEnv, subcmd serveMode) *ffcli.Command {
	...
	return &ffcli.Command{
		...
		ShortUsage: strings.Join([]string{
			fmt.Sprintf("rspscale %s <target>", info.Name),
			fmt.Sprintf("rspscale %s status [--json]", info.Name),
			fmt.Sprintf("rspscale %s reset", info.Name),
		}, "\n"),
		...
		Subcommands: []*ffcli.Command{
			{
				Name:       "status",
				ShortUsage: "rspscale " + info.Name + " status [--json]",
			...
			{
				Name:       "reset",
				ShortUsage: "rspscale " + info.Name + " reset",
...
func (e *serveEnv) validateArgs(subcmd serveMode, args []string) error {
	if translation, ok := isLegacyInvocation(subcmd, args); ok {
		...
		fmt.Fprint(e.stderr(), "\nPlease see https://scale.ropsoft.cloud/kb/1242/rspscale-serve for more information.\n")
...
// runServeCombined is the entry point for the "rspscale {serve,funnel}" commands.
func (e *serveEnv) runServeCombined(subcmd serveMode) execFunc {
	...
	return func(ctx context.Context, args []string) error {
		...
		var watcher *rspscale.IPNBusWatcher
		wantFg := !e.bg && !turnOff
		if wantFg {
		...
		if err := e.lc.SetServeConfig(ctx, parentSC); err != nil {
			if rspscale.IsPreconditionsFailedError(err) {
...
const backgroundExistsMsg = "background configuration already exists, use `rspscale %s --%s=%d off` to remove the existing configuration"
...
var (
	...
	msgDisableProxy        = "To disable the proxy, run: rspscale %s --%s=%d off"
...
func (e *serveEnv) applyWebServe(sc *ipn.ServeConfig, dnsName string, srvPort uint16, useTLS bool, mount, target string) error {
	...
	case filepath.IsAbs(target):
		if version.IsMacAppStore() || version.IsMacSys() {
			// The Rspscale network extension cannot serve arbitrary paths on macOS due to sandbox restrictions (2024-03-26)
			return errors.New("Path serving is not supported on macOS due to sandbox restrictions. To use Rspscale Serve on macOS, switch to the open-source rspscaled distribution. See https://scale.ropsoft.cloud/kb/1065/macos-variants for more information.")
		}
