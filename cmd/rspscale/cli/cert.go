...
var certCmd = &ffcli.Command{
	...
	ShortUsage: "rspscale cert [flags] <domain>",
...
func runCert(ctx context.Context, args []string) error {
	if certArgs.serve {
		s := &http.Server{
			...
			Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				fmt.Fprintf(w, "<h1>Hello from Rspscale</h1>It works.")
			}),
		}
		switch len(args) {
		case 0:
	...
	if len(args) != 1 {
		...
		return fmt.Errorf("Usage: rspscale cert [flags] <domain>%s", hint.Bytes())
	}
	domain := args[0]
	...
	macWarn := func() {
		...
		dir := "io.rspscale.ipn.macos"
		if version.IsMacSysExt() {
			dir = "io.rspscale.ipn.macsys"
		}
...
func convertToPKCS12(certPEM, keyPEM []byte) ([]byte, error) {
	...
	// work for https://github.com/ropsoft7/rspscale/issues/2928 but I'm still
	// fighting Windows.
