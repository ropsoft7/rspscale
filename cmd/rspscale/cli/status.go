...
var statusCmd = &ffcli.Command{
	Name:       "status",
	ShortUsage: "rspscale status [--active] [--web] [--json]",
	ShortHelp:  "Show state of rspscaled and its connections",
...
https://github.com/ropsoft7/rspscale/blob/main/ipn/ipnstate/ipnstate.go

(and be sure to select branch/tag that corresponds to the version
 of Rspscale you're running)
...
func runStatus(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return errors.New("unexpected non-flag arguments to 'rspscale status'")
	}
	...
	if statusArgs.web {
		...
		printf("Serving Rspscale status at %v ...\n", statusURL)
		go func() {
			<-ctx.Done()
			ln.Close()
		}()
	...
	if locBasedExitNode {
		outln()
		printf("# To see the full list of exit nodes, including location-based exit nodes, run `rspscale exit-node list`  \n")
	}
	if len(st.Health) > 0 {
		outln()
		printHealth()
...
func isRunningOrStarting(st *ipnstate.Status) (description string, ok bool) {
	...
	case ipn.Stopped.String():
		return "Rspscale is stopped.", false
