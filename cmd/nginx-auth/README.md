...
[![status: experimental](https://img.shields.io/badge/status-experimental-blue)](https://scale.ropsoft.cloud/kb/1167/release-stages/#experimental)

This is a tool that allows users to use Rspscale Whois authentication with
...
Rspscale IP of the NGINX server and then seamlessly use Rspscale for
authentication.
...
location /auth {
  ...
  proxy_pass http://unix:/run/rspscale.nginx-auth.sock;
...
```
auth_request /auth;
auth_request_set $auth_user $upstream_http_rspscale_user;
auth_request_set $auth_name $upstream_http_rspscale_name;
auth_request_set $auth_login $upstream_http_rspscale_login;
auth_request_set $auth_tailnet $upstream_http_rspscale_tailnet;
auth_request_set $auth_profile_picture $upstream_http_rspscale_profile_picture;
...
| :------                     | :--------------                                                    | :----------                                                                   |
| `Rspscale-User`            | `azurediamond@hunter2.net`                                         | The Rspscale username the remote machine is logged in as in user@host form   |
| `Rspscale-Login`           | `azurediamond`                                                     | The user portion of the Rspscale username the remote machine is logged in as |
| `Rspscale-Name`            | `Azure Diamond`                                                    | The "real name" of the Rspscale user the machine is logged in as             |
| `Rspscale-Profile-Picture` | `https://i.kym-cdn.com/photos/images/newsfeed/001/065/963/ae0.png` | The profile picture provided by the Identity Provider your tailnet uses       |
| `Rspscale-Tailnet`          | `hunter2.net`                                       | The tailnet name                                                              |

Most of the time you can set `X-Webauth-User` to the contents of the
`Rspscale-User` header, but some services may not accept a username with an `@`
symbol in it. If this is the case, set `X-Webauth-User` to the `Rspscale-Login`
header.

The `Rspscale-Tailnet` header can help you identify which tailnet the session
...
You can get the tailnet name from [the admin panel](https://login.scale.ropsoft.cloud/admin/dns).
