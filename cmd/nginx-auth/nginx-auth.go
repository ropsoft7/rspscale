...
// Command nginx-auth is a tool that allows users to use Rspscale Whois
...
// to point those domains to the Rspscale IP of the NGINX server and
// then seamlessly use Rspscale for authentication.
package main
...
func main() {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		info, err := rspscale.WhoIs(r.Context(), remoteAddr.String())
		if err != nil {
			w.WriteHeader(http.StatusUnauthorized)
		...
		if !info.Node.Hostinfo.ShareeNode() {
			...
			tailnet = strings.TrimSuffix(tailnet, ".beta.rspscale.net")
		}
		...
		h := w.Header()
		h.Set("Rspscale-Login", strings.Split(info.UserProfile.LoginName, "@")[0])
		h.Set("Rspscale-User", info.UserProfile.LoginName)
		h.Set("Rspscale-Name", info.UserProfile.DisplayName)
		h.Set("Rspscale-Profile-Picture", info.UserProfile.ProfilePicURL)
		h.Set("Rspscale-Tailnet", tailnet)
