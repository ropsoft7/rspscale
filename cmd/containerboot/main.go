...
// The containerboot binary is a wrapper for starting rspscaled in a container.
// It handles reading the desired mode of operation out of environment
// variables, bringing up and authenticating Rspscale, and any other
...
//     in --accept-routes.
//   - TS_DEST_IP: proxy all incoming Rspscale traffic to the given
//     destination defined by an IP address.
//   - TS_EXPERIMENTAL_DEST_DNS_NAME: proxy all incoming Rspscale traffic to the given
...
//   - TS_TAILNET_TARGET_IP: proxy all incoming non-Rspscale traffic to the given
//     destination defined by an IP.
//   - TS_TAILNET_TARGET_FQDN: proxy all incoming non-Rspscale traffic to the given
//     destination defined by a MagicDNS name.
//   - TS_RSPSCALED_EXTRA_ARGS: extra arguments to 'rspscaled'.
//   - TS_EXTRA_ARGS: extra arguments to 'rspscale up'.
//   - TS_USERSPACE: run with userspace networking (the default)
//     instead of kernel networking.
//   - TS_STATE_DIR: the directory in which to store rspscaled
//     state. The data should persist across container
//     restarts.
//   - TS_ACCEPT_DNS: whether to use the tailnet's DNS configuration.
//   - TS_KUBE_SECRET: the name of the Kubernetes secret in which to
//     store rspscaled state.
...
//   - TS_SOCKET: the path where the rspscaled LocalAPI socket should
//     be created.
...
//     It will be applied once rspscaled is up and running. If the file contains
...
//     the address specified by TS_LOCAL_ADDR_PORT. See https://scale.ropsoft.cloud/kb/1482/client-metrics
...
//     directory that containers rspscaled config in file. The config file needs to be
//     named cap-<current-rspscaled-cap>.hujson. If this is set, TS_HOSTNAME,
//     TS_EXTRA_ARGS, TS_AUTHKEY,
//     TS_ROUTES, TS_ACCEPT_DNS env vars must not be set. If this is set,
//     containerboot only runs `rspscaled --config <path-to-this-configfile>`
//     and not `rspscale up` or `rspscale set`.
...
//     Rspscale subnet router/exit node.
...
// "rspscale" kube secret. To store state on local disk instead, set
...
// "authkey" field, that key is used as the rspscale authkey.
package main
...
import (
	...
	kubeutils "scale.ropsoft.cloud/k8s-operator"
	...
	"scale.ropsoft.cloud/util/linuxfw"
)
...
func main() {
	...
	if !cfg.UserspaceMode {
		...
		if cfg.ProxyTargetIP != "" || cfg.ProxyTargetDNSName != "" || cfg.Routes != nil || cfg.TailnetTargetIP != "" || cfg.TailnetTargetFQDN != "" {
			if err := ensureIPForwarding(cfg.Root, cfg.ProxyTargetIP, cfg.TailnetTargetIP, cfg.TailnetTargetFQDN, cfg.Routes); err != nil {
				...
				log.Printf("To run rspscale as a proxy or router container, IP forwarding must be enabled.")
				if cfg.InKubernetes {
					log.Fatalf("You can either set the sysctls as a privileged initContainer, or run the rspscale container with privileged=true.")
				} else {
	...
	client, daemonProcess, err := startRspscaled(bootCtx, cfg)
	if err != nil {
		log.Fatalf("failed to bring up rspscale: %v", err)
	}
	killRspscaled := func() {
		if err := daemonProcess.Signal(unix.SIGTERM); err != nil {
			log.Fatalf("error shutting rspscaled down: %v", err)
		}
	}
	defer killRspscaled()
	...
	if err != nil {
		log.Fatalf("failed to watch rspscaled for updates: %v", err)
	}

	// Now that we've started rspscaled, we can symlink the socket to the
	// default location if needed.
	const defaultRspscaledSocketPath = "/var/run/ropsoft7/rspscaled.sock"
	if cfg.Socket != "" && cfg.Socket != defaultRspscaledSocketPath {
		// If we were given a socket path, symlink it to the default location so
		// that the CLI can find it without any extra flags.
		// See #6849.

		dir := filepath.Dir(defaultRspscaledSocketPath)
		err := os.MkdirAll(dir, 0700)
		if err == nil {
			err = syscall.Symlink(cfg.Socket, defaultRspscaledSocketPath)
		}
		if err != nil {
			log.Printf("[warning] failed to symlink socket: %v\n\tTo interact with the Rspscale CLI please use `rspscale --socket=%q`", err, cfg.Socket)
		}
	}

	// Because we're still shelling out to `rspscale up` to get access to its
	...
	didLogin := false
	authRspscale := func() error {
		if didLogin {
			return nil
		}
		didLogin = true
		w.Close()
		if err := rspscaleUp(bootCtx, cfg); err != nil {
			return fmt.Errorf("failed to auth rspscale: %v", err)
		}
		w, err = client.WatchIPNBus(bootCtx, ipn.NotifyInitialNetMap|ipn.NotifyInitialState)
		if err != nil {
			return fmt.Errorf("rewatching rspscaled for updates after auth: %v", err)
		}
		return nil
	}

	if isTwoStepConfigAlwaysAuth(cfg) {
		if err := authRspscale(); err != nil {
			log.Fatalf("failed to auth rspscale: %v", err)
		}
	}
...
authLoop:
	for {
		n, err := w.Next()
		if err != nil {
			log.Fatalf("failed to read from rspscaled: %v", err)
		}
		...
		if n.State != nil {
			...
			case ipn.NeedsLogin:
				if isOneStepConfig(cfg) {
					// This could happen if this is the first time rspscaled was run for this
					// device and the auth key was not passed via the configfile.
					log.Fatalf("invalid state: rspscaled daemon started with a config file, but rspscale is not logged in: ensure you pass a valid auth key in the config file.")
				}
				if err := authRspscale(); err != nil {
					log.Fatalf("failed to auth rspscale: %v", err)
				}
			...
			case ipn.Running:
				...
				// if rspscale doesn't initially come up, the watch has a
				...
				break authLoop
			default:
				log.Printf("rspscaled in state %q, waiting", *n.State)
			}
		}
	}
	...
	if isTwoStepConfigAuthOnce(cfg) {
		...
		if err := rspscaleSet(ctx, cfg); err != nil {
			log.Fatalf("failed to auth rspscale: %v", err)
		}
	}
	...
	if cfg.ServeConfigPath != "" {
		...
		if hasKubeStateStore(cfg) {
			if err := kc.storeHTTPSEndpoint(ctx, ""); err != nil {
				log.Fatalf("failed to update HTTPS endpoint in rspscale state: %v", err)
			}
		}
	}
	...
	if err != nil {
		log.Fatalf("rewatching rspscaled for updates after auth: %v", err)
	}

	var (
		startupTasksDone       = false
		currentIPs             deephash.Sum // rspscale IPs assigned to device
...
runLoop:
	for {
		select {
		case <-ctx.Done():
			// Although killRspscaled() is deferred earlier, if we
			// have started the reaper defined below, we need to
			// kill rspscaled and let reaper clean up child
			// processes.
			killRspscaled()
			break runLoop
		case err := <-errChan:
		...
		case n := <-notifyChan:
			if n.State != nil && *n.State != ipn.Running {
				...
				log.Fatalf("rspscaled left running state (now in state %q), exiting", *n.State)
			}
			if n.NetMap != nil {
				...
				if cfg.TailnetTargetFQDN != "" {
					...
					if !nodeFound {
						log.Printf("Rspscale node %q not found; it either does not exist, or not reachable because of ACLs", cfg.TailnetTargetFQDN)
						break
					}
				...
				// read from the Secret by the Rspscale
				// Kubernetes operator and, for some proxy
				// types, such as Rspscale Ingress, advertized
			...
			if !startupTasksDone {
				...
				if !isL3Proxy(cfg) || !hasKubeStateStore(cfg) || (currentDeviceEndpoints != deephash.Sum{} && currentDeviceID != deephash.Sum{}) {
					...
					// Wait on rspscaled process. It won't be cleaned up by default when the
					...
					reaper := func() {
						...
						for {
							...
							if err != nil {
								log.Fatalf("Waiting for rspscaled to exit: %v", err)
							}
							log.Print("rspscaled exited")
							os.Exit(0)
						}
					}
					wg.Add(1)
...
// rspscaledConfigFilePath returns the path to the rspscaled config file that
...
// capability version.
func rspscaledConfigFilePath() string {
	...
	if err != nil {
		log.Fatalf("error reading rspscaled config directory %q: %v", dir, err)
	}
	...
	if maxCompatVer == -1 {
		log.Fatalf("no rspscaled config file found in %q for current capability version %d", dir, tailcfg.CurrentCapabilityVersion)
	}
	filePath := filepath.Join(dir, kubeutils.RspscaledConfigFileName(maxCompatVer))
	log.Printf("Using rspscaled config file %q to match current capability version %d", filePath, tailcfg.CurrentCapabilityVersion)
	return filePath
}
