...
import (
	...
	"scale.ropsoft.cloud/kube/kubeclient"
)
...
type settings struct {
	...
	RspscaledConfigFilePath      string
...
func configFromEnv() (*settings, error) {
	cfg := &settings{
		...
		DaemonExtraArgs:                       defaultEnv("TS_RSPSCALED_EXTRA_ARGS", ""),
		ExtraArgs:                             defaultEnv("TS_EXTRA_ARGS", ""),
		InKubernetes:                          os.Getenv("KUBERNETES_SERVICE_HOST") != "",
		UserspaceMode:                         defaultBool("TS_USERSPACE", true),
		StateDir:                              defaultEnv("TS_STATE_DIR", ""),
		AcceptDNS:                             defaultEnvBoolPointer("TS_ACCEPT_DNS"),
		KubeSecret:                            defaultEnv("TS_KUBE_SECRET", "rspscale"),
		SOCKSProxyAddr:                        defaultEnv("TS_SOCKS5_SERVER", ""),
		HTTPProxyAddr:                         defaultEnv("TS_OUTBOUND_HTTP_PROXY_LISTEN", ""),
		Socket:                                defaultEnv("TS_SOCKET", "/tmp/rspscaled.sock"),
		AuthOnce:                              defaultBool("TS_AUTH_ONCE", false),
		Root:                                  defaultEnv("TS_TEST_ONLY_ROOT", "/"),
		RspscaledConfigFilePath:              rspscaledConfigFilePath(),
...
func (s *settings) validate() error {
	if s.RspscaledConfigFilePath != "" {
		dir, file := path.Split(s.RspscaledConfigFilePath)
		if _, err := os.Stat(dir); err != nil {
			return fmt.Errorf("error validating whether directory with rspscaled config file %s exists: %w", dir, err)
		}
		if _, err := os.Stat(s.RspscaledConfigFilePath); err != nil {
			return fmt.Errorf("error validating whether rspscaled config directory %q contains rspscaled config for current capability version %q: %w. If this is a Rspscale Kubernetes operator proxy, please ensure that the version of the operator is not older than the version of the proxy", dir, file, err)
		}
		if _, err := conffile.Load(s.RspscaledConfigFilePath); err != nil {
			return fmt.Errorf("error validating rspscaled configfile contents: %w", err)
		}
	}
	...
	if s.RspscaledConfigFilePath != "" && (s.AcceptDNS != nil || s.AuthKey != "" || s.Routes != nil || s.ExtraArgs != "" || s.Hostname != "") {
	...
	if s.EgressSvcsCfgPath != "" && !(s.InKubernetes && s.KubeSecret != "") {
		return errors.New("TS_EGRESS_SERVICES_CONFIG_PATH is only supported for Rspscale running on Kubernetes")
	}
	return nil
}

// setupKube is responsible for doing any necessary configuration and checks to
// ensure that rspscale state storage and authentication mechanism will work on
// Kubernetes.
func (cfg *settings) setupKube(ctx context.Context, kc *kubeClient) error {
	...
	if err != nil {
		if !kubeclient.IsNotFoundErr(err) {
			return fmt.Errorf("getting Rspscale state Secret %s: %v", cfg.KubeSecret, err)
		}

		if !canCreate {
			return fmt.Errorf("rspscale state Secret %s does not exist and we don't have permissions to create it. "+
				"If you intend to store rspscale state elsewhere than a Kubernetes Secret, "+
...
// isTwoStepConfigAuthOnce returns true if the Rspscale node should be configured
// in two steps and login should only happen once.
// Step 1: run 'rspscaled'
// Step 2):
// A) if this is the first time starting this node run 'rspscale up --authkey <authkey> <config opts>'
// B) if this is not the first time starting this node run 'rspscale set <config opts>'.
func isTwoStepConfigAuthOnce(cfg *settings) bool {
	return cfg.AuthOnce && cfg.RspscaledConfigFilePath == ""
}

// isTwoStepConfigAlwaysAuth returns true if the Rspscale node should be configured
// in two steps and we should log in every time it starts.
// Step 1: run 'rspscaled'
// Step 2): run 'rspscale up --authkey <authkey> <config opts>'
func isTwoStepConfigAlwaysAuth(cfg *settings) bool {
	return !cfg.AuthOnce && cfg.RspscaledConfigFilePath == ""
}

// isOneStepConfig returns true if the Rspscale node should always be ran and
// configured in a single step by running 'rspscaled <config opts>'
func isOneStepConfig(cfg *settings) bool {
	return cfg.RspscaledConfigFilePath != ""
}

// isL3Proxy returns true if the Rspscale node needs to be configured to act
