...
// kubeClient is a wrapper around Rspscale's internal kube client that knows how to talk to the kube API server. We use
...
func newKubeClient(root string, stateSecret string) (*kubeClient, error) {
	...
	var err error
	kc, err := kubeclient.New("rspscale-container")
	if err != nil {
...
func (kc *kubeClient) storeDeviceID(ctx context.Context, deviceID tailcfg.StableNodeID) error {
	...
	return kc.StrategicMergePatchSecret(ctx, kc.stateSecret, s, "rspscale-container")
}
...
// storeHTTPSEndpoint writes an HTTPS endpoint exposed by this device via 'rspscale serve' to the client's state
...
// storeCapVerUID stores the current capability version of rspscale and, if provided, UID of the Pod in the rspscale
// state Secret.
// These two fields are used by the Kubernetes Operator to observe the current capability version of rspscaled running in this container.
