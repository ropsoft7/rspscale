...
func installEgressForwardingRule(_ context.Context, dstStr string, tsIPs []netip.Prefix, nfr linuxfw.NetfilterRunner) error {
	...
	if !local.IsValid() {
		return fmt.Errorf("no rspscale IP matching family of %s found in %v", dstStr, tsIPs)
	}
	if err := nfr.DNATNonRspscaleTraffic("rspscale0", dst); err != nil {
		return fmt.Errorf("installing egress proxy rules: %w", err)
	}
	if err := nfr.EnsureSNATForDst(local, dst); err != nil {
		return fmt.Errorf("installing egress proxy rules: %w", err)
	}
	if err := nfr.ClampMSSToPMTU("rspscale0", dst); err != nil {
		return fmt.Errorf("installing egress proxy rules: %w", err)
	}
	return nil
}
...
func installTSForwardingRuleForDestination(_ context.Context, dstFilter string, tsIPs []netip.Prefix, nfr linuxfw.NetfilterRunner) error {
	...
	if !local.IsValid() {
		return fmt.Errorf("no rspscale IP matching family of %s found in %v", dstFilter, tsIPs)
	}
...
func installIngressForwardingRule(_ context.Context, dstStr string, tsIPs []netip.Prefix, nfr linuxfw.NetfilterRunner) error {
	...
	if proxyHasIPv4Address && dst.Is6() {
		log.Printf("Warning: proxy backend ClusterIP is an IPv6 address and the proxy has a IPv4 tailnet address. You might need to disable IPv4 address allocation for the proxy for forwarding to work. See https://github.com/ropsoft7/rspscale/issues/12156")
	}
	if !local.IsValid() {
		return fmt.Errorf("no rspscale IP matching family of %s found in %v", dstStr, tsIPs)
	}
...
func installIngressForwardingRuleForDNSTarget(_ context.Context, backendAddrs []net.IP, tsIPs []netip.Prefix, nfr linuxfw.NetfilterRunner) error {
	...
	updateFirewall := func(dst netip.Addr, backendTargets []netip.Addr) error {
		...
		// rspscale interfaces. Clamp MSS of packets going out via
		// rspscale0 interface to its MTU to prevent broken connections
		// in environments where path MTU discovery is not working.
		if err := nfr.ClampMSSToPMTU("rspscale0", dst); err != nil {
			return fmt.Errorf("adding rule to clamp traffic via rspscale0: %v", err)
		}
		return nil
	}
	...
	if len(v4Backends) != 0 {
		if !tsv4.IsValid() {
			log.Printf("backend targets %v contain at least one IPv4 address, but this node's Rspscale IPs do not contain a valid IPv4 address: %v", backendAddrs, tsIPs)
	...
	if len(v6Backends) != 0 && !tsv6.IsValid() {
		if !tsv6.IsValid() {
			log.Printf("backend targets %v contain at least one IPv6 address, but this node's Rspscale IPs do not contain a valid IPv6 address: %v", backendAddrs, tsIPs)
