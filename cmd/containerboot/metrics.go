...
// the rspscaled's LocalAPI usermetrics endpoint at /localapi/v0/usermetrics.
type metrics struct {
...
// requests to rspscaled's /localapi/v0/usermetrics API.
//
// In 1.78.x and 1.80.x, it also proxies debug paths to rspscaled's debug
...
func metricsHandlers(mux *http.ServeMux, lc *rspscale.LocalClient, debugAddrPort string) {
	m := &metrics{
		lc:            lc,
