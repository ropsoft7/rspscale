...
// re-read and applied.
func watchServeConfigChanges(ctx context.Context, path string, cdChanged <-chan bool, certDomainAtomic *atomic.Pointer[string], lc *rspscale.LocalClient, kc *kubeClient) {
...
func updateServeConfig(ctx context.Context, sc *ipn.ServeConfig, certDomain string, lc *rspscale.LocalClient) error {
...
func validateHTTPSServe(certDomain string, sc *ipn.ServeConfig) {
	...
	log.Printf(
		...
		To make it work, ensure that HTTPS is enabled for your tailnet, see https://scale.ropsoft.cloud/kb/1153/enabling-https for more details.`)
}
