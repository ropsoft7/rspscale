...
import (
	...
	"scale.ropsoft.cloud/kube/egressservices"
...
func TestContainerBoot(t *testing.T) {
	...
	rspscaledConf := &ipn.ConfigVAlpha{AuthKey: ptr.To("foo"), Version: "alpha0"}
	rspscaledConfBytes, err := json.Marshal(rspscaledConf)
	if err != nil {
		t.Fatalf("error unmarshaling rspscaled config: %v", err)
	}
	...
	files := map[string][]byte{
		"usr/bin/rspscaled":                         fakeRspscaled,
		"usr/bin/rspscale":                          fakeRspscale,
		"usr/bin/iptables":                           fakeRspscale,
		"usr/bin/ip6tables":                          fakeRspscale,
		"dev/net/tun":                                []byte(""),
		"proc/sys/net/ipv4/ip_forward":               []byte("0"),
		"proc/sys/net/ipv6/conf/all/forwarding":      []byte("0"),
		"etc/rspscaled/cap-95.hujson":               rspscaledConfBytes,
		"etc/rspscaled/serve-config.json":           serveConfBytes,
		"etc/rspscaled/egress-services-config.json": egressSvcsCfgBytes,
	}
	resetFiles := func() {
	...
	if err := exec.Command("go", "build", "-o", boot, "scale.ropsoft.cloud/cmd/containerboot").Run(); err != nil {
		t.Fatalf("Building containerboot: %v", err)
	}

	argFile := filepath.Join(d, "args")
	runningSockPath := filepath.Join(d, "tmp/rspscaled.sock")
	...
	tests := []struct {
	...
	}{
		{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false",
					},
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key",
					},
				},
				{
					Notify: runningNotify,
		...
		},
		{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key",
					},
				},
				{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key --advertise-routes=1.2.3.0/24,10.20.30.0/24",
					},
				},
				{
					Notify: runningNotify,
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key --advertise-routes=",
					},
				},
				{
					Notify: runningNotify,
		...
		},
		{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key --advertise-routes=1.2.3.0/24,10.20.30.0/24",
					},
				},
				{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key --advertise-routes=::/64,1::/64",
					},
				},
				{
					Notify: runningNotify,
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key --advertise-routes=::/64,1.2.3.0/24",
					},
				},
				{
					Notify: runningNotify,
		...
		},
		{
			...
			Phases: []phase{
				...
				},
				{
					Notify: runningNotify,
					WantCmds: []string{
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock set --accept-dns=false",
					},
				},
			},
		},
		{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=kube:rspscale --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --authkey=tskey-key",
					},
					WantKubeSecret: map[string]string{
						"authkey": "tskey-key",
		...
		},
		{
			...
			Env: map[string]string{
				"KUBERNETES_SERVICE_HOST":       kube.Host,
				"KUBERNETES_SERVICE_PORT_HTTPS": kube.Port,
				// Explicitly set to an empty value, to override the default of "rspscale".
				"TS_KUBE_SECRET": "",
				"TS_STATE_DIR":   filepath.Join(d, "tmp"),
				"TS_AUTHKEY":     "tskey-key",
			},
		...
		},
		{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking --socks5-server=localhost:1080 --outbound-http-proxy-listen=localhost:8080",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false",
					},
				},
				{
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=true",
					},
				},
				{
					Notify: runningNotify,
		...
		},
		{
			...
			Env: map[string]string{
				...
				"TS_RSPSCALED_EXTRA_ARGS": "--experiments=widgets",
			},
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking --experiments=widgets",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --widget=rotated",
					},
				}, {
					Notify: runningNotify,
				},
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --accept-routes",
					},
				}, {
					Notify: runningNotify,
				},
			...
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking",
						"/usr/bin/rspscale --socket=/tmp/rspscaled.sock up --accept-dns=false --hostname=my-server",
					},
				}, {
					Notify: runningNotify,
				},
		...
		{
			Name: "experimental rspscaled config path",
			Env: map[string]string{
				"TS_EXPERIMENTAL_VERSIONED_CONFIG_DIR": filepath.Join(d, "etc/rspscaled/"),
			},
			Phases: []phase{
				{
					WantCmds: []string{
						"/usr/bin/rspscaled --socket=/tmp/rspscaled.sock --state=mem: --statedir=/tmp --tun=userspace-networking --config=/etc/rspscaled/cap-95.hujson",
					},
				}, {
					Notify: runningNotify,
				},
		...
		},
		{
			...
			Env: map[string]string{
				"TS_SERVE_CONFIG": filepath.Join(d, "etc/rspscaled/serve-config.json"),
				"TS_AUTHKEY":      "tskey-key",
			},
			Phases: []phase{
				{
		...
		},
		{
			...
			Env: map[string]string{
				"KUBERNETES_SERVICE_HOST":        kube.Host,
				"KUBERNETES_SERVICE_PORT_HTTPS":  kube.Port,
				"TS_EGRESS_SERVICES_CONFIG_PATH": filepath.Join(d, "etc/rspscaled/egress-services-config.json"),
			},
			KubeSecret: map[string]string{
				"authkey": "tskey-key",
			},
		...
		},
		{
			...
			Phases: []phase{
				{
					WantFatalLog: "TS_EGRESS_SERVICES_CONFIG_PATH is only supported for Rspscale running on Kubernetes",
				},
			},
		},
	}
...
// of command lines recorded by test_rspscale.sh and
// test_rspscaled.sh.
//
...
//go:embed test_rspscaled.sh
var fakeRspscaled []byte

//go:embed test_rspscale.sh
var fakeRspscale []byte

// localAPI is a minimal fake rspscaled LocalAPI server that presents
// just enough functionality for containerboot to function
// correctly. In practice this means it only supports querying
// rspscaled status, and panics on all other uses to make it very
...
func (l *localAPI) Start() error {
	path := filepath.Join(l.FSRoot, "tmp/rspscaled.sock.fake")
...
func (k *kubeServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	switch r.URL.Path {
	case "/api/v1/namespaces/default/secrets/rspscale":
		k.serveSecret(w, r)
