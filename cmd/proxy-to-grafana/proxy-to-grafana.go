...
// originating Rspscale identity and maps them to corresponding Grafana
...
var (
	hostname     = flag.String("hostname", "", "Rspscale hostname to serve on, used as the base name for MagicDNS or subdomain in your domain alias for HTTPS.")
	...
	rspscaleDir = flag.String("state-dir", "./", "Alternate directory to use for Rspscale state storage. If empty, a default is used.")
	useHTTPS     = flag.Bool("use-https", false, "Serve over HTTPS via your *.ts.net subdomain if enabled in Rspscale admin.")
	loginServer  = flag.String("login-server", "", "URL to alternative control server. If empty, the default Rspscale control is used.")
)
...
func main() {
	...
	if *useHTTPS {
		...
		go func() {
			// wait for rspscale to start before trying to fetch cert names
			for range 60 {
				st, err := localClient.Status(context.Background())
				if err != nil {
					log.Printf("error retrieving rspscale status; retrying: %v", err)
				} else {
					log.Printf("rspscale status: %v", st.BackendState)
...
func modifyRequest(req *http.Request, localClient *rspscale.LocalClient) {
	...
	user, err := getRspscaleUser(req.Context(), localClient, req.RemoteAddr)
	if err != nil {
		log.Printf("error getting Rspscale user: %v", err)
		return
	}
...
func getRspscaleUser(ctx context.Context, localClient *rspscale.LocalClient, ipPort string) (*tailcfg.UserProfile, error) {
