...
var (
	hostname     = flag.String("hostname", "", "Rspscale hostname to serve on")
	...
	rspscaleDir = flag.String("state-dir", "", "Directory in which to store the Rspscale auth state")
)
...
func newProxy(upstreamAddr, upstreamCAPath string, client *rspscale.LocalClient) (*proxy, error) {
...
// connections must originate from rspscale IPs that can be verified
// with WhoIs.
