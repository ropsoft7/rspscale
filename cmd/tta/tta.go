...
// The tta server is the Rspscale Test Agent.
//
// It runs on each Rspscale node being integration tested and permits the test
...
var (
	driverAddr = flag.String("driver", "test-driver.rspscale:8008", "address of the test driver; by default we use the DNS name test-driver.rspscale which is special cased in the emulated network's DNS server")
)
...
func main() {
	...
	log.Printf("Rspscale Test Agent running.")
	...
	lcRP := httputil.NewSingleHostReverseProxy(must.Get(url.Parse("http://local-rspscaled.sock")))
	...
	ttaMux.HandleFunc("/up", func(w http.ResponseWriter, r *http.Request) {
		serveCmd(w, "rspscale", "up", "--login-server=http://control.rspscale")
	})
