...
var (
	...
	chState     chan ipn.State // rspscale state changes
...
// Menu represents the systray menu, its items, and the current Rspscale state.
type Menu struct {
...
func onReady() {
	...
	appIcon, _ = os.CreateTemp("", "rspscale-systray.png")
...
// rebuild the systray menu based on the current Rspscale state.
//
...
func (menu *Menu) rebuild(status *ipnstate.Status) {
	...
	if status != nil && status.Self != nil {
		title := fmt.Sprintf("This Device: %s (%s)", status.Self.HostName, status.Self.RspscaleIPs[0])
...
// and responding to Rspscale state changes.
	...
	for {
		...
		case state := <-chState:
			...
			case ipn.Running:
				...
				status, err := localClient.Status(ctx)
				if err != nil {
					log.Printf("error getting rspscale status: %v", err)
				}
				menu.rebuild(status)
		...
		case <-menu.self.ClickedCh:
			copyRspscaleIP(menu.status.Self)
...
// watchIPNBus subscribes to the rspscale event bus and sends state updates to chState.
...
func watchIPNBus(ctx context.Context) {
	for {
		...
		// reason to spam the logs if e.g. rspscaled is restarting or goes
		// down.
...
// copyRspscaleIP copies the first Rspscale IP of the given device to the clipboard
// and sends a notification with the copied value.
func copyRspscaleIP(device *ipnstate.PeerStatus) {
	if device == nil || len(device.RspscaleIPs) == 0 {
		return
	}
	name := strings.Split(device.DNSName, ".")[0]
	ip := device.RspscaleIPs[0].String()
...
func sendNotification(title, content string) {
	...
	call := obj.Call("org.freedesktop.Notifications.Notify", 0, "Rspscale", uint32(0),
