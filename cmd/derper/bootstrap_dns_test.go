...
import (
	...
	"scale.ropsoft.cloud/tstest/nettest"
)

func BenchmarkHandleBootstrapDNS(b *testing.B) {
	tstest.Replace(b, bootstrapDNS, "log.rspscale.io,login.scale.ropsoft.cloud,controlplane.scale.ropsoft.cloud,login.us.scale.ropsoft.cloud")
	refreshBootstrapDNS()
	w := new(bitbucketResponseWriter)
	req, _ := http.NewRequest("GET", "https://localhost/bootstrap-dns?q="+url.QueryEscape("log.rspscale.io"), nil)
	b.ReportAllocs()
	b.ResetTimer()
...
func TestUnpublishedDNS(t *testing.T) {
	...
	const published = "login.scale.ropsoft.cloud"
	const unpublished = "log.rspscale.io"
...
func TestUnpublishedDNSEmptyList(t *testing.T) {
	pub := &dnsEntryMap{
		IPs: map[string][]net.IP{"scale.ropsoft.cloud": {net.IPv4(10, 10, 10, 10)}},
	}
	dnsCache.Store(pub)
	dnsCacheBytes.Store([]byte(`{"scale.ropsoft.cloud":["10.10.10.10"]}`))

	unpublishedDNSCache.Store(&dnsEntryMap{
		IPs: map[string][]net.IP{
			"log.rspscale.io":           {},
			"controlplane.scale.ropsoft.cloud": {net.IPv4(1, 2, 3, 4)},
		},
		Percent: map[string]float64{
			"log.rspscale.io":           1.0,
			"controlplane.scale.ropsoft.cloud": 1.0,
		},
	})

	t.Run("CacheMiss", func(t *testing.T) {
		// One domain in map but empty, one not in map at all
		for _, q := range []string{"log.rspscale.io", "login.scale.ropsoft.cloud"} {
			resetMetrics()
	...
	t.Run("CacheHit", func(t *testing.T) {
		resetMetrics()
		ips := getBootstrapDNS(t, "controlplane.scale.ropsoft.cloud")
		want := map[string][]net.IP{"controlplane.scale.ropsoft.cloud": {net.IPv4(1, 2, 3, 4)}}
