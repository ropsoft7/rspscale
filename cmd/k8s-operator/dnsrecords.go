...
import (
	...
	operatorutils "scale.ropsoft.cloud/k8s-operator"
...
const (
	dnsRecordsRecocilerFinalizer = "scale.ropsoft.cloud/dns-records-reconciler"
	annotationTSMagicDNSName     = "scale.ropsoft.cloud/magic-dnsname"
)

// dnsRecordsReconciler knows how to update dnsrecords ConfigMap with DNS
// records.
// The records that it creates are:
//   - For rspscale Ingress, a mapping of the Ingress's MagicDNSName to the IP address of
//     the ingress proxy Pod.
//   - For egress proxies configured via scale.ropsoft.cloud/tailnet-fqdn annotation, a
...
// scale.ropsoft.cloud/v1alpha1.DNSConfig instance in the cluster (so that we know
// that there is a ts.net nameserver deployed in the cluster).
type dnsRecordsReconciler struct {
	client.Client
	tsNamespace           string // namespace in which we provision rspscale resources
...
// rspscale proxy and updates DNS Records in dnsrecords ConfigMap for the
...
func (dnsRR *dnsRecordsReconciler) Reconcile(ctx context.Context, req reconcile.Request) (res reconcile.Result, err error) {
	...
	if !(isManagedByType(headlessSvc, "svc") || isManagedByType(headlessSvc, "ingress")) {
		logger.Debugf("Service is not a headless Service for a rspscale ingress or egress proxy; do nothing")
		return reconcile.Result{}, nil
	}
	...
	// checking that there is scale.ropsoft.cloud/v1alpha1.DNSConfig resource in a
	// Ready state.
...
// The record is only provisioned if the proxy is for a rspscale Ingress or
// egress configured via scale.ropsoft.cloud/tailnet-fqdn annotation.
//
...
//
// For egress, the record is a mapping between scale.ropsoft.cloud/tailnet-fqdn
// annotation and the proxy Pod IP addresses, retrieved from the EndpointSlice
// associated with this headless Service, i.e
// Records{IP4: {<scale.ropsoft.cloud/tailnet-fqdn>: <[IPs of the egress proxy Pods]>}
//
// If records need to be created for this proxy, maybeProvision will also:
// - update the headless Service with a scale.ropsoft.cloud/magic-dnsname annotation
...
func (dnsRR *dnsRecordsReconciler) maybeProvision(ctx context.Context, headlessSvc *corev1.Service, logger *zap.SugaredLogger) error {
	...
	if oldFqdn != "" && oldFqdn != fqdn { // i.e user has changed the value of scale.ropsoft.cloud/tailnet-fqdn annotation
...
// dnsrecords ConfigMap and the scale.ropsoft.cloud/dns-records-reconciler finalizer
...
// scale.ropsoft.cloud/magic-dnsname annotation, just remove the finalizer.
...
// If the headless Service is for a rspscale Ingress proxy, returns ingress.status.loadBalancer.ingress.hostname.
// If the headless Service is for an rspscale egress proxy configured via scale.ropsoft.cloud/tailnet-fqdn annotation, returns the annotation value.
...
// created for a proxy for a rspscale egress Service configured via
// scale.ropsoft.cloud/tailnet-fqdn annotation.
