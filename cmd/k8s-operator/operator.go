...
// rspscale-operator provides a way to expose services running in a Kubernetes
...
// Generate static manifests for deploying Rspscale operator on Kubernetes from the operator's Helm chart.
//go:generate go run scale.ropsoft.cloud/cmd/k8s-operator/generate staticmanifests
...
func main() {
	...
	var (
		...
		image                 = defaultEnv("PROXY_IMAGE", "ropsoft7/rspscale:latest")
	...
	// https://scale.ropsoft.cloud/kb/1236/kubernetes-operator/?q=kubernetes#accessing-the-kubernetes-control-plane-using-an-api-server-proxy.
...
// initTSNet initializes the tsnet.Server and logs in to Rspscale. It uses the
// CLIENT_ID_FILE and CLIENT_SECRET_FILE environment variables to authenticate
// with Rspscale.
func initTSNet(zlog *zap.SugaredLogger) (*tsnet.Server, *rspscale.Client) {
	var (
		clientIDPath     = defaultEnv("CLIENT_ID_FILE", "")
		clientSecretPath = defaultEnv("CLIENT_SECRET_FILE", "")
		hostname         = defaultEnv("OPERATOR_HOSTNAME", "rspscale-operator")
	...
	credentials := clientcredentials.Config{
		...
		TokenURL:     "https://login.scale.ropsoft.cloud/api/v2/oauth/token",
	}
	tsClient := rspscale.NewClient("-", nil)
	tsClient.UserAgent = "rspscale-k8s-operator"
	tsClient.HTTPClient = credentials.Client(context.Background())

	s := &tsnet.Server{
		Hostname: hostname,
		Logf:     zlog.Named("rspscaled").Debugf,
	}
	...
	if err := s.Start(); err != nil {
		startlog.Fatalf("starting rspscale server: %v", err)
	}
...
waitOnline:
	for {
		startlog.Debugf("querying rspscaled status")
		...
		case "NeedsLogin":
			...
			if err := lc.Start(ctx, ipn.Options{
				...
				startlog.Fatalf("starting rspscale: %v", err)
			}
		...
		default:
			startlog.Debugf("waiting for rspscale to start: %v", st.BackendState)
		}
...
func runReconcilers(opts reconcilerOpts) {
	...
	nsFilter := cache.ByObject{
		Field: client.InNamespace(opts.rspscaleNamespace).AsSelector(),
	}
	...
	eventRecorder := mgr.GetEventRecorderFor("rspscale-operator")
	ssr := &rspscaleSTSReconciler{
		Client:                 mgr.GetClient(),
		tsnetServer:            opts.tsServer,
		tsClient:               opts.tsClient,
		defaultTags:            strings.Split(opts.proxyTags, ","),
		operatorNamespace:      opts.rspscaleNamespace,
	...
	err = builder.
		...
		Complete(&ServiceReconciler{
			...
			tsNamespace:           opts.rspscaleNamespace,
	...
	// Enque Ingress if a managed Service or backend Service associated with a rspscale Ingress changes.
	...
	podsFilter := handler.EnqueueRequestsFromMapFunc(egressEpsFromPGPods(mgr.GetClient(), opts.rspscaleNamespace))
	secretsFilter := handler.EnqueueRequestsFromMapFunc(egressEpsFromPGStateSecrets(mgr.GetClient(), opts.rspscaleNamespace))
	epsFromExtNSvcFilter := handler.EnqueueRequestsFromMapFunc(epsFromExternalNameService(mgr.GetClient(), opts.log, opts.rspscaleNamespace))
	...
	dnsRRDNSConfigOpts := handler.EnqueueRequestsFromMapFunc(enqueueAllIngressEgressProxySvcsInNS(opts.rspscaleNamespace, mgr.GetClient(), logger))
	...
	// On Ingress events, if it is a rspscale Ingress or if rspscale is the default ingress controller, reconcile the proxy
	// headless Service.
	dnsRRIngressOpts := handler.EnqueueRequestsFromMapFunc(dnsRecordsReconcilerIngressHandler(opts.rspscaleNamespace, opts.proxyActAsDefaultLoadBalancer, mgr.GetClient(), logger))
	err = builder.ControllerManagedBy(mgr).
...
type reconcilerOpts struct {
	...
	tsClient           *rspscale.Client
	rspscaleNamespace string       // namespace in which operator resources will be deployed
	...
	// default to tag:k8s.
	// https://scale.ropsoft.cloud/kb/1085/auth-keys
	proxyTags string
...
func enqueueAllIngressEgressProxySvcsInNS(ns string, cl client.Client, logger *zap.SugaredLogger) handler.MapFunc {
	return func(ctx context.Context, _ client.Object) []reconcile.Request {
		...
		if err := cl.List(ctx, svcHeadlessSvcList, client.InNamespace(ns), client.MatchingLabels(svcProxyLabels)); err != nil {
			logger.Errorf("error listing headless Services for rspscale ingress/egress Services in operator namespace: %v", err)
			return nil
		}
		...
		if err := cl.List(ctx, ingHeadlessSvcList, client.InNamespace(ns), client.MatchingLabels(ingProxyLabels)); err != nil {
			logger.Errorf("error listing headless Services for rspscale Ingresses in operator namespace: %v", err)
			return nil
		}
...
// dns-records-reconciler only reconciles on rspscale Ingress events. When an
// event is observed on a rspscale Ingress, reconcile the proxy headless Service.
func dnsRecordsReconcilerIngressHandler(ns string, isDefaultLoadBalancer bool, cl client.Client, logger *zap.SugaredLogger) handler.MapFunc {
	return func(ctx context.Context, o client.Object) []reconcile.Request {
		...
		if !isDefaultLoadBalancer && (ing.Spec.IngressClassName == nil || *ing.Spec.IngressClassName != "rspscale") {
			return nil
		}
...
type tsClient interface {
	CreateKey(ctx context.Context, caps rspscale.KeyCapabilities) (string, *rspscale.Key, error)
	Device(ctx context.Context, deviceID string, fields *rspscale.DeviceFieldsOpts) (*rspscale.Device, error)
...
// scale.ropsoft.cloud/proxy-class: <proxy class name>.
...
// The Services of interest are backend Services for rspscale Ingress and
// managed Services for an StatefulSet for a proxy configured for rspscale
// Ingress
func serviceHandlerForIngress(cl client.Client, logger *zap.SugaredLogger) handler.MapFunc {
	return func(ctx context.Context, o client.Object) []reconcile.Request {
		...
		reqs := make([]reconcile.Request, 0)
		for _, ing := range ingList.Items {
			if ing.Spec.IngressClassName == nil || *ing.Spec.IngressClassName != rspscaleIngressClassName {
				return nil
			}
...
// request for it , if the object is a Rspscale egress Service meant to be
...
// epsFromExternalNameService is an event handler for ExternalName Services that define a Rspscale egress service that
...
// indexEgressServices adds a local index to a cached Rspscale egress Services meant to be exposed on a ProxyGroup. The
