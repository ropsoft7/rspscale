...
import (
	...
	ksr "scale.ropsoft.cloud/k8s-operator/sessionrecording"
...
// that authenticates requests using the Rspscale LocalAPI and then proxies
...
// Rspscale LocalAPI and then proxies them to the Kubernetes API.
// It listens on :443 and uses the Rspscale HTTPS certificate.
...
//     caller's identity from the Rspscale LocalAPI.
...
// apiserverProxy is an [net/http.Handler] that authenticates requests using the Rspscale
...
func determineRecorderConfig(who *apitype.WhoIsResponse) (failOpen bool, recorderAddresses []netip.AddrPort, _ error) {
	...
	for _, rule := range rules {
		if len(rule.RecorderAddrs) != 0 {
			...
			// spend 30s trying to reach a recorder whose rspscale
			// status is offline.
