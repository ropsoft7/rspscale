...
func TestDNSRecordsReconciler(t *testing.T) {
	...
	ing := &networkingv1.Ingress{
		...
		Spec: networkingv1.IngressSpec{
			IngressClassName: ptr.To("rspscale"),
		},
	...
	cm := &corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Name: "dnsrecords", Namespace: "rspscale"}}
	...
	egressSvcFQDN := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "egress-fqdn",
			Namespace:   "test",
			Annotations: map[string]string{"scale.ropsoft.cloud/tailnet-fqdn": "foo.bar.ts.net"},
		},
	...
	expectReconciled(t, dnsRR, "rspscale", "egress-fqdn") // dns-records-reconciler reconcile the headless Service
	...
	// 2. DNS record is updated if scale.ropsoft.cloud/tailnet-fqdn annotation's
	// value changes
	mustUpdate(t, fc, "test", "egress-fqdn", func(svc *corev1.Service) {
		svc.Annotations["scale.ropsoft.cloud/tailnet-fqdn"] = "baz.bar.ts.net"
	})
	expectReconciled(t, dnsRR, "rspscale", "egress-fqdn") // dns-records-reconciler reconcile the headless Service
	...
	mustCreate(t, fc, ep)
	expectReconciled(t, dnsRR, "rspscale", "ts-ingress") // dns-records-reconciler should reconcile the headless Service
	...
	expectReconciled(t, dnsRR, "rspscale", "ts-ingress")
...
func expectHostsRecords(t *testing.T, cl client.Client, wantsHosts map[string][]string) {
	...
	if err := cl.Get(context.Background(), types.NamespacedName{Name: "dnsrecords", Namespace: "rspscale"}, cm); err != nil {
