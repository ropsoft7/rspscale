...
func expectedSTS(t *testing.T, cl client.Client, opts configOpts) *appsv1.StatefulSet {
	...
	if opts.confFileHash != "" {
		annots["scale.ropsoft.cloud/operator-last-set-config-file-hash"] = opts.confFileHash
	}
	...
	if opts.tailnetTargetIP != "" {
		annots["scale.ropsoft.cloud/operator-last-set-ts-tailnet-target-ip"] = opts.tailnetTargetIP
		tsContainer.Env = append(tsContainer.Env, corev1.EnvVar{
			Name:  "TS_TAILNET_TARGET_IP",
			Value: opts.tailnetTargetIP,
		})
	} else if opts.tailnetTargetFQDN != "" {
		annots["scale.ropsoft.cloud/operator-last-set-ts-tailnet-target-fqdn"] = opts.tailnetTargetFQDN
		tsContainer.Env = append(tsContainer.Env, corev1.EnvVar{
		...
		annots["scale.ropsoft.cloud/operator-last-set-cluster-ip"] = opts.clusterTargetIP
	} else if opts.clusterTargetDNS != "" {
		tsContainer.Env = append(tsContainer.Env, corev1.EnvVar{
			Name:  "TS_EXPERIMENTAL_DEST_DNS_NAME",
			Value: opts.clusterTargetDNS,
		})
		annots["scale.ropsoft.cloud/operator-last-set-cluster-dns-name"] = opts.clusterTargetDNS
	}
	if opts.serveConfig != nil {
		...
		tsContainer.VolumeMounts = append(tsContainer.VolumeMounts, corev1.VolumeMount{Name: "serve-config", ReadOnly: true, MountPath: "/etc/rspscaled"})
	}
	tsContainer.Env = append(tsContainer.Env, corev1.EnvVar{
	...
	ss := &appsv1.StatefulSet{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"scale.ropsoft.cloud/managed":              "true",
				"scale.ropsoft.cloud/parent-resource":      "test",
				"scale.ropsoft.cloud/parent-resource-ns":   opts.namespace,
				"scale.ropsoft.cloud/parent-resource-type": opts.parentType,
			},
		},
	...
	if opts.proxyClass != "" {
		...
		return applyProxyClassToStatefulSet(proxyClass, ss, new(rspscaleSTSConfig), zl.Sugar())
	}
	return ss
}
...
func expectedSTSUserspace(t *testing.T, cl client.Client, opts configOpts) *appsv1.StatefulSet {
	...
	tsContainer := corev1.Container{
		...
		Env: []corev1.EnvVar{
			...
			{Name: "TS_KUBE_SECRET", Value: opts.secretName},
			{Name: "TS_EXPERIMENTAL_VERSIONED_CONFIG_DIR", Value: "/etc/tsconfig"},
			{Name: "TS_SERVE_CONFIG", Value: "/etc/rspscaled/serve-config"},
			{Name: "TS_INTERNAL_APP", Value: opts.app},
		},
		ImagePullPolicy: "Always",
		VolumeMounts: []corev1.VolumeMount{
			{Name: "rspscaledconfig", ReadOnly: true, MountPath: "/etc/tsconfig"},
			{Name: "serve-config", ReadOnly: true, MountPath: "/etc/rspscaled"},
		},
	}
	if opts.enableMetrics {
		tsContainer.Env = append(tsContainer.Env,
	...
	if opts.confFileHash != "" {
		ss.Spec.Template.Annotations["scale.ropsoft.cloud/operator-last-set-config-file-hash"] = opts.confFileHash
	}
	// If opts.proxyClass is set, retrieve the ProxyClass and apply
	// configuration from that to the StatefulSet.
	if opts.proxyClass != "" {
	...
	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"scale.ropsoft.cloud/managed":              "true",
				"scale.ropsoft.cloud/parent-resource":      "test",
				"scale.ropsoft.cloud/parent-resource-ns":   "default",
				"scale.ropsoft.cloud/parent-resource-type": parentType,
			},
		},
...
func expectedMetricsService(opts configOpts) *corev1.Service {
	...
	if opts.namespaced {
		selector["scale.ropsoft.cloud/parent-resource-ns"] = opts.namespace
	}
	return &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      metricsResourceName(opts.stsName),
			Namespace: opts.rspscaleNamespace,
			Labels:    labels,
		},
		Spec: corev1.ServiceSpec{
			Selector: selector,
...
func metricsLabels(opts configOpts) map[string]string {
	...
	labels := map[string]string{
		"scale.ropsoft.cloud/managed":        "true",
		"scale.ropsoft.cloud/metrics-target": opts.stsName,
		"ts_prom_job":                  promJob,
	...
	sm := &ServiceMonitor{
		...
		Spec: ServiceMonitorSpec{
			...
			NamespaceSelector: ServiceMonitorNamespaceSelector{
				MatchNames: []string{opts.rspscaleNamespace},
			},
...
func expectedSecret(t *testing.T, cl client.Client, opts configOpts) *corev1.Secret {
	...
	if opts.proxyClass != "" {
		...
		if proxyClass.Spec.RspscaleConfig != nil && proxyClass.Spec.RspscaleConfig.AcceptRoutes {
	...
	if err != nil {
		t.Fatalf("error marshalling rspscaled config")
	}
	...
	if opts.parentType == "connector" {
		labels["scale.ropsoft.cloud/parent-resource-ns"] = "" // Connector is cluster scoped
	}
	s.Labels = labels
...
type fakeTSClient struct {
	sync.Mutex
	keyRequests []rspscale.KeyCapabilities
	deleted     []string
}
...
func (c *fakeTSClient) CreateKey(ctx context.Context, caps rspscale.KeyCapabilities) (string, *rspscale.Key, error) {
	c.Lock()
	defer c.Unlock()
...
func (c *fakeTSClient) Device(ctx context.Context, deviceID string, fields *rspscale.DeviceFieldsOpts) (*rspscale.Device, error) {
	return &rspscale.Device{
		DeviceID: deviceID,
...
func (c *fakeTSClient) KeyRequests() []rspscale.KeyCapabilities {
	c.Lock()
	defer c.Unlock()
...
// removeHashAnnotation can be used to remove declarative rspscaled config hash
