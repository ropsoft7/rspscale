...
func Test_generate(t *testing.T) {
	...
	helmPackagePath := filepath.Join(tempDir, "rspscale-operator-0.0.1.tgz")
	...
	if !strings.Contains(installContentsWithCRD.String(), "name: connectors.scale.ropsoft.cloud") {
		t.Errorf("Connector CRD not found in default chart install")
	}
	if !strings.Contains(installContentsWithCRD.String(), "name: proxyclasses.scale.ropsoft.cloud") {
		t.Errorf("ProxyClass CRD not found in default chart install")
	}
	if !strings.Contains(installContentsWithCRD.String(), "name: dnsconfigs.scale.ropsoft.cloud") {
		t.Errorf("DNSConfig CRD not found in default chart install")
	}
	if !strings.Contains(installContentsWithCRD.String(), "name: recorders.scale.ropsoft.cloud") {
		t.Errorf("Recorder CRD not found in default chart install")
	}
	if !strings.Contains(installContentsWithCRD.String(), "name: proxygroups.scale.ropsoft.cloud") {
	...
	if strings.Contains(installContentsWithoutCRD.String(), "name: connectors.scale.ropsoft.cloud") {
	...
	if strings.Contains(installContentsWithoutCRD.String(), "name: dnsconfigs.scale.ropsoft.cloud") {
		t.Errorf("DNSConfig CRD found in chart install that should not contain a CRD")
	}
	if strings.Contains(installContentsWithoutCRD.String(), "name: recorders.scale.ropsoft.cloud") {
		t.Errorf("Recorder CRD found in chart install that should not contain a CRD")
	}
	if strings.Contains(installContentsWithoutCRD.String(), "name: proxygroups.scale.ropsoft.cloud") {
