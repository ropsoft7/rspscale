...
func TestRspscaleIngress(t *testing.T) {
	tsIngressClass := &networkingv1.IngressClass{ObjectMeta: metav1.ObjectMeta{Name: "rspscale"}, Spec: networkingv1.IngressClassSpec{Controller: "scale.ropsoft.cloud/ts-ingress"}}
	...
	expectReconciled(t, ingR, "default", "test")
	ing.Finalizers = append(ing.Finalizers, "scale.ropsoft.cloud/finalizer")
...
func TestRspscaleIngressHostname(t *testing.T) {
	tsIngressClass := &networkingv1.IngressClass{ObjectMeta: metav1.ObjectMeta{Name: "rspscale"}, Spec: networkingv1.IngressClassSpec{Controller: "scale.ropsoft.cloud/ts-ingress"}}
	fc := fake.NewFakeClient(tsIngressClass)
	ft := &fakeTSClient{}
	fakeTsnetServer := &fakeTSNetServer{certDomains: []string{"foo.com"}}
	...
	mustUpdate(t, fc, "operator-ns", opts.secretName, func(secret *corev1.Secret) {
		mak.Set(&secret.Data, "device_id", []byte("1234"))
		mak.Set(&secret.Data, "rspscale_capver", []byte("110"))
...
func TestRspscaleIngressWithProxyClass(t *testing.T) {
	// Setup
	pc := &tsapi.ProxyClass{
	...
	// 4. scale.ropsoft.cloud/proxy-class label is removed from the Ingress, the
...
func TestRspscaleIngressWithServiceMonitor(t *testing.T) {
	pc := &tsapi.ProxyClass{
	...
	ing := &networkingv1.Ingress{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				"scale.ropsoft.cloud/proxy-class": "metrics",
			},
		},
		Spec: networkingv1.IngressSpec{
			IngressClassName: ptr.To("rspscale"),
