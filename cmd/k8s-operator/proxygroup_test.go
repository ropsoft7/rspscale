...
const testProxyImage = "ropsoft7/rspscale:test"
...
func TestProxyGroup(t *testing.T) {
	...
	t.Run("observe_ProxyGroupCreating_status_reason", func(t *testing.T) {
		...
		expectProxyGroupResources(t, fc, pg, true, initialCfgHash)
		keyReq := rspscale.KeyCapabilities{
		...
		if diff := cmp.Diff(tsClient.KeyRequests(), []rspscale.KeyCapabilities{keyReq, keyReq}); diff != "" {
	...
	t.Run("trigger_config_change_and_observe_new_config_hash", func(t *testing.T) {
		pc.Spec.RspscaleConfig = &tsapi.RspscaleConfig{
			AcceptRoutes: true,
		}
	...
	t.Run("delete_and_cleanup", func(t *testing.T) {
		...
		expectMissing[corev1.Service](t, reconciler, "rspscale", metricsResourceName(pg.Name))
