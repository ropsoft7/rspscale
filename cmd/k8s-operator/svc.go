...
func (a *ServiceReconciler) isRspscaleService(svc *corev1.Service) bool {
...
func (a *ServiceReconciler) Reconcile(ctx context.Context, req reconcile.Request) (_ reconcile.Result, err error) {
	...
	if !svc.DeletionTimestamp.IsZero() || !a.isRspscaleService(svc) {
		logger.Debugf("service is being deleted or is (no longer) referring to Rspscale ingress/egress, ensuring any created resources are cleaned up")
...
// maybeCleanup removes any existing resources related to serving svc over rspscale.
//
...
// maybeProvision ensures that svc is exposed over rspscale, taking any actions
...
func (a *ServiceReconciler) maybeProvision(ctx context.Context, logger *zap.SugaredLogger, svc *corev1.Service) (err error) {
	...
	if !slices.Contains(svc.Finalizers, FinalizerName) {
		...
		logger.Infof("exposing service over rspscale")
	...
	if !isRspscaleLoadBalancerService(svc, a.isDefaultLoadBalancer) {
	...
	if dev == nil || dev.hostname == "" {
		msg := "no Rspscale hostname known yet, waiting for proxy pod to finish auth"
		logger.Debug(msg)
...
func validateService(svc *corev1.Service) []string {
	...
	if err := dnsname.ValidLabel(svcName); err != nil {
		if _, ok := svc.Annotations[AnnotationHostname]; ok {
			violations = append(violations, fmt.Sprintf("invalid Rspscale hostname specified %q: %s", svcName, err))
		} else {
			violations = append(violations, fmt.Sprintf("invalid Rspscale hostname %q, use %q annotation to override: %s", svcName, AnnotationHostname, err))
		}
	}
	return violations
}
...
func (a *ServiceReconciler) shouldExposeClusterIP(svc *corev1.Service) bool {
	...
	return isRspscaleLoadBalancerService(svc, a.isDefaultLoadBalancer) || hasExposeAnnotation(svc)
}

func isRspscaleLoadBalancerService(svc *corev1.Service, isDefaultLoadBalancer bool) bool {
	return svc != nil &&
		svc.Spec.Type == corev1.ServiceTypeLoadBalancer &&
		(svc.Spec.LoadBalancerClass != nil && *svc.Spec.LoadBalancerClass == "rspscale" ||
			svc.Spec.LoadBalancerClass == nil && isDefaultLoadBalancer)
}

// hasExposeAnnotation reports whether Service has the scale.ropsoft.cloud/expose
// annotation set
func hasExposeAnnotation(svc *corev1.Service) bool {
	return svc != nil && svc.Annotations[AnnotationExpose] == "true"
}

// tailnetTargetAnnotation returns the value of scale.ropsoft.cloud/tailnet-ip
// annotation or of the deprecated scale.ropsoft.cloud/ts-tailnet-target-ip
