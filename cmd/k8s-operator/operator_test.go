...
import (
	...
	"scale.ropsoft.cloud/net/dns/resolvconffile"
...
func TestLoadBalancerClass(t *testing.T) {
	...
	mustCreate(t, fc, &corev1.Service{
		...
		Spec: corev1.ServiceSpec{
			...
			LoadBalancerClass: ptr.To("rspscale"),
		},
	})
	...
	want := &corev1.Service{
		...
		Status: corev1.ServiceStatus{
			Conditions: []metav1.Condition{{
				...
				Message:            `unable to provision proxy resources: invalid Service: invalid value of annotation scale.ropsoft.cloud/tailnet-fqdn: "invalid.example.com" does not appear to be a valid MagicDNS name`,
			}},
		},
	}
	...
	want.ObjectMeta.Finalizers = []string{"scale.ropsoft.cloud/finalizer"}
	want.Status = corev1.ServiceStatus{
		Conditions: []metav1.Condition{{
			...
			Message:            "no Rspscale hostname known yet, waiting for proxy pod to finish auth",
		}},
	}
	expectEqual(t, fc, want, nil)

	// Normally the Rspscale proxy pod would come up here and write its info
	...
	mustUpdate(t, fc, "operator-ns", fullName, func(s *corev1.Secret) {
		...
		s.Data["device_fqdn"] = []byte("rspscale.device.name.")
	...
	// Note that the Rspscale-specific condition status should be gone now.
	want = &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test",
			Namespace: "default",
...
func TestTailnetTargetFQDNAnnotation(t *testing.T) {
	...
	want := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:       "test",
			Namespace:  "default",
			Finalizers: []string{"scale.ropsoft.cloud/finalizer"},
			UID:        types.UID("1234-UID"),
			Annotations: map[string]string{
				AnnotationTailnetTargetFQDN: tailnetTargetFQDN,
			},
	...
	// Change the rspscale-target-fqdn annotation which should update the
	// StatefulSet
	...
	// Remove the rspscale-target-fqdn annotation which should make the
	// operator clean up
	mustUpdate(t, fc, "default", "test", func(s *corev1.Service) {
		s.ObjectMeta.Annotations = map[string]string{}
	})
...
func TestTailnetTargetIPAnnotation(t *testing.T) {
	...
	// Change the rspscale-target-ip annotation which should update the
	// StatefulSet
	...
	// Remove the rspscale-target-ip annotation which should make the
	// operator clean up
	mustUpdate(t, fc, "default", "test", func(s *corev1.Service) {
		s.ObjectMeta.Annotations = map[string]string{}
	})
	...
	want := &corev1.Service{
		...
		Status: corev1.ServiceStatus{
			Conditions: []metav1.Condition{{
				...
				LastTransitionTime: t0,
				Reason:             reasonProxyInvalid,
				Message:            `unable to provision proxy resources: invalid Service: invalid value of annotation scale.ropsoft.cloud/tailnet-ip: "invalid-ip" could not be parsed as a valid IP Address, error: ParseAddr("invalid-ip"): unable to parse IP`,
			}},
		},
	}
	...
	want := &corev1.Service{
		...
		Status: corev1.ServiceStatus{
			Conditions: []metav1.Condition{{
				...
				LastTransitionTime: t0,
				Reason:             reasonProxyInvalid,
				Message:            `unable to provision proxy resources: invalid Service: invalid value of annotation scale.ropsoft.cloud/tailnet-ip: "999.999.999.999" could not be parsed as a valid IP Address, error: ParseAddr("999.999.999.999"): IPv4 field has value >255`,
			}},
		},
	}
...
func TestAnnotations(t *testing.T) {
	...
	mustCreate(t, fc, &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			UID: types.UID("1234-UID"),
			Annotations: map[string]string{
				"scale.ropsoft.cloud/expose": "true",
			},
		},
		Spec: corev1.ServiceSpec{
			ClusterIP: "10.20.30.40",
	...
	// operator clean up.
	mustUpdate(t, fc, "default", "test", func(s *corev1.Service) {
		delete(s.ObjectMeta.Annotations, "scale.ropsoft.cloud/expose")
	})
	// synchronous StatefulSet deletion triggers a requeue. But, the StatefulSet
	// didn't create any child resources since this is all faked, so the
	// deletion goes through immediately.
...
func TestAnnotationIntoLB(t *testing.T) {
	...
	// Remove Rspscale's annotation, and at the same time convert the service
	// into a rspscale LoadBalancer.
	mustUpdate(t, fc, "default", "test", func(s *corev1.Service) {
		delete(s.ObjectMeta.Annotations, "scale.ropsoft.cloud/expose")
		s.Spec.Type = corev1.ServiceTypeLoadBalancer
		s.Spec.LoadBalancerClass = ptr.To("rspscale")
	})
	expectReconciled(t, sr, "default", "test")
...
func TestCustomHostname(t *testing.T) {
	...
	mustCreate(t, fc, &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				"scale.ropsoft.cloud/expose":   "true",
				"scale.ropsoft.cloud/hostname": "reindeer-flotilla",
			},
		},
		Spec: corev1.ServiceSpec{
			ClusterIP: "10.20.30.40",
...
func TestCustomPriorityClassName(t *testing.T) {
	...
	mustCreate(t, fc, &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				"scale.ropsoft.cloud/expose":   "true",
				"scale.ropsoft.cloud/hostname": "rspscale-critical",
			},
		},
		Spec: corev1.ServiceSpec{
			ClusterIP: "10.20.30.40",
...
func TestProxyClassForService(t *testing.T) {
	...
	pc := &tsapi.ProxyClass{
		...
		Spec: tsapi.ProxyClassSpec{
			RspscaleConfig: &tsapi.RspscaleConfig{
				AcceptRoutes: true,
			},
	...
	// 1. A new rspscale LoadBalancer Service is created without any
	...
	// 2. The Service gets updated with scale.ropsoft.cloud/proxy-class label
	...
	// 4. scale.ropsoft.cloud/proxy-class label is removed from the Service, the
...
func TestRspscaledConfigfileHash(t *testing.T) {
	fc := fake.NewFakeClient()
	ft := &fakeTSClient{}
	zl, err := zap.NewDevelopment()
	if err != nil {
...
func Test_serviceHandlerForIngress(t *testing.T) {
	...
	// 1. An event on a headless Service for a rspscale Ingress results in
	...
	mustCreate(t, fc, &networkingv1.Ingress{
		...
		Spec: networkingv1.IngressSpec{IngressClassName: ptr.To(rspscaleIngressClassName)},
	})
	svc1 := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "headless-1",
	...
	// 2. An event on a Service that is the default backend for a rspscale
	...
	mustCreate(t, fc, &networkingv1.Ingress{
		...
		Spec: networkingv1.IngressSpec{
			...
			IngressClassName: ptr.To(rspscaleIngressClassName),
		},
	})
	...
	// a rspscale Ingress results in the Ingress being reconciled.
	mustCreate(t, fc, &networkingv1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "ing-3",
			Namespace: "ns-3",
	...
	// rspscale Ingress does not result in an Ingress reconcile.
	mustCreate(t, fc, &networkingv1.Ingress{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "ing-4",
			Namespace: "ns-4",
	...
	if len(gotReqs) > 0 {
		t.Errorf("unexpected reconcile request for a Service that does not belong to a Rspscale Ingress: %#+v\n", gotReqs)
	}
...
func Test_authKeyRemoval(t *testing.T) {
	...
	// 1. A new Service that should be exposed via Rspscale gets created, a Secret with a config that contains auth
	// key is generated.
	clock := tstest.NewClock(tstest.ClockOpts{})
	sr := &ServiceReconciler{
		Client: fc,
...
func Test_externalNameService(t *testing.T) {
	...
	// 1. A External name Service that should be exposed via Rspscale gets
	// created.
	clock := tstest.NewClock(tstest.ClockOpts{})
	sr := &ServiceReconciler{
		Client: fc,
