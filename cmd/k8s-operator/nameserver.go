...
const (
	...
	defaultNameserverImageRepo = "rspscale/k8s-nameserver"
...
func (a *NameserverReconciler) Reconcile(ctx context.Context, req reconcile.Request) (res reconcile.Result, err error) {
	...
	if len(dnsCfgs.Items) > 1 { // enforce DNSConfig to be a singleton
		msg := "invalid cluster configuration: more than one scale.ropsoft.cloud/dnsconfigs found. Please ensure that no more than one is created."
		logger.Error(msg)
...
func nameserverResourceLabels(name, namespace string) map[string]string {
	...
	labels["app.kubernetes.io/name"] = "rspscale"
