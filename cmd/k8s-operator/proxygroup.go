...
func (r *ProxyGroupReconciler) Reconcile(ctx context.Context, req reconcile.Request) (_ reconcile.Result, err error) {
	...
	if apierrors.IsNotFound(err) {
		...
		return reconcile.Result{}, fmt.Errorf("failed to get scale.ropsoft.cloud ProxyGroup: %w", err)
	}
...
func (r *ProxyGroupReconciler) ensureConfigSecretsCreated(ctx context.Context, pg *tsapi.ProxyGroup, proxyClass *tsapi.ProxyClass) (hash string, err error) {
	...
	for i := range pgReplicas(pg) {
		...
		configs, err := pgRspscaledConfig(pg, proxyClass, i, authKey, existingCfgSecret)
		if err != nil {
			return "", fmt.Errorf("error creating rspscaled config: %w", err)
		}

		for cap, cfg := range configs {
			cfgJSON, err := json.Marshal(cfg)
			if err != nil {
				return "", fmt.Errorf("error marshalling rspscaled config: %w", err)
			}
			mak.Set(&cfgSecret.StringData, tsoperator.RspscaledConfigFileName(cap), string(cfgJSON))
		}

		// The config sha256 sum is a value for a hash annotation used to trigger
		// pod restarts when rspscaled config changes. Any config changes apply
...
func pgRspscaledConfig(pg *tsapi.ProxyGroup, class *tsapi.ProxyClass, idx int32, authKey string, oldSecret *corev1.Secret) (rspscaledConfigs, error) {
