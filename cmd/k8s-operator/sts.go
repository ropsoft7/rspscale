...
import (
	...
	tsoperator "scale.ropsoft.cloud/k8s-operator"
	tsapi "scale.ropsoft.cloud/k8s-operator/apis/v1alpha1"
...
const (
	...
	// new label here, do also add it to rspscaleManagedLabels var to
	// ensure that it does not get overwritten by ProxyClass configuration.
	LabelManaged         = "scale.ropsoft.cloud/managed"
	LabelParentType      = "scale.ropsoft.cloud/parent-resource-type"
	LabelParentName      = "scale.ropsoft.cloud/parent-resource"
	LabelParentNamespace = "scale.ropsoft.cloud/parent-resource-ns"
	labelSecretType      = "scale.ropsoft.cloud/secret-type" // "config" or "state".

	// LabelProxyClass can be set by users on rspscale Ingresses and Services that define cluster ingress or
	...
	LabelProxyClass = "scale.ropsoft.cloud/proxy-class"

	FinalizerName = "scale.ropsoft.cloud/finalizer"

	// Annotations settable by users on services.
	AnnotationExpose             = "scale.ropsoft.cloud/expose"
	AnnotationTags               = "scale.ropsoft.cloud/tags"
	AnnotationHostname           = "scale.ropsoft.cloud/hostname"
	annotationTailnetTargetIPOld = "scale.ropsoft.cloud/ts-tailnet-target-ip"
	AnnotationTailnetTargetIP    = "scale.ropsoft.cloud/tailnet-ip"
	//MagicDNS name of tailnet node.
	AnnotationTailnetTargetFQDN = "scale.ropsoft.cloud/tailnet-fqdn"

	AnnotationProxyGroup = "scale.ropsoft.cloud/proxy-group"

	// Annotations settable by users on ingresses.
	AnnotationFunnel = "scale.ropsoft.cloud/funnel"
	...
	// rspscale in non-userspace, with NET_ADMIN cap for rspscale
	// container and will also run a privileged init container that enables
	// forwarding.
	// Eventually this behaviour might become the default.
	AnnotationExperimentalForwardClusterTrafficViaL7IngresProxy = "scale.ropsoft.cloud/experimental-forward-cluster-traffic-via-ingress"

	// Annotations set by the operator on pods to trigger restarts when the
	// hostname, IP, FQDN or rspscaled config changes. If you add a new
	// annotation here, also add it to rspscaleManagedAnnotations var to
	// ensure that it does not get removed when a ProxyClass configuration
	// is applied.
	podAnnotationLastSetClusterIP         = "scale.ropsoft.cloud/operator-last-set-cluster-ip"
	podAnnotationLastSetClusterDNSName    = "scale.ropsoft.cloud/operator-last-set-cluster-dns-name"
	podAnnotationLastSetTailnetTargetIP   = "scale.ropsoft.cloud/operator-last-set-ts-tailnet-target-ip"
	podAnnotationLastSetTailnetTargetFQDN = "scale.ropsoft.cloud/operator-last-set-ts-tailnet-target-fqdn"
	// podAnnotationLastSetConfigFileHash is sha256 hash of the current rspscaled configuration contents.
	podAnnotationLastSetConfigFileHash = "scale.ropsoft.cloud/operator-last-set-config-file-hash"
...
var (
	// rspscaleManagedLabels are label keys that rspscale operator sets on StatefulSets and Pods.
	rspscaleManagedLabels = []string{LabelManaged, LabelParentType, LabelParentName, LabelParentNamespace, "app"}
	// rspscaleManagedAnnotations are annotation keys that rspscale operator sets on StatefulSets and Pods.
	rspscaleManagedAnnotations = []string{podAnnotationLastSetClusterIP, podAnnotationLastSetTailnetTargetIP, podAnnotationLastSetTailnetTargetFQDN, podAnnotationLastSetConfigFileHash}
)

type rspscaleSTSConfig struct {
...
type rspscaleSTSReconciler struct {
	client.Client
...
func (sts rspscaleSTSReconciler) validate() error {
...
func (a *rspscaleSTSReconciler) IsHTTPSEnabledOnTailnet() bool {
	return len(a.tsnetServer.CertDomains()) > 0
}

// Provision ensures that the StatefulSet for the given service is running and
// up to date.
func (a *rspscaleSTSReconciler) Provision(ctx context.Context, logger *zap.SugaredLogger, sts *rspscaleSTSConfig) (*corev1.Service, error) {
	// Do full reconcile.
...
func (a *rspscaleSTSReconciler) Cleanup(ctx context.Context, logger *zap.SugaredLogger, labels map[string]string, typ string) (done bool, _ error) {
	...
	// rspscale device deletion races where we fail to notice a device that
	// should be removed.
...
func (a *rspscaleSTSReconciler) reconcileHeadlessService(ctx context.Context, logger *zap.SugaredLogger, sts *rspscaleSTSConfig) (*corev1.Service, error) {
...
func (a *rspscaleSTSReconciler) createOrGetSecret(ctx context.Context, logger *zap.SugaredLogger, stsC *rspscaleSTSConfig, hsvc *corev1.Service) (secretName, hash string, configs rspscaledConfigs, _ error) {
	...
	var authKey string
	if orig == nil {
		// Initially it contains only rspscaled config, but when the
		...
		if sts != nil {
			...
			logger.Errorf("Rspscale proxy secret doesn't exist, but the corresponding StatefulSet %s/%s already does. Something is wrong, please delete the StatefulSet.", sts.GetNamespace(), sts.GetName())
			return "", "", nil, nil
		}
		// Create API Key secret which is going to be used by the statefulset
		// to authenticate with Rspscale.
		logger.Debugf("creating authkey for new rspscale proxy")
		tags := stsC.Tags
		if len(tags) == 0 {
			tags = a.defaultTags
		}
	...
	configs, err := rspscaledConfig(stsC, authKey, orig)
	if err != nil {
		return "", "", nil, fmt.Errorf("error creating rspscaled config: %w", err)
	}
	hash, err = rspscaledConfigHash(configs)
	if err != nil {
		return "", "", nil, fmt.Errorf("error calculating hash of rspscaled configs: %w", err)
	}

	latest := tailcfg.CapabilityVersion(-1)
	var latestConfig ipn.ConfigVAlpha
	for key, val := range configs {
		fn := tsoperator.RspscaledConfigFileName(key)
		b, err := json.Marshal(val)
		if err != nil {
			return "", "", nil, fmt.Errorf("error marshalling rspscaled config: %w", err)
		}
	...
	if orig != nil {
		logger.Debugf("patching the existing proxy Secret with rspscaled config %s", sanitizeConfigBytes(latestConfig))
		if err := a.Patch(ctx, secret, client.MergeFrom(orig)); err != nil {
			return "", "", nil, err
		}
	} else {
		logger.Debugf("creating a new Secret for the proxy with rspscaled config %s", sanitizeConfigBytes(latestConfig))
...
// DeviceInfo returns the device ID, hostname and IPs for the Rspscale device
...
func (a *rspscaleSTSReconciler) DeviceInfo(ctx context.Context, childLabels map[string]string, logger *zap.SugaredLogger) (dev *device, err error) {
...
// device contains rspscale state of a proxy device as gathered from its rspscale state Secret.
type device struct {
	id       tailcfg.StableNodeID // device's stable ID
	hostname string               // MagicDNS name of the device
	ips      []string             // Rspscale IPs of the device
	// ingressDNSName is the L7 Ingress DNS name. In practice this will be the same value as hostname, but only set
	// when the device has been configured to serve traffic on it via 'rspscale serve'.
...
func (a *rspscaleSTSReconciler) reconcileSTS(ctx context.Context, logger *zap.SugaredLogger, sts *rspscaleSTSConfig, headlessSvc *corev1.Service, proxySecret, tsConfigHash string) (*appsv1.StatefulSet, error) {
	...
	// Configure containeboot to run rspscaled with a configfile read from the state Secret.
	mak.Set(&ss.Spec.Template.Annotations, podAnnotationLastSetConfigFileHash, tsConfigHash)

	configVolume := corev1.Volume{
		Name: "rspscaledconfig",
	...
	if sts.ClusterTargetIP != "" {
		...
		container.Env = append(container.Env, corev1.EnvVar{
			...
			Value: "/etc/rspscaled/serve-config",
		})
		container.VolumeMounts = append(container.VolumeMounts, corev1.VolumeMount{
			Name:      "serve-config",
			ReadOnly:  true,
			MountPath: "/etc/rspscaled",
		})
...
func appInfoForProxy(cfg *rspscaleSTSConfig) (string, error) {
...
// to this StatefulSet whilst any rspscale managed labels/annotations remain present.
...
func debugSetting(pc *tsapi.ProxyClass) bool {
	if pc == nil ||
		...
		pc.Spec.StatefulSet.Pod.RspscaleContainer == nil ||
		pc.Spec.StatefulSet.Pod.RspscaleContainer.Debug == nil {
		// This default will change to false in 1.82.0.
		return pc.Spec.Metrics != nil && pc.Spec.Metrics.Enable
	}

	return pc.Spec.StatefulSet.Pod.RspscaleContainer.Debug.Enable
}

func applyProxyClassToStatefulSet(pc *tsapi.ProxyClass, ss *appsv1.StatefulSet, stsCfg *rspscaleSTSConfig, logger *zap.SugaredLogger) *appsv1.StatefulSet {
	if pc == nil || ss == nil {
		return ss
	}
	...
	if metricsEnabled || debugEnabled {
		...
		if isEgress {
			...
			// scale.ropsoft.cloud/experimental-forward-cluster-traffic-via-ingress
	...
	if wantsSSLabels := pc.Spec.StatefulSet.Labels; len(wantsSSLabels) > 0 {
		ss.ObjectMeta.Labels = mergeStatefulSetLabelsOrAnnots(ss.ObjectMeta.Labels, wantsSSLabels, rspscaleManagedLabels)
	}
	if wantsSSAnnots := pc.Spec.StatefulSet.Annotations; len(wantsSSAnnots) > 0 {
		ss.ObjectMeta.Annotations = mergeStatefulSetLabelsOrAnnots(ss.ObjectMeta.Annotations, wantsSSAnnots, rspscaleManagedAnnotations)
	}
	...
	if wantsPodLabels := wantsPod.Labels; len(wantsPodLabels) > 0 {
		ss.Spec.Template.ObjectMeta.Labels = mergeStatefulSetLabelsOrAnnots(ss.Spec.Template.ObjectMeta.Labels, wantsPodLabels, rspscaleManagedLabels)
	}
	if wantsPodAnnots := wantsPod.Annotations; len(wantsPodAnnots) > 0 {
		ss.Spec.Template.ObjectMeta.Annotations = mergeStatefulSetLabelsOrAnnots(ss.Spec.Template.ObjectMeta.Annotations, wantsPodAnnots, rspscaleManagedAnnotations)
	}
	...
	for i, c := range ss.Spec.Template.Spec.Containers {
		if c.Name == "rspscale" {
			ss.Spec.Template.Spec.Containers[i] = updateContainer(wantsPod.RspscaleContainer, ss.Spec.Template.Spec.Containers[i])
			break
		}
	}
	if initContainers := ss.Spec.Template.Spec.InitContainers; len(initContainers) > 0 {
		for i, c := range initContainers {
			if c.Name == "sysctler" {
				ss.Spec.Template.Spec.InitContainers[i] = updateContainer(wantsPod.RspscaleInitContainer, initContainers[i])
				break
			}
		}
	}
...
func readAuthKey(secret *corev1.Secret, key string) (*string, error) {
	...
	if err := json.Unmarshal([]byte(secret.Data[key]), origConf); err != nil {
		return nil, fmt.Errorf("error unmarshaling previous rspscaled config in %q: %w", key, err)
	}
	return origConf.AuthKey, nil
}

// rspscaledConfig takes a proxy config, a newly generated auth key if generated and a Secret with the previous proxy
// state and auth key and returns rspscaled config files for currently supported proxy versions and a hash of that
// configuration.
func rspscaledConfig(stsC *rspscaleSTSConfig, newAuthkey string, oldSecret *corev1.Secret) (rspscaledConfigs, error) {
...
func shouldAcceptRoutes(pc *tsapi.ProxyClass) bool {
	return pc != nil && pc.Spec.RspscaleConfig != nil && pc.Spec.RspscaleConfig.AcceptRoutes
}
...
type rspscaledConfigs map[tailcfg.CapabilityVersion]ipn.ConfigVAlpha

// hashBytes produces a hash for the provided rspscaled config that is the same across
// different invocations of this code. We do not use the
// scale.ropsoft.cloud/deephash.Hash here because that produces a different hash for
// the same value in different rspscale builds. The hash we are producing here
// is used to determine if the container running the Connector Rspscale node
...
// changed).
func rspscaledConfigHash(c rspscaledConfigs) (string, error) {
	b, err := json.Marshal(c)
	if err != nil {
		return "", fmt.Errorf("error marshalling rspscaled configs: %w", err)
	}
	h := sha256.New()
