...
// Command gitops-pusher allows users to use a GitOps flow for managing Rspscale ACLs.
//
...
var (
	...
	apiServer         = rootFlagSet.String("api-server", "api.scale.ropsoft.cloud", "API server to contact")
...
func main() {
	...
	if !ok && (!oiok || !osok) {
		log.Fatal("set envvar TS_API_KEY to your Rspscale API key or TS_OAUTH_ID and TS_OAUTH_SECRET to your Rspscale OAuth ID and Secret")
	}
	...
	root := &ffcli.Command{
		...
		ShortHelp:   "Push Rspscale ACLs to CONTROL using a GitOps workflow",
