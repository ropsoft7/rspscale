...
// Rspscale on one or more TCP ports and sends them out to the same SNI
...
const configCapKey = "scale.ropsoft.cloud/sniproxy"
...
func run(ctx context.Context, ts *tsnet.Server, wgPort int, hostname string, promoteHTTPS bool, debugPort int, ports, forwards string) {
	// Wire up Rspscale node + app connector server
