...
// Capture of first TCP data segment for a connection to https://pkgs.scale.ropsoft.cloud
...
func TestTCPSNIHandler(t *testing.T) {
	h := tcpSNIHandler{
		Allowlist: []string{"pkgs.scale.ropsoft.cloud"},
		DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
			if network != "tcp" {
				t.Errorf("network = %s, want %s", network, "tcp")
			}
			if addr != "pkgs.scale.ropsoft.cloud:443" {
				t.Errorf("addr = %s, want %s", addr, "pkgs.scale.ropsoft.cloud:443")
			}
