...
func New(luid uint64) (*Firewall, error) {
	...
	if err := session.AddSublayer(&wf.Sublayer{
		ID:     sublayerID,
		Name:   "Rspscale permissive and blocking filters",
		Weight: 0,
	}); err != nil {
		return nil, err
	}
...
const (
	weightRspscaleTraffic weight = 15
	weightKnownTraffic     weight = 12
	weightCatchAll         weight = 0
)

func (f *Firewall) enable() error {
	if err := f.permitRspscaleService(weightRspscaleTraffic); err != nil {
		return fmt.Errorf("permitRspscaleService failed: %w", err)
	}

	if err := f.permitTunInterface(weightRspscaleTraffic); err != nil {
		return fmt.Errorf("permitTunInterface failed: %w", err)
	}

	if err := f.permitDNS(weightRspscaleTraffic); err != nil {
		return fmt.Errorf("permitDNS failed: %w", err)
	}

	if err := f.permitLoopback(weightRspscaleTraffic); err != nil {
...
func (f *Firewall) permitRspscaleService(w weight) error {
	...
	_, err = f.addRules("unrestricted traffic for Rspscale service", w, conditions, wf.ActionPermit, protocolAll, directionBoth)
	return err
}
