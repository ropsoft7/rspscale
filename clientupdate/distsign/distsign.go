...
func (c *Client) Download(ctx context.Context, srcPath, dstPath string) error {
	...
	if !VerifyAny(sigPub, msg, sig) {
		// Best-effort clean up of downloaded package.
		os.Remove(dstPathUnverified)
		return fmt.Errorf("signature %q for file %q does not validate with the current release signing key; either you are under attack, or attempting to download an old version of Rspscale which was signed with an older signing key", sigURL, srcURL)
	}
	c.logf("Signature OK")
...
func (c *Client) ValidateLocalBinary(srcURLPath, localFilePath string) error {
	...
	if !VerifyAny(sigPub, msg, sig) {
		return fmt.Errorf("signature %q for file %q does not validate with the current release signing key; either you are under attack, or attempting to download an old version of Rspscale which was signed with an older signing key", sigURL, localFilePath)
	}
	c.logf("Signature OK")
...
func (c *Client) signingKeys() ([]ed25519.PublicKey, error) {
	...
	if !VerifyAny(c.roots, raw, sig) {
		return nil, fmt.Errorf("signature %q for key %q does not validate with any known root key; either you are under attack, or running a very old version of Rspscale with outdated root keys", sigURL, keyURL)
	}
