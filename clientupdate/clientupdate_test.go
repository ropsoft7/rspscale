...
func TestUpdateDebianAptSourcesListBytes(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			in:      "# Rspscale packages for debian buster\ndeb https://pkgs.scale.ropsoft.cloud/stable/debian bullseye main\n",
			want:    "# Rspscale packages for debian buster\ndeb https://pkgs.scale.ropsoft.cloud/unstable/debian bullseye main\n",
		},
		{
		...
		},
		{
			name:    "signed-by-form",
			toTrack: UnstableTrack,
			in:      "# Rspscale packages for ubuntu jammy\ndeb [signed-by=/usr/share/keyrings/rspscale-archive-keyring.gpg] https://pkgs.scale.ropsoft.cloud/stable/ubuntu jammy main\n",
			want:    "# Rspscale packages for ubuntu jammy\ndeb [signed-by=/usr/share/keyrings/rspscale-archive-keyring.gpg] https://pkgs.scale.ropsoft.cloud/unstable/ubuntu jammy main\n",
		},
		{
			name:    "unsupported-lines",
			toTrack: UnstableTrack,
			in:      "# Rspscale packages for ubuntu jammy\ndeb [signed-by=/usr/share/keyrings/rspscale-archive-keyring.gpg] https://pkgs.scale.ropsoft.cloud/foobar/ubuntu jammy main\n",
			wantErr: "unexpected/unsupported /etc/apt/sources.list.d/rspscale.list contents",
		},
	}
...
func TestUpdateYUMRepoTrack(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			before: `
[rspscale-stable]
name=Rspscale stable
baseurl=https://pkgs.scale.ropsoft.cloud/stable/fedora/$basearch
enabled=1
type=rpm
repo_gpgcheck=1
gpgcheck=0
gpgkey=https://pkgs.scale.ropsoft.cloud/stable/fedora/repo.gpg
`,
			track: StableTrack,
			after: `
[rspscale-stable]
...
`,
			...
			after: `
[rspscale-unstable]
name=Rspscale unstable
baseurl=https://pkgs.scale.ropsoft.cloud/unstable/fedora/$basearch
enabled=1
type=rpm
repo_gpgcheck=1
gpgcheck=0
gpgkey=https://pkgs.scale.ropsoft.cloud/unstable/fedora/repo.gpg
`,
			rewrote: true,
		},
		{
			desc: "non-rspscale repo file",
			before: `
[fedora]
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "rspscale.repo")
...
rspscale-1.44.2-r0 webpage:
https://scale.ropsoft.cloud/

rspscale-1.44.2-r0 installed size:
32 MiB
`,
			want: "1.44.2",
		},
...
`,
		...
		},
		{
			...
			out: `
rspscale description:
The easiest, most secure way to use WireGuard and 2FA

rspscale webpage:
https://scale.ropsoft.cloud/

rspscale installed size:
32 MiB
`,
			wantErr: true,
		},
		...
		},
		{
			...
			out: `
rspscale-1.54.1-r0 description:
The easiest, most secure way to use WireGuard and 2FA

rspscale-1.54.1-r0 webpage:
https://scale.ropsoft.cloud/

rspscale-1.54.1-r0 installed size:
34 MiB

rspscale-1.58.2-r0 description:
The easiest, most secure way to use WireGuard and 2FA

rspscale-1.58.2-r0 webpage:
https://scale.ropsoft.cloud/

rspscale-1.58.2-r0 installed size:
35 MiB
`,
			want: "1.58.2",
		},
...
func TestUnpackLinuxTarball(t *testing.T) {
	...
	tests := []struct {
		...
		{
			desc: "tarball missing rspscaled",
			before: map[string]string{
				"rspscale":  "v1",
				"rspscaled": "v1",
			},
		...
		{
			desc: "duplicate rspscale binary",
			before: map[string]string{
				"rspscale":  "v1",
				"rspscaled": "v1",
			},
	...
	for _, tt := range tests {
		t.Run(tt.desc, func(t *testing.T) {
			...
			tmp := t.TempDir()
			rspscalePath := filepath.Join(tmp, "rspscale")
			rspscaledPath := filepath.Join(tmp, "rspscaled")
			binaryPaths = func() (string, string, error) {
				return rspscalePath, rspscaledPath, nil
			}
			...
			tarPath := filepath.Join(tmp, "rspscale.tgz")
...
func TestCleanupOldDownloads(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			desc: "MSIs",
			before: []string{
				"MSICache/rspscale-1.0.0.msi",
				"MSICache/rspscale-1.1.0.msi",
				"MSICache/readme.txt",
			},
			glob: "MSICache/*.msi",
			after: []string{
		...
		},
		{
			desc: "SPKs",
			before: []string{
				"tmp/rspscale-update-1/rspscale-1.0.0.spk",
				"tmp/rspscale-update-2/rspscale-1.1.0.spk",
				"tmp/readme.txt",
				"tmp/rspscale-update-3",
				"tmp/rspscale-update-4/rspscale-1.3.0",
			},
			glob: "tmp/rspscale-update*/*.spk",
			after: []string{
				"tmp/readme.txt",
				"tmp/rspscale-update-3",
				"tmp/rspscale-update-4/rspscale-1.3.0",
		...
		},
		{
			...
			symlinks: map[string]string{
				"MSICache/rspscale-1.3.0.msi": "MSICache/rspscale-1.0.0.msi",
				"MSICache/rspscale-1.4.0.msi": "MSICache/readme.txt",
			},
			glob: "MSICache/*.msi",
			after: []string{
				"MSICache/rspscale-1.3.0.msi",
				"MSICache/rspscale-1.4.0.msi",
				"MSICache/readme.txt",
			},
		},
	}
...
func TestParseUnraidPluginVersion(t *testing.T) {
	tests := []struct {
		...
		{plgPath: "testdata/rspscale-1.52.0.plg", wantVer: "1.52.0"},
		{plgPath: "testdata/rspscale-1.54.0.plg", wantVer: "1.54.0"},
		{plgPath: "testdata/rspscale-nover.plg", wantErr: "version not found in plg file"},
		{plgPath: "testdata/rspscale-nover-path-mentioned.plg", wantErr: "version not found in plg file"},
	}
	for _, tt := range tests {
