...
import (
	...
	"scale.ropsoft.cloud/util/winutil"
	"scale.ropsoft.cloud/util/winutil/authenticode"
)
...
const (
	...
	// rspscale.exe process from running before the msiexec process runs and
	// tries to overwrite ourselves.
	winMSIEnv = "TS_UPDATE_WIN_MSI"
	// winExePathEnv is the environment variable that is set along with
	// winMSIEnv and carries the full path of the calling rspscale.exe binary.
	// It is used to re-launch the GUI process (rspscale-ipn.exe) after
	// install is complete.
...
func makeSelfCopy() (origPathExe, tmpPathExe string, err error) {
	...
	defer f.Close()
	f2, err := os.CreateTemp("", "rspscale-updater-*.exe")
	if err != nil {
		return "", "", err
	}
...
const certSubjectRspscale = "Rspscale Inc."

func verifyAuthenticode(path string) error {
	return authenticode.Verify(path, certSubjectRspscale)
}

func (up *Updater) updateWindows() error {
	if msi := os.Getenv(winMSIEnv); msi != "" {
		// stdout/stderr from this part of the install could be lost since the
		// parent rspscaled is replaced. Create a temp log file to have some
...
* press Windows+r, type in "cmd", then press Ctrl+Shift+Enter`)
	...
	tsDir := filepath.Join(os.Getenv("ProgramData"), "Rspscale")
	...
	pkgsPath := fmt.Sprintf("%s/rspscale-setup-%s-%s.msi", up.Track, ver, arch)
	...
	up.Logf("making rspscale.exe copy to switch to...")
	up.cleanupOldDownloads(filepath.Join(os.TempDir(), "rspscale-updater-*.exe"))
	selfOrig, selfCopy, err := makeSelfCopy()
	if err != nil {
		return err
	}
	defer os.Remove(selfCopy)
	up.Logf("running rspscale.exe copy for final install...")
...
func msiUUIDForVersion(ver string) string {
	...
	msiURL := fmt.Sprintf("https://pkgs.scale.ropsoft.cloud/%s/rspscale-setup-%s-%s.msi", track, ver, arch)
...
func (up *Updater) switchOutputToFile() (io.Closer, error) {
	...
	if err != nil {
		logFilePath = filepath.Join(os.TempDir(), "rspscale-updater.log")
	} else {
