...
// Package clientupdate implements rspscale client update for all supported
// platforms. This package can be used from both rspscaled and rspscale
// binaries.
...
import (
	...
	"scale.ropsoft.cloud/util/cmpver"
...
type Arguments struct {
	...
	// Defaults to "https://pkgs.scale.ropsoft.cloud".
	PkgsAddr string
...
type Updater struct {
	...
	// nil (not all platforms support updates from within Rspscale).
	Update func() error
...
func NewUpdater(args Arguments) (*Updater, error) {
	...
	if up.Arguments.PkgsAddr == "" {
		up.Arguments.PkgsAddr = "https://pkgs.scale.ropsoft.cloud"
	}
	return &up, nil
}
...
func (up *Updater) getUpdateFunction() (fn updateFunction, canAutoUpdate bool) {
	...
	case "linux":
		...
		case distro.Synology:
			// Synology updates use our own pkgs.scale.ropsoft.cloud instead of the
...
func (up *Updater) updateSynology() error {
	...
	// Get the latest version and list of SPKs from pkgs.scale.ropsoft.cloud.
	...
	up.cleanupOldDownloads(filepath.Join(os.TempDir(), "rspscale-update*", "*.spk"))
	// Download the SPK into a temporary directory.
	spkDir, err := os.MkdirTemp("", "rspscale-update")
	if err != nil {
		return err
	}
	...
	// connected over rspscale ssh and this parent process dies. Otherwise, if
	// you abort synopkg install mid-way, rspscaled is not restarted.
	...
	if dsmVersion == 6 {
		...
		// manually.
		cmd := exec.Command("nohup", "synopkg", "start", "Rspscale")
		out, err := cmd.CombinedOutput()
		if err != nil {
...
// architectures served from pkgs.scale.ropsoft.cloud.
func synoArch(goArch, synoinfoPath string) (string, error) {
	...
	if arch == "" {
		...
		case "88f6281", "88f6282", "hi3535", "alpine", "armada370",
			...
			arch = cpu
		default:
			return "", fmt.Errorf("unsupported Synology CPU architecture %q (Go arch %q), please report a bug at https://github.com/ropsoft7/rspscale/issues/new/choose", cpu, goArch)
		}
	}
	return arch, nil
}
...
func (up *Updater) updateDebLike() error {
	...
	if err := exec.Command("dpkg", "--status", "rspscale").Run(); err != nil && isExitError(err) {
		// Rspscale was not installed via apt, update via tarball download
		// instead.
		return up.updateLinuxBinary()
	}
	ver, err := requestedRspscaleVersion(up.Version, up.Track)
	if err != nil {
		return err
	}
	if !up.confirm(ver) {
	...
	cmd := exec.Command("apt-get", "update",
		// Only update the rspscale repo, not the other ones, treating
		// the rspscale.list file as the main "sources.list" file.
		"-o", "Dir::Etc::SourceList=sources.list.d/rspscale.list",
	...
	for range 2 {
		out, err := exec.Command("apt-get", "install", "--yes", "--allow-downgrades", "rspscale="+ver).CombinedOutput()
		if err != nil {
			...
			up.Logf("running dpkg --configure rspscale")
			out, err = exec.Command("dpkg", "--force-confdef,downgrade", "--configure", "rspscale").CombinedOutput()
			if err != nil {
				return fmt.Errorf("dpkg --configure rspscale failed: %w; output:\n%s", err, out)
			}
			continue
		}
		break
...
const aptSourcesFile = "/etc/apt/sources.list.d/rspscale.list"

// updateDebianAptSourcesList updates the /etc/apt/sources.list.d/rspscale.list
...
func updateDebianAptSourcesListBytes(was []byte, dstTrack string) (newContent []byte, err error) {
	trackURLPrefix := []byte("https://pkgs.scale.ropsoft.cloud/" + dstTrack + "/")
	var buf bytes.Buffer
	var changes int
	bs := bufio.NewScanner(bytes.NewReader(was))
	hadCorrect := false
	commentLine := regexp.MustCompile(`^\s*\#`)
	pkgsURL := regexp.MustCompile(`\bhttps://pkgs\.rspscale\.com/((un)?stable)/`)
	for bs.Scan() {
		line := bs.Bytes()
...
func (up *Updater) archPackageInstalled() bool {
	err := exec.Command("pacman", "--query", "rspscale").Run()
	return err == nil
}

func (up *Updater) updateArchLike() error {
	// Arch maintainer asked us not to implement "rspscale update" or
	// auto-updates on Arch-based distros:
	// https://github.com/ropsoft7/rspscale/issues/6995#issuecomment-1687080106
...
you can use "pacman --sync --refresh --sysupgrade" or "pacman -Syu" to upgrade the system, including Rspscale.`)
}
...
func (up *Updater) updateNixos() error {
	...
	return errors.New(`individual package updates are not supported on NixOS installations. Update your system channel or flake inputs to get the latest Rspscale version from nixpkgs.`)
}

const yumRepoConfigFile = "/etc/yum.repos.d/rspscale.repo"

// updateFedoraLike updates rspscale on any distros in the Fedora family,
...
func (up *Updater) updateFedoraLike(packageManager string) func() error {
	return func() (err error) {
		...
		if err := exec.Command(packageManager, "info", "--installed", "rspscale").Run(); err != nil && isExitError(err) {
			// Rspscale was not installed via yum/dnf, update via tarball
			// download instead.
			return up.updateLinuxBinary()
		}
		defer func() {
			if err != nil {
				err = fmt.Errorf(`%w; you can try updating using "%s upgrade rspscale"`, err, packageManager)
			}
		}()
		...
		cmd := exec.Command(packageManager, "install", "--assumeyes", fmt.Sprintf("rspscale-%s-1", ver))
...
func updateYUMRepoTrack(repoFile, dstTrack string) (rewrote bool, err error) {
	...
	urlRe := regexp.MustCompile(`^(baseurl|gpgkey)=https://pkgs\.rspscale\.com/(un)?stable/`)
	urlReplacement := fmt.Sprintf("$1=https://pkgs.scale.ropsoft.cloud/%s/", dstTrack)

	s := bufio.NewScanner(bytes.NewReader(was))
	newContent := bytes.NewBuffer(make([]byte, 0, len(was)))
	for s.Scan() {
		line := s.Text()
		// Handle repo section name, like "[rspscale-stable]".
		if len(line) > 0 && line[0] == '[' {
			if !strings.HasPrefix(line, "[rspscale-") {
				return false, fmt.Errorf("%q does not look like a rspscale repo file, it contains an unexpected %q section", repoFile, line)
			}
			fmt.Fprintf(newContent, "[rspscale-%s]\n", dstTrack)
			continue
		}
		// Update the track mentioned in repo name.
		if strings.HasPrefix(line, "name=") {
			fmt.Fprintf(newContent, "name=Rspscale %s\n", dstTrack)
			continue
		}
...
func (up *Updater) updateAlpineLike() (err error) {
	...
	if err := exec.Command("apk", "info", "--installed", "rspscale").Run(); err != nil && isExitError(err) {
		// Rspscale was not installed via apk, update via tarball download
		// instead.
		return up.updateLinuxBinary()
	}
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf(`%w; you can try updating using "apk upgrade rspscale"`, err)
		}
	}()
	...
	out, err = exec.Command("apk", "info", "rspscale").CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed checking apk for latest rspscale version: %w, output:\n%s", err, out)
	}
	ver, err := parseAlpinePackageVersion(out)
	if err != nil {
		return fmt.Errorf(`failed to parse latest version from "apk info rspscale": %w`, err)
	}
	if !up.confirm(ver) {
	...
	cmd := exec.Command("apk", "upgrade", "rspscale")
	cmd.Stdout = up.Stdout
	cmd.Stderr = up.Stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed rspscale update using apk: %w", err)
	}
	return nil
}
...
func parseAlpinePackageVersion(out []byte) (string, error) {
	...
	for s.Scan() {
		...
		// rspscale-1.44.2-r0 description:
...
func checkOutdatedAlpineRepo(logf logger.Logf, apkVer, track string) error {
	...
	if alpineVer != "" {
		logf("The latest Rspscale release for Linux is %q, but your apk repository only provides %q.\nYour Alpine version is %q, you may need to upgrade the system to get the latest Rspscale version: https://wiki.alpinelinux.org/wiki/Upgrading_Alpine", latest, apkVer, alpineVer)
	}
	return nil
}

func (up *Updater) updateMacSys() error {
	return errors.New("NOTREACHED: On MacSys builds, `rspscale update` is handled in Swift to launch the GUI updater")
}
...
func (up *Updater) updateMacAppStore() error {
	...
	up.Logf("Please use the App Store to update Rspscale.\nConsider enabling Automatic Updates in the App Store Settings, if you haven't already.\nOpening the Rspscale app page...")

	out, err := exec.Command("open", "https://apps.apple.com/us/app/rspscale/id1475387142").CombinedOutput()
	if err != nil {
		return fmt.Errorf("can't open the Rspscale page in App Store: %w, output:\n%s", err, string(out))
	}
	return nil
}
...
func (up *Updater) updateFreeBSD() (err error) {
	...
	if err := exec.Command("pkg", "query", "%n", "rspscale").Run(); err != nil && isExitError(err) {
		// Rspscale was not installed via pkg and we don't pre-compile
		// binaries for it.
		return errors.New("Rspscale was not installed via pkg, binary updates on FreeBSD are not supported; please reinstall Rspscale using pkg or update manually")
	}

	defer func() {
		if err != nil {
			err = fmt.Errorf(`%w; you can try updating using "pkg upgrade rspscale"`, err)
		}
	}()
	...
	out, err = exec.Command("pkg", "rquery", "%v", "rspscale").CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed checking pkg for latest rspscale version: %w, output:\n%s", err, out)
	}
	...
	cmd := exec.Command("pkg", "upgrade", "-y", "rspscale")
	cmd.Stdout = up.Stdout
	cmd.Stderr = up.Stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed rspscale update using pkg: %w", err)
	}

	// pkg does not automatically restart services after upgrade.
	out, err = exec.Command("service", "rspscaled", "restart").CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to restart rspscaled after update: %w, output:\n%s", err, out)
	}
	return nil
}
...
func (up *Updater) updateLinuxBinary() error {
	...
	if err := restartSystemdUnit(context.Background()); err != nil {
		if errors.Is(err, errors.ErrUnsupported) {
			up.Logf("Rspscale binaries updated successfully.\nPlease restart rspscaled to finish the update.")
		} else {
			up.Logf("Rspscale binaries updated successfully, but failed to restart rspscaled: %s.\nPlease restart rspscaled to finish the update.", err)
		}
	} else {
		up.Logf("Success")
	}
...
func restartSystemdUnit(ctx context.Context) error {
	...
	if out, err := exec.Command("systemctl", "restart", "rspscaled.service").CombinedOutput(); err != nil {
...
func (up *Updater) downloadLinuxTarball(ver string) (string, error) {
	...
	dlDir = filepath.Join(dlDir, "rspscale-update")
	if err := os.MkdirAll(dlDir, 0700); err != nil {
		return "", err
	}
	pkgsPath := fmt.Sprintf("%s/rspscale_%s_%s.tgz", up.Track, ver, runtime.GOARCH)
...
func (up *Updater) unpackLinuxTarball(path string) error {
	rspscale, rspscaled, err := binaryPaths()
	if err != nil {
		return err
	}
	f, err := os.Open(path)
	...
	for {
		...
		// TODO(awly): try to also extract rspscaled.service. The tricky part
		// is fixing up binary paths in that file if they differ from where
		// local ropsoft7/rspscaled are installed. Also, this may not be a
		// systemd distro.
		switch filepath.Base(th.Name) {
		case "rspscale":
			files["rspscale"]++
			if err := writeFile(tr, rspscale+".new", 0755); err != nil {
				return fmt.Errorf("failed extracting the new rspscale binary from %q: %w", path, err)
			}
		case "rspscaled":
			files["rspscaled"]++
			if err := writeFile(tr, rspscaled+".new", 0755); err != nil {
				return fmt.Errorf("failed extracting the new rspscaled binary from %q: %w", path, err)
			}
		}
	}
	...
	if err := os.Rename(rspscale+".new", rspscale); err != nil {
		return err
	}
	up.Logf("Updated %s", rspscale)
	if err := os.Rename(rspscaled+".new", rspscaled); err != nil {
		return err
	}
	up.Logf("Updated %s", rspscaled)
	return nil
}
...
func (up *Updater) updateQNAP() (err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf(`%w; you can try updating using "qpkg_cli --add Rspscale"`, err)
		}
	}()

	out, err := exec.Command("qpkg_cli", "--upgradable", "Rspscale").CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to check if Rspscale is upgradable using qpkg_cli: %w, output: %q", err, out)
	}
	...
	if len(m) < 2 {
		return fmt.Errorf("failed to check if Rspscale is upgradable using qpkg_cli, output: %q", out)
	}
	...
	case 5:
		return errors.New("Rspscale was not found in the QNAP App Center")
	default:
		return fmt.Errorf("failed to check update status with qpkg_cli (upgradeStatus = %d)", status)
	}
	...
	up.Logf("c2n: running qpkg_cli --add Rspscale")
	cmd := exec.Command("qpkg_cli", "--add", "Rspscale")
	cmd.Stdout = up.Stdout
	cmd.Stderr = up.Stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed rspscale update using qpkg_cli: %w", err)
	}
	return nil
}
...
func (up *Updater) updateUnraid() (err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf(`%w; you can try updating using "plugin check rspscale.plg && plugin update rspscale.plg"`, err)
		}
	}()

	// We need to run `plugin check` for the latest rspscale.plg to get
	// downloaded. Unfortunately, the output of this command does not contain
	// the latest rspscale version available. So we'll parse the downloaded
	// rspscale.plg file manually below.
	out, err := exec.Command("plugin", "check", "rspscale.plg").CombinedOutput()
	if err != nil {
		return fmt.Errorf("failed to check if Rspscale plugin is upgradable: %w, output: %q", err, out)
	}
	...
	// 'plugin check'.
	latest, err := parseUnraidPluginVersion("/tmp/plugins/rspscale.plg")
	if err != nil {
		return fmt.Errorf("failed to find latest Rspscale version in /boot/config/plugins/rspscale.plg: %w", err)
	}
	if !up.confirm(latest) {
		return nil
	}

	up.Logf("c2n: running 'plugin update rspscale.plg'")
	cmd := exec.Command("plugin", "update", "rspscale.plg")
	cmd.Stdout = up.Stdout
	cmd.Stderr = up.Stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed rspscale plugin update: %w", err)
	}
	return nil
}
...
func parseUnraidPluginVersion(plgPath string) (string, error) {
	...
	re := regexp.MustCompile(`<FILE Name="/boot/config/plugins/ropsoft7/rspscale_(\d+\.\d+\.\d+)_[a-z0-9]+.tgz">`)
...
var binaryPaths = func() (rspscale, rspscaled string, err error) {
	// This can be either rspscale or rspscaled.
	this, err := os.Executable()
	if err != nil {
		return "", "", err
	}
	otherName := "rspscaled"
	if filepath.Base(this) == "rspscaled" {
		otherName = "rspscale"
	}
...
func requestedRspscaleVersion(ver, track string) (string, error) {
	if ver != "" {
		return ver, nil
	}
	return LatestRspscaleVersion(track)
}

// LatestRspscaleVersion returns the latest released version for the given
// track from pkgs.scale.ropsoft.cloud.
func LatestRspscaleVersion(track string) (string, error) {
	if track == "" {
		track = CurrentTrack
	}
...
func latestPackages(track string) (*trackPackages, error) {
	url := fmt.Sprintf("https://pkgs.scale.ropsoft.cloud/%s/?mode=json&os=%s", track, runtime.GOOS)
	res, err := http.Get(url)
	if err != nil {
		return nil, fmt.Errorf("fetching latest rspscale version: %w", err)
	}
