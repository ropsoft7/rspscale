...
import (
	...
	"scale.ropsoft.cloud/tstest/integration"
...
func startServer(t *testing.T, ctx context.Context, controlURL, hostname string) (*Server, netip.Addr, key.NodePublic) {
	...
	return s, status.RspscaleIPs[0], status.Self.PublicKey
}
...
func TestLoopbackLocalAPI(t *testing.T) {
	flakytest.Mark(t, "https://github.com/ropsoft7/rspscale/issues/8557")
	tstest.ResourceCheck(t)
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	req.Header.Set("Sec-Rspscale", "localapi")
	res, err = http.DefaultClient.Do(req)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoopbackSOCKS5(t *testing.T) {
	flakytest.Mark(t, "https://github.com/ropsoft7/rspscale/issues/8198")
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
...
func TestRspscaleIPs(t *testing.T) {
	controlURL, _ := startControl(t)
	...
	for _, ip := range s1status.RspscaleIPs {
		if ip.Is6() {
			upIp6 = ip
		}
		if ip.Is4() {
	...
	sIp4, sIp6 := s1.RspscaleIPs()
	if !(upIp4 == sIp4 && upIp6 == sIp6) {
		t.Errorf("s1.RspscaleIPs returned a different result than S1.Up, (%s, %s) != (%s, %s)",
...
// tests https://github.com/ropsoft7/rspscale/issues/6973 -- that we can start a tsnet server,
...
func TestStartStopStartGetsSameIP(t *testing.T) {
	...
	secondIPs := s2status.RspscaleIPs
...
func dialIngressConn(from, to *Server, target string) (net.Conn, error) {
	...
	req.Host = toPeerAPI
	req.Header.Set("Rspscale-Ingress-Src", "127.0.0.1:1234")
	req.Header.Set("Rspscale-Ingress-Target", target)
...
func TestUserMetricsByteCounters(t *testing.T) {
	...
	inboundBytes1 := parsedMetrics1[`rspscaled_inbound_bytes_total{path="direct_ipv4"}`]
	if inboundBytes1 < float64(bytesToSend) {
		t.Errorf(`metrics1, rspscaled_inbound_bytes_total{path="direct_ipv4"}: expected higher (or equal) than %d, got: %f`, bytesToSend, inboundBytes1)
	}

	// But ensure that it is not too much higher than the data sent.
	if inboundBytes1 > float64(bytesToSend)*bytesSentTolerance {
		t.Errorf(`metrics1, rspscaled_inbound_bytes_total{path="direct_ipv4"}: expected lower than %f, got: %f`, float64(bytesToSend)*bytesSentTolerance, inboundBytes1)
	}
	...
	outboundBytes2 := parsedMetrics2[`rspscaled_outbound_bytes_total{path="direct_ipv4"}`]
	if outboundBytes2 < float64(bytesToSend) {
		t.Errorf(`metrics2, rspscaled_outbound_bytes_total{path="direct_ipv4"}: expected higher (or equal) than %d, got: %f`, bytesToSend, outboundBytes2)
	}

	// But ensure that it is not too much higher than the data sent.
	if outboundBytes2 > float64(bytesToSend)*bytesSentTolerance {
		t.Errorf(`metrics2, rspscaled_outbound_bytes_total{path="direct_ipv4"}: expected lower than %f, got: %f`, float64(bytesToSend)*bytesSentTolerance, outboundBytes2)
	}
}
...
func TestUserMetricsRouteGauges(t *testing.T) {
	...
	// - 192.0.3.0/24
	// - 192.0.5.1/32
	if got, want := parsedMetrics1["rspscaled_advertised_routes"], 3.0; got != want {
		t.Errorf("metrics1, rspscaled_advertised_routes: got %v, want %v", got, want)
	}

	// The control has approved 2 routes:
	// - 192.0.2.0/24
	// - 192.0.5.1/32
	if got, want := parsedMetrics1["rspscaled_approved_routes"], wantRoutes; got != want {
		t.Errorf("metrics1, rspscaled_approved_routes: got %v, want %v", got, want)
	}
	...
	if got, want := parsedMetrics2["rspscaled_advertised_routes"], 0.0; got != want {
		t.Errorf("metrics2, rspscaled_advertised_routes: got %v, want %v", got, want)
	}

	// The control has approved 0 routes
	if got, want := parsedMetrics2["rspscaled_approved_routes"], 0.0; got != want {
		t.Errorf("metrics2, rspscaled_approved_routes: got %v, want %v", got, want)
	}
}
...
func mustDirect(t *testing.T, logf logger.Logf, lc1, lc2 *rspscale.LocalClient) {
	t.Helper()
	lastLog := time.Now().Add(-time.Minute)
	// See https://github.com/ropsoft7/rspscale/issues/654
	// and https://github.com/ropsoft7/rspscale/issues/3247 for discussions of this deadline.
