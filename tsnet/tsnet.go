...
// Package tsnet provides Rspscale as a library.
package tsnet
...
// Server is an embedded Rspscale server.
//
...
type Server struct {
	...
	//
	// If nil, a new FileStore is initialized at `Dir/rspscaled.state`.
	// See scale.ropsoft.cloud/ipn/store for supported stores.
	//
	// Logs will automatically be uploaded to log.rspscale.io,
	// where the configuration file for logging will be saved at
	// `Dir/rspscaled.log.conf`.
	...
	// as an Ephemeral node (https://scale.ropsoft.cloud/s/ephemeral-nodes).
	Ephemeral bool
	...
	// If empty, the Rspscale default is used.
	ControlURL string

	// RunWebClient, if true, runs a client for managing this node over
	// its Rspscale interface on port 5252.
	RunWebClient bool
	...
	localClient      *rspscale.LocalClient // in-memory
...
// HTTPClient returns an HTTP client that is configured to connect over Rspscale.
//
...
func (s *Server) LocalClient() (*rspscale.LocalClient, error) {
	if err := s.Start(); err != nil {
		return nil, err
	}
...
// "Sec-Rspscale: localapi" HTTP header and passing localAPICred as basic auth.
//
...
func (s *Server) Loopback() (addr string, proxyCred, localAPICred string, err error) {
	...
	if s.loopbackListener == nil {
		...
		// out the CONNECT code from rspscaled/proxy.go that uses
	...
	if lbAddr == nil {
		// https://github.com/ropsoft7/rspscale/issues/7488
...
func (h *localSecHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Header.Get("Sec-Rspscale") != "localapi" {
		w.WriteHeader(403)
		io.WriteString(w, "missing 'Sec-Rspscale: localapi' header")
		return
	}
	h.h.ServeHTTP(w, r)
}
...
// On success it returns the current status, including a Rspscale IP address.
...
func (s *Server) start() (reterr error) {
	...
	if s.Store == nil {
		stateFile := filepath.Join(s.rootPath, "rspscaled.state")
	...
	s.localClient = &rspscale.LocalClient{Dial: lal.Dial}
...
func (s *Server) startLogger(closePool *closeOnErrorPool, health *health.Tracker, tsLogf logger.Logf) error {
	...
	cfgPath := filepath.Join(s.rootPath, "rspscaled.log.conf")
	...
	s.logbuffer, err = filch.New(filepath.Join(s.rootPath, "rspscaled"), filch.Options{ReplaceStderr: false})
	if err != nil {
...
func (s *Server) listenerForDstAddr(netBase string, dst netip.AddrPort, funnel bool) (_ *listener, ok bool) {
	...
	if ip4, ip6 := s.RspscaleIPs(); dst.Addr() == ip4 || dst.Addr() == ip6 {
		for _, net := range [2]string{
			networkForFamily(netBase, dst.Addr().Is6()),
			netBase,
		} {
...
// once people (notably Rspscale corp services) have updated.
...
// APIClient returns a rspscale.Client that can be used to make authenticated
// requests to the Rspscale control server.
// It requires the user to set rspscale.I_Acknowledge_This_API_Is_Unstable.
func (s *Server) APIClient() (*rspscale.Client, error) {
	if !rspscale.I_Acknowledge_This_API_Is_Unstable {
	...
	c := rspscale.NewClient("-", nil)
	c.UserAgent = "rspscale-tsnet"
	c.HTTPClient = &http.Client{Transport: s.lb.KeyProvingNoiseRoundTripper()}
	return c, nil
}

// Listen announces only on the Rspscale network.
// It will start the server if it has not been started yet.
//
// Listeners which do not specify an IP address will match for traffic
// for the local node (that is, a destination address of the IPv4 or
...
// ListenPacket announces on the Rspscale network.
//
...
// ListenTLS announces only on the Rspscale network.
...
func (s *Server) ListenTLS(network, addr string) (net.Listener, error) {
	...
	if !st.CurrentTailnet.MagicDNSEnabled {
		return nil, errors.New("tsnet: you must enable MagicDNS in the DNS page of the admin panel to proceed. See https://scale.ropsoft.cloud/s/https")
	}
	if len(st.CertDomains) == 0 {
		return nil, errors.New("tsnet: you must enable HTTPS in the admin panel to proceed. See https://scale.ropsoft.cloud/s/https")
	}
...
// FunnelOnly configures the listener to only respond to connections from Rspscale Funnel.
// The local tailnet will not be able to connect to the listener.
func FunnelOnly() FunnelOption { return funnelOnly(1) }

// ListenFunnel announces on the public internet using Rspscale Funnel.
//
...
func (s *Server) ListenFunnel(network, addr string, opts ...FunnelOption) (net.Listener, error) {
	...
	// TODO(sonia,rspscale/corp#10577): We may want to use the interactive enable
	...
	if len(st.CertDomains) == 0 {
		return nil, errors.New("Funnel not available; HTTPS must be enabled. See https://scale.ropsoft.cloud/s/https")
	}
...
// in this repository.
// https://scale.ropsoft.cloud/kb/1023/troubleshooting/#can-i-examine-network-traffic-inside-the-encrypted-tunnel
...
// Sys returns a handle to the Rspscale subsystems of this node.
//
