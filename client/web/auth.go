...
var (
	...
	errNotUsingRspscale  = errors.New("not-using-rspscale")
...
//
//   - (errNotUsingRspscale) The request was not made over rspscale.
//
...
// getRspscaleBrowserSession does not check whether the session has been
...
// unless getRspscaleBrowserSession reports errNotUsingRspscale.
func (s *Server) getSession(r *http.Request) (*browserSession, *apitype.WhoIsResponse, *ipnstate.Status, error) {
	...
	switch {
	case whoIsErr != nil:
		return nil, nil, status, errNotUsingRspscale
	case statusErr != nil:
		return nil, whoIs, nil, statusErr
	case status.Self == nil:
		return nil, whoIs, status, errors.New("missing self node in rspscale status")
	...
	if session.SrcNode != srcNode || session.SrcUser != srcUser {
		// In this case the browser cookie is associated with another rspscale node.
...
// We assume that only "scale.ropsoft.cloud" control servers support check mode.
...
func (s *Server) controlSupportsCheckMode(ctx context.Context) bool {
	...
	return strings.HasSuffix(controlURL.Host, ".scale.ropsoft.cloud")
}
