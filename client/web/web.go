...
// Package web provides the Rspscale client for web.
package web
...
// ListenPort is the static port used for the web client when run inside rspscaled.
// (5252 are the numbers above the letters "TSTS" on a qwerty keyboard.)
const ListenPort = 5252

// Server is the backend server for a Rspscale web client.
type Server struct {
	mode ServerMode

	logf    logger.Logf
	lc      *rspscale.LocalClient
	...
	// full management web client, which is only accessible over Rspscale.
	//
	// Users obtain a valid browser session by connecting to the web client
	// over Rspscale and verifying their identity by authenticating on the
	// control server.
	//
...
const (
	...
	// node's current Rspscale settings.
	//
	...
	// ManageServerMode serves a management client for editing rspscale
	// settings of a node.
	//
	// This mode restricts the app to only being assessible over Rspscale,
	// and API calls are authenticated via browser sessions associated with
	// the source's Rspscale identity. If the source browser does not have
...
type ServerOpts struct {
	...
	// LocalClient is the rspscale.LocalClient to use for this web server.
	// If nil, a new one will be created.
	LocalClient *rspscale.LocalClient
...
// NewServer constructs a new Rspscale web client server.
...
func NewServer(opts ServerOpts) (s *Server, err error) {
	...
	if opts.LocalClient == nil {
		opts.LocalClient = &rspscale.LocalClient{}
	}
	s = &Server{
		mode:        opts.Mode,
		logf:        opts.Logf,
...
// ServeHTTP processes all requests for the Rspscale web client.
...
func (s *Server) serve(w http.ResponseWriter, r *http.Request) {
	if s.mode == ManageServerMode {
		// In manage mode, requests must be sent directly to the bare Rspscale IP address.
		// If a request comes in on any other hostname, redirect.
		if s.requireRspscaleIP(w, r) {
...
// requireRspscaleIP redirects an incoming request if the HTTP request was not made to a bare Rspscale IP address.
// The request will be redirected to the Rspscale IP, port 5252, with the original request path.
...
func (s *Server) requireRspscaleIP(w http.ResponseWriter, r *http.Request) (handled bool) {
	const (
		ipv4ServiceHost = tsaddr.RspscaleServiceIPString
		ipv6ServiceHost = "[" + tsaddr.RspscaleServiceIPv6String + "]"
	)
	...
	if r.Host == fmt.Sprintf("%s:%d", ipv4.String(), ListenPort) {
		return false // already accessing over Rspscale IP
	}
	if r.Host == fmt.Sprintf("[%s]:%d", ipv6.String(), ListenPort) {
		return false // already accessing over Rspscale IP
	}

	// Not currently accessing via Rspscale IP,
	// redirect them.
...
// selfNodeAddresses return the Rspscale IPv4 and IPv6 addresses for the self node.
...
func (s *Server) selfNodeAddresses(r *http.Request, st *ipnstate.Status) (ipv4, ipv6 netip.Addr) {
	for _, ip := range st.Self.RspscaleIPs {
		if ip.Is4() {
			ipv4 = ip
		} else if ip.Is6() {
			ipv6 = ip
...
func (s *Server) authorizeRequest(w http.ResponseWriter, r *http.Request) (ok bool) {
	if s.mode == ManageServerMode { // client using rspscale auth
		session, _, _, err := s.getSession(r)
		switch {
		case errors.Is(err, errNotUsingRspscale):
			// All requests must be made over rspscale.
			http.Error(w, "must access over rspscale", http.StatusUnauthorized)
			return false
...
func (a *apiHandler[data]) getPeer() (peerCapabilities, error) {
	// TODO(rspscale/corp#16695,sonia): We also call StatusWithoutPeers and
...
// viewerIdentity is the Rspscale identity of the source node
...
func (s *Server) serveAPIAuth(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case sErr != nil && errors.Is(sErr, errNotUsingRspscale):
	...
	default:
		if whois == nil || (whois.Node.StableID == status.Self.ID) {
			// whois being nil implies local as the request did not come over Rspscale.
			s.lc.IncrementCounter(r.Context(), "web_client_viewing_local", 1)
		} else {
...
func (s *Server) serveAPIAuthSessionNew(w http.ResponseWriter, r *http.Request) {
	...
	if session == nil {
		...
		http.SetCookie(w, &http.Cookie{
			...
			// (but only on Rspscale IPs, hence over encrypted
...
func (s *Server) serveGetNodeData(w http.ResponseWriter, r *http.Request) {
	...
	if e := st.ExitNodeStatus; e != nil {
		...
		if data.UsingExitNode.Name == "" {
			// Falling back to RspscaleIP/StableNodeID when the peer
			// is no longer included in status.
			if len(e.RspscaleIPs) > 0 {
				data.UsingExitNode.Name = e.RspscaleIPs[0].Addr().String()
			} else {
...
func availableFeatures() map[string]bool {
	features := map[string]bool{
		...
		"ssh":                 featureknob.CanRunRspscaleSSH() == nil,
...
// rspscaleUp starts the daemon with the provided options.
// If reauthentication has been requested, an authURL is returned to complete device registration.
func (s *Server) rspscaleUp(ctx context.Context, st *ipnstate.Status, opt rspscaleUpOptions) (authURL string, retErr error) {
...
type rspscaleUpOptions struct {
	// If true, force reauthentication of the client.
	// Otherwise simply reconnect, the same as running `rspscale up`.
	Reauthenticate bool
...
// serveRspscaleUp serves requests to /api/up.
// If the user needs to authenticate, an authURL is provided in the response.
func (s *Server) serveRspscaleUp(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	...
	s.logf("rspscaleUp(reauth=%v) ...", opt.Reauthenticate)
	url, err := s.rspscaleUp(r.Context(), st, opt)
	s.logf("rspscaleUp = (URL %v, %v)", url != "", err)
	if err != nil {
...
func (s *Server) proxyRequestToLocalAPI(w http.ResponseWriter, r *http.Request) {
	...
	// Make request to rspscaled localapi.
...
func (s *Server) csrfKey() []byte {
	csrfFile := filepath.Join(os.TempDir(), "rspscale-web-csrf.key")
