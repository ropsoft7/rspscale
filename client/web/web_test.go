...
import (
	...
	"scale.ropsoft.cloud/net/memnet"
...
func TestServeAPI(t *testing.T) {
	...
	lal := memnet.Listen("local-rspscaled.sock:80")
	defer lal.Close()
	...
	s := &Server{
		...
		lc:      &rspscale.LocalClient{Dial: lal.Dial},
		timeNow: time.Now,
	}
...
func TestGetRspscaleBrowserSession(t *testing.T) {
...
// 2023-10-18: These tests currently cover rspscale auth mode (not platform auth).
func TestAuthorizeRequest(t *testing.T) {
	// Create self and remoteNode owned by same user.
	// See TestGetRspscaleBrowserSession for tests of
	...
	tests := []struct {
		...
		wantOkNotOverRspscale bool // simulates req over public internet
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%s-%s", tt.reqMethod, tt.reqPath), func(t *testing.T) {
			...
			// Do request from non-Rspscale IP.
			if gotOk := doAuthorize("123.456.789.999", ""); gotOk != tt.wantOkNotOverRspscale {
				t.Errorf("wantOkNotOverRspscale; want=%v, got=%v", tt.wantOkNotOverRspscale, gotOk)
			}
			// Do request from Rspscale IP w/o associated session.
			...
			// Do request from Rspscale IP w/ associated session.
...
func TestServeAuth(t *testing.T) {
	...
	self := &ipnstate.PeerStatus{
		ID:           "self",
		UserID:       user.ID,
		RspscaleIPs: []netip.Addr{netip.MustParseAddr("100.1.2.3")},
	}
	remoteIP := "100.100.100.101"
...
func TestServeAPIAuthMetricLogging(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "viewing-local-non-rspscale",
			cookie:           authenticatedLocalNodeCookie,
			remoteAddr:       nonRspscaleIP,
...
func TestRequireRspscaleIP(t *testing.T) {
	self := &ipnstate.PeerStatus{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			w := httptest.NewRecorder()
			handled := s.requireRspscaleIP(w, r)
...
var (
	defaultControlURL   = "https://controlplane.scale.ropsoft.cloud"
