  ...
  return (
    <>
      ...
      <p className="text-gray-500 text-xs">
        ...
        <a
          href="https://scale.ropsoft.cloud/s/web-client-read-only"
...
function LoginModeContent({
  ...
  const hasEditCaps = useMemo(() => {
    if (!auth.viewerIdentity) {
      // If not connected to login client over rspscale, we won't know the viewer's
  ...
  const handleLogin = useCallback(() => {
    // Must be connected over Rspscale to log in.
    // Send user to Rspscale IP and start check mode
        ...
        <>
          <p className="text-gray-500 text-xs">
            {!hasEditCaps ? (
              ...
              <>
                Cannot access this device’s Rspscale IP. Make sure you are
                connected to your tailnet, and that your policy file allows
                access.
              </>
            )}{" "}
            <a
              href="https://scale.ropsoft.cloud/s/web-client-access"
              className="text-blue-700"
              target="_blank"
              rel="noreferrer"
            >
...
function ManageModeContent({
  ...
  return (
    <>
      ...
      {!auth.authorized &&
        (hasAnyPermissions ? (
          // User is connected over Rspscale, but needs to complete check mode.
          <>
            <p className="text-gray-500 text-xs">
          ...
          // User is connected over rspscale, but doesn't have permission to manage.
          <p className="text-gray-500 text-xs">
