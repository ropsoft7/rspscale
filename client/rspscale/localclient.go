...
import (
	...
	"scale.ropsoft.cloud/util/syspolicy/setting"
)
...
// LocalClient is a client to Rspscale's "LocalAPI", communicating with the
// Rspscale daemon on the local machine. Its API is not necessarily stable and
...
type LocalClient struct {
	...
	// machine's rspscaled or equivalent. If nil, a default is used.
	Dial func(ctx context.Context, network, addr string) (net.Conn, error)

	// Socket specifies an alternate path to the local Rspscale socket.
	// If empty, a platform-specific default is used.
	Socket string

	// UseSocketOnly, if true, tries to only connect to rspscaled via the
	// Unix socket and not via fallback mechanisms as done on macOS when
	// connecting to the GUI client variants.
	UseSocketOnly bool

	// OmitAuth, if true, omits sending the local Rspscale daemon any
	...
	// tsClient does HTTP requests to the local Rspscale daemon.
...
func (lc *LocalClient) socket() string {
	...
	return paths.DefaultRspscaledSocket()
}
...
func (lc *LocalClient) defaultDialer(ctx context.Context, network, addr string) (net.Conn, error) {
	if addr != "local-rspscaled.sock:80" {
...
// DoLocalRequest makes an HTTP request to the local machine's Rspscale daemon.
//
// URLs are of the form http://local-rspscaled.sock/localapi/v0/whois?ip=1.2.3.4.
//
// The hostname must be "local-rspscaled.sock", even though it
// doesn't actually do any DNS lookup. The actual means of connecting to and
// authenticating to the local Rspscale daemon vary by platform.
//
// DoLocalRequest may mutate the request to add Authorization headers.
func (lc *LocalClient) DoLocalRequest(req *http.Request) (*http.Response, error) {
	req.Header.Set("Rspscale-Cap", strconv.Itoa(int(tailcfg.CurrentCapabilityVersion)))
...
func (lc *LocalClient) doLocalRequestNiceError(req *http.Request) (*http.Response, error) {
	...
	if err == nil {
		if server := res.Header.Get("Rspscale-Version"); server != "" && server != envknob.IPCVersion() && onVersionMismatch != nil {
	...
	if ue, ok := err.(*url.Error); ok {
		if oe, ok := ue.Err.(*net.OpError); ok && oe.Op == "dial" {
			...
			return nil, fmt.Errorf("Failed to connect to local Rspscale daemon for %s; %s Error: %w", pathPrefix, rspscaledConnectHint(), oe)
		}
	}
	return nil, err
}
...
//
// For connections proxied by rspscaled, this looks up the owner of the given
...
// Goroutines returns a dump of the Rspscale daemon's current goroutines.
func (lc *LocalClient) Goroutines(ctx context.Context) ([]byte, error) {
	return lc.get200(ctx, "/localapi/v0/goroutines")
}

// DaemonMetrics returns the Rspscale daemon's metrics in
...
// IncrementCounter increments the value of a Rspscale daemon's counter
...
// TailDaemonLogs returns a stream the Rspscale daemon's logs as they arrive.
...
// Pprof returns a pprof profile of the Rspscale daemon.
...
// BugReportOpts contains options to pass to the Rspscale daemon when
...
// user with support.
//
// The opts type specifies options to pass to the Rspscale daemon when
...
// Status returns the Rspscale daemon's status.
...
// StatusWithoutPeers returns the Rspscale daemon's status, without the peer info.
...
// received by the Rspscale daemon in its staging/cache directory but not yet
...
// CheckIPForwarding asks the local Rspscale daemon whether it looks like the
...
// CheckUDPGROForwarding asks the local Rspscale daemon whether it looks like
...
// See https://scale.ropsoft.cloud/kb/1320/performance-best-practices#linux-optimizations-for-subnet-routers-and-exit-nodes
...
// That is, it returns the DNS configuration that the system would use if Rspscale weren't being used.
...
//
// This is a low-level interface; it's expected that most Rspscale
...
// DialTCP connects to the host's port via Rspscale.
//
// The host may be a base DNS name (resolved from the netmap inside
// rspscaled), a FQDN, or an IP address.
//
...
// UserDial connects to the host's port via Rspscale for the given network.
//
// The host may be a base DNS name (resolved from the netmap inside rspscaled),
...
// CurrentDERPMap returns the current DERPMap that is being used by the local rspscaled.
...
// rspscaledConnectHint gives a little thing about why rspscaled (or
// platform equivalent) is not answering localapi connections.
//
// It ends in a punctuation. See caller.
func rspscaledConnectHint() string {
	if runtime.GOOS != "linux" {
		// TODO(bradfitz): flesh this out
		return "not running?"
	}
	out, err := exec.Command("systemctl", "show", "rspscaled.service", "--no-page", "--property", "LoadState,ActiveState,SubState").Output()
	if err != nil {
		return "not running?"
	}
	// Parse:
	...
	if st["LoadState"] == "loaded" &&
		...
		return "systemd rspscaled.service not running."
	}
	return "not running?"
}
...
// IPNBusWatcher is an active subscription (watch) of the local rspscaled IPN bus.
