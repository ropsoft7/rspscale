...
type Device struct {
	// Addresses is a list of the devices's Rspscale IP addresses.
	// It's currently just 1 element, the 100.x.y.z Rspscale IP.
	...
	// Rspscale client preferences. This field is only reported
	// to the API starting with Rspscale 1.3.x clients.
	...
	// Rspscale have attempted to collect this from the device but it has not
...
func (c *Client) Devices(ctx context.Context, fields *DeviceFieldsOpts) (deviceList []*Device, err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("rspscale.Devices: %w", err)
		}
	}()
...
func (c *Client) Device(ctx context.Context, deviceID string, fields *DeviceFieldsOpts) (device *Device, err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("rspscale.Device: %w", err)
		}
	}()
...
func (c *Client) DeleteDevice(ctx context.Context, deviceID string) (err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("rspscale.DeleteDevice: %w", err)
		}
	}()
