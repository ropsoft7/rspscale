...
import (
	...
	"scale.ropsoft.cloud/net/wsconn"
	"scale.ropsoft.cloud/types/key"
)

// AcceptHTTP upgrades the HTTP request given by w and r into a Rspscale
...
func acceptHTTP(ctx context.Context, w http.ResponseWriter, r *http.Request, private key.MachinePrivate, earlyWrite func(protocolVersion int, w io.Writer) error) (_ *controlbase.Conn, retErr error) {
	...
	if initB64 == "" {
		http.Error(w, "missing Rspscale handshake header", http.StatusBadRequest)
		return nil, errors.New("no rspscale handshake header in HTTP request")
	}
	init, err := base64.StdEncoding.DecodeString(initB64)
	if err != nil {
		http.Error(w, "invalid rspscale handshake header", http.StatusBadRequest)
...
// speak HTTP) to a Rspscale control protocol base transport connection.
func acceptWebsocket(ctx context.Context, w http.ResponseWriter, r *http.Request, private key.MachinePrivate) (*controlbase.Conn, error) {
	...
	if initB64 == "" {
		c.Close(websocket.StatusPolicyViolation, "missing Rspscale handshake parameter")
		return nil, errors.New("no rspscale handshake parameter in HTTP request")
	}
	init, err := base64.StdEncoding.DecodeString(initB64)
	if err != nil {
		c.Close(websocket.StatusPolicyViolation, "invalid rspscale handshake parameter")
