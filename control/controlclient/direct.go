...
import (
	...
	"scale.ropsoft.cloud/util/singleflight"
...
func NewDirect(opts Options) (*Direct, error) {
	...
	if strings.Contains(opts.ServerURL, "controlplane.scale.ropsoft.cloud") && envknob.Bool("TS_PANIC_IF_HIT_MAIN_CONTROL") {
		c.panicOnUse = true
	}
	return c, nil
}
...
func (c *Direct) TryLogin(ctx context.Context, flags LoginFlags) (url string, err error) {
	if strings.Contains(c.serverURL, "controlplane.scale.ropsoft.cloud") && envknob.Bool("TS_PANIC_IF_HIT_MAIN_CONTROL") {
...
var macOSScreenTime = health.Register(&health.Warnable{
	...
	Title:    "Rspscale blocked by Screen Time",
	Text: func(args health.Args) string {
		return "macOS Screen Time seems to be blocking Rspscale. Try disabling Screen Time in System Settings > Screen Time > Content & Privacy > Access to Web Content."
	},
...
func (c *Direct) doLogin(ctx context.Context, opt loginOpt) (mustRegen bool, newURL string, nks tkatype.MarshaledSignature, err error) {
	...
	if opt.Logout {
		...
		if expired {
			...
			systemd.Status("key expired; run 'rspscale up' to authenticate")
			regen = true
		}
