...
// from /Library/Rspscale.
//
// In that case the files are:
//
//	/Library/Rspscale/ipnport => $port (symlink with localhost port number target)
//	/Library/Rspscale/sameuserproof-$port is a file with auth
func localTCPPortAndTokenMacsys() (port int, token string, err error) {

	const dir = "/Library/Rspscale"
	...
	// /Applications/Rspscale.app, so check we can connect to avoid returning a
...
func localTCPPortAndTokenDarwin() (port int, token string, err error) {
	...
	if dir := os.Getenv("TS_MACOS_CLI_SHARED_DIR"); dir != "" {
		...
		// running the CLI via /Applications/Rspscale.app/Contents/MacOS/Rspscale
		...
		if os.Geteuid() == 0 {
			...
			warnAboutRootOnce.Do(func() {
				fmt.Fprintf(os.Stderr, "Warning: The CLI is running as root from within a sandboxed binary. It cannot reach the local rspscaled, please try again as a regular user.\n")
			})
		}
	...
	subStr := []byte(".rspscale.ipn.macos/sameuserproof-")
	for bs.Scan() {
		line := bs.Bytes()
