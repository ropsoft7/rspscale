...
func TestOmitDropLogging(t *testing.T) {
	tests := []struct {
		...
		{
			name: "v4_igmp_out", // on Windows, from https://github.com/ropsoft7/rspscale/issues/618
...
func TestLoggingPrivacy(t *testing.T) {
	...
	var (
		...
		ts6       = netip.AddrPortFrom(tsaddr.RspscaleULARange().Addr().Next(), 1234)
...
func benchmarkFile(b *testing.B, file string, opt benchOpt) {
	...
	logIPs.AddPrefix(tsaddr.RspscaleULARange())
