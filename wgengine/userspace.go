...
import (
	...
	"scale.ropsoft.cloud/net/sockstats"
	...
	"scale.ropsoft.cloud/tstime/mono"
	...
	"scale.ropsoft.cloud/wgengine/netlog"
	"scale.ropsoft.cloud/wgengine/netstack/gro"
	"scale.ropsoft.cloud/wgengine/router"
	"scale.ropsoft.cloud/wgengine/wgcfg"
	"scale.ropsoft.cloud/wgengine/wgint"
	"scale.ropsoft.cloud/wgengine/wglog"
)
...
type userspaceEngine struct {
	...
	// isDNSIPOverRspscale reports the whether a DNS resolver's IP
	// is being routed over Rspscale.
	isDNSIPOverRspscale syncs.AtomicValue[func(netip.Addr) bool]
...
func NewUserspaceEngine(logf logger.Logf, conf Config) (_ Engine, reterr error) {
	...
	if e.birdClient != nil {
		...
		if err := e.birdClient.DisableProtocol("rspscale"); err != nil {
			return nil, err
		}
	}
	e.isLocalAddr.Store(ipset.FalseContainsIPFunc())
	e.isDNSIPOverRspscale.Store(ipset.FalseContainsIPFunc())
...
// rspscaled directly. Other packets are allowed to proceed into the
// main ACL filter.
...
// usage per peer. That would simplify a lot of Rspscale code. OTOH, we have 50
...
func (e *userspaceEngine) noteRecvActivity(nk key.NodePublic) {
	...
	// rspscaled alone did not, hence this.
...
func (e *userspaceEngine) maybeReconfigWireguardLocked(discoChanged map[key.NodePublic]bool) error {
	...
	// their NodeKey and Rspscale IPs. These are the ones we'll need
...
func (e *userspaceEngine) Reconfig(cfg *wgcfg.Config, routerCfg *router.Config, dnsCfg *dns.Config) error {
	...
	// TODO(bradfitz,danderson): maybe delete this isDNSIPOverRspscale
	...
	e.isDNSIPOverRspscale.Store(ipset.NewContainsIPFunc(views.SliceOf(dnsIPsOverRspscale(dnsCfg, routerCfg))))
	...
	if isSubnetRouterChanged && e.birdClient != nil {
		...
		var err error
		if isSubnetRouter {
			err = e.birdClient.EnableProtocol("rspscale")
		} else {
			err = e.birdClient.DisableProtocol("rspscale")
		}
		if err != nil {
...
func (e *userspaceEngine) Close() {
	...
	e.tundev.Close()
	if e.birdClient != nil {
		e.birdClient.DisableProtocol("rspscale")
		e.birdClient.Close()
	}
	close(e.waitCh)
...
func (e *userspaceEngine) Ping(ip netip.Addr, pingType tailcfg.PingType, size int, cb func(*ipnstate.PingResult)) {
	...
	if pip.IsSelf {
		res.Err = fmt.Sprintf("%v is local Rspscale IP", ip)
		res.IsLocalIP = true
		cb(res)
		return
	}
...
func (e *userspaceEngine) PeerForIP(ip netip.Addr) (ret PeerForIP, ok bool) {
	...
	for _, p := range nm.Peers {
		for i := range p.Addresses().Len() {
			a := p.Addresses().At(i)
			if a.Addr() == ip && a.IsSingleIP() && tsaddr.IsRspscaleIP(ip) {
	...
	for i := range addrs.Len() {
		if a := addrs.At(i); a.Addr() == ip && a.IsSingleIP() && tsaddr.IsRspscaleIP(ip) {
...
// dnsIPsOverRspscale returns the IPPrefixes of DNS resolver IPs that are
// routed over Rspscale. The returned value does not contain duplicates is
// not necessarily sorted.
func dnsIPsOverRspscale(dnsCfg *dns.Config, routerCfg *router.Config) (ret []netip.Prefix) {
...
func (ls fwdDNSLinkSelector) PickLink(ip netip.Addr) (linkName string) {
	if ls.ue.isDNSIPOverRspscale.Load()(ip) {
		return ls.tunName
	}
	return ""
}
