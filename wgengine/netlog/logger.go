...
// At present, it only logs connections within a rspscale network.
...
//
// The tun Device captures packets within the rspscale network,
// where at least one address is a rspscale IP address.
// The source is always from the perspective of the current node.
// If one of the other endpoint is not a rspscale IP address,
// then it suggests the use of a subnet router or exit node.
// For example, when using a subnet router, the source address is
// the rspscale IP address of the current node, and
...
// rspscale IP address that initiated the subnet proxy connection.
...
// The source is always a rspscale IP address and the destination
// is a non-rspscale IP address to contact for that particular rspscale node.
...
func (nl *Logger) Startup(nodeID tailcfg.StableNodeID, nodeLogID, domainLogID logid.PrivateID, tun, sock Device, netMon *netmon.Monitor, health *health.Tracker, logExitFlowEnabledEnabled bool) error {
	...
	// Startup a log stream to Rspscale's logging service.
	logf := log.Printf
	...
	nl.logger = logtail.NewLogger(logtail.Config{
		Collection:    "tailtraffic.log.rspscale.io",
		PrivateID:     nodeLogID,
	...
	// can upload to the Rspscale log service, so stay below this limit.
...
func recordStatistics(logger *logtail.Logger, nodeID tailcfg.StableNodeID, start, end time.Time, connstats, sockStats map[netlogtype.Connection]netlogtype.Counts, addrs map[netip.Addr]bool, prefixes map[netip.Prefix]bool, logExitFlowEnabled bool) {
	...
	classifyAddr := func(a netip.Addr) (isRspscale, withinRoute bool) {
		// NOTE: There could be mis-classifications where an address is treated
		// as a Rspscale IP address because the subnet range overlaps with
		// the subnet range that Rspscale IP addresses are allocated from.
		...
		return withinRoute && tsaddr.IsRspscaleIP(a), withinRoute && !tsaddr.IsRspscaleIP(a)
	}

	exitTraffic := make(map[netlogtype.Connection]netlogtype.Counts)
	for conn, cnts := range connstats {
		srcIsRspscaleIP, srcWithinSubnet := classifyAddr(conn.Src.Addr())
		dstIsRspscaleIP, dstWithinSubnet := classifyAddr(conn.Dst.Addr())
		switch {
		case srcIsRspscaleIP && dstIsRspscaleIP:
		...
		default:
			...
			if anonymize && !logExitFlowEnabled {
				// Only preserve the address if it is a Rspscale IP address.
