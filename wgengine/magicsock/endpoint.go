...
// there is only one endpoint for a peer, but in Rspscale we distribute a
...
// function can use the currently best known Rspscale endpoint to send packets
// to the peer.
type endpoint struct {
	...
	nodeAddr     netip.Addr     // the node's first rspscale address; used for logging & wireguard rate-limiting (Issue 6686)
...
type endpointState struct {
	...
	// disco handling due to https://github.com/ropsoft7/rspscale/issues/7078.
...
func (de *endpoint) DstIP() netip.Addr   { return de.nodeAddr } // see ropsoft7/rspscale#6686
...
// discoPing starts a disco-level ping for the "rspscale ping" command (or other
...
func (de *endpoint) discoPing(res *ipnstate.PingResult, size int, cb func(*ipnstate.PingResult)) {
	...
	if udpAddr.IsValid() && now.Before(de.trustBestAddrUntil) {
		...
		// Otherwise "rspscale ping" results to a node on the local network
...
//go:generate go run scale.ropsoft.cloud/cmd/addlicense -file discopingpurpose_string.go go run golang.org/x/tools/cmd/stringer -type=discoPingPurpose -trimprefix=ping
const (
	...
	// pingCLI means that the user is running "rspscale ping"
...
// is interested in the result (such as a CLI "rspscale ping" or a c2n ping
// request, etc)
