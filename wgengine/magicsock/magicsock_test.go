...
// IP returns the Rspscale IP address assigned to this magicStack.
//
...
// and purpose-built Rspscale control plane.
...
func TestTwoDevicePing(t *testing.T) {
	flakytest.Mark(t, "https://github.com/ropsoft7/rspscale/issues/11762")
...
func assertConnStatsAndUserMetricsEqual(t *testing.T, ms *magicStack) {
	...
	// TODO(kradalby): https://github.com/ropsoft7/rspscale/issues/13420
...
// goMajorVersion reports the major Go version and whether it is a Rspscale fork.
...
func TestReceiveFromAllocs(t *testing.T) {
	...
	// Go 1.17, Rspscale fork: allow 1 alloc.
...
//
// https://github.com/ropsoft7/rspscale/issues/1391
...
// https://github.com/ropsoft7/rspscale/issues/6680: don't ignore
...
func applyNetworkMap(t *testing.T, m *magicStack, nm *netmap.NetworkMap) {
	...
	// Turn the network map into a wireguard config (for the rspscale internal wireguard device).
	cfg, err := nmcfg.WGCfg(nm, t.Logf, netmap.AllowSubnetRoutes, "")
	if err != nil {
		t.Fatal(err)
	}
	// Apply the wireguard config to the rspscale internal wireguard device.
...
func TestIsWireGuardOnlyPickEndpointByPing(t *testing.T) {
	t.Skip("This test is flaky; see https://github.com/ropsoft7/rspscale/issues/8037")
...
func TestNetworkDownSendErrors(t *testing.T) {
	...
	if !strings.Contains(resp.Body.String(), `rspscaled_outbound_dropped_packets_total{reason="error"} 1`) {
