...
import (
	...
	"scale.ropsoft.cloud/net/neterror"
	...
	"scale.ropsoft.cloud/util/ringbuffer"
...
func registerMetrics(reg *usermetric.Registry) *metrics {
	...
	inboundPacketsTotal := usermetric.NewMultiLabelMapWithRegistry[pathLabel](
		reg,
		"rspscaled_inbound_packets_total",
		"counter",
...
func (c *Conn) GetEndpointChanges(peer tailcfg.NodeView) ([]EndpointChange, error) {
	...
	if c.privateKey.IsZero() {
		c.mu.Unlock()
		return nil, fmt.Errorf("rspscaled stopped")
	}
	ep, ok := c.peerMap.endpointForNodeKey(peer.Key())
	c.mu.Unlock()
...
func (c *Conn) determineEndpoints(ctx context.Context) ([]tailcfg.Endpoint, error) {
	...
	if len(v4Addrs) >= 1 {
		...
		// port that rspscaled is running with. Worst case
	...
	// https://github.com/ropsoft7/rspscale/issues/12578
...
// msg was a Rspscale inter-node discovery message.
//
...
//   - nonce             [24]byte
//   - naclbox of payload (see scale.ropsoft.cloud/disco package for inner payload format)
//
...
func (c *Conn) handleDiscoMessage(msg []byte, src netip.AddrPort, derpNodeSrc key.NodePublic, via discoRXPath) (isDiscoMsg bool) {
	...
	if err != nil {
		...
		// newer version of Rspscale that we don't
	...
	for _, n := range nm.Peers {
		...
		if ok {
			...
			if n.DiscoKey().IsZero() && !n.IsWireGuardOnly() {
				...
				// 2. A Rspscale node key was extracted and used on a
				//    non-Rspscale node (should not enter here due to the
...
func (c *Conn) goroutinesRunningLocked() bool {
	...
	// and appear to leak, as observed in https://github.com/ropsoft7/rspscale/issues/554.
...
func (c *Conn) ReSTUN(why string) {
	...
	// user stops Rspscale via the GUI apps, ipn/local.go
...
// can't map to a Rspscale peer. But Wireguard most likely can, once it
