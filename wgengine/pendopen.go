...
type pendingOpenFlow struct {
	...
	// TSMP "reject" header.
	problem packet.RspscaleRejectReason
}
...
func (e *userspaceEngine) noteFlowProblemFromPeer(f flowtrack.Tuple, problem packet.RspscaleRejectReason) {
	e.mu.Lock()
	defer e.mu.Unlock()
	of, ok := e.pendOpen[f]
	if !ok {
...
func (e *userspaceEngine) trackOpenPreFilterIn(pp *packet.Parsed, t *tstun.Wrapper) (res filter.Response) {
	...
	if pp.IPProto == ipproto.TSMP {
		...
		rh, ok := pp.AsRspscaleRejectedHeader()
		if !ok {
			return
		}
		if rh.MaybeBroken {
...
func (e *userspaceEngine) isOSNetworkProbe(dst netip.AddrPort) bool {
	...
	// NetworkManager; https://github.com/ropsoft7/rspscale/issues/13687
