...
// Package netstack wires up gVisor's netstack into Rspscale.
package netstack
...
// (i.e. keyed by the remote Rspscale IP).
...
var (
	serviceIP   = tsaddr.RspscaleServiceIP()
	serviceIPv6 = tsaddr.RspscaleServiceIPv6()
)
...
// stack when Rspscale is running in fake mode.
type Impl struct {
	...
	// is a local (non-subnet) Rspscale IP address of this
	...
	// connections by the client ("Rspscale") IP. This is used to apply a
...
// the rspscale MTU to anything up to this size so we can potentially
...
func setTCPBufSizes(ipstack *stack.Stack) error {
	...
	// write path within Rspscale. Therefore, we loosen our understanding of
...
func Create(logf logger.Logf, tundev *tstun.Wrapper, e wgengine.Engine, mc *magicsock.Conn, dialer *tsdial.Dialer, dns *dns.Manager, pm *proxymap.Mapper) (*Impl, error) {
	...
	if runtime.GOOS == "windows" {
		// See https://github.com/ropsoft7/rspscale/issues/9707
	...
	// Add IPv4 and IPv6 default routes, so all incoming packets from the Rspscale side
...
// the host and arriving at rspscaled. This method returns filter.DropSilently
...
// host (i.e the current machine running Rspscale), in which case it will
// return true. It will return false if the packet should be sent outbound, for
// transit via WireGuard to another Rspscale node.
	...
	case header.IPv6:
		...
		if viaRange.Contains(srcIP) {
			...
			if ns.lb != nil && ns.lb.ShouldHandleViaIP(srcIP) {
				...
				//     https://github.com/ropsoft7/rspscale/issues/12448
...
// isLocalIP reports whether ip is a Rspscale IP assigned to this
...
var viaRange = tsaddr.RspscaleViaRange()
...
func (ns *Impl) shouldProcessInbound(p *packet.Parsed, t *tstun.Wrapper) bool {
	...
	// Handle TCP connection to the Rspscale IP(s) in some cases:
...
const (
	...
	// "inbound"–i.e. from Rspscale to another process on this host.
...
func (ns *Impl) acceptTCP(r *tcp.ForwarderRequest) {
	...
	isRspscaleIP := tsaddr.IsRspscaleIP(dialIP)
	...
	getConnOrReset := func(opts ...tcpip.SettableSocketOption) *gonet.TCPConn {
		...
		// TODO(ropsoft7/rspscale#4522): Netstack defaults match the Linux
...
func (ns *Impl) forwardTCP(getClient func(...tcpip.SettableSocketOption) *gonet.TCPConn, clientRemoteIP netip.Addr, wq *waiter.Queue, dialAddr netip.AddrPort) (handled bool) {
	...
	// https://github.com/ropsoft7/rspscale/issues/1616.
...
//
// dstAddr may be either a local Rspscale IP, in which we case we proxy to
...
//
// See https://github.com/ropsoft7/rspscale/issues/13654
//
