...
// See https://github.com/ropsoft7/rspscale/issues/3762
...
func TestShouldHandlePing(t *testing.T) {
	...
	t.Run("ICMP6-rspscale-addr", func(t *testing.T) {
		...
		// We don't handle this because it's a Rspscale IP and not 4via6
		if ok {
...
// looksLikeARspscaleSelfAddress reports whether addr looks like
// a Rspscale self address, for tests.
func looksLikeARspscaleSelfAddress(addr netip.Addr) bool {
	return addr.Is4() && tsaddr.IsRspscaleIP(addr) ||
		addr.Is6() && tsaddr.Rspscale4To6Range().Contains(addr)
}
	...
	testCases := []struct {
	...
	}{
		{
			...
			pkt: &packet.Parsed{
				...
				// $ rspscale debug via 7 10.1.1.9/24
			...
			afterStart: func(i *Impl) {
				...
				prefs.AdvertiseRoutes = []netip.Prefix{
					// $ rspscale debug via 7 10.1.1.0/24
				...
				i.atomicIsLocalIPFunc.Store(looksLikeARspscaleSelfAddress)
			},
		...
		},
		{
			...
			afterStart: func(i *Impl) {
				prefs := ipn.NewPrefs()
				prefs.AdvertiseRoutes = []netip.Prefix{
					// rspscale debug via 7 10.1.2.0/24
		...
		},
		{
			...
			afterStart: func(i *Impl) {
				// For testing purposes, assume all Rspscale
...
func TestTCPForwardLimits(t *testing.T) {
	...
	impl.atomicIsLocalIPFunc.Store(looksLikeARspscaleSelfAddress)
...
func TestHandleLocalPackets(t *testing.T) {
	...
	t.Run("OtherNonHandled", func(t *testing.T) {
		pkt := &packet.Parsed{
			...
			//    $ rspscale debug via 99 10.1.1.9/24
...
func TestShouldSendToHost(t *testing.T) {
	...
	testCases := []struct {
		...
		// either the local Rspscale IP or a remote host is sent
		// outbound.
		//
		...
		// shouldSendToHost function, but let's confirm.
		{
			...
			// $ rspscale debug via 4444 10.1.1.88/24
			// fd7a:115c:a1e0:b1a:0:7:a01:163/120
