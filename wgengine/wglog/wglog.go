...
// strCache holds a wireguard-go and a Rspscale style peer string.
...
// and rewrites peer keys from wireguard-go into Rspscale format.
func NewLogger(logf logger.Logf) *Logger {
	...
	wrapper := func(format string, args ...any) {
		...
		if strings.Contains(format, "Failed to send data packet") {
			// Drop. See https://github.com/ropsoft7/rspscale/issues/1239.
			return
		}
		if strings.Contains(format, "Interface up requested") || strings.Contains(format, "Interface down requested") {
			...
			// See https://github.com/ropsoft7/rspscale/issues/1388.
			return
		}
		...
		for i, arg := range newargs {
			// We want to replace *device.Peer args with the Rspscale-formatted version of themselves.
