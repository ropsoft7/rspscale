...
// See https://github.com/ropsoft7/rspscale/issues/785.
...
// set takes CIDRs to allow, and the routes that point into the Rspscale tun interface.
...
//
// local is the list of local Rspscale addresses (formatted as CIDR
// prefixes) to allow through the Windows firewall.
// killswitch, if true, enables the wireguard-windows based internet
// killswitch to prevent use of non-Rspscale default routes.
// clear, if true, removes all rspscale address firewall rules before
// adding local.
// procRule, if true, installs a firewall rule that permits the Rspscale
...
func (ft *firewallTweaker) doSet(local []string, killswitch bool, clear bool, procRule bool, allowedRoutes []netip.Prefix) error {
	if clear {
		ft.logf("clearing Rspscale-In firewall rules...")
		...
		d, _ := ft.runFirewall("delete", "rule", "name=Rspscale-In", "dir=in")
		ft.logf("cleared Rspscale-In firewall rules in %v", d)
	}
	if procRule {
		ft.logf("deleting any prior Rspscale-Process rule...")
		d, err := ft.runFirewall("delete", "rule", "name=Rspscale-Process", "dir=in") // best effort
		if err == nil {
			ft.logf("removed old Rspscale-Process rule in %v", d)
		}
		var exe string
		exe, err = os.Executable()
		if err != nil {
			ft.logf("failed to find Executable for Rspscale-Process rule: %v", err)
		} else {
			ft.logf("adding Rspscale-Process rule to allow UDP for %q ...", exe)
			d, err = ft.runFirewall("add", "rule", "name=Rspscale-Process",
				"dir=in",
				"action=allow",
				"edge=yes",
				"program="+exe,
			...
			if err != nil {
				ft.logf("error adding Rspscale-Process rule: %v", err)
			} else {
				ft.mu.Lock()
				ft.didProcRule = true
				ft.mu.Unlock()
				ft.logf("added Rspscale-Process rule in %v", d)
			}
		}
	}
	for _, cidr := range local {
		ft.logf("adding Rspscale-In rule to allow %v ...", cidr)
		var d time.Duration
		d, err := ft.runFirewall("add", "rule", "name=Rspscale-In", "dir=in", "action=allow", "localip="+cidr, "profile=private,domain", "enable=yes")
		if err != nil {
			ft.logf("error adding Rspscale-In rule to allow %v: %v", cidr, err)
			return err
		}
		ft.logf("added Rspscale-In rule to allow %v in %v", cidr, d)
	}
