...
import (
	...
	"scale.ropsoft.cloud/wgengine/winnet"
...
// the Rspscale tunnel interface's MTU to match that of the
...
// ICMP fragmentation-needed messages within rspscaled. This code may
...
func getDefaultRouteMTU() (uint32, error) {
	mtus, err := netmon.NonRspscaleMTUs()
	if err != nil {
		return 0, err
	}
...
var networkCategoryWarnable = health.Register(&health.Warnable{
	...
	Text: func(args health.Args) string {
		return fmt.Sprintf("Failed to set the network category to private on the Rspscale adapter. This may prevent Rspscale from working correctly. Error: %s", args[health.ArgError])
	},
...
func configureInterface(cfg *Config, tun *tun.NativeTun, ht *health.Tracker) (retErr error) {
	...
	iface, err := interfaceFromLUID(luid,
		...
		// coming up, if the Rspscale service comes up first,
		// the Rspscale adapter it finds might not have the
	...
	if err != nil {
		...
		log.Printf("AF_INET interface not found on Rspscale adapter, skipping IPv4 programming")
		ipif4 = nil
	}
	...
	if err != nil {
		...
		log.Printf("AF_INET6 interface not found on Rspscale adapter, skipping IPv6 programming")
		ipif6 = nil
	}
	...
	// This does not happen with single-host routes, such as routes to Rspscale IP addresses,
	...
	// arbitrary nexthop address, such as RspscaleServiceIP, to prevent the
	// routes from being marked as on-link. We can still create on-link routes
	// for single-host Rspscale routes, but we shouldn't attempt to create a
	...
	for _, addr := range cfg.LocalAddrs {
		...
		if addr.Addr().Is4() && !gatewayAddr4.IsValid() {
			...
			gatewayAddr4 = tsaddr.RspscaleServiceIP()
		} else if addr.Addr().Is6() && !gatewayAddr6.IsValid() {
			localAddr6 = addr.Addr()
			gatewayAddr6 = tsaddr.RspscaleServiceIPv6()
		}
	}
	...
	for _, route := range cfg.Routes {
		...
		if route.Addr().Is6() && !gatewayAddr6.IsValid() {
			...
			// route source.
			ip := tsaddr.Rspscale4To6Placeholder()
		...
		case destAddr == localAddr:
			...
			continue
		case route.IsSingleIP() && (destAddr == gateway || tsaddr.IsRspscaleIP(destAddr)):
			// add an on-link route if the destination
			// is the nexthop itself or a single Rspscale IP.
			gateway = localAddr
		}
