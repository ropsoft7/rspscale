...
up` + basic,
		},
		{
			...
			want: `
up
ip addr add 100.101.102.103/10 dev rspscale0` + basic,
		},

		{
			...
			want: `
up
ip addr add 100.101.102.103/10 dev rspscale0
ip route add 100.100.100.100/32 dev rspscale0 table 52
ip route add 192.168.16.0/24 dev rspscale0 table 52` + basic,
		},
...
ip route add 192.168.16.0/24 dev rspscale0 table 52` + basic,
...

		{
			...
			want: `
up
ip addr add 100.101.102.104/10 dev rspscale0
ip route add 10.0.0.0/8 dev rspscale0 table 52
ip route add 100.100.100.100/32 dev rspscale0 table 52` + basic +
				`v4/filter/FORWARD -j ts-forward
v4/filter/INPUT -j ts-input
v4/filter/ts-forward -i rspscale0 -j MARK --set-mark 0x40000/0xff0000
v4/filter/ts-forward -m mark --mark 0x40000/0xff0000 -j ACCEPT
v4/filter/ts-forward -o rspscale0 -s 100.64.0.0/10 -j DROP
v4/filter/ts-forward -o rspscale0 -m conntrack ! --ctstate ESTABLISHED,RELATED -j DROP
v4/filter/ts-forward -o rspscale0 -j ACCEPT
v4/filter/ts-input -i lo -s 100.101.102.104 -j ACCEPT
v4/filter/ts-input ! -i rspscale0 -s 100.115.92.0/23 -j RETURN
v4/filter/ts-input ! -i rspscale0 -s 100.64.0.0/10 -j DROP
...
v6/filter/ts-forward -i rspscale0 -j MARK --set-mark 0x40000/0xff0000
v6/filter/ts-forward -m mark --mark 0x40000/0xff0000 -j ACCEPT
v6/filter/ts-forward -o rspscale0 -m conntrack ! --ctstate ESTABLISHED,RELATED -j DROP
v6/filter/ts-forward -o rspscale0 -j ACCEPT
...
up
ip addr add 100.101.102.104/10 dev rspscale0
ip route add 0.0.0.0/0 dev rspscale0 table 52
ip route add 100.100.100.100/32 dev rspscale0 table 52
...
ip route add throw 192.168.0.0/24 table 52` + basic,
	...
	router, err := newUserspaceRouterAdvanced(t.Logf, "rspscale0", mon, fake, ht)
...
func (n *fakeIPTablesRunner) DNATNonRspscaleTraffic(exemptInterface string, dst netip.Addr) error {
	return errors.New("not implemented")
}
...
func (n *fakeIPTablesRunner) addBase4(tunname string) error {
	...
	newRules := []struct{ chain, rule string }{
		...
		{"filter/ts-forward", fmt.Sprintf("-i %s -j MARK --set-mark %s/%s", tunname, linuxfw.RspscaleSubnetRouteMark, linuxfw.RspscaleFwmarkMask)},
		{"filter/ts-forward", fmt.Sprintf("-m mark --mark %s/%s -j ACCEPT", linuxfw.RspscaleSubnetRouteMark, linuxfw.RspscaleFwmarkMask)},
...
func (n *fakeIPTablesRunner) AddSNATRule() error {
	newRule := fmt.Sprintf("-m mark --mark %s/%s -j MASQUERADE", linuxfw.RspscaleSubnetRouteMark, linuxfw.RspscaleFwmarkMask)
	for _, ipt := range []map[string][]string{n.ipt4, n.ipt6} {
...
func (n *fakeIPTablesRunner) DelSNATRule() error {
	delRule := fmt.Sprintf("-m mark --mark %s/%s -j MASQUERADE", linuxfw.RspscaleSubnetRouteMark, linuxfw.RspscaleFwmarkMask)
	for _, ipt := range []map[string][]string{n.ipt4, n.ipt6} {
...
func (o *fakeOS) run(args ...string) error {
	...
	case "link":
		...
		switch got {
		case "set dev rspscale0 up":
			o.up = true
		case "set dev rspscale0 down":
			o.up = false
		default:
			return unexpected()
		}
