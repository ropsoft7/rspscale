...
// Config is the subset of Rspscale configuration that is relevant to
// the OS's network stack.
type Config struct {
	// LocalAddrs are the address(es) for this node. This is
	// typically one IPv4/32 (the 100.x.y.z CGNAT) and one
	// IPv6/128 (Rspscale ULA).
	LocalAddrs []netip.Prefix

	// Routes are the routes that point into the Rspscale
	...
	// LocalRoutes are the routes that should not be routed through Rspscale.
	...
	// advertising to other Rspscale nodes.
