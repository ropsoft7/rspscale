...
func newUserspaceRouterAdvanced(logf logger.Logf, tunname string, netMon *netmon.Monitor, cmd commandRunner, health *health.Tracker) (Router, error) {
	...
	//
	// has the effect of gobbling rspscale packets, because rspscale by default installs
...
//
// If an ip rule is deleted (with pref number 52xx, as Rspscale sets), then
...
func (r *linuxRouter) onIPRuleDeleted(table uint8, priority uint32) {
	...
	time.AfterFunc(rr.Delay()+250*time.Millisecond, func() {
		if r.ruleRestorePending.Swap(false) && !r.closed.Load() {
			r.logf("somebody (likely systemd-networkd) deleted ip rules; restoring Rspscale's")
			r.justAddIPRules()
		}
	})
}
...
var dockerStatefulFilteringWarnable = health.Register(&health.Warnable{
	...
	Text:     health.StaticMessage("Stateful filtering is enabled and Docker was detected; this may prevent Docker containers on this host from resolving DNS and connecting to Rspscale nodes. See https://scale.ropsoft.cloud/s/stateful-docker"),
})
...
func (r *linuxRouter) updateStatefulFilteringWithDockerWarning(cfg *Config) {
	...
	// containers from connecting through the Rspscale in this container.
	//
	// For now, just check for the case where we're running Rspscale on
	// the host and Docker is also running.

	// If this node isn't a subnet router or exit node, then we would never
	// have allowed traffic from a Docker container in to Rspscale, since
...
func (r *linuxRouter) setNetfilterMode(mode preftype.NetfilterMode) error {
	...
	// have created the Rspscale netfilter chains. If so, we have to
...
// required rspscale filter rules for IPv6.
...
func (r *linuxRouter) addRouteDef(routeDef []string, cidr netip.Prefix) error {
	...
	if r.ipRuleAvailable {
		args = append(args, "table", rspscaleRouteTable.ipCmdArg())
	}
	err := r.cmd.run(args...)
	if err == nil {
		return nil
...
// addIPRules adds the policy routing rule that avoids rspscaled
// routing loops. If the rule exists and appears to be a
// rspscale-managed rule, it is gracefully replaced.
...
var (
	...
	// rspscaleRouteTable is the routing table number for Rspscale
	...
	rspscaleRouteTable = newRouteTable("rspscale", 52)
)

// ipRules are the policy routing rules that Rspscale uses.
...
var ipRules = []netlink.Rule{
	...
	{
		Priority: 10,
		Mark:     linuxfw.RspscaleBypassMarkNum,
	...
	// to the rspscale routes, because that would create routing loops.
	{
		Priority: 50,
		Mark:     linuxfw.RspscaleBypassMarkNum,
	...
	// through to the rspscale route table. For apps other than us
...
func (r *linuxRouter) justAddIPRules() error {
	...
	for _, family := range r.addrFamilies() {
		...
		for _, ru := range ipRules {
			...
			if ru.Mark != 0 {
				ru.Mask = linuxfw.RspscaleFwmarkMaskNum
			}
			ru.Goto = -1
...
func (r *linuxRouter) addIPRulesWithIPCommand() error {
	...
	for _, family := range r.addrFamilies() {
		for _, rule := range ipRules {
			...
			if rule.Mark != 0 {
				if r.fwmaskWorks() {
					args = append(args, "fwmark", fmt.Sprintf("0x%x/%s", rule.Mark, linuxfw.RspscaleFwmarkMask))
				} else {
...
// rspscaled routing loops, if it exists.
...
func platformCanNetfilter() bool {
	...
	case distro.Synology:
		...
		//
		// See https://github.com/ropsoft7/rspscale/issues/11737 for one such
