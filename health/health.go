...
// Tracker tracks the health of various Rspscale subsystems,
...
type Warnable struct {
	// Code is a string that uniquely identifies this Warnable across the entire Rspscale backend,
...
// SetLatestVersion records the latest version of the Rspscale client.
...
func (t *Tracker) updateBuiltinWarnablesLocked() {
	...
	// Whether user recently turned on Rspscale.
	recentlyOn := now.Sub(t.ipnWantRunningLastTrue) < 5*time.Second

	homeDERP := t.derpHomeRegion
	if recentlyOn {
		// If user just turned Rspscale on, don't warn for a bit.
