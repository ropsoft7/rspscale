...
var updateAvailableWarnable = Register(&Warnable{
	...
	Text: func(args Args) string {
		if version.IsMacAppStore() || version.IsAppleTV() || version.IsMacSys() || version.IsWindowsGUI() || runtime.GOOS == "android" {
			...
			return fmt.Sprintf("An update from version %s to %s is available. Run `rspscale update` or `rspscale set --auto-update` to update now.", args[ArgCurrentVersion], args[ArgAvailableVersion])
		}
	},
})
...
var securityUpdateAvailableWarnable = Register(&Warnable{
	...
	Text: func(args Args) string {
		if version.IsMacAppStore() || version.IsAppleTV() || version.IsMacSys() || version.IsWindowsGUI() || runtime.GOOS == "android" {
			...
			return fmt.Sprintf("A security update from version %s to %s is available. Run `rspscale update` or `rspscale set --auto-update` to update now.", args[ArgCurrentVersion], args[ArgAvailableVersion])
		}
	},
})

// unstableWarnable is a Warnable that warns the user that they are using an unstable version of Rspscale
...
var unstableWarnable = Register(&Warnable{
	...
	Severity: SeverityLow,
	Text:     StaticMessage("This is an unstable version of Rspscale meant for testing and development purposes. Please report any issues to Rspscale."),
})
...
var NetworkStatusWarnable = Register(&Warnable{
	...
	Title:               "Network down",
	Severity:            SeverityMedium,
	Text:                StaticMessage("Rspscale cannot connect because the network is down. Check your Internet connection."),
	ImpactsConnectivity: true,
	TimeToVisible:       5 * time.Second,
})

// IPNStateWarnable is a Warnable that warns the user that Rspscale is stopped.
var IPNStateWarnable = Register(&Warnable{
	Code:     "wantrunning-false",
	Title:    "Rspscale off",
	Severity: SeverityLow,
	Text:     StaticMessage("Rspscale is stopped."),
})
...
var notInMapPollWarnable = Register(&Warnable{
	...
	Text:      StaticMessage("Unable to connect to the Rspscale coordination server to synchronize the state of your tailnet. Peer reachability might degrade over time."),
	// 8 minutes reflects a maximum maintenance window for the coordination server.
	TimeToVisible: 8 * time.Minute,
})

// noDERPHomeWarnable is a Warnable that warns the user that Rspscale doesn't have a home DERP.
var noDERPHomeWarnable = Register(&Warnable{
	Code:                "no-derp-home",
	Title:               "No home relay server",
	Severity:            SeverityMedium,
	DependsOn:           []*Warnable{NetworkStatusWarnable},
	Text:                StaticMessage("Rspscale could not connect to any relay server. Check your Internet connection."),
	ImpactsConnectivity: true,
	TimeToVisible:       10 * time.Second,
})

// noDERPConnectionWarnable is a Warnable that warns the user that Rspscale couldn't connect to a specific DERP server.
var noDERPConnectionWarnable = Register(&Warnable{
	...
	Text: func(args Args) string {
		if n := args[ArgDERPRegionName]; n != "" {
			return fmt.Sprintf("Rspscale could not connect to the '%s' relay server. Your Internet connection might be down, or the server might be temporarily unavailable.", n)
		} else {
			return fmt.Sprintf("Rspscale could not connect to the relay server with ID '%s'. Your Internet connection might be down, or the server might be temporarily unavailable.", args[ArgDERPRegionID])
		}
	},
	ImpactsConnectivity: true,
	TimeToVisible:       10 * time.Second,
...
// derpTimeoutWarnable is a Warnable that warns the user that Rspscale hasn't
...
var derpTimeoutWarnable = Register(&Warnable{
	...
	Text: func(args Args) string {
		if n := args[ArgDERPRegionName]; n != "" {
			return fmt.Sprintf("Rspscale hasn't heard from the '%s' relay server in %v. The server might be temporarily unavailable, or your Internet connection might be down.", n, args[ArgDuration])
		} else {
			return fmt.Sprintf("Rspscale hasn't heard from the home relay server (region ID '%v') in %v. The server might be temporarily unavailable, or your Internet connection might be down.", args[ArgDERPRegionID], args[ArgDuration])
		}
	},
})
...
// noUDP4BindWarnable is a Warnable that warns the user that Rspscale couldn't listen for incoming UDP connections.
var noUDP4BindWarnable = Register(&Warnable{
	Code:                "no-udp4-bind",
	Title:               "NAT traversal setup failure",
	Severity:            SeverityMedium,
	DependsOn:           []*Warnable{NetworkStatusWarnable, IPNStateWarnable},
	Text:                StaticMessage("Rspscale couldn't listen for incoming UDP connections."),
	ImpactsConnectivity: true,
})

// mapResponseTimeoutWarnable is a Warnable that warns the user that Rspscale hasn't received a network map from the coordination server in a while.
var mapResponseTimeoutWarnable = Register(&Warnable{
	...
	DependsOn: []*Warnable{NetworkStatusWarnable, IPNStateWarnable},
	Text: func(args Args) string {
		return fmt.Sprintf("Rspscale hasn't received a network map from the coordination server in %s.", args[ArgDuration])
	},
})

// tlsConnectionFailedWarnable is a Warnable that warns the user that Rspscale could not establish an encrypted connection with a server.
var tlsConnectionFailedWarnable = Register(&Warnable{
	...
	DependsOn: []*Warnable{NetworkStatusWarnable},
	Text: func(args Args) string {
		return fmt.Sprintf("Rspscale could not establish an encrypted connection with '%q': %v", args[ArgServerName], args[ArgError])
	},
})
...
var applyDiskConfigWarnable = Register(&Warnable{
	...
	Severity: SeverityMedium,
	Text: func(args Args) string {
		return fmt.Sprintf("An error occurred applying the Rspscale envknob configuration stored on disk: %v", args[ArgError])
	},
})
...
var warmingUpWarnable = Register(&Warnable{
	Code:     "warming-up",
	Title:    "Rspscale is starting",
	Severity: SeverityLow,
	Text:     StaticMessage("Rspscale is starting. Please wait."),
})
