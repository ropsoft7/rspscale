...
// Package tsweb contains code used in various Rspscale webservers.
package tsweb
...
func DefaultCertDir(leafDir string) string {
	...
	if err == nil {
		return filepath.Join(cacheDir, "rspscale", leafDir)
	}
	return ""
}
...
func AllowDebugAccess(r *http.Request) bool {
	...
	if tsaddr.IsRspscaleIP(ip) || ip.IsLoopback() || ipStr == envknob.String("TS_ALLOW_DEBUG_IP") {
		return true
	}
	return false
}
...
//   - any path component consisting of a common Rspscale Stable ID
...
var errCallback = ctxkey.New[func(HTTPError)]("scale.ropsoft.cloud/tsweb.errCallback", nil)
