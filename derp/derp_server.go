...
import (
	...
	"scale.ropsoft.cloud/tstime/rate"
...
type Server struct {
	...
	// verifyClientsLocalRspscaled only accepts client connections to the DERP
	// server if the clientKey is a known peer in the network, as specified by a
	// running rspscaled's client's LocalAPI.
	verifyClientsLocalRspscaled bool
...
// SetVerifyClients sets whether this DERP server verifies clients through rspscaled.
//
// It must be called before serving begins.
func (s *Server) SetVerifyClient(v bool) {
	s.verifyClientsLocalRspscaled = v
}
...
//go:generate go run scale.ropsoft.cloud/cmd/addlicense -file dropreason_string.go go run golang.org/x/tools/cmd/stringer -type=dropReason -trimprefix=dropReason
...
const (
	...
	dropReasonGoneDisconnected                   // destination rspscaled disconnected before we could send
...
var localClient rspscale.LocalClient
...
func (s *Server) verifyClient(ctx context.Context, clientKey key.NodePublic, info *clientInfo, clientIP netip.Addr) error {
	if s.isMeshPeer(info) {
		...
		// further wouldn't work: it's not part of the tailnet so rspscaled and
		// likely the admission control URL wouldn't know about it.
		return nil
	}

	// rspscaled-based verification:
	if s.verifyClientsLocalRspscaled {
		_, err := localClient.WhoIsNodeKey(ctx, clientKey)
		if err == rspscale.ErrPeerNotFound {
			return fmt.Errorf("peer %v not authorized (not found in local rspscaled)", clientKey)
		}
		if err != nil {
			if strings.Contains(err.Error(), "invalid 'addr' parameter") {
				// Issue 12617
				return errors.New("rspscaled version is too old (out of sync with derper binary)")
			}
			return fmt.Errorf("failed to query local rspscaled status for %v: %w", clientKey, err)
		}
	}
...
func (s *Server) ConsistencyCheck() error {
	...
	if s.verifyClientsLocalRspscaled {
		if err := s.checkVerifyClientsLocalRspscaled(); err != nil {
...
// checkVerifyClientsLocalRspscaled checks that a verifyClients call can be made successfully for the derper hosts own node key.
func (s *Server) checkVerifyClientsLocalRspscaled() error {
