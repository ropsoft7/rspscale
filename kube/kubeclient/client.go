...
// This package is Rspscale-internal and not meant for external consumption.
...
// pod. It returns an error if the basic permissions rspscale needs are
...
// setEventPerms checks whether this client will be able to write rspscaled Events to its Pod and updates the state
...
func (c *client) setEventPerms() {
	...
	defer func() {
		...
		if !hasPerms {
			log.Printf(`kubeclient: this client is not able to write rspscaled Events to the Pod in which it is running.
