...
// Package tsd (short for "Rspscale Daemon") contains a System type that
// containing all the subsystems a Rspscale node (rspscaled or platform
// equivalent) uses.
//
// The goal of this package (as of 2023-05-03) is to eventually unify
// initialization across rspscaled, rspscaled as a Windows services, the mac
...
//
// This package depends on nearly all parts of Rspscale, so it should not be
// imported by (or thus passed to) any package that does not want to depend on
// the world. In practice this means that only things like cmd/rspscaled,
...
// System contains all the subsystems of a Rspscale node (rspscaled, etc.)
type System struct {
...
// IsNetstackRouter reports whether Rspscale is either fully netstack based
...
// IsNetstack reports whether Rspscale is running as a netstack-based TUN-free engine.
...
// SubSystem represents some subsystem of the Rspscale node daemon.
//
