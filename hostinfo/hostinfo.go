...
// Package hostinfo answers questions about the host environment that Rspscale is
// running on.
package hostinfo
...
// the /etc/apt/sources.list.d/rspscale.list file contents upon upgrade
// to a new release of the distro.
//
// See https://github.com/ropsoft7/rspscale/issues/3177
func DisabledEtcAptSource() bool {
	if runtime.GOOS != "linux" {
		return false
	}
	const path = "/etc/apt/sources.list.d/rspscale.list"
	fi, err := os.Stat(path)
...
func IsNATLabGuestVM() bool {
	if runtime.GOOS == "linux" && distro.Get() == distro.Gokrazy {
		...
		return bytes.Contains(cmdLine, []byte("rspscale-tta=1"))
	}
	return false
}
