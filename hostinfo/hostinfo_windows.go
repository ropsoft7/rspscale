...
import (
	...
	"scale.ropsoft.cloud/util/winutil/winenv"
)
...
func packageTypeWindows() string {
	if _, err := os.Stat(`C:\ProgramData\chocolatey\lib\rspscale`); err == nil {
		return "choco"
	}
	...
	if strings.HasPrefix(exe, filepath.Join(home, "scoop", "apps", "rspscale")) {
		return "scoop"
	}
	msiSentinel, _ := winutil.GetRegInteger("MSI")
	if msiSentinel != 1 {
		// Atypical. Not worth trying to detect. Likely open
		// source rspscaled or a developer running by hand.
		return ""
	}
	result := "msi"
