...
// debug settings.
//
// These are primarily knobs used by Rspscale developers during
// development or by users when instructed to by Rspscale developers
...
// UseWIPCode is whether RSPSCALE_USE_WIP_CODE is set to permit use
// of Work-In-Progress code.
func UseWIPCode() bool { return Bool("RSPSCALE_USE_WIP_CODE") }

// CanSSHD reports whether the Rspscale SSH server is allowed to run.
//
...
// App returns the rspscale app type of this instance, if set via
// TS_INTERNAL_APP env var. TS_INTERNAL_APP can be used to set app type for
// components that wrap rspscaled, such as containerboot. App type is intended
// to only be used to set known predefined app types, such as Rspscale
...
// CrashOnUnexpected reports whether the Rspscale client should panic
...
// Rspscale control plane initiate a Rspscale update (e.g. on behalf of an
...
// /etc/default/rspscaled on Linux.
//
// On Windows, you use %ProgramData%\Rspscale\rspscaled-env.txt instead.
//
// On macOS, use one of:
//
//   - /private/var/root/Library/Containers/io.rspscale.ipn.macsys.network-extension/Data/rspscaled-env.txt
//     for standalone macOS GUI builds
//   - ~/Library/Containers/io.rspscale.ipn.macos.network-extension/Data/rspscaled-env.txt
//     for App Store builds
//   - /etc/ropsoft7/rspscaled-env.txt for rspscaled-on-macOS (homebrew, etc)
...
// optional rspscaled-env.txt file. It returns an empty list if none is
// defined for the platform.
func getPlatformEnvFiles() []string {
	switch runtime.GOOS {
	case "windows":
		return []string{
			filepath.Join(os.Getenv("ProgramData"), "Rspscale", "rspscaled-env.txt"),
		}
	case "linux":
		if distro.Get() == distro.Synology {
			return []string{"/etc/ropsoft7/rspscaled-env.txt"}
		}
	case "darwin":
		if version.IsSandboxedMacOS() { // the two GUI variants (App Store or separate download)
			...
			// to something like:
			//	~/Library/Containers/io.rspscale.ipn.macos.network-extension/Data
			//
			// On the macsys (downloadable Mac GUI) variant, the
			// home directory can be unset, but we have a working
			// directory that looks like:
			//	/private/var/root/Library/Containers/io.rspscale.ipn.macsys.network-extension/Data
			//
			// Try both and see if we can find the file in either
			// location.
			var candidates []string
			if home := os.Getenv("HOME"); home != "" {
				candidates = append(candidates, filepath.Join(home, "rspscaled-env.txt"))
			}
			if wd, err := os.Getwd(); err == nil {
				candidates = append(candidates, filepath.Join(wd, "rspscaled-env.txt"))
			}

			return candidates
		} else {
			// Open source / homebrew variable, running rspscaled-on-macOS.
			return []string{"/etc/ropsoft7/rspscaled-env.txt"}
		}
	}
	return nil
