...
const (
	DeeplinkRspscaleURLScheme = "rspscale"
...
func (a *Authority) NewDeeplink(params NewDeeplinkParams) (string, error) {
	...
	u := url.URL{
		Scheme: DeeplinkRspscaleURLScheme,
...
//
// rspscale://sign-device/v1/?nk=xxx&tp=xxx&dn=xxx&os=xxx&em=xxx&hm=xxx
//
// where:
...
func (a *Authority) ValidateDeeplink(urlString string) DeeplinkValidationResult {
	...
	if parsedUrl.Scheme != DeeplinkRspscaleURLScheme {
		return DeeplinkValidationResult{
			IsValid: false,
			Error:   fmt.Sprintf("unhandled scheme %s, expected %s", parsedUrl.Scheme, DeeplinkRspscaleURLScheme),
		}
	}
