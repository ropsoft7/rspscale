...
// Package qnap contains dist Targets for building QNAP Rspscale packages.
//
...
func (t *target) buildQPKG(b *dist.Build, qnapBuilds *qnapBuilds, inner *innerPkg) ([]string, error) {
	...
	filename := fmt.Sprintf("Rspscale_%s-%s_%s.qpkg", b.Version.Short, qnapTag, t.arch)
	...
	cmd := b.Command(b.Repo, "docker", "run", "--rm",
		...
		"-v", fmt.Sprintf("%s:/rspscale", inner.rspscalePath),
		"-v", fmt.Sprintf("%s:/rspscaled", inner.rspscaledPath),
		// Rspscale folder has QNAP package setup files needed for building.
		"-v", fmt.Sprintf("%s:/Rspscale", filepath.Join(qnapBuilds.tmpDir, "files/Rspscale")),
		"-v", fmt.Sprintf("%s:/build-qpkg.sh", filepath.Join(qnapBuilds.tmpDir, "files/scripts/build-qpkg.sh")),
		"-v", fmt.Sprintf("%s:/out", b.Out),
		"build.rspscale.io/qdk:latest",
		"/build-qpkg.sh",
	)
...
// FS for building.
//
// We do this to allow for this scale.ropsoft.cloud/release/dist/qnap package to be
...
func newQNAPBuilds(b *dist.Build, signer *signer) (*qnapBuilds, error) {
	...
	if signer != nil {
		...
		// of the project directory (in our case release/dist/qnap/Rspscale).
		...
		keyPath := filepath.Join(m.tmpDir, "files/Rspscale/private_key")
		if err := os.WriteFile(keyPath, key, 0400); err != nil {
			return nil, err
		}
		certPath := filepath.Join(m.tmpDir, "files/Rspscale/certificate")
...
// These binaries get embedded with Rspscale package metadata to form qnap
// releases.
func (m *qnapBuilds) buildInnerPackage(b *dist.Build, goenv map[string]string) (*innerPkg, error) {
	return m.innerPkgs.Do(goenv, func() (*innerPkg, error) {
		...
		ts, err := b.BuildGoBinary("scale.ropsoft.cloud/cmd/rspscale", goenv)
		if err != nil {
			return nil, err
		}
		tsd, err := b.BuildGoBinary("scale.ropsoft.cloud/cmd/rspscaled", goenv)
		if err != nil {
			return nil, err
		}
		...
		tsPath := filepath.Join(tmpDir, "rspscale")
		if err := os.WriteFile(tsPath, tsBytes, 0755); err != nil {
			return nil, err
		}
		tsdPath := filepath.Join(tmpDir, "rspscaled")
		if err := os.WriteFile(tsdPath, tsdBytes, 0755); err != nil {
			return nil, err
		}

		return &innerPkg{rspscalePath: tsPath, rspscaledPath: tsdPath}, nil
	})
}
...
func (m *qnapBuilds) makeDockerImage(b *dist.Build) error {
	return b.Once("make-qnap-docker-image", func() error {
		...
		cmd := b.Command(b.Repo, "docker", "build",
			...
			"-t", "build.rspscale.io/qdk:latest",
