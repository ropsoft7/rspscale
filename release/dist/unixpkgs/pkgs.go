...
// Package unixpkgs contains dist Targets for building unix Rspscale packages.
package unixpkgs
...
import (
	...
	"scale.ropsoft.cloud/release/dist"
)
...
func (t *tgzTarget) Build(b *dist.Build) ([]string, error) {
	...
	if t.goEnv["GOOS"] == "linux" {
		...
		filename = fmt.Sprintf("rspscale_%s_%s.tgz", b.Version.Short, t.arch())
	} else {
		filename = fmt.Sprintf("rspscale_%s_%s_%s.tgz", b.Version.Short, t.os(), t.arch())
	}
	if err := b.BuildWebClientAssets(); err != nil {
		return nil, err
	}
	ts, err := b.BuildGoBinary("scale.ropsoft.cloud/cmd/rspscale", t.goEnv)
	if err != nil {
		return nil, err
	}
	tsd, err := b.BuildGoBinary("scale.ropsoft.cloud/cmd/rspscaled", t.goEnv)
	if err != nil {
		return nil, err
	}
	...
	if err := addFile(tsd, filepath.Join(dir, "rspscaled"), 0755); err != nil {
		return nil, err
	}
	if err := addFile(ts, filepath.Join(dir, "rspscale"), 0755); err != nil {
		return nil, err
	}
	if t.os() == "linux" {
		...
		rspscaledDir, err := b.GoPkg("scale.ropsoft.cloud/cmd/rspscaled")
		if err != nil {
			return nil, err
		}
		if err := addFile(filepath.Join(rspscaledDir, "rspscaled.service"), filepath.Join(dir, "rspscaled.service"), 0644); err != nil {
			return nil, err
		}
		if err := addFile(filepath.Join(rspscaledDir, "rspscaled.defaults"), filepath.Join(dir, "rspscaled.defaults"), 0644); err != nil {
			return nil, err
		}
	}
...
func (t *debTarget) Build(b *dist.Build) ([]string, error) {
	...
	repoDir, err := b.GoPkg("scale.ropsoft.cloud")
	if err != nil {
		return nil, err
	}
	...
	contents, err := files.PrepareForPackager(files.Contents{
		&files.Content{
			Type:        files.TypeFile,
			Source:      ts,
			Destination: "/usr/bin/rspscale",
		},
		&files.Content{
			Type:        files.TypeFile,
			Source:      tsd,
			Destination: "/usr/sbin/rspscaled",
		},
		&files.Content{
			Type:        files.TypeFile,
			Source:      filepath.Join(rspscaledDir, "rspscaled.service"),
			Destination: "/lib/systemd/system/rspscaled.service",
		},
		&files.Content{
			Type:        files.TypeConfigNoReplace,
			Source:      filepath.Join(rspscaledDir, "rspscaled.defaults"),
			Destination: "/etc/default/rspscaled",
		},
	}, 0, "deb", false)
	if err != nil {
		return nil, err
	...
	info := nfpm.WithDefaults(&nfpm.Info{
		...
		Maintainer:  "Rspscale Inc <info@scale.ropsoft.cloud>",
		Description: "The easiest, most secure, cross platform way to use WireGuard + oauth2 + 2FA/SSO",
		Homepage:    "https://www.scale.ropsoft.cloud",
		License:     "MIT",
		Section:     "net",
		Priority:    "extra",
		Overridables: nfpm.Overridables{
			...
			Recommends: []string{
				"rspscale-archive-keyring (>= 1.35.181)",
				...
				// https://github.com/ropsoft7/rspscale/issues/391.
			...
			Replaces:  []string{"rspscale-relay"},
			Conflicts: []string{"rspscale-relay"},
		},
	})
	...
	filename := fmt.Sprintf("rspscale_%s_%s.deb", b.Version.Short, arch)
	log.Printf("Building %s", filename)
...
func (t *rpmTarget) Build(b *dist.Build) ([]string, error) {
	...
	contents, err := files.PrepareForPackager(files.Contents{
		...
		&files.Content{
			Type:        files.TypeDir,
			Destination: "/var/cache/rspscale",
		},
	}, 0, "rpm", false)
	if err != nil {
		return nil, err
	...
	filename := fmt.Sprintf("rspscale_%s_%s.rpm", b.Version.Short, arch)
	log.Printf("Building %s", filename)
