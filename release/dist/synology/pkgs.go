...
// Package synology contains dist Targets for building Synology Rspscale packages.
package synology
...
func (t *target) buildSPK(b *dist.Build, inner *innerPkg) ([]string, error) {
	...
	filename := fmt.Sprintf("rspscale-%s-%s-%d-dsm%s.spk", t.filenameArch, b.Version.Short, synoVersion, t.dsmVersionString())
	...
	err = writeTar(tw, b.Time,
		...
		static("Rspscale.sc", "Rspscale.sc", 0644),
		dir("conf"),
...
func (t *target) mkInfo(b *dist.Build, uncompressedSz int64) []byte {
	...
	f("displayname", "Rspscale")
	f("maintainer", "Rspscale, Inc.")
	f("maintainer_url", "https://github.com/ropsoft7/rspscale")
	f("create_time", b.Time.Format("20060102-15:04:05"))
	f("dsmuidir", "ui")
	f("dsmappname", "SYNO.SDS.Rspscale")
...
func (m *synologyBuilds) buildInnerPackage(b *dist.Build, dsmVersion int, goenv map[string]string) (*innerPkg, error) {
	...
	return m.innerPkgs.Do(key, func() (*innerPkg, error) {
		...
		err = writeTar(tw, b.Time,
			dir("bin"),
			file(tsd, "bin/rspscaled", 0755),
			file(ts, "bin/rspscale", 0755),
			dir("conf"),
			static("Rspscale.sc", "conf/Rspscale.sc", 0644),
...
func dir(name string) tarEntry {
	return func(tw *tar.Writer, modTime time.Time) error {
		return tw.WriteHeader(&tar.Header{
			...
			Mode:     0755,
			ModTime:  modTime,
			// TODO: why rspscale? Files are being written as owned by root.
			Uname: "rspscale",
			Gname: "rspscale",
		})
	}
