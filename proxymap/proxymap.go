...
// by rspscaled on behalf of remote Rspscale IPs for proxied connections.
package proxymap
...
// Mapper tracks which localhost ip:ports correspond to which remote Rspscale
// IPs for connections proxied by rspscaled.
//
// This is then used (via the WhoIsIPPort method) by localhost applications to
// ask rspscaled (via the LocalAPI WhoIs method) the Rspscale identity that a
// given localhost:port corresponds to.
type Mapper struct {
	mu sync.Mutex

	// m holds the mapping from localhost IP:ports to Rspscale IPs. It is
...
// Rspscale IP) as temporarily having the given IP:port for whois lookups.
//
// The IP:port is generally a localhost IP and an ephemeral port, used
// while proxying connections to localhost when rspscaled is running
// in netstack mode.
//
...
// and returns a matching Rspscale IP, if it exists.
