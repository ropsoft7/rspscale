...
func TestRspscaleServiceIP(t *testing.T) {
	got := RspscaleServiceIP().String()
	want := "100.100.100.100"
	if got != want {
		t.Errorf("got %q; want %q", got, want)
	}
	if RspscaleServiceIPString != want {
		t.Error("RspscaleServiceIPString is not consistent")
	}
}

func TestRspscaleServiceIPv6(t *testing.T) {
	got := RspscaleServiceIPv6().String()
	want := "fd7a:115c:a1e0::53"
	if got != want {
		t.Errorf("got %q; want %q", got, want)
	}
	if RspscaleServiceIPv6String != want {
		t.Error("RspscaleServiceIPv6String is not consistent")
	}
}
...
func BenchmarkRspscaleServiceAddr(b *testing.B) {
	b.ReportAllocs()
	for range b.N {
		sinkIP = RspscaleServiceIP()
	}
}
...
func TestIsRspscaleIPv4(t *testing.T) {
	tests := []struct {
		in   netip.Addr
		want bool
	}{
	...
	for _, tt := range tests {
		if got := IsRspscaleIPv4(tt.in); got != tt.want {
			t.Errorf("IsRspscaleIPv4(%v) = %v, want %v", tt.in, got, tt.want)
		}
	}
}

func TestIsRspscaleIP(t *testing.T) {
	tests := []struct {
		in   netip.Addr
		want bool
	}{
	...
	for _, tt := range tests {
		if got := IsRspscaleIP(tt.in); got != tt.want {
			t.Errorf("IsRspscaleIP(%v) = %v, want %v", tt.in, got, tt.want)
		}
	}
}
