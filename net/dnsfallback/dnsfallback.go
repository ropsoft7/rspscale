...
// for starting up Rspscale when the system DNS is broken or otherwise unavailable.
//
...
import (
	...
	"scale.ropsoft.cloud/net/dns/recursive"
...
func (fr *fallbackResolver) Lookup(ctx context.Context, host string) ([]netip.Addr, error) {
	...
	// recursive resolver. (rspscale/corp#15261) In the future, we might
...
// serverName and serverIP of are, say, "derpN.scale.ropsoft.cloud".
// queryName is the name being sought (e.g. "controlplane.scale.ropsoft.cloud"), passed as hint.
//
// ht may be nil.
...
// https://derp10.scale.ropsoft.cloud/bootstrap-dns
...
func UpdateCache(c *tailcfg.DERPMap, logf logger.Logf) {
	...
	// when we don't have a state path (e.g. /var/lib/rspscale) configured.
	if cachePath != "" {
