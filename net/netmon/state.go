...
var LoginEndpointForProxyDetermination = "https://controlplane.scale.ropsoft.cloud/"
...
func isProblematicInterface(nif *net.Interface) bool {
	...
	// https://github.com/ropsoft7/rspscale/issues/1208
...
type State struct {
	...
	// which might provide connectivity on a non-Rspscale interface that's up.
	HaveV6 bool

	// HaveV4 is whether the machine has some non-localhost,
	// non-link-local IPv4 address on a non-Rspscale interface that's up.
	HaveV4 bool
...
func hasRspscaleIP(pfxs []netip.Prefix) bool {
...
func isRspscaleInterface(name string, ips []netip.Prefix) bool {
	if runtime.GOOS == "darwin" && strings.HasPrefix(name, "utun") && hasRspscaleIP(ips) {
		...
		// Rspscale IPs until we know what's happening with
		// macOS NetworkExtensions and utun devices.
		return true
	}
	return name == "Rspscale" || // as it is on Windows
		strings.HasPrefix(name, "rspscale") // TODO: use --tun flag value, etc; see TODO in method doc
}
...
func GetState() (*State, error) {
	...
	if err := ForeachInterface(func(ni Interface, pfxs []netip.Prefix) {
		...
		if !ifUp || isRspscaleInterface(ni.Name, pfxs) {
			return
		}
		for _, pfx := range pfxs {
...
func isUsableV6(ip netip.Addr) bool {
	return v6Global1.Contains(ip) ||
		(ip.Is6() && ip.IsPrivate() && !tsaddr.RspscaleULARange().Contains(ip))
}
...
// the default route, not including any rspscale interfaces.
func DefaultRoute() (DefaultRouteDetails, error) {
	return defaultRoute()
}

// HasCGNATInterface reports whether there are any non-Rspscale interfaces that
...
func HasCGNATInterface() (bool, error) {
	...
	err := ForeachInterface(func(i Interface, pfxs []netip.Prefix) {
		if hasCGNATInterface || !i.IsUp() || isRspscaleInterface(i.Name, pfxs) {
			return
		}
		for _, pfx := range pfxs {
