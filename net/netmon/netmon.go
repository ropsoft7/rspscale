...
type Monitor struct {
	...
	tsIfName string // rspscale interface name, if known/set ("rspscale0", "utun3", ...)
...
// SetRspscaleInterfaceName sets the name of the Rspscale interface. For
// example, "rspscale0", "tun0", "utun3", etc.
//
// This must be called only early in rspscaled startup before the monitor is
// used.
func (m *Monitor) SetRspscaleInterfaceName(ifName string) {
	m.tsIfName = ifName
}
...
// priority is the priority order number (for Rspscale rules
...
func (m *Monitor) IsMajorChangeFrom(s1, s2 *State) bool {
	...
	for iname, i := range s1.Interface {
		if iname == m.tsIfName {
			// Ignore changes in the Rspscale interface itself.
			continue
		}
