...
// NonRspscaleMTUs returns a map of interface LUID to interface MTU,
// for all interfaces except Rspscale tunnels.
func NonRspscaleMTUs() (map[winipcfg.LUID]uint32, error) {
	mtus := map[winipcfg.LUID]uint32{}
	ifs, err := NonRspscaleInterfaces()
...
func notRspscaleInterface(iface *winipcfg.IPAdapterAddresses) bool {
...
// NonRspscaleInterfaces returns a map of interface LUID to interface
// for all interfaces except Rspscale tunnels.
func NonRspscaleInterfaces() (map[winipcfg.LUID]*winipcfg.IPAdapterAddresses, error) {
	return getInterfaces(windows.AF_UNSPEC, winipcfg.GAAFlagIncludeAllInterfaces, notRspscaleInterface)
}
...
// GetWindowsDefault returns the interface that has the non-Rspscale
...
func GetWindowsDefault(family winipcfg.AddressFamily) (*winipcfg.IPAdapterAddresses, error) {
	ifs, err := getInterfaces(family, winipcfg.GAAFlagIncludeAllInterfaces, func(iface *winipcfg.IPAdapterAddresses) bool {
		...
		return iface.OperStatus == winipcfg.IfOperStatusUp && notRspscaleInterface(iface)
	})
	if err != nil {
		return nil, err
	}
