...
const testDomain = "scale.ropsoft.cloud"
...
func TestResolveFallbackToTCP(t *testing.T) {
	...
	ctx := context.Background()
	resp, err := r.queryNameserverProto(ctx, 0, "scale.ropsoft.cloud", netip.MustParseAddr("9.9.9.9"), "udp", dns.Type(dns.TypeA))
	if err != nil {
		t.Fatal(err)
	}
	...
	resp2, err := r.queryNameserverProto(ctx, 0, "scale.ropsoft.cloud", netip.MustParseAddr("9.9.9.9"), "udp", dns.Type(dns.TypeA))
	if err != nil {
		t.Fatal(err)
	}
...
var (
	...
	// Nameservers for the scale.ropsoft.cloud domain
	rspscaleNameservers = &dns.Msg{
		Ns: []dns.RR{
			nsRR("scale.ropsoft.cloud.", "ns-197.awsdns-24.com."),
			nsRR("scale.ropsoft.cloud.", "ns-557.awsdns-05.net."),
			nsRR("scale.ropsoft.cloud.", "ns-1558.awsdns-02.co.uk."),
			nsRR("scale.ropsoft.cloud.", "ns-1359.awsdns-41.org."),
		},
		Extra: []dns.RR{
...
func TestBasicRecursion(t *testing.T) {
	mock := &replyMock{
		...
		replies: map[netip.Addr][]mockReply{
			...
			rootServerAddr: {
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: comRecord},
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: comRecord},
			},

			// Query to the ".com" server return the nameservers for scale.ropsoft.cloud
			comNSAddr: {
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: rspscaleNameservers},
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: rspscaleNameservers},
			},

			// Query to the actual nameserver works.
			amazonNS: {
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: &dns.Msg{
					MsgHdr: dns.MsgHdr{Authoritative: true},
					Answer: []dns.RR{
						dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("13.248.141.131")),
						dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("76.223.15.28")),
					},
				}},
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: &dns.Msg{
					MsgHdr: dns.MsgHdr{Authoritative: true},
					Answer: []dns.RR{
						dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("2600:9000:a602:b1e6:86d:8165:5e8c:295b")),
						dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("2600:9000:a51d:27c1:1530:b9ef:2a6:b9e5")),
					},
				}},
			},
		},
	...
	// Query for scale.ropsoft.cloud, verify we get the right responses
	ctx := context.Background()
	addrs, minTTL, err := r.Resolve(ctx, "scale.ropsoft.cloud")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNoAnswers(t *testing.T) {
	...
	// Query for scale.ropsoft.cloud, verify we get the right responses
	_, _, err := r.Resolve(context.Background(), "scale.ropsoft.cloud")
	if err == nil {
	...
	mock := &replyMock{
		...
		replies: map[netip.Addr][]mockReply{
			...
			rootServerAddr: {
				...
				{name: "subdomain.scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: comRecord},
				{name: "subdomain.scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: comRecord},
			},

			// Query to the ".com" server return the nameservers for scale.ropsoft.cloud
			comNSAddr: {
				{name: "subdomain.otherdomain.com.", qtype: dns.Type(dns.TypeA), resp: rspscaleNameservers},
				{name: "subdomain.otherdomain.com.", qtype: dns.Type(dns.TypeAAAA), resp: rspscaleNameservers},

				{name: "subdomain.scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: rspscaleNameservers},
				{name: "subdomain.scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: rspscaleNameservers},
			},
			...
			amazonNS: {
				{name: "subdomain.otherdomain.com.", qtype: dns.Type(dns.TypeA), resp: &dns.Msg{
					MsgHdr: dns.MsgHdr{Authoritative: true},
					Answer: []dns.RR{cnameRR("subdomain.otherdomain.com.", "subdomain.scale.ropsoft.cloud.")},
				}},
				...
				{name: "subdomain.scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: &dns.Msg{
					MsgHdr: dns.MsgHdr{Authoritative: true},
					Answer: []dns.RR{dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("13.248.141.131"))},
				}},
				{name: "subdomain.scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: &dns.Msg{
					MsgHdr: dns.MsgHdr{Authoritative: true},
					Answer: []dns.RR{dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("2600:9000:a602:b1e6:86d:8165:5e8c:295b"))},
				}},
			},
		},
	}
...
func TestRecursionNoGlue(t *testing.T) {
	...
	rspscaleNameservers := &dns.Msg{
		Ns: []dns.RR{
			nsRR("scale.ropsoft.cloud.", amazonNameserver),
		},
	}

	rspscaleResponses := []mockReply{
		{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: &dns.Msg{
			MsgHdr: dns.MsgHdr{Authoritative: true},
			Answer: []dns.RR{dnsIPRR("scale.ropsoft.cloud.", netip.MustParseAddr("13.248.141.131"))},
		}},
	...
	mock := &replyMock{
		...
		replies: map[netip.Addr][]mockReply{
			...
			// for scale.ropsoft.cloud, which don't contain a glue
			// record.
			comNSAddr: {
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeA), resp: rspscaleNameservers},
				{name: "scale.ropsoft.cloud.", qtype: dns.Type(dns.TypeAAAA), resp: rspscaleNameservers},
			...
			amazonNS:   rspscaleResponses,
			amazonNSv6: rspscaleResponses,
		},
	}
	...
	// Query for scale.ropsoft.cloud, verify we get the right responses
	addrs, minTTL, err := r.Resolve(context.Background(), "scale.ropsoft.cloud")
	if err != nil {
		t.Fatal(err)
	}
	wantAddrs := []netip.Addr{
...
func TestRecursionLimit(t *testing.T) {
	...
	for i := range maxDepth + 1 {
		curr := fmt.Sprintf("%d-scale.ropsoft.cloud.", i)
		...
		// Query to the ".com" server return the nameservers for NN-scale.ropsoft.cloud
		mock.replies[comNSAddr] = append(mock.replies[comNSAddr],
			mockReply{name: curr, qtype: dns.Type(dns.TypeA), resp: rspscaleNameservers},
			mockReply{name: curr, qtype: dns.Type(dns.TypeAAAA), resp: rspscaleNameservers},
		)

		// Queries to the nameserver return a CNAME for the n+1th server.
		next := fmt.Sprintf("%d-scale.ropsoft.cloud.", i+1)
	...
	// Query for the first node in the chain, 0-scale.ropsoft.cloud, and verify
	// we get a max-depth error.
	ctx := context.Background()
	_, _, err := r.Resolve(ctx, "0-scale.ropsoft.cloud")
	if err == nil {
...
func TestInvalidResponses(t *testing.T) {
	...
	// Query for scale.ropsoft.cloud, verify we get no responses since the
