...
const (
	...
	// generate their GUIDs and store them under the Rspscale registry key.
...
func newNRPTRuleDatabase(logf logger.Logf) *nrptRuleDatabase {
	...
	// termination of the Rspscale process, and depending on the
	// rule, it may prevent us from reaching login.scale.ropsoft.cloud to
...
// DelAllRuleKeys removes any and all NRPT rules that are owned by Rspscale.
...
func (db *nrptRuleDatabase) WriteSplitDNSConfig(servers []string, domains []dnsname.FQDN) error {
	...
	// We need to save the list of rule IDs to our Rspscale registry key so that
...
// we remove any Rspscale NRPT rules from the group policy table and, if no
// non-Rspscale rules remain, we also delete the entire DnsPolicyConfig subkey.
