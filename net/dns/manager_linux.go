...
func dnsMode(logf logger.Logf, health *health.Tracker, env newOSConfigEnv) (ret string, err error) {
	...
	case "systemd-resolved":
		...
		// https://github.com/ropsoft7/rspscale/issues/2136
		...
		// https://github.com/ropsoft7/rspscale/issues/1699,
		// https://github.com/ropsoft7/rspscale/pull/1945)
	...
	case "NetworkManager":
		...
		if err := resolvedIsActuallyResolver(logf, env, dbg, bs); err != nil {
			...
			// Rspscale network interface. So, when we can avoid it,
		...
		health.SetDNSManagerHealth(errors.New("systemd-resolved and NetworkManager are wired together incorrectly; MagicDNS will probably not work. For more info, see https://scale.ropsoft.cloud/s/resolved-nm"))
		dbg("nm-safe", "no")
		return "systemd-resolved", nil
	default:
		dbg("rc", "unknown")
