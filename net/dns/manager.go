...
var osConfigurationReadWarnable = health.Register(&health.Warnable{
	...
	Text: func(args health.Args) string {
		return fmt.Sprintf("Rspscale failed to fetch the DNS configuration of your device: %v", args[health.ArgError])
	},
...
var osConfigurationSetWarnable = health.Register(&health.Warnable{
	...
	Text: func(args health.Args) string {
		return fmt.Sprintf("Rspscale failed to set the DNS configuration of your device: %v", args[health.ArgError])
	},
...
func (m *Manager) compileConfig(cfg Config) (rcfg resolver.Config, ocfg OSConfig, err error) {
	...
	case !cfg.needsOSResolver():
		...
		// configs clear all Rspscale DNS settings.
	...
	case cfg.hasDefaultIPResolversOnly() && !cfg.hasHostsWithoutSplitDNSRoutes():
		...
		// https://github.com/ropsoft7/rspscale/issues/1666
	...
	// Rspscale, we need to set up quad-100 as a "full proxy"
...
// in case the Rspscale daemon terminated without closing the router.
