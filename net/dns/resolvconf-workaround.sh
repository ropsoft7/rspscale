...
# means that if Rspscale wants to install a DNS configuration, that
...
# contain Rspscale's configuration - effectively implementing
# exclusive mode ourselves in post-production.

set -e

if [ -n "$RSPSCALE_RESOLVCONF_HOOK_LOOP" ]; then
	# Hook script being invoked by itself, skip.
	exit 0
fi

if [ ! -f tun-rspscale.inet ]; then
	# Rspscale isn't trying to manage DNS, do nothing.
	exit 0
fi
...
(
	...
	echo "# Rspscale workaround applied to set exclusive DNS configuration."
	cat tun-rspscale.inet
...
if [ -d /etc/resolvconf/update-libc.d ] ; then
	...
	export RSPSCALE_RESOLVCONF_HOOK_LOOP=1
