...
func (m openresolvManager) deleteRspscaleConfig() error {
	cmd := exec.Command("resolvconf", "-f", "-d", "rspscale")
...
func (m openresolvManager) SetDNS(config OSConfig) error {
	if config.IsZero() {
		return m.deleteRspscaleConfig()
	}

	var stdin bytes.Buffer
	writeResolvConf(&stdin, config.Nameservers, config.SearchDomains)

	cmd := exec.Command("resolvconf", "-m", "0", "-x", "-a", "rspscale")
	cmd.Stdin = &stdin
	out, err := cmd.CombinedOutput()
	if err != nil {
		m.logCmdErr(cmd, err)
...
func (m openresolvManager) GetBaseConfig() (OSConfig, error) {
	...
	// Remove the "rspscale" snippet from the list.
	args := []string{"-l"}
	...
	// "everyone except rspscale", which is what would be in use if
	// rspscale hadn't set exclusive mode.
	//
	...
	// predated rspscale, and stick to only those. However, in
