...
func (m *windowsManager) openInterfaceKey(pfx winutil.RegistryPathPrefix) (registry.Key, error) {
	...
	if closing {
		...
		// the key would already be gone by now and will not reappear until rspscaled is restarted.
...
//
// If no resolvers are provided, the Rspscale NRPT rules are deleted.
...
func setRspscaleHosts(prevHostsFile []byte, hosts []*HostEntry) ([]byte, error) {
	b := bytes.ReplaceAll(prevHostsFile, []byte("\r\n"), []byte("\n"))
	sc := bufio.NewScanner(bytes.NewReader(b))
	const (
		header = "# RspscaleHostsSectionStart"
		footer = "# RspscaleHostsSectionEnd"
	)
	var comments = []string{
		"# This section contains MagicDNS entries for Rspscale.",
...
func (m *windowsManager) setHosts(hosts []*HostEntry) error {
	...
	outB, err := setRspscaleHosts(b, hosts)
	if err != nil {
		return err
	}
...
// setPrimaryDNS sets the given resolvers and domains as the Rspscale
...
func (m *windowsManager) setPrimaryDNS(resolvers []netip.Addr, domains []dnsname.FQDN) error {
	...
	// Disable LLMNR on the Rspscale interface. We don't do multicast, and we
...
func (m *windowsManager) SetDNS(cfg OSConfig) error {
	...
	if len(cfg.MatchDomains) == 0 {
		...
		// to wait for a MDNS response from the Rspscale interface.
...
func (m *windowsManager) GetBaseConfig() (OSConfig, error) {
	...
	return OSConfig{
		...
		// onto the Rspscale config unnecessarily.
	}, nil
}

// getBasePrimaryResolver returns a guess of the non-Rspscale primary
...
func (m *windowsManager) getBasePrimaryResolver() (resolvers []netip.Addr, err error) {
	...
	if len(candidates) == 0 {
		// No resolvers set outside of Rspscale.
		return nil, nil
	}
