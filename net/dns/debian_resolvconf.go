...
const resolvconfConfigName = "tun-rspscale.inet"
...
// to force Rspscale's DNS config to take effect.
var resolvconfHookPath = filepath.Join(resolvconfLibcHookPath, "rspscale")
...
func (m *resolvconfManager) deleteRspscaleConfig() error {
...
func (m *resolvconfManager) SetDNS(config OSConfig) error {
	...
	if config.IsZero() {
		if err := m.deleteRspscaleConfig(); err != nil {
			return err
		}
	} else {
