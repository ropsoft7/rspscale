...
func TestOSConfigPrintable(t *testing.T) {
	ocfg := OSConfig{
		...
		SearchDomains: []dnsname.FQDN{
			dnsname.FQDN("foo.beta.rspscale.net."),
			dnsname.FQDN("bar.beta.rspscale.net."),
		},
	...
	const expected = `{Nameservers:[8.8.8.8] SearchDomains:[foo.beta.rspscale.net. bar.beta.rspscale.net.] MatchDomains:[ts.com.] Hosts:[&{Addr:100.1.2.3 Hosts:[server client]} &{Addr:100.1.2.4 Hosts:[otherhost]}]}`
	if s != expected {
