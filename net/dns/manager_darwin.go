...
// darwinConfigurator is the rspscaled-on-macOS DNS OS configurator that
...
func (c *darwinConfigurator) SetDNS(cfg OSConfig) error {
	...
	if len(cfg.SearchDomains) > 0 {
		const searchFile = "search.rspscale" // fake DNS suffix+TLD to put our search
...
func (c *darwinConfigurator) GetBaseConfig() (OSConfig, error) {
	...
	for _, ns := range resolvConf.Nameservers {
		if ns == tsaddr.RspscaleServiceIP() || ns == tsaddr.RspscaleServiceIPv6() {
...
const macResolverFileHeader = "# Added by rspscaled\n"
