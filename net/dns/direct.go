...
// to the disappearance of the Rspscale interface.
...
// ownedByRspscale reports whether /etc/resolv.conf seems to be a
// rspscale-managed file.
func (m *directManager) ownedByRspscale() (bool, error) {
	...
	if bytes.Contains(bs, []byte("generated by rspscale")) {
		return true, nil
	}
	return false, nil
}
...
// resolv.conf does not currently contain a Rspscale-managed config.
...
func (m *directManager) restoreBackup() (restored bool, err error) {
	...
	if resolvConfExists && !owned {
		// There's already a non-rspscale config in place, get rid of
		// our backup.
		m.fs.Remove(backupConf)
		return false, nil
	}
...
func (m *directManager) SetDNS(config OSConfig) (err error) {
	defer func() {
		if err != nil && errors.Is(err, fs.ErrPermission) && runtime.GOOS == "linux" &&
			...
			// See https://github.com/ropsoft7/rspscale/issues/4017
...
func (m *directManager) GetBaseConfig() (OSConfig, error) {
	...
	if removed {
		m.logf("[v1] dropped Rspscale IP from base config that was a symlink")
	}
	return oscfg, nil
}
...
func (m *directManager) Close() error {
	...
	// We used to keep a file for the rspscale config and symlinked
	...
	m.fs.Remove("/etc/resolv.rspscale.conf")
