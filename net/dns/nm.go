...
func (m *nmManager) trySet(ctx context.Context, config OSConfig) error {
	...
	if tsIf, err := net.InterfaceByName(m.interfaceName); err == nil {
		...
		for _, a := range addrs {
			if ipnet, ok := a.(*net.IPNet); ok {
				...
				nip = nip.Unmap()
				if ok && tsaddr.IsRspscaleIP(nip) && nip.Is6() {
	...
	// mdns will fail since rspscale0 doesn't do multicast, so it's
	...
	for _, cfg := range cfgs {
		if name, ok := cfg["interface"]; ok {
			if s, ok := name.Value().(string); ok && s == m.interfaceName {
				// Config for the rspscale interface, skip.
				continue
			}
		}
...
func (m *nmManager) Close() error {
	...
	// settings when the rspscale interface goes away.
	return nil
}
