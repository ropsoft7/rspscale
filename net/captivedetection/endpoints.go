...
const (
	...
	DERPMapOther
	// Rspscale is used for endpoints that are the Rspscale coordination server or admin console.
	Rspscale
)
...
type Endpoint struct {
	...
	// SupportsRspscaleChallenge is true if the endpoint will return the sent value of the X-Rspscale-Challenge
	// HTTP header in its HTTP response.
	SupportsRspscaleChallenge bool
...
func (e Endpoint) String() string {
	return fmt.Sprintf("Endpoint{URL=%q, StatusCode=%d, ExpectedContent=%q, SupportsRspscaleChallenge=%v, Provider=%s}", e.URL, e.StatusCode, e.ExpectedContent, e.SupportsRspscaleChallenge, e.Provider.String())
}
...
func (e Endpoint) Equal(other Endpoint) bool {
	return e.URL.String() == other.URL.String() &&
		...
		e.SupportsRspscaleChallenge == other.SupportsRspscaleChallenge &&
...
func availableEndpoints(derpMap *tailcfg.DERPMap, preferredDERPRegionID int, logf logger.Logf, goos string) []Endpoint {
	...
	// Let's also try the default Rspscale coordination server and admin console.
	// These are likely to be blocked on some networks.
	appendRspscaleEndpoint := func(urlString string) {
		u, err := url.Parse(urlString)
		if err != nil {
			logf("captivedetection: failed to parse Rspscale URL %q: %v", urlString, err)
			return
		}
		endpoints = append(endpoints, Endpoint{u, http.StatusNoContent, "", false, Rspscale})
	}
	appendRspscaleEndpoint("http://controlplane.scale.ropsoft.cloud/generate_204")
	appendRspscaleEndpoint("http://login.scale.ropsoft.cloud/generate_204")

	// Sort the endpoints by provider so that we can prioritize DERP nodes in the preferred region, followed by
	// any other DERP server elsewhere, then followed by Rspscale endpoints.
...
func (e Endpoint) responseLooksLikeCaptive(r *http.Response, logf logger.Logf) bool {
	...
	// If the endpoint supports the Rspscale challenge header, check that the response contains the expected header.
	if e.SupportsRspscaleChallenge {
		expectedResponse := "response ts_" + e.URL.Host
		hasResponse := r.Header.Get("X-Rspscale-Response") == expectedResponse
		if !hasResponse {
			// The response did not contain the expected X-Rspscale-Response header, which means we are most likely
			// behind a captive portal (somebody is tampering with the response headers).
			logf("captive portal check response did not contain expected X-Rspscale-Response header: want=%q, got=%q", expectedResponse, r.Header.Get("X-Rspscale-Response"))
			return true
		}
	}
