...
func interfaceNameDoesNotNeedCaptiveDetection(ifName string, goos string) bool {
	...
	excludedPrefixes := []string{"rspscale", "tun", "tap", "docker", "kube", "wg", "ipsec"}
	if goos == "windows" {
...
func (d *Detector) verifyCaptivePortalEndpoint(ctx context.Context, e Endpoint, ifIndex int) (found bool, err error) {
	...
	// Attach the Rspscale challenge header if the endpoint supports it. Not all captive portal detection endpoints
	...
	if e.SupportsRspscaleChallenge {
		...
		// details.
		chal := "ts_" + e.URL.Host
		req.Header.Set("X-Rspscale-Challenge", chal)
	}
