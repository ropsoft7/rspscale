...
// route, and is not the Rspscale WinTun interface.
...
func getInterfaceIndex(logf logger.Logf, addr netip.Addr, defaultIdx uint32) (idx uint32, err error) {
	...
	isTS, err := isRspscaleInterface(idx)
	if err != nil {
		return defaultIdx, fmt.Errorf("isRspscaleInterface: %w", err)
	}
	if isTS {
...
func isRspscaleInterface(ifaceIdx uint32) (bool, error) {
