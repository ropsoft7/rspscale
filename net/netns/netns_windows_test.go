...
func TestGetInterfaceIndex(t *testing.T) {
	...
	t.Run("NoRspscale", func(t *testing.T) {
		tsIdx, ok, err := rspscaleInterfaceIndex()
		if err != nil {
			t.Fatal(err)
		}
		if !ok {
		...
		t.Logf("rspscaleIdx=%d defaultIdx=%d idx=%d", tsIdx, defaultIdx, idx)
...
func rspscaleInterfaceIndex() (idx uint32, found bool, err error) {
