...
func controlC(network, address string, c syscall.RawConn) error {
	...
	if sockErr != nil && ignoreErrors() {
		// TODO(bradfitz): maybe log once? probably too spammy for e.g. CLI tools like rspscale netcheck.
		return nil
	}
	return sockErr
}
...
func setBypassMark(fd uintptr) error {
	if err := unix.SetsockoptInt(int(fd), unix.SOL_SOCKET, unix.SO_MARK, linuxfw.RspscaleBypassMarkNum); err != nil {
