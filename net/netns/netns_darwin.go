...
func getInterfaceIndex(logf logger.Logf, netMon *netmon.Monitor, address string) (int, error) {
	...
	// Verify that we didn't just choose the Rspscale interface;
	// if so, we fall back to binding from the default.
	tsif, err2 := rspscaleInterface()
	if err2 == nil && tsif != nil && tsif.Index == idx {
		logf("[unexpected] netns: interfaceIndexFor returned Rspscale interface")
		return defaultIdx()
	}
...
// rspscaleInterface returns the current machine's Rspscale interface, if any.
// If none is found, (nil, nil) is returned.
// A non-nil error is only returned on a problem listing the system interfaces.
func rspscaleInterface() (*net.Interface, error) {
	...
	for _, iface := range ifs {
		...
		for _, a := range addrs {
			if ipnet, ok := a.(*net.IPNet); ok {
				...
				if ok && tsaddr.IsRspscaleIP(nip.Unmap()) {
					return &iface, nil
				}
			}
		}
