...
// TSMP is our ICMP-like "Rspscale Message Protocol" for signaling
// Rspscale-specific messages between nodes. It uses IP protocol 99
...
// RspscaleRejectedHeader is a TSMP message that says that one
// Rspscale node has rejected the connection from another. Unlike a
...
type RspscaleRejectedHeader struct {
	...
	Reason RspscaleRejectReason // why the connection was rejected
	...
	// it might be. For example, the target rspscaled might think
	// its host firewall or IP forwarding aren't configured
	// properly, but rspscaled might be wrong (not having enough
...
func (rh RspscaleRejectedHeader) Flow() flowtrack.Tuple {
	return flowtrack.MakeTuple(rh.Proto, rh.Src, rh.Dst)
}

func (rh RspscaleRejectedHeader) String() string {
...
const (
	// TSMPTypeRejectedConn is the type byte for a RspscaleRejectedHeader.
	TSMPTypeRejectedConn TSMPType = '!'

	// TSMPTypePing is the type byte for a RspscalePingRequest.
	TSMPTypePing TSMPType = 'p'

	// TSMPTypePong is the type byte for a RspscalePongResponse.
	TSMPTypePong TSMPType = 'o'
)

type RspscaleRejectReason byte

// IsZero reports whether r is the zero value, representing no rejection.
func (r RspscaleRejectReason) IsZero() bool { return r == RspscaleRejectReasonNone }

const (
	// RspscaleRejectReasonNone is the RspscaleRejectReason zero value.
	RspscaleRejectReasonNone RspscaleRejectReason = 0

	// RejectedDueToACLs means that the host rejected the connection due to ACLs.
	RejectedDueToACLs RspscaleRejectReason = 'A'

	// RejectedDueToShieldsUp means that the host rejected the connection due to shields being up.
	RejectedDueToShieldsUp RspscaleRejectReason = 'S'

	// RejectedDueToIPForwarding means that the relay node's IP
	// forwarding is disabled.
	RejectedDueToIPForwarding RspscaleRejectReason = 'F'

	// RejectedDueToHostFirewall means that the target host's
	// firewall is blocking the traffic.
	RejectedDueToHostFirewall RspscaleRejectReason = 'W'
)

func (r RspscaleRejectReason) String() string {
	switch r {
...
func (h RspscaleRejectedHeader) hasFlags() bool {
	return h.MaybeBroken // the only one currently
}

func (h RspscaleRejectedHeader) Len() int {
	v := 1 + // TSMPType byte
		1 + // IPProto byte
		1 + // RspscaleRejectReason byte
		2*2 // 2 uint16 ports
	if h.IPSrc.Is4() {
		v += ip4HeaderLength
	} else if h.IPSrc.Is6() {
...
func (h RspscaleRejectedHeader) Marshal(buf []byte) error {
	if len(buf) < h.Len() {
...
// AsRspscaleRejectedHeader parses pp as an incoming rejection
// connection TSMP message.
//
// ok reports whether pp was a valid TSMP rejection packet.
func (pp *Parsed) AsRspscaleRejectedHeader() (h RspscaleRejectedHeader, ok bool) {
	p := pp.Payload()
	if len(p) < 7 || p[0] != byte(TSMPTypeRejectedConn) {
		return
	}
	h = RspscaleRejectedHeader{
		Proto:  ipproto.Proto(p[1]),
		Reason: RspscaleRejectReason(p[2]),
		IPSrc:  pp.Src.Addr(),
		IPDst:  pp.Dst.Addr(),
