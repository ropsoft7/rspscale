...
import (
	...
	tsmetrics "scale.ropsoft.cloud/metrics"
	"scale.ropsoft.cloud/net/connstats"
	"scale.ropsoft.cloud/net/packet"
	"scale.ropsoft.cloud/net/packet/checksum"
...
//
// Start must be called exactly once after the various Rspscale
...
// findV4 returns the first Rspscale IPv4 address in addrs.
func findV4(addrs []netip.Prefix) netip.Addr {
	for _, ap := range addrs {
		a := ap.Addr()
		if a.Is4() && tsaddr.IsRspscaleIP(a) {
			return a
		}
	}
	return netip.Addr{}
...
// findV6 returns the first Rspscale IPv6 address in addrs.
func findV6(addrs []netip.Prefix) netip.Addr {
	for _, ap := range addrs {
		a := ap.Addr()
		if a.Is6() && tsaddr.IsRspscaleIP(a) {
			return a
		}
	}
	return netip.Addr{}
...
type peerConfigTable struct {
	// nativeAddr4 and nativeAddr6 are the IPv4/IPv6 Rspscale Addresses of
	// the current node.
	//
...
func (pc *peerConfigTable) selectSrcIP(oldSrc, dst netip.Addr) netip.Addr {
	...
	// If this packet doesn't originate from this Rspscale node, don't
...
var (
	magicDNSIPPort   = netip.AddrPortFrom(tsaddr.RspscaleServiceIP(), 0) // 100.100.100.100:0
	magicDNSIPPortv6 = netip.AddrPortFrom(tsaddr.RspscaleServiceIPv6(), 0)
)
...
func (t *Wrapper) filterPacketOutboundToWireGuard(p *packet.Parsed, pc *peerConfigTable, gro *gro.GRO) (filter.Response, *gro.GRO) {
	...
	// Rspscale from ourselves, then drop them, as that shouldn't
