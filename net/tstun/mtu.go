...
//
// Wire MTU: The MTU of an interface underneath the rspscale TUN, e.g. an
// Ethernet network card will default to a 1500 byte MTU. The user may change
// this MTU at any time.
//
// TUN MTU: The current MTU of the rspscale TUN. This MTU is adjusted downward
// to make room for the wireguard/rspscale headers. For example, if the
// underlying network interface's MTU is 1500 bytes, the maximum size of a
// packet entering the rspscale TUN is 1420 bytes. The user may change this MTU
// at any time via the OS's tools (ifconfig, ip, etc.).
//
// User configured initial MTU: The MTU the rspscale TUN should be created
...
// the MTU() method of rspscale's TUN wrapper.
//
// Maximum probed MTU: This is the largest MTU size that we send probe packets
// for.
//
// Safe MTU: If the rspscale TUN MTU is set to this value, almost all packets
// will get to their destination. Rspscale defaults to this MTU in the absence
...
//
// Initial MTU: This is the MTU rspscaled creates the TUN with. In order of
// priority, it is:
//
...
//
// Current MTU: This the MTU of the rspscale TUN at any given moment
...
// TUNMTU is the MTU for the rspscale TUN.
type TUNMTU uint32
...
const (
	// maxTUNMTU is the largest MTU we will consider for the Rspscale
	...
	// safeTUNMTU is the default "safe" MTU for the Rspscale TUN that we
...
var WireMTUsToProbe = []WireMTU{
	WireMTU(safeTUNMTU),      // Rspscale over Rspscale :)
...
// (e.g., calculating the MTU for the Rspscale interface.
//
...
// TUNToWireMTU takes the MTU that the Rspscale TUN presents to the user and
...
// largest possible MTU for a Rspscale TUN operating on top of that device,
...
// DefaultTUNMTU returns the MTU we use to set the Rspscale TUN
