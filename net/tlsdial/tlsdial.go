...
// doesn't have TLS roots, we can at least connect to Rspscale's LetsEncrypt
// services.  It's the unified point where we can add shared policy on outgoing
// TLS connections from the three places in the client that connect to Rspscale
// (logs, control, DERP).
package tlsdial
...
import (
	...
	"scale.ropsoft.cloud/net/tlsdial/blockblame"
)
...
var mitmBlockWarnable = health.Register(&health.Warnable{
	...
	Title: "Network may be blocking Rspscale",
	Text: func(args health.Args) string {
		return fmt.Sprintf("Network equipment from %q may be blocking Rspscale traffic on this network. Connect to another network, or contact your network administrator for assistance.", args["manufacturer"])
	},
...
func Config(host string, ht *health.Tracker, base *tls.Config) *tls.Config {
	...
	conf.VerifyConnection = func(cs tls.ConnectionState) (retErr error) {
		if host == "log.rspscale.io" && hostinfo.IsNATLabGuestVM() {
			// Allow log.rspscale.io TLS MITM for integration tests when
		...
		if ht != nil {
			defer func() {
				if retErr != nil && cert != nil {
					...
					if ok {
						log.Printf("tlsdial: server cert for %q looks like %q equipment (could be blocking Rspscale)", host, m.Name)
...
(Rspscale runs on some ancient devices.)
...
Then restart rspscaled. To also test dnsfallback's use of it, nuke
