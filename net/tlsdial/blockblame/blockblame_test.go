// Copyright (c) Rspscale Inc & AUTHORS
// SPDX-License-Identifier: BSD-3-Clause

package blockblame

import (
	"crypto/x509"
	"encoding/pem"
	"testing"
)

const controlplaneDotRspscaleDotComPEM = `
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

func TestVerifyCertificateOurControlPlane(t *testing.T) {
	p, _ := pem.Decode([]byte(controlplaneDotRspscaleDotComPEM))
	if p == nil {
		t.Fatalf("failed to extract certificate bytes for controlplane.scale.ropsoft.cloud")
		return
	}
	cert, err := x509.ParseCertificate(p.Bytes)
	if err != nil {
		t.Fatalf("failed to parse certificate: %v", err)
		return
	}
	m, found := VerifyCertificate(cert)
	if found {
		t.Fatalf("expected to not get a result for the controlplane.scale.ropsoft.cloud certificate")
	}
	if m != nil {
		t.Fatalf("expected nil manufacturer for controlplane.scale.ropsoft.cloud certificate")
	}
}
