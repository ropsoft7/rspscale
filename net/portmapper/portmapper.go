...
//go:generate go run scale.ropsoft.cloud/cmd/addlicense -file pmpresultcode_string.go go run golang.org/x/tools/cmd/stringer -type=pmpResultCode -trimprefix=pmpCode
...
func (c *Client) Probe(ctx context.Context) (res ProbeResult, err error) {
	...
	// https://github.com/ropsoft7/rspscale/issues/1001
	if c.sawPMPRecently() {
		res.PMP = true
	...
	if c.sawUPnPRecently() {
		...
		// multicast query.
		//
		// See https://github.com/ropsoft7/rspscale/issues/3197 for
		...
		// in response to ssdp:all. https://github.com/ropsoft7/rspscale/issues/3557
	...
	for {
		...
		handleUPnPResponse := func() {
			...
			if ip != gw {
				// https://github.com/ropsoft7/rspscale/issues/5502
		...
		default:
			// https://github.com/ropsoft7/rspscale/issues/7377
			if mem.Contains(mem.B(buf[:n]), mem.S(":InternetGatewayDevice:")) {
...
// https://github.com/ropsoft7/rspscale/issues/3557
