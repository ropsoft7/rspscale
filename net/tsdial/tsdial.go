...
// Package tsdial provides a Dialer type that can dial out of rspscaled.
package tsdial
...
// NewDialer returns a new Dialer that can dial out of rspscaled.
...
// Dialer dials out of rspscaled, while taking care of details while
...
type Dialer struct {
	...
	routes atomic.Pointer[bart.Table[bool]] // or nil if UserDial should not use routes. `true` indicates routes that point into the Rspscale interface
...
// SetTUNName sets the name of the tun device in use ("rspscale0", "utun6",
...
// Example format ("rspscale0", "utun6").
...
// SetRoutes configures the dialer to dial the specified routes via Rspscale,
...
func changeAffectsConn(delta *netmon.ChangeDelta, conn net.Conn) bool {
	...
	// Android; see rspscale/corp#19124); if so, pessimistically assume
...
// Rspscale. It prefers going over the default interface and closes existing
...
func (d *Dialer) UserDial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if routes := d.routes.Load(); routes != nil {
		if isRspscaleRoute, _ := routes.Lookup(ipp.Addr()); isRspscaleRoute {
	...
	// Workaround for macOS for now: dial Rspscale IPs with peer dialer.
	...
	if version.IsMacGUIVariant() && tsaddr.IsRspscaleIP(ipp.Addr()) {
		return d.getPeerDialer().DialContext(ctx, network, ipp.String())
	}
...
// dialPeerAPI connects to a Rspscale peer's peerapi over TCP.
//
...
// "rspscale nc", or querying internal DNS servers over Rspscale)
//
