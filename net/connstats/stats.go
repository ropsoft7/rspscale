...
var (
	rspscaleServiceIPv4 = tsaddr.RspscaleServiceIP()
	rspscaleServiceIPv6 = tsaddr.RspscaleServiceIPv6()
)
...
func (s *Statistics) updateVirtual(b []byte, receive bool) {
	...
	// Network logging is defined as traffic between two Rspscale nodes.
	// Traffic with the internal Rspscale service is not with another node
	...
	case rspscaleServiceIPv4, rspscaleServiceIPv6:
		return
	}
...
// The src is always a Rspscale IP address, representing some remote peer.
// The dst is a remote IP address and port that corresponds
// with some physical peer backing the Rspscale IP address.
