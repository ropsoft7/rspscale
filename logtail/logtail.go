...
// Package logtail sends logs to log.rspscale.io.
package logtail
...
import (
	...
	tslogger "scale.ropsoft.cloud/types/logger"
	"scale.ropsoft.cloud/types/logid"
	"scale.ropsoft.cloud/util/set"
	"scale.ropsoft.cloud/util/truncate"
	"scale.ropsoft.cloud/util/zstdframe"
)
...
const DefaultHost = "log.rspscale.io"

const defaultFlushDelay = 2 * time.Second

const (
	// CollectionNode is the name of a logtail Config.Collection
	// for rspscaled (or equivalent: IPNExtension, Android app).
	CollectionNode = "tailnode.log.rspscale.io"
)
...
type Config struct {
	...
	BaseURL        string          // if empty defaults to "https://log.rspscale.io"
...
func (l *Logger) upload(ctx context.Context, body []byte, origlen int) (retryAfter time.Duration, err error) {
	...
	if runtime.GOOS == "js" {
		...
		// on log.rspscale.io but then Rspscale SSH js/wasm clients prompted
...
// unrecoverable error.
//
// TODO(bradfitz): this apparently just returns nil, as of rspscale/corp@9c2ec35.
...
// appendText appends a raw text message in the Rspscale JSON log entry format.
...
// in the Rspscale JSON log format.
func (l *Logger) appendTextOrJSONLocked(dst, src []byte, level int) []byte {
	...
	// That's okay as the Rspscale log service limit is actually 2*maxSize.
...
// redactIPs is a helper function used in Write() to redact IPs (other than rspscale IPs).
...
func redactIPs(buf []byte) []byte {
	out := regexMatchesIPv6.ReplaceAllFunc(buf, func(b []byte) []byte {
		...
		if err != nil || tsaddr.IsRspscaleIP(ip) {
...
// rspscaled. It works less well for a binary with multiple tsnet.Servers. Oh
...
// (somebody running "rspscale debug daemon-logs").
