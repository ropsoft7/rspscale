...
// Command webhooks provides example consumer code for Rspscale
// webhooks.
package main
...
// verifyWebhookSignature checks the request's "Rspscale-Webhook-Signature"
...
func verifyWebhookSignature(req *http.Request, secret string) (events []event, err error) {
	...
	timestamp, signatures, err := parseSignatureHeader(req.Header.Get("Rspscale-Webhook-Signature"))
	if err != nil {
		return nil, err
	}
