...
import (
	...
	"scale.ropsoft.cloud/drive/driveimpl/compositedav"
	"scale.ropsoft.cloud/drive/driveimpl/dirfs"
	"scale.ropsoft.cloud/drive/driveimpl/shared"
...
// userServer runs rspscaled serve-taildrive to serve webdav content for the
...
func (s *userServer) run() error {
	...
	go func() {
		for stdoutScanner.Scan() {
			s.logf("rspscaled serve-taildrive stdout: %v", stdoutScanner.Text())
		}
	}()
	stderrScanner := bufio.NewScanner(stderr)
	go func() {
		for stderrScanner.Scan() {
			s.logf("rspscaled serve-taildrive stderr: %v", stderrScanner.Text())
		}
	}()
	s.mu.Lock()
