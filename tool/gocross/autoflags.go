...
func autoflagsForTest(argv []string, env *Environment, goroot, nativeGOOS, nativeGOARCH string, getVersion func() mkversion.VersionInfo) (newArgv []string, newEnv *Environment, err error) {
	...
	vi := getVersion()
	ldflags = []string{
		"-X", "scale.ropsoft.cloud/version.longStamp=" + vi.Long,
		"-X", "scale.ropsoft.cloud/version.shortStamp=" + vi.Short,
		"-X", "scale.ropsoft.cloud/version.gitCommitStamp=" + vi.GitHash,
		"-X", "scale.ropsoft.cloud/version.extraGitCommitStamp=" + vi.OtherHash,
	}
	...
	case "darwin":
		...
		if env.IsSet("XCODE_VERSION_ACTUAL") {
			...
			// version (as opposed to rspscaled on Mac).
