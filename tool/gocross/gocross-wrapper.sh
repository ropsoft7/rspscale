...
*)
    ...
    if [[ ! -f "$toolchain.extracted" ]]; then
        ...
        # (we do not build rspscale-go for other targets).
...
if [[ "$gocross_ok" == "0" ]]; then
    ...
    "$toolchain/bin/go" build -o "$gocross_path" -ldflags "-X scale.ropsoft.cloud/version.gitCommitStamp=$wantver" scale.ropsoft.cloud/tool/gocross
fi
