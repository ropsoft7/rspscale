...
// Package tailcfg contains types used by the Rspscale protocol with between
// the node and the coordination server.
package tailcfg

//go:generate go run scale.ropsoft.cloud/cmd/viewer --type=User,Node,Hostinfo,NetInfo,Login,DNSConfig,RegisterResponse,RegisterResponseAuth,RegisterRequest,DERPHomeParams,DERPRegion,DERPMap,DERPNode,SSHRule,SSHAction,SSHPrincipal,ControlDialPlan,Location,UserProfile --clonefunc
...
import (
	...
	"scale.ropsoft.cloud/types/structs"
	"scale.ropsoft.cloud/types/tkatype"
	"scale.ropsoft.cloud/util/dnsname"
	"scale.ropsoft.cloud/util/slicesx"
)
...
//   - 20: 2021-06-11: MapResponse.LastSeen used even less (https://github.com/ropsoft7/rspscale/issues/2107)
...
//   - 24: 2021-09-18: MapResponse.Health from control to node; node shows in "rspscale status"
...
//   - 42: 2022-09-06: NextDNS DoH support; see https://github.com/ropsoft7/rspscale/pull/5556
...
// In particular: domain. If a user has a "rspscale.io" domain login, they cannot
...
type Node struct {
	...
	// such as:
	//    "https://scale.ropsoft.cloud/cap/is-admin"
	//    "https://scale.ropsoft.cloud/cap/file-sharing"
	//
	// Deprecated: use CapMap instead. See https://github.com/ropsoft7/rspscale/issues/11508
	...
	// it do anything. It is the rspscaled client's job to double-check the
	...
	// IsWireGuardOnly indicates that this is a non-Rspscale WireGuard peer, it
...
type Service struct {
	...
	//        node's Rspscale IPv4 address.
	//     * "peerapi6": peerapi is available on IPv6; Port is the
	//        port number that the peerapi is running on the
	//        node's Rspscale IPv6 address.
...
// Rspscale host. Location is optional and only set if
...
// Hostinfo contains a summary of a Rspscale host.
//
...
type Hostinfo struct {
	...
	//
	// For Linux, prior to Rspscale 1.32, we jammed a bunch of fields into this
	// string on Linux, like "Debian 10.4; kernel=xxx; container; env=kn" and so
	// on. As of Rspscale 1.32, this is simply the kernel version on Linux, like
	// "5.10.0-17-amd64".
	...
	// App is used to disambiguate Rspscale clients that run using tsnet.
	App string `json:",omitempty"` // "k8s-operator", "golinks", ...

	Desktop         opt.Bool       `json:",omitempty"` // if a desktop was detected on Linux
	Package         string         `json:",omitempty"` // Rspscale package to disambiguate ("choco", "appstore", etc; "" for unknown)
...
// RspscaleSSHEnabled reports whether or not this node is acting as a
// Rspscale SSH server.
func (hi *Hostinfo) RspscaleSSHEnabled() bool {
	// Currently, we use `SSH_HostKeys` as a proxy for this. However, we may later
	// include non-Rspscale host keys, and will add a separate flag to rely on.
	return hi != nil && len(hi.SSH_HostKeys) > 0
}

func (v HostinfoView) RspscaleSSHEnabled() bool { return v.ж.RspscaleSSHEnabled() }

// RspscaleFunnelEnabled reports whether or not this node has explicitly
// enabled Funnel.
func (hi *Hostinfo) RspscaleFunnelEnabled() bool {
	return hi != nil && hi.WireIngress
}

func (v HostinfoView) RspscaleFunnelEnabled() bool { return v.ж.RspscaleFunnelEnabled() }
...
//
//	https://login.scale.ropsoft.cloud/machine/<mkey hex>
...
// using the local machine key, and sent to:
//
//	https://login.scale.ropsoft.cloud/machine/<mkey hex>/map
...
// application-defined.
//
// It must be a URL like "https://scale.ropsoft.cloud/cap/file-send".
...
const (
	...
	PeerCapabilityFileSharingTarget PeerCapability = "https://scale.ropsoft.cloud/cap/file-sharing-target"
	// PeerCapabilityFileSharingSend grants the ability to receive files from a
	// node that's owned by a different user.
	PeerCapabilityFileSharingSend PeerCapability = "https://scale.ropsoft.cloud/cap/file-send"
	// PeerCapabilityDebugPeer grants the ability for a peer to read this node's
	// goroutines, metrics, magicsock internal state, etc.
	PeerCapabilityDebugPeer PeerCapability = "https://scale.ropsoft.cloud/cap/debug-peer"
	// PeerCapabilityWakeOnLAN grants the ability to send a Wake-On-LAN packet.
	PeerCapabilityWakeOnLAN PeerCapability = "https://scale.ropsoft.cloud/cap/wake-on-lan"
	// PeerCapabilityIngress grants the ability for a peer to send ingress traffic.
	PeerCapabilityIngress PeerCapability = "https://scale.ropsoft.cloud/cap/ingress"
	// PeerCapabilityWebUI grants the ability for a peer to edit features from the
	// device Web UI.
	PeerCapabilityWebUI PeerCapability = "scale.ropsoft.cloud/cap/webui"
	// PeerCapabilityTaildrive grants the ability for a peer to access Taildrive
	// shares.
	PeerCapabilityTaildrive PeerCapability = "scale.ropsoft.cloud/cap/drive"
	// PeerCapabilityTaildriveSharer indicates that a peer has the ability to
	// share folders with us.
	PeerCapabilityTaildriveSharer PeerCapability = "scale.ropsoft.cloud/cap/drive-sharer"

	// PeerCapabilityKubernetes grants a peer Kubernetes-specific
	// capabilities, such as the ability to impersonate specific Rspscale
	// user groups as Kubernetes user groups. This capability is read by
	// peers that are Rspscale Kubernetes operator instances.
	PeerCapabilityKubernetes PeerCapability = "scale.ropsoft.cloud/cap/kubernetes"
	...
	PeerCapabilityServicesDestination PeerCapability = "scale.ropsoft.cloud/cap/services-destination"
)
...
//
// The values are opaque to Rspscale, but are passed through from the ACLs to
...
type FilterRule struct {
	...
	// ignored.
	//
	// This is still present in this file because the Rspscale control plane
	...
	// reserved for Rspscale's use. Unknown ones are ignored.
	//
...
type DNSConfig struct {
	...
	// be handled by Rspscale's built-in resolver (100.100.100.100), such
	...
	// https://github.com/ropsoft7/rspscale/issues/1743
...
const (
	...
	// PingICMP performs a ping between two rspscale nodes using ICMP that is
	// received by the target systems IP stack.
	PingICMP PingType = "ICMP"
	// PingPeerAPI performs a ping between two rspscale nodes using ICMP that is
	// received by the target systems IP stack.
...
// Typically populated from an ipnstate.PingResult used in `rspscale ping`.
type PingResponse struct {
	Type PingType // ping type, such as TSMP or disco.

	IP       string `json:",omitempty"` // ping destination
	NodeIP   string `json:",omitempty"` // Rspscale IP of node handling IP (different for subnet routers)
	...
	// e.g. "100.1.2.3 is local Rspscale IP"
...
//
// It must be a URL like "https://scale.ropsoft.cloud/cap/file-sharing", or a
// well-known capability name like "funnel". The latter is only allowed for
// Rspscale-defined capabilities.
//
...
const (
	CapabilityFileSharing        NodeCapability = "https://scale.ropsoft.cloud/cap/file-sharing"
	CapabilityAdmin              NodeCapability = "https://scale.ropsoft.cloud/cap/is-admin"
	CapabilitySSH                NodeCapability = "https://scale.ropsoft.cloud/cap/ssh"                   // feature enabled/available
	CapabilitySSHRuleIn          NodeCapability = "https://scale.ropsoft.cloud/cap/ssh-rule-in"           // some SSH rule reach this node
	CapabilityDataPlaneAuditLogs NodeCapability = "https://scale.ropsoft.cloud/cap/data-plane-audit-logs" // feature enabled
	CapabilityDebug              NodeCapability = "https://scale.ropsoft.cloud/cap/debug"                 // exposes debug endpoints over the PeerAPI
	...
	CapabilityBindToInterfaceByRoute NodeCapability = "https://scale.ropsoft.cloud/cap/bind-to-interface-by-route"
	...
	// table).
	CapabilityDebugDisableAlternateDefaultRouteInterface NodeCapability = "https://scale.ropsoft.cloud/cap/debug-disable-alternate-default-route-interface"
	...
	CapabilityDebugDisableBindConnToInterface NodeCapability = "https://scale.ropsoft.cloud/cap/debug-disable-bind-conn-to-interface"

	// CapabilityTailnetLock indicates the node may initialize tailnet lock.
	CapabilityTailnetLock NodeCapability = "https://scale.ropsoft.cloud/cap/tailnet-lock"
	...
	CapabilityWarnFunnelNoInvite NodeCapability = "https://scale.ropsoft.cloud/cap/warn-funnel-no-invite"

	// CapabilityWarnFunnelNoHTTPS indicates HTTPS has not been enabled for the tailnet.
	// This cap is no longer used 2023-08-09 onwards.
	CapabilityWarnFunnelNoHTTPS NodeCapability = "https://scale.ropsoft.cloud/cap/warn-funnel-no-https"

	// Debug logging capabilities

	// CapabilityDebugTSDNSResolution enables verbose debug logging for DNS
	// resolution for Rspscale-controlled domains (the control server, log
	// server, DERP servers, etc.)
	CapabilityDebugTSDNSResolution NodeCapability = "https://scale.ropsoft.cloud/cap/debug-ts-dns-resolution"
	...
	// e.g. https://scale.ropsoft.cloud/cap/funnel-ports?ports=80,443,8080-8090
	CapabilityFunnelPorts NodeCapability = "https://scale.ropsoft.cloud/cap/funnel-ports"
	...
	// "full" ones as rspscaled did in 1.48.x and earlier.
	...
	// Added 2024-05-29 in Rspscale version 1.68.
	...
	// V1 and V2 behavior.
	// Added 2024-08-06 in Rspscale version 1.72.
	...
	// magicsock cryptorouting hook. See rspscale/corp#20732.
...
// using the local machine key, and sent to:
//
//	https://login.scale.ropsoft.cloud/machine/<mkey hex>/set-dns
...
type SSHAction struct {
	...
	// session is open.
	//
	// The following variables in the URL are expanded by rspscaled:
	//
...
// over HTTPS (regular TLS) to the Rspscale control plane server,
...
type TokenResponse struct {
	...
	//   `nbf` | Not before time
	//
	// It also encodes the following Rspscale specific claims:
	//
	//   `key`       | the node public key
	//   `addresses` | the Rspscale IPs of the node
	//   `nid`       | the node ID
