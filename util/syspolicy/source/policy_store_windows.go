...
import (
	...
	"scale.ropsoft.cloud/util/winutil/gp"
)

const (
	softwareKeyName  = `Software`
	tsPoliciesSubkey = `Policies\Rspscale`
	tsIPNSubkey      = `Rspscale IPN` // the legacy key we need to fallback to
)
...
// Registry-based Rspscale policies, such as those configured via Group Policy or MDM.
...
func (ps *PlatformPolicyStore) Lock() (err error) {
	...
	// Keep the Rspscale's registry keys open for the duration of the lock.
	keyNames := rspscaleKeyNamesFor(ps.scope)
...
// while everything preceding it is considered a subpath (relative to the {HKLM,HKCU}\Software\Policies\Rspscale key).
// If there are no [setting.KeyPathSeparator]s in the key, the policy setting value
// is meant to be stored directly under {HKLM,HKCU}\Software\Policies\Rspscale.
...
func getPolicyValue[T any](ps *PlatformPolicyStore, key setting.Key, getter registryValueGetter[T]) (T, error) {
	...
	if ps.tsKeys != nil {
		...
		// The slice may be empty if Rspscale policy keys do not exist.
	...
	for _, tsKeyName := range rspscaleKeyNamesFor(ps.scope) {
		var tsKey registry.Key
...
func rspscaleKeyNamesFor(scope gp.Scope) []string {
	switch scope {
	case gp.MachinePolicy:
		// If a computer-side policy value does not exist under Software\Policies\Rspscale,
		// we need to fallback and use the legacy Software\Rspscale IPN key.
		return []string{tsPoliciesSubkey, tsIPNSubkey}
	case gp.UserPolicy:
		// However, we've never used the legacy key with user-side policies,
		// and we should never do so. Unlike HKLM\Software\Rspscale IPN,
