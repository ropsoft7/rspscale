...
import (
	...
	"scale.ropsoft.cloud/util/syspolicy/internal/loggerx"
...
//
// See https://github.com/ropsoft7/rspscale/issues/2798 for some background.
func SelectControlURL(reg, disk string) string {
	const def = "https://controlplane.scale.ropsoft.cloud"

	// Prior to Dec 2020's commit 739b02e6, the installer
	// wrote a LoginURL value of https://login.scale.ropsoft.cloud to the registry.
	const oldRegDef = "https://login.scale.ropsoft.cloud"
	...
	if reg != "" {
		...
		if disk != def && disk != oldRegDef {
			...
			// default (login.scale.ropsoft.cloud) but the value
