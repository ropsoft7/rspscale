...
func TestAddAndDeleteBase(t *testing.T) {
	...
	tsRulesCommon := []fakeRule{ // table/chain/rule
		...
		{"filter", "ts-forward", []string{"-i", tunname, "-j", "MARK", "--set-mark", RspscaleSubnetRouteMark + "/" + RspscaleFwmarkMask}},
		{"filter", "ts-forward", []string{"-m", "mark", "--mark", RspscaleSubnetRouteMark + "/" + RspscaleFwmarkMask, "-j", "ACCEPT"}},
...
func TestAddAndDelSNATRule(t *testing.T) {
	...
	rule := fakeRule{ // table/chain/rule
		"nat", "ts-postrouting", []string{"-m", "mark", "--mark", RspscaleSubnetRouteMark + "/" + RspscaleFwmarkMask, "-j", "MASQUERADE"},
	}
