...
func Test_iptablesRunner_EnsurePortMapRuleForSvc(t *testing.T) {
	...
	v4Rule := argsForPortMapRule("test-svc", "rspscale0", v4Addr, testPM)
	tests := []struct {
		name              string
		targetIP          netip.Addr
		svc               string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := iptr.EnsurePortMapRuleForSvc(tt.svc, "rspscale0", tt.targetIP, tt.pm); err != nil {
				t.Errorf("[unexpected error] iptablesRunner.EnsurePortMapRuleForSvc() = %v", err)
			}
			args := argsForPortMapRule(tt.svc, "rspscale0", tt.targetIP, tt.pm)
...
func Test_iptablesRunner_DeletePortMapRuleForSvc(t *testing.T) {
	...
	v6Addr := netip.MustParseAddr("fd7a:115c:a1e0::701:b62a")
	testPM := PortMap{Protocol: "tcp", MatchPort: 4003, TargetPort: 80}
	v4Rule := argsForPortMapRule("test", "rspscale0", v4Addr, testPM)
	v6Rule := argsForPortMapRule("test", "rspscale0", v6Addr, testPM)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := iptr.DeletePortMapRuleForSvc(tt.svc, "rspscale0", tt.targetIP, tt.pm); err != nil {
				t.Errorf("iptablesRunner.DeletePortMapRuleForSvc() errored: %v ", err)
			}
			deletedRule := argsForPortMapRule(tt.svc, "rspscale0", tt.targetIP, tt.pm)
...
func Test_iptablesRunner_DeleteSvc(t *testing.T) {
	...
	s1R1 := argsForPortMapRule("svc1", "rspscale0", v4Addr, testPM)
	mustPrecreatePortMapRule(t, s1R1, iptr.getIPTByAddr(v4Addr))
	s1R2 := argsForPortMapRule("svc1", "rspscale0", v6Addr, testPM)
	mustPrecreatePortMapRule(t, s1R2, iptr.getIPTByAddr(v6Addr))

	// create two rules that will consitute svc2
	s2R1 := argsForPortMapRule("svc2", "rspscale0", v4Addr, testPM)
	mustPrecreatePortMapRule(t, s2R1, iptr.getIPTByAddr(v4Addr))
	s2R2 := argsForPortMapRule("svc2", "rspscale0", v6Addr, testPM)
	mustPrecreatePortMapRule(t, s2R2, iptr.getIPTByAddr(v6Addr))

	// delete svc1
	if err := iptr.DeleteSvc("svc1", "rspscale0", []netip.Addr{v4Addr, v6Addr}, []PortMap{testPM}); err != nil {
