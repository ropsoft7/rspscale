...
func Test_nftablesRunner_EnsurePortMapRuleForSvc(t *testing.T) {
	...
	runner.EnsurePortMapRuleForSvc("foo", "rspscale0", ipv4, pmTCP)
	svcChains(t, 1, conn)
	...
	runner.EnsurePortMapRuleForSvc("foo", "rspscale0", ipv4, pmTCP1)
	svcChains(t, 1, conn)
	...
	runner.EnsurePortMapRuleForSvc("foo", "rspscale0", ipv6, pmTCP)
	svcChains(t, 2, conn)
	...
	runner.EnsurePortMapRuleForSvc("bar", "rspscale0", ipv4, pmTCP)
	svcChains(t, 3, conn)
	...
	runner.EnsurePortMapRuleForSvc("bar", "rspscale0", ipv6, pmTCP)
	svcChains(t, 4, conn)
	chainRuleCount(t, "bar", 1, conn, nftables.TableFamilyIPv6)
	checkPortMapRule(t, "bar", ipv6, pmTCP, runner, nftables.TableFamilyIPv6)

	// Delete service bar
	runner.DeleteSvc("bar", "rspscale0", []netip.Addr{ipv4, ipv6}, []PortMap{pmTCP})
	svcChains(t, 2, conn)

	// Delete a rule from service foo
	runner.DeletePortMapRuleForSvc("foo", "rspscale0", ipv4, pmTCP)
	svcChains(t, 2, conn)
	chainRuleCount(t, "foo", 1, conn, nftables.TableFamilyIPv4)

	// Delete service foo
	runner.DeleteSvc("foo", "rspscale0", []netip.Addr{ipv4, ipv6}, []PortMap{pmTCP, pmTCP1})
	svcChains(t, 0, conn)
}
...
func checkPortMapRule(t *testing.T, svc string, targetIP netip.Addr, pm PortMap, runner *nftablesRunner, fam nftables.TableFamily) {
	...
	wantsRule := portMapRule(chain.Table, chain, "rspscale0", targetIP, pm.MatchPort, pm.TargetPort, p, meta)
