...
func checkSupportsV6NAT(ipt *iptables.IPTables, logf logger.Logf) bool {
	...
	// https://github.com/ropsoft7/rspscale/issues/11344), but also should
...
// tsChain returns the name of the rspscale sub-chain corresponding
...
// AddHooks inserts calls to rspscale's netfilter chains in
// the relevant main netfilter chains. The rspscale chains must
// already exist. If they do not, an error is returned.
func (i *iptablesRunner) AddHooks() error {
	// divert inserts a jump to the rspscale chain in the given table/chain.
...
// AddChains creates custom Rspscale chains in netfilter via iptables
...
func (i *iptablesRunner) addBase4(tunname string) error {
	// Only allow CGNAT range traffic to come from rspscale0. There
	// is an exception carved out for ranges used by ChromeOS, for
	// which we fall out of the Rspscale chain.
	//
	...
	// Forward all traffic from the Rspscale interface, and drop
	// traffic to the rspscale interface by default. We use packet
	...
	// nat/POSTROUTING to packets that originated from the Rspscale
	...
	args = []string{"-i", tunname, "-j", "MARK", "--set-mark", RspscaleSubnetRouteMark + "/" + RspscaleFwmarkMask}
	if err := i.ipt4.Append("filter", "ts-forward", args...); err != nil {
		return fmt.Errorf("adding %v in v4/filter/ts-forward: %w", args, err)
	}
	args = []string{"-m", "mark", "--mark", RspscaleSubnetRouteMark + "/" + RspscaleFwmarkMask, "-j", "ACCEPT"}
	if err := i.ipt4.Append("filter", "ts-forward", args...); err != nil {
		return fmt.Errorf("adding %v in v4/filter/ts-forward: %w", args, err)
	}
...
func (i *iptablesRunner) DNATNonRspscaleTraffic(tun string, dst netip.Addr) error {
	table := i.getIPTByAddr(dst)
...
func (i *iptablesRunner) addBase6(tunname string) error {
	// TODO: only allow traffic from Rspscale's ULA range to come
	// from rspscale0.
	...
	// TODO: drop forwarded traffic to rspscale0 from rspscale's ULA
...
// DelChains removes the custom Rspscale chains from netfilter via iptables.
...
// DelHooks deletes the calls to rspscale's netfilter chains
...
func (i *iptablesRunner) AddSNATRule() error {
	args := []string{"-m", "mark", "--mark", RspscaleSubnetRouteMark + "/" + RspscaleFwmarkMask, "-j", "MASQUERADE"}
	for _, ipt := range i.getNATTables() {
...
func (i *iptablesRunner) AddStatefulRule(tunname string) error {
	// Drop packets that are destined for the rspscale interface if
	...
	// packets on to the Rspscale network.
	//
	...
	// Rspscale from other hosts on the same network segment; we drop
...
// IPTablesCleanUp removes all Rspscale added iptables rules.
...
// clearRules clears all the iptables rules created by Rspscale
