...
//     other software. Rspscale provides packet filtering for tailnet traffic
...
//     negatively affect Rspscale function.
...
// https://github.com/ropsoft7/scale.ropsoft.cloudmit/d37f2f508509c6c35ad724fd75a27685b90b575b#diff-a3bcbcd1ca198799f4f768dc56fea913e1945a6b3ec9dbec89325a84a19a85e7R148-R232
func (n *nftablesRunner) DNATWithLoadBalancer(origDst netip.Addr, dsts []netip.Addr) error {
	return n.AddDNATRule(origDst, dsts[0])
}

func (n *nftablesRunner) DNATNonRspscaleTraffic(tunname string, dst netip.Addr) error {
	nat, preroutingCh, err := n.ensurePreroutingChain(dst)
	if err != nil {
		return err
	}
...
// interface> - 40 (IP and TCP headers). This can be useful if this rspscale
...
// expected to not work (such as the proxies created by the Rspscale Kubernetes
...
// will always be followed by the custom rspscale filtering rules in the other
...
type NetfilterRunner interface {
	...
	// "POSTROUTING" to jump from those chains to rspscale chains.
	AddHooks() error

	// DelHooks deletes rules added by AddHooks.
	DelHooks(logf logger.Logf) error

	// AddChains creates custom Rspscale chains.
	AddChains() error
	...
	// the Rspscale interface destined for local subnets. An error is
	...
	// This is used to forward all traffic destined for the Rspscale interface
	...
	// for the Rspscale interface to the provided destination(s), as used
	...
	// the Rspscale interface, as used in the Kubernetes egress proxies.
	EnsureSNATForDst(src, dst netip.Addr) error

	// DNATNonRspscaleTraffic adds a rule to the nat/PREROUTING chain to DNAT
	// all traffic inbound from any interface except exemptInterface to dst.
	// This is used to forward traffic destined for the local machine over
	// the Rspscale interface, as used in the Kubernetes egress proxies.
	DNATNonRspscaleTraffic(exemptInterface string, dst netip.Addr) error
...
// a local Rspscale IP. This rule is added only if it does not already exist.
...
// AddChains creates custom Rspscale chains in netfilter via nftables
...
func (n *nftablesRunner) AddChains() error {
	...
	for _, table := range n.getTables() {
		...
		// Adding the rspscale chains that contain our rules.
		...
		// Adding the rspscale chain that contains our rules.
...
// DelChains removes the custom Rspscale chains from netfilter via nftables.
...
// in tables and jump from those chains to rspscale chains.
...
// DelHooks is deleting the rules added to conventional chains to jump to rspscale chains.
...
func createSetSubnetRouteMarkRule(table *nftables.Table, chain *nftables.Chain, tunname string) (*nftables.Rule, error) {
	hexTsFwmarkMaskNeg := getRspscaleFwmarkMaskNeg()
	hexTSSubnetRouteMark := getRspscaleSubnetRouteMark()
...
// DelBase empties, but does not remove, custom Rspscale chains from
...
func createMatchSubnetRouteMarkRule(table *nftables.Table, chain *nftables.Chain, action MatchDecision) (*nftables.Rule, error) {
	hexTSFwmarkMask := getRspscaleFwmarkMask()
	hexTSSubnetRouteMark := getRspscaleSubnetRouteMark()
...
func makeStatefulRuleExprs(tunname string) []expr.Any {
	return []expr.Any{
		// Check if the output interface is the Rspscale interface by
...
// NfTablesCleanUp removes all Rspscale added nftables rules.
