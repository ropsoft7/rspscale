...
// The following bits are added to packet marks for Rspscale use.
//
...
const (
	...
	RspscaleFwmarkMask    = "0xff0000"
	RspscaleFwmarkMaskNum = 0xff0000

	// Packet is from Rspscale and to a subnet route destination, so
	// is allowed to be routed through this machine.
	RspscaleSubnetRouteMark    = "0x40000"
	RspscaleSubnetRouteMarkNum = 0x40000

	// Packet was originated by rspscaled itself, and must not be
	// routed over the Rspscale network.
	RspscaleBypassMark    = "0x80000"
	RspscaleBypassMarkNum = 0x80000
)

// getRspscaleFwmarkMaskNeg returns the negation of RspscaleFwmarkMask in bytes.
func getRspscaleFwmarkMaskNeg() []byte {
	return []byte{0xff, 0x00, 0xff, 0xff}
}

// getRspscaleFwmarkMask returns the RspscaleFwmarkMask in bytes.
func getRspscaleFwmarkMask() []byte {
	return []byte{0x00, 0xff, 0x00, 0x00}
}

// getRspscaleSubnetRouteMark returns the RspscaleSubnetRouteMark in bytes.
func getRspscaleSubnetRouteMark() []byte {
...
func CheckIPRuleSupportsV6(logf logger.Logf) error {
	...
	rule.Priority = 1234
	rule.Mark = RspscaleBypassMarkNum
	rule.Table = 52
