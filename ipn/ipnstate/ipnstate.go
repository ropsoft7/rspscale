...
// Package ipnstate captures the entire state of the Rspscale network.
//
...
//go:generate go run scale.ropsoft.cloud/cmd/cloner  -clonefunc=false -type=TKAPeer
...
type Status struct {
	...
	RspscaleIPs []netip.Addr // Rspscale IP(s) assigned to this node
	Self         *PeerStatus
	...
	// version of the Rspscale client that's available. Depending on
...
type ExitNodeStatus struct {
	...
	// RspscaleIPs are the exit node's IP addresses assigned to the node.
	RspscaleIPs []netip.Prefix
}
...
type PeerStatusLite struct {
	...
	// since this peer was last known to WireGuard. (Rspscale removes peers
...
type PeerStatus struct {
	...
	// RspscaleIPs are the IP addresses assigned to the node.
	...
	// to us. These nodes should be hidden by "rspscale status"
	// etc by default.
...
// AddIP adds a Rspscale IP address to the status.
func (sb *StatusBuilder) AddRspscaleIP(ip netip.Addr) {
	if sb.locked {
		log.Printf("[unexpected] ipnstate: AddIP after Locked")
		return
	}

	sb.st.RspscaleIPs = append(sb.st.RspscaleIPs, ip)
}
...
<body>
<h1>Rspscale State</h1>
`)

	//f("<p><b>logid:</b> %s</p>\n", logid)
	//f("<p><b>opts:</b> <code>%s</code></p>\n", html.EscapeString(fmt.Sprintf("%+v", opts)))

	ips := make([]string, 0, len(st.RspscaleIPs))
	for _, ip := range st.RspscaleIPs {
		ips = append(ips, ip.String())
	}
	f("<p>Rspscale IP: %s", strings.Join(ips, ", "))
	...
	for _, ps := range peers {
		...
		var tailAddr string
		if len(ps.RspscaleIPs) > 0 {
			tailAddr = ps.RspscaleIPs[0].String()
		}
...
// PingResult contains response information for the "rspscale ping" subcommand,
// saying how Rspscale can reach a Rspscale IP or subnet-routed IP.
...
type PingResult struct {
	...
	NodeIP   string // Rspscale IP of node handling IP (different for subnet routers)
...
// SortPeers sorts peers by either their DNS name, hostname, Rspscale IP,
...
func (a *PeerStatus) compare(b *PeerStatus) int {
	...
	if len(a.RspscaleIPs) > 0 && len(b.RspscaleIPs) > 0 {
		if v := a.RspscaleIPs[0].Compare(b.RspscaleIPs[0]); v != 0 {
			return v
		}
	}
...
// DebugDERPRegionReport is the result of a "rspscale debug derp" command,
