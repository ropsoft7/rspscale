...
type ServiceConfig struct {
	// TCP are the list of TCP port numbers that rspscaled should handle for
	// the Rspscale IP addresses. (not subnet routers, etc)
...
type FunnelConn struct {
	...
	// Target is what was presented in the "Rspscale-Ingress-Target"
	// HTTP header.
	Target HostPort

	// Src is the source address of the connection.
	// This is the address of the client that initiated the
	// connection, not the address of the Rspscale Funnel
...
type TCPPortHandler struct {
	// HTTPS, if true, means that rspscaled should handle this connection as an
	// HTTPS request as configured by ServeConfig.Web.
	//
	// It is mutually exclusive with TCPForward.
	HTTPS bool `json:",omitempty"`

	// HTTP, if true, means that rspscaled should handle this connection as an
	...
	// Whether or not TLS is terminated by rspscaled depends on
	// TerminateTLS.
	//
	// It is mutually exclusive with HTTPS.
	TCPForward string `json:",omitempty"`

	// TerminateTLS, if non-empty, means that rspscaled should terminate the
...
// for Rspscale Funnel usage.
func NodeCanFunnel(node *ipnstate.PeerStatus) error {
	if !node.HasCap(tailcfg.CapabilityHTTPS) {
		return errors.New("Funnel not available; HTTPS must be enabled. See https://scale.ropsoft.cloud/s/https.")
	}
	if !node.HasCap(tailcfg.NodeAttrFunnel) {
		return errors.New("Funnel not available; \"funnel\" node attribute not set. See https://scale.ropsoft.cloud/s/no-funnel.")
	}
	return nil
}
