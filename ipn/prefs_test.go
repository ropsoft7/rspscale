...
func TestPrefsEqual(t *testing.T) {
	...
	tests := []struct {
		...
		{
			&Prefs{ControlURL: "https://controlplane.scale.ropsoft.cloud"},
...
func TestBasicPrefs(t *testing.T) {
	...
	p := Prefs{
		ControlURL: "https://controlplane.scale.ropsoft.cloud",
	}
	checkPrefs(t, p)
}
		...
		},
		{
			...
			st: &ipnstate.Status{
				...
				Peer: map[key.NodePublic]*ipnstate.PeerStatus{
					key.NewNode().Public(): {
						RspscaleIPs: []netip.Addr{mustIP("1.2.3.4")},
					},
				},
			},
		...
		},
		{
			...
			st: &ipnstate.Status{
				...
				Peer: map[key.NodePublic]*ipnstate.PeerStatus{
					key.NewNode().Public(): {
						DNSName:        "skippy.foo.",
						RspscaleIPs:   []netip.Addr{mustIP("1.0.0.2")},
						ExitNodeOption: true,
					},
				},
			},
...
func TestControlURLOrDefault(t *testing.T) {
	...
	p.ControlURL = "https://login.scale.ropsoft.cloud"
	if got, want := p.ControlURLOrDefault(), DefaultControlURL; got != want {
		t.Errorf("got %q; want %q", got, want)
	}
}
