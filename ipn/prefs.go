...
// The default control plane is the hosted version run by Rspscale.com.
const DefaultControlURL = "https://controlplane.scale.ropsoft.cloud"
...
// for the primary Rspscale login server.
func IsLoginServerSynonym(val any) bool {
	return val == "https://login.scale.ropsoft.cloud" || val == "https://controlplane.scale.ropsoft.cloud"
}

// Prefs are the user modifiable settings of the Rspscale node agent.
...
type Prefs struct {
	...
	// other nodes on the Rspscale network. Note that this does not
	...
	// linux CLI, before rspscaled has a netmap). For those
	...
	// CorpDNS specifies whether to install the Rspscale network's
	...
	// its web client over Rspscale at port 5252,
	...
	// forced Windows to go into "server mode" where Rspscale is
	...
	// Rspscale network as reachable through the current
	// node.
	...
	// machine rather than the peer's Rspscale IP.
	//
	// Disabling SNAT requires additional manual configuration in your
	// network to route Rspscale traffic back to the subnet relay
	// machine.
	//
	// Linux-only.
	NoSNAT bool
	...
	// operate rspscaled without being root or using sudo.
	...
	// for the past 4.5 years. It controlled whether Rspscale
	// peers got /32 or /127 routes for each other.
	// As of 2024-05-17 we're starting to ignore it, but to let
	// people still downgrade Rspscale versions and not break
...
type AutoUpdatePrefs struct {
	...
	// enabled, rspscaled will periodically check for available updates and
	// notify the user about them.
	Check bool
	// Apply specifies whether background auto-updates are enabled. When
	// enabled, rspscaled will apply available updates in the background.
...
func (m *MaskedPrefs) Pretty() string {
	...
	for i := 1; i < mt.NumField(); i++ {
		...
		case reflect.Struct:
			...
			// https://github.com/ropsoft7/rspscale/issues/10627#issuecomment-1861211945
			//
...
func (p *Prefs) AdminPageURL() string {
	...
	if IsLoginServerSynonym(url) {
		// TODO(crawshaw): In future release, make this https://console.scale.ropsoft.cloud
		url = "https://login.scale.ropsoft.cloud"
	}
	return url + "/admin"
}
...
// peerWithRspscaleIP returns the peer in st with the provided
// Rspscale IP.
func peerWithRspscaleIP(st *ipnstate.Status, ip netip.Addr) (ps *ipnstate.PeerStatus, ok bool) {
	for _, ps := range st.Peer {
		for _, ip2 := range ps.RspscaleIPs {
			if ip == ip2 {
				return ps, true
			}
		}
...
func isRemoteIP(st *ipnstate.Status, ip netip.Addr) bool {
	for _, selfIP := range st.RspscaleIPs {
		if ip == selfIP {
			return false
		}
	}
...
func exitNodeIPOfArg(s string, st *ipnstate.Status) (ip netip.Addr, err error) {
	...
	if err == nil {
		...
		if st.BackendState == "Running" {
			ps, ok := peerWithRspscaleIP(st, ip)
			if !ok {
	...
	for _, ps := range st.Peer {
		...
		if len(ps.RspscaleIPs) == 0 {
			return ip, fmt.Errorf("node %q has no Rspscale IP?", s)
		}
		if !ps.ExitNodeOption {
...
// "foo.bar.beta.rspscale.net"). This method does not mutate ExitNodeID and
...
func LoadPrefsWindows(filename string) (*Prefs, error) {
	...
	if bytes.Contains(data, jsonEscapedZero) {
		// Rspscale 1.2.0 - 1.2.8 on Windows had a memory corruption bug
