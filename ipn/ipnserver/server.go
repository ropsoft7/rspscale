...
import (
	...
	"scale.ropsoft.cloud/ipn/localapi"
...
// come up. See https://github.com/ropsoft7/rspscale/issues/6522.
...
func (s *Server) serveHTTP(w http.ResponseWriter, r *http.Request) {
	...
	io.WriteString(w, "<html><title>Rspscale</title><body><h1>Rspscale</h1>This is the local Rspscale daemon.\n")
}
	...
	if len(s.activeReqs) > 0 {
		...
		if active != nil {
			...
			if ci.UserID() != active.UserID() {
				...
				b.WriteString("Rspscale already in use")
...
// controlling the rspscaled process.
...
// the Rspscale local daemon API. The operatorUID is only used on
// Unix-like platforms and specifies the ID of a local user
// (in the os/user.User.Uid string form) who is allowed
// to operate rspscaled without being root or using sudo.
...
func (s *Server) SetLocalBackend(lb *ipnlocal.LocalBackend) {
	...
	// https://github.com/ropsoft7/rspscale/issues/6522
}
...
// Windows and via $DEBUG_LISTENER/debug/ipn when rspscaled's --debug flag
