...
// log.rspscale.io (or whatever the configured log server is). This
...
// instead go directly, like rspscaled's. The dialer tried to do that
// in the unprivileged GUI by binding to a specific interface, but the
// "Internet Kill Switch" installed by rspscaled for exit nodes
// precludes that from working and instead the GUI fails to dial out.
// So, go through rspscaled (with a CONNECT request) instead.
