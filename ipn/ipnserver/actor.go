...
// we progress on rspscale/corp#18342.
type actor struct {
	logf logger.Logf
...
func newActor(logf logger.Logf, c net.Conn) (*actor, error) {
	...
	if pid := ci.Pid(); pid != 0 {
		...
		// progress on rspscale/corp#18342. At minimum, we should use a 2-tuple
...
// current OS.
//
// This is useful because rspscaled itself always runs with elevated rights:
...
func connIsLocalAdmin(logf logger.Logf, ci *ipnauth.ConnIdentity, operatorUID string) bool {
	...
	case "darwin":
		...
		// This is a standalone rspscaled setup, use the same logic as on
		// Linux.
		fallthrough
	case "linux":
		...
		// if non-root, must be operator AND able to execute "sudo rspscale".
		...
		defer cancel()
		if err := exec.CommandContext(ctx, "sudo", "--other-user="+u.Name, "--list", "rspscale").Run(); err != nil {
			return false
		}
		return true
