...
// Notify is a communication from a backend (e.g. rspscaled) to a frontend
// (cmd/rspscale, iOS, macOS, Win Tasktray).
...
type Notify struct {
	...
	// the Rspscale daemon and ready for local transfer to the
	...
	// This is currently only used by Rspscale when run in the
...
// and off individually using the rspscale debug command.
