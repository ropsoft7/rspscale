...
type ConfigVAlpha struct {
	...
	Locked  opt.Bool `json:",omitempty"` // whether the config is locked from being changed by 'rspscale set'; it defaults to true

	ServerURL *string  `json:",omitempty"` // defaults to https://controlplane.scale.ropsoft.cloud
	...
	OperatorUser *string `json:",omitempty"` // local user name who is allowed to operate rspscaled without being root or using sudo
	...
	RunSSHServer    opt.Bool         `json:",omitempty"` // Rspscale SSH
