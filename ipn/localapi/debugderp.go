...
func (h *Handler) serveDebugDERPRegion(w http.ResponseWriter, r *http.Request) {
	...
	if len(dm.Regions) == 1 {
		st.Warnings = append(st.Warnings, "Having only a single DERP region (i.e. removing the default Rspscale-provided regions) is a single point of failure and could hamper connectivity")
	}
