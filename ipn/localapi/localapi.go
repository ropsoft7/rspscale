...
// Package localapi contains the HTTP server handlers for rspscaled's API server.
package localapi
...
import (
	...
	"scale.ropsoft.cloud/net/portmapper"
	...
	"scale.ropsoft.cloud/util/httphdr"
	"scale.ropsoft.cloud/util/httpm"
	"scale.ropsoft.cloud/util/mak"
	"scale.ropsoft.cloud/util/osdiag"
	"scale.ropsoft.cloud/util/progresstracking"
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Rspscale-Version", version.Long())
	w.Header().Set("Rspscale-Cap", strconv.Itoa(int(tailcfg.CurrentCapabilityVersion)))
...
func (h *Handler) validHost(hostname string) bool {
	// The client code sends a hostname of "local-rspscaled.sock".
	switch hostname {
...
func (h *Handler) serveBugReport(w http.ResponseWriter, r *http.Request) {
	...
	case errors.Is(err, io.ErrUnexpectedEOF):
		// this happens when Ctrl-C'ing the rspscale client; don't
...
// serveLogTap taps into the rspscaled/logtail server output and streams
// it to the client.
...
func authorizeServeConfigForGOOSAndUserContext(goos string, configIn *ipn.ServeConfig, h *Handler) error {
	...
	// Only check for local admin on rspscaled-on-mac (based on "sudo"
	// permissions). On sandboxed variants (MacSys and AppStore), rspscaled
	...
	case "linux", "darwin":
		return errors.New("must be root, or be an operator and able to run 'sudo rspscale' to serve a path")
	default:
...
// rspscaled, except in the case of rspscaled running in
// userspace-networking ("netstack") mode, in which case rspscaled
// needs to a do a netstack dial out.
//
// Instead, the CLI also goes through rspscaled so it doesn't need to be
...
func (h *Handler) serveSetGUIVisible(w http.ResponseWriter, r *http.Request) {
	...
	type setGUIVisibleRequest struct {
		IsVisible bool   // whether the Rspscale client UI is now presented to the user
...
// serveUpdateInstall sends a request to the LocalBackend to start a Rspscale
...
// serveUpdateProgress returns the status of an in-progress Rspscale self-update.
