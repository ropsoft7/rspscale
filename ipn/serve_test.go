...
func TestCheckFunnelAccess(t *testing.T) {
	...
	const portAttr tailcfg.NodeCapability = "https://scale.ropsoft.cloud/cap/funnel-ports?ports=443,8080-8090,8443,"
	tests := []struct {
		port    uint16
...
func TestExpandProxyTargetDev(t *testing.T) {
	tests := []struct {
		...
		{name: "not-localhost", input: "https://scale.ropsoft.cloud:8080", expected: "", wantErr: true},
