...
import (
	...
	"scale.ropsoft.cloud/kube/kubeapi"
...
const (
	...
	reasonRspscaleStateUpdated      = "RspscaledStateUpdated"
	reasonRspscaleStateLoaded       = "RspscaleStateLoaded"
	reasonRspscaleStateUpdateFailed = "RspscaleStateUpdateFailed"
	reasonRspscaleStateLoadFailed   = "RspscaleStateLoadFailed"
...
type Store struct {
	...
	// memory holds the latest rspscale state. Writes write state to a kube Secret and memory, Reads read from
	// memory.
	memory mem.Store
}
...
func New(_ logger.Logf, secretName string) (*Store, error) {
	c, err := kubeclient.New("rspscale-state-store")
	if err != nil {
		return nil, err
	}
...
func (s *Store) WriteState(id ipn.StateKey, bs []byte) (err error) {
	...
	defer func() {
		...
		if err != nil {
			if err := s.client.Event(ctx, eventTypeWarning, reasonRspscaleStateUpdateFailed, err.Error()); err != nil {
				log.Printf("kubestore: error creating rspscaled state update Event: %v", err)
			}
		} else {
			if err := s.client.Event(ctx, eventTypeNormal, reasonRspscaleStateUpdated, "Successfully updated rspscaled state Secret"); err != nil {
				log.Printf("kubestore: error creating rspscaled state Event: %v", err)
			}
		}
		cancel()
	}()
...
func (s *Store) loadState() (err error) {
	...
	if err != nil {
		...
		if err := s.client.Event(ctx, eventTypeWarning, reasonRspscaleStateLoadFailed, err.Error()); err != nil {
			log.Printf("kubestore: error creating Event: %v", err)
		}
		return err
	}
	if err := s.client.Event(ctx, eventTypeNormal, reasonRspscaleStateLoaded, "Successfully loaded rspscaled state from Secret"); err != nil {
		log.Printf("kubestore: error creating Event: %v", err)
	}
