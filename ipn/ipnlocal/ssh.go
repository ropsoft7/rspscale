...
// potential Rspscale SSH user targets.
//
...
func (b *LocalBackend) GetSSH_HostKeys() (keys []ssh.Signer, err error) {
	...
	return b.getRspscaleSSH_HostKeys(existing)
}

// getRspscaleSSH_HostKeys returns the three (rsa, ecdsa, ed25519) SSH host
// keys, reusing the provided ones in existing if present in the map.
func (b *LocalBackend) getRspscaleSSH_HostKeys(existing map[string]ssh.Signer) (keys []ssh.Signer, err error) {
	var keyDir string // lazily initialized $RSPSCALE_VAR/ssh dir.
...
// rspscaleSSHEnabled reports whether Rspscale SSH is currently enabled based
// on prefs. It returns false if there are no prefs set.
func (b *LocalBackend) rspscaleSSHEnabled() bool {
	b.mu.Lock()
	defer b.mu.Unlock()
	p := b.pm.CurrentPrefs()
