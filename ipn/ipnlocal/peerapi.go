...
func (s *peerAPIServer) listen(ip netip.Addr, ifState *netmon.State) (ln net.Listener, err error) {
	...
	// Rspscale addresses (at least currently), so we'll usually
...
// from a browser and false for requests from rspscaled (Go) clients.
//
// PeerAPI is primarily an RPC mechanism between Rspscale instances. Some of
// the HTTP handlers are useful for debugging with curl or browsers, but in
// general the client is always rspscaled itself. Because PeerAPI only uses
...
func peerAPIRequestShouldGetSecurityHeaders(r *http.Request) bool {
	...
	// it's probably a browser and not a Rspscale PeerAPI (Go) client.
	...
	// Rspscale/PeerAPI/Go clients don't have an Accept-Language.
...
<body>
<h1>Hello, %s (%v)</h1>
This is my Rspscale device. Your device is %v.
...
func (h *peerAPIHandler) handleServeIngress(w http.ResponseWriter, r *http.Request) {
	...
	srcAddrStr := r.Header.Get("Rspscale-Ingress-Src")
	if srcAddrStr == "" {
		bad("Rspscale-Ingress-Src header not set")
		return
	}
	srcAddr, err := netip.ParseAddrPort(srcAddrStr)
	if err != nil {
		bad("Rspscale-Ingress-Src header invalid; want ip:port")
		return
	}
	target := ipn.HostPort(r.Header.Get("Rspscale-Ingress-Target"))
	if target == "" {
		bad("Rspscale-Ingress-Target header not set")
		return
	}
	if _, _, err := net.SplitHostPort(string(target)); err != nil {
		bad("Rspscale-Ingress-Target header invalid; want host:port")
		return
	}
