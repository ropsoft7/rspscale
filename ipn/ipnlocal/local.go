...
// Package ipnlocal is the heart of the Rspscale node agent that controls
// all the other misc pieces of the Rspscale node.
package ipnlocal
...
import (
	...
	"scale.ropsoft.cloud/appc"
	"scale.ropsoft.cloud/client/rspscale/apitype"
	"scale.ropsoft.cloud/clientupdate"
	"scale.ropsoft.cloud/control/controlclient"
	"scale.ropsoft.cloud/control/controlknobs"
	"scale.ropsoft.cloud/doctor"
	"scale.ropsoft.cloud/doctor/ethtool"
	"scale.ropsoft.cloud/doctor/permissions"
	"scale.ropsoft.cloud/doctor/routetable"
	"scale.ropsoft.cloud/drive"
	"scale.ropsoft.cloud/envknob"
	"scale.ropsoft.cloud/envknob/featureknob"
	"scale.ropsoft.cloud/health"
	"scale.ropsoft.cloud/health/healthmsg"
	"scale.ropsoft.cloud/hostinfo"
	"scale.ropsoft.cloud/ipn"
	"scale.ropsoft.cloud/ipn/conffile"
	"scale.ropsoft.cloud/ipn/ipnauth"
	"scale.ropsoft.cloud/ipn/ipnstate"
	"scale.ropsoft.cloud/ipn/policy"
	"scale.ropsoft.cloud/log/sockstatlog"
	"scale.ropsoft.cloud/logpolicy"
	"scale.ropsoft.cloud/net/captivedetection"
	"scale.ropsoft.cloud/net/dns"
	"scale.ropsoft.cloud/net/dnscache"
	"scale.ropsoft.cloud/net/dnsfallback"
	"scale.ropsoft.cloud/net/ipset"
	"scale.ropsoft.cloud/net/netcheck"
	"scale.ropsoft.cloud/net/netkernelconf"
	"scale.ropsoft.cloud/net/netmon"
	"scale.ropsoft.cloud/net/netns"
	"scale.ropsoft.cloud/net/netutil"
	"scale.ropsoft.cloud/net/tsaddr"
	"scale.ropsoft.cloud/net/tsdial"
	"scale.ropsoft.cloud/paths"
	"scale.ropsoft.cloud/portlist"
	"scale.ropsoft.cloud/syncs"
	"scale.ropsoft.cloud/tailcfg"
	"scale.ropsoft.cloud/taildrop"
	"scale.ropsoft.cloud/tka"
	"scale.ropsoft.cloud/tsd"
	"scale.ropsoft.cloud/tstime"
	"scale.ropsoft.cloud/types/appctype"
	"scale.ropsoft.cloud/types/dnstype"
	"scale.ropsoft.cloud/types/empty"
	"scale.ropsoft.cloud/types/key"
	"scale.ropsoft.cloud/types/logger"
	"scale.ropsoft.cloud/types/logid"
	...
	"scale.ropsoft.cloud/util/httpm"
	"scale.ropsoft.cloud/util/mak"
	"scale.ropsoft.cloud/util/multierr"
	"scale.ropsoft.cloud/util/osshare"
	"scale.ropsoft.cloud/util/osuser"
	"scale.ropsoft.cloud/util/rands"
	"scale.ropsoft.cloud/util/set"
	"scale.ropsoft.cloud/util/syspolicy"
	"scale.ropsoft.cloud/util/syspolicy/rsop"
	"scale.ropsoft.cloud/util/systemd"
	"scale.ropsoft.cloud/util/testenv"
	"scale.ropsoft.cloud/util/uniq"
	...
	"scale.ropsoft.cloud/wgengine/magicsock"
	"scale.ropsoft.cloud/wgengine/router"
	"scale.ropsoft.cloud/wgengine/wgcfg"
	"scale.ropsoft.cloud/wgengine/wgcfg/nmcfg"
)
...
type SSHServer interface {
	...
	// Shutdown is called when rspscaled is shutting down.
	Shutdown()
}
...
// LocalBackend is the glue between the major pieces of the Rspscale
// network software: the cloud control plane (via controlclient), the
// network data plane (via wgengine), and the user-facing UIs and CLIs
// (collectively called "frontends", via LocalBackend's implementation
// of the Backend interface).
//
// LocalBackend implements the overall state machine for the Rspscale
...
func NewLocalBackend(logf logger.Logf, logID logid.PublicID, sys *tsd.System, loginFlags controlclient.LoginFlags) (_ *LocalBackend, err error) {
	...
	m := metrics{
		advertisedRoutes: sys.UserMetricsRegistry().NewGauge(
			"rspscaled_advertised_routes", "Number of advertised network routes (e.g. by a subnet router)"),
		approvedRoutes: sys.UserMetricsRegistry().NewGauge(
			"rspscaled_approved_routes", "Number of approved network routes (e.g. by a subnet router)"),
	}
...
func (b *LocalBackend) linkChange(delta *netmon.ChangeDelta) {
	...
	if needReconfig {
		...
		case ipn.NoState, ipn.Stopped:
			// Do nothing.
		default:
			// TODO(raggi,rspscale/corp#22574): authReconfig should be refactored such that we can call the
...
func (b *LocalBackend) UpdateStatus(sb *ipnstate.StatusBuilder) {
				...
				if !prefs.ExitNodeID().IsZero() {
					if exitPeer, ok := b.netMap.PeerWithStableID(prefs.ExitNodeID()); ok {
						...
						s.ExitNodeStatus = &ipnstate.ExitNodeStatus{
							ID:           prefs.ExitNodeID(),
							Online:       online,
							RspscaleIPs: exitPeer.Addresses().AsSlice(),
						}
					}
				}
			}
	...
	var rspscaleIPs []netip.Addr
	if b.netMap != nil {
		addrs := b.netMap.GetAddresses()
		for i := range addrs.Len() {
			if addr := addrs.At(i); addr.IsSingleIP() {
				sb.AddRspscaleIP(addr.Addr())
				rspscaleIPs = append(rspscaleIPs, addr.Addr())
			}
		}
	}
		...
		if b.netMap != nil {
			...
			if sn := b.netMap.SelfNode; sn.Valid() {
				...
				if cm := sn.CapMap(); cm.Len() > 0 {
					...
					ss.Capabilities[0] = "HTTPS://RSPSCALE.COM/s/DEPRECATED-NODE-CAPS#see-https://github.com/ropsoft7/rspscale/issues/11508"
			...
			for _, addr := range rspscaleIPs {
				ss.RspscaleIPs = append(ss.RspscaleIPs, addr)
			}
...
func (b *LocalBackend) populatePeerStatusLocked(sb *ipnstate.StatusBuilder) {
	...
	for _, p := range b.peers {
		...
		rspscaleIPs := make([]netip.Addr, 0, p.Addresses().Len())
		for i := range p.Addresses().Len() {
			addr := p.Addresses().At(i)
			if addr.IsSingleIP() && tsaddr.IsRspscaleIP(addr.Addr()) {
				rspscaleIPs = append(rspscaleIPs, addr.Addr())
			}
		}
		online := p.Online()
...
// If the IP address is a Rspscale IP, the provided port may be 0.
//
// The 'proto' is used when looking up the IP:port in our proxy mapper; it
// tracks which local IP:ports correspond to connections proxied by rspscaled,
// and since rspscaled proxies both TCP and UDP, the 'proto' is needed to look
...
func (b *LocalBackend) SetControlClientStatus(c controlclient.Client, st controlclient.Status) {
	...
	if prefs.ControlURL == "" {
		...
		// future "rspscale up" to start checking for
	...
	if st.NetMap != nil {
		if envknob.NoLogsNoSupport() && st.NetMap.HasCap(tailcfg.CapabilityDataPlaneAuditLogs) {
			msg := "tailnet requires logging to be enabled. Remove --no-logs-no-support from rspscaled command line."
...
// contains some settings that are one-shot things done by `rspscale
...
func (b *LocalBackend) updateFilterLocked(netMap *netmap.NetworkMap, prefs ipn.PrefsView) {
	...
	// Log traffic for Rspscale IPs.
	logNetsB.AddPrefix(tsaddr.CGNATRange())
	logNetsB.AddPrefix(tsaddr.RspscaleULARange())
...
func internalAndExternalInterfacesFrom(il netmon.InterfaceList, goos string) (internal, external []netip.Prefix, err error) {
	...
	if err := il.ForeachInterfaceAddress(func(iface netmon.Interface, pfx netip.Prefix) {
		if tsaddr.IsRspscaleIP(pfx.Addr()) {
			return
		}
		if pfx.IsSingleIP() {
			return
...
func (b *LocalBackend) WatchNotificationsAs(ctx context.Context, actor ipnauth.Actor, mask ipn.NotifyWatchOpt, onWatchAdded func(), fn func(roNotify *ipn.Notify) (keepGoing bool)) {
	...
	// rspscaled yet, so just do the polling here. This ends up causing all IPN
...
// has started a new interactive login (e.g., by running `rspscale login` or clicking Login in the GUI),
...
func (b *LocalBackend) validPopBrowserURL(urlStr string) bool {
	...
	if ipn.IsLoginServerSynonym(serverURL) {
		// When connected to the official Rspscale control plane, only allow
		// URLs from scale.ropsoft.cloud or its subdomains.
		if h := u.Hostname(); h != "scale.ropsoft.cloud" && !strings.HasSuffix(u.Hostname(), ".scale.ropsoft.cloud") {
			return false
		}
...
// InServerMode reports whether the Rspscale backend is explicitly running in
...
// rspscaled behavior is to shut down whenever the GUI disconnects.
//
...
// as we progress on rspscale/corp#18342.
func (b *LocalBackend) CheckIPNConnectionAllowed(actor ipnauth.Actor) error {
	...
	// even if Rspscale is currently being used by another user.
	...
	if uid != serverModeUid {
		return fmt.Errorf("Rspscale running in server mode (%q); connection from %q not allowed", b.tryLookupUserName(string(serverModeUid)), b.tryLookupUserName(string(uid)))
	}
	return nil
}
...
func (b *LocalBackend) pingPeerAPI(ctx context.Context, ip netip.Addr) (peer tailcfg.NodeView, peerBase string, err error) {
	...
	peer, ok := nm.PeerByRspscaleIP(ip)
	if !ok {
		return zero, "", fmt.Errorf("no peer found with Rspscale IP %v", ip)
	}
	if peer.Expired() {
...
func (b *LocalBackend) checkPrefsLocked(p *ipn.Prefs) error {
	if b.isConfigLocked_Locked() {
		return errors.New("can't reconfigure rspscaled when using a config file; config file is locked")
	}
	var errs []error
	if p.Hostname == "badhostname.rspscale." {
...
func (b *LocalBackend) checkSSHPrefsLocked(p *ipn.Prefs) error {
	...
	if err := featureknob.CanRunRspscaleSSH(); err != nil {
		return err
	}
	...
	if b.netMap != nil {
		if !b.netMap.HasCap(tailcfg.CapabilitySSH) {
			if b.isDefaultServerLocked() {
				return errors.New("Unable to enable local Rspscale SSH server; not enabled on Tailnet. See https://scale.ropsoft.cloud/s/ssh")
			}
			return errors.New("Unable to enable local Rspscale SSH server; not enabled on Tailnet.")
		}
	}
	return nil
}
...
func (b *LocalBackend) sshOnButUnusableHealthCheckMessageLocked() (healthMessage string) {
	...
	if !nm.HasCap(tailcfg.CapabilityAdmin) {
		return healthmsg.RspscaleSSHOnBut + "access controls don't allow anyone to access this device. Ask your admin to update your tailnet's ACLs to allow access."
	}
	if !isDefault {
		return healthmsg.RspscaleSSHOnBut + "access controls don't allow anyone to access this device. Update your tailnet's ACLs to allow access."
	}
	return healthmsg.RspscaleSSHOnBut + "access controls don't allow anyone to access this device. Update your tailnet's ACLs at https://scale.ropsoft.cloud/s/ssh-policy"
}
...
func updateExitNodeUsageWarning(p ipn.PrefsView, state *netmon.State, healthTracker *health.Tracker) {
	...
	if p.ExitNodeIP().IsValid() || p.ExitNodeID() != "" {
		...
		const comment = "please set rp_filter=2 instead of rp_filter=1; see https://github.com/ropsoft7/rspscale/issues/3310"
		if len(warn) > 0 {
...
func (b *LocalBackend) editPrefsLockedOnEntry(mp *ipn.MaskedPrefs, unlock unlockOnce) (ipn.PrefsView, error) {
	...
	if p1.RunSSH && !envknob.CanSSHD() {
		...
		return ipn.PrefsView{}, errors.New("Rspscale SSH server administratively disabled.")
	}
...
// The local parameter is the local address (either a Rspscale IPv4
...
var (
	magicDNSIP   = tsaddr.RspscaleServiceIP()
	magicDNSIPv6 = tsaddr.RspscaleServiceIPv6()
)
...
func (b *LocalBackend) reconfigAppConnectorLocked(nm *netmap.NetworkMap, prefs ipn.PrefsView) {
	const appConnectorCapName = "scale.ropsoft.cloud/app-connectors"
	defer func() {
		if b.hostinfo != nil {
...
//
// The versionOS is a Rspscale-style version ("iOS", "macOS") and not
// a runtime.GOOS.
func shouldUseOneCGNATRoute(logf logger.Logf, controlKnobs *controlknobs.Knobs, versionOS string) bool {
	...
	// Chrome, see https://github.com/ropsoft7/rspscale/issues/3102). Only
...
func dnsConfigForNetmap(nm *netmap.NetworkMap, peers map[tailcfg.NodeID]tailcfg.NodeView, prefs ipn.PrefsView, selfExpired bool, logf logger.Logf, versionOS string) *dns.Config {
	...
	// Rspscale, we don't break connectivity for the user.
	//
	...
	// magic. Details in
	// https://github.com/ropsoft7/rspscale/issues/1886.
	set := func(name string, addrs views.Slice[netip.Prefix]) {
		...
		for _, addr := range addrs.All() {
			...
			//
			// https://github.com/ropsoft7/rspscale/issues/1152
	...
	// https://github.com/ropsoft7/rspscale/issues/1743 for
	// details.
	switch {
	...
	case !prefs.ExitNodeID().IsZero():
		...
		//
		// https://github.com/ropsoft7/rspscale/issues/1713
...
// SetVarRoot sets the root directory of Rspscale's writable
// storage area . (e.g. "/var/lib/rspscale")
//
// It should only be called before the LocalBackend is used.
...
// RspscaleVarRoot returns the root directory of Rspscale's writable
// storage area. (e.g. "/var/lib/rspscale")
//
// It returns an empty string if there's no configured or discovered
// location.
func (b *LocalBackend) RspscaleVarRoot() string {
	if b.varRoot != "" {
		return b.varRoot
	}
	switch runtime.GOOS {
...
func peerRoutes(logf logger.Logf, peers []wgcfg.Peer, cgnatThreshold int) (routes []netip.Prefix) {
	...
	for _, peer := range peers {
		for _, aip := range peer.AllowedIPs {
			...
			// Only add the Rspscale IPv6 ULA once, if we see anybody using part of it.
...
func (b *LocalBackend) routerConfig(cfg *wgcfg.Config, prefs ipn.PrefsView, oneCGNATRoute bool) *router.Config {
	...
	if slices.ContainsFunc(rs.LocalAddrs, tsaddr.PrefixIs4) {
		rs.Routes = append(rs.Routes, netip.PrefixFrom(tsaddr.RspscaleServiceIP(), 32))
	}
	if slices.ContainsFunc(rs.LocalAddrs, tsaddr.PrefixIs6) {
		rs.Routes = append(rs.Routes, netip.PrefixFrom(tsaddr.RspscaleServiceIPv6(), 128))
	}
...
func (b *LocalBackend) applyPrefsToHostinfoLocked(hi *tailcfg.Hostinfo, prefs ipn.PrefsView) {
	...
	// true; the actual policy is controlled in rspscaled by ServeConfig. But
...
func (b *LocalBackend) enterStateLockedOnEntry(newState ipn.State, unlock unlockOnce) {
	...
	// https://github.com/ropsoft7/rspscale/issues/11962 in the act.
	if prefs.WantRunning() &&
	...
	case ipn.Stopped:
		...
		if authURL == "" {
			systemd.Status("Stopped; run 'rspscale up' to log in")
		}
...
// traditional (simple). See https://github.com/ropsoft7/rspscale/issues/11649
...
// https://github.com/ropsoft7/rspscale/issues/11649 for background.
//
...
func (b *LocalBackend) resetControlClientLocked() controlclient.Client {
	...
	//
	// See https://github.com/ropsoft7/rspscale/issues/7392
	if b.nmExpiryTimer != nil {
		b.nmExpiryTimer.Stop()
		b.nmExpiryTimer = nil
...
// connect to rspscaled next. But it does not trigger a logout; we
...
// within this rspscaled instance. ShouldRunWebClient is safe to
...
// accept connections via [rspscale IP]:5252 in addition to the default
...
// Rspscale ULA's v6 "via" range embedding an IPv4 address to be forwarded to
// by Rspscale.
...
// the ports that rspscaled should handle as a function of b.netMap and b.prefs.
//
// b.mu must be held.
...
func (b *LocalBackend) FileTargets() ([]*apitype.FileTarget, error) {
	...
	if !b.capFileSharing {
		return nil, errors.New("file sharing not enabled by Rspscale admin")
	}
	for _, p := range b.peers {
...
// forward UDP packets between the default route and Rspscale TUN interfaces.
...
// acting as Rspscale subnet routers or exit nodes. Currently (9/5/2024) this
...
// https://scale.ropsoft.cloud/kb/1320/performance-best-practices#linux-optimizations-for-subnet-routers-and-exit-nodes
func (b *LocalBackend) SetUDPGROForwarding() error {
	...
	netmonSys, ok := b.sys.NetMon.GetOK()
	if !ok {
		return errors.New("[unexpected] unable to retrieve rspscale netmon configuration")
	}
	state := netmonSys.InterfaceState()
	if state == nil {
...
func peerCanProxyDNS(p tailcfg.NodeView) bool {
	if p.Cap() >= 26 {
		...
		// (https://github.com/ropsoft7/rspscale/blob/3ae6f898cfdb58fd0e30937147dd6ce28c6808dd/tailcfg/tailcfg.go#L51)
...
func (n keyProvingNoiseRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	// genNodeProofHeaderValue returns the Rspscale-Node-Proof header's value to prove
	...
	req.Header.Set("Rspscale-Node-Proof", genNodeProofHeaderValue(priv, ep.NodeKeyChallenge))
...
var warnSSHSELinuxWarnable = health.Register(&health.Warnable{
	...
	Title:    "Rspscale SSH and SELinux",
	Severity: health.SeverityLow,
	Text:     health.StaticMessage("SELinux is enabled; Rspscale SSH may not work. See https://scale.ropsoft.cloud/s/ssh-selinux"),
})
...
// the equivalent tsaddr.RspscaleServiceIPv6 address).
...
func validQuad100Host(h string) bool {
	switch h {
	case "",
		tsaddr.RspscaleServiceIPString,
		tsaddr.RspscaleServiceIPv6String,
		"[" + tsaddr.RspscaleServiceIPv6String + "]":
		return true
	}
	return false
...
func (b *LocalBackend) handleQuad100Port80Conn(w http.ResponseWriter, r *http.Request) {
	...
	io.WriteString(w, "<h1>Rspscale</h1>\n")
	if b.netMap == nil {
...
func (b *LocalBackend) Doctor(ctx context.Context, logf logger.Logf) {
	...
	// Print a log message if any of the global DNS resolvers are Rspscale
	// IPs; this can interfere with our ability to connect to the Rspscale
	// controlplane.
	checks = append(checks, doctor.CheckFunc("dns-resolvers", func(_ context.Context, logf logger.Logf) error {
		...
		for i, resolver := range nm.DNS.Resolvers {
			...
			if ok && tsaddr.IsRspscaleIP(ipp.Addr()) {
				logf("resolver %d is a Rspscale address: %v", i, resolver)
			}
		}
		for i, resolver := range nm.DNS.FallbackResolvers {
			ipp, ok := resolver.IPPort()
			if ok && tsaddr.IsRspscaleIP(ipp.Addr()) {
				logf("fallback resolver %d is a Rspscale address: %v", i, resolver)
			}
		}
		return nil
	}))
...
// Rspscale IP (not a subnet router, service IP, etc) should be intercepted by
// Rspscaled and handled in-process.
...
// rspscaled to the provided response writer.
...
func (b *LocalBackend) GetPeerEndpointChanges(ctx context.Context, ip netip.Addr) ([]magicsock.EndpointChange, error) {
	...
	if pip.IsSelf {
		return nil, fmt.Errorf("%v is local Rspscale IP", ip)
	}
	peer := pip.Node
...
func (b *LocalBackend) DoSelfUpdate() {
	...
	if err != nil {
		...
		b.pushSelfUpdateProgress(ipnstate.NewUpdateProgress(ipnstate.UpdateFinished, "rspscaled did not restart; please restart Rspscale manually."))
	}
}
...
func allowedAutoRoute(ipp netip.Prefix) bool {
	...
	// TODO(raggi): exclude rspscale service IPs and so on as well.
	return true
}
...
func (b *LocalBackend) startAutoUpdate(logPrefix string) (retErr error) {
	...
	cmdTS, err := findCmdRspscale()
	if err != nil {
		return fmt.Errorf("failed to find cmd/rspscale binary: %w", err)
	}
	var ver struct {
	...
	if err := json.Unmarshal(out, &ver); err != nil {
		return fmt.Errorf("invalid JSON from cmd/rspscale version --json: %w", err)
	}
	if ver.Long != version.Long() {
		return fmt.Errorf("cmd/rspscale version %q does not match rspscaled version %q", ver.Long, version.Long())
	}

	cmd := rspscaleUpdateCmd(cmdTS)
	buf := new(bytes.Buffer)
	cmd.Stdout = buf
	cmd.Stderr = buf
	b.logf("%s: running %q", logPrefix, strings.Join(cmd.Args, " "))
	if err := cmd.Start(); err != nil {
		return fmt.Errorf("failed to start cmd/rspscale update: %w", err)
	}
