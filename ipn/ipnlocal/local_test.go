...
import (
	...
	"scale.ropsoft.cloud/appc/appctest"
	...
	"scale.ropsoft.cloud/util/syspolicy/source"
	...
	tests := []struct {
		...
		},
		{
			...
			out: []string{
				"fe80::1",
				"ff00::1",
				tsaddr.RspscaleULARange().Addr().String(),
			},
...
func TestFileTargets(t *testing.T) {
	...
	b.state = ipn.Running
	_, err = b.FileTargets()
	if got, want := fmt.Sprint(err), "file sharing not enabled by Rspscale admin"; got != want {
...
// legacyBackend was the interface between Rspscale frontends
// (e.g. cmd/rspscale, iOS/MacOS/Windows GUIs) and the rspscale
// backend (e.g. cmd/rspscaled) running on the same machine.
...
func TestDNSConfigForNetmapForExitNodeConfigs(t *testing.T) {
	...
	peers := []tailcfg.NodeView{
		...
		// regular rspscale exit node with DNS capabilities
		(&tailcfg.Node{
			Cap:      26,
			ID:       2,
			StableID: "ts",
...
func TestReconfigureAppConnector(t *testing.T) {
	...
	b.netMap.SelfNode = (&tailcfg.Node{
		...
		CapMap: (tailcfg.NodeCapMap)(map[tailcfg.NodeCapability][]tailcfg.RawMessage{
			"scale.ropsoft.cloud/app-connectors": {tailcfg.RawMessage(appCfg)},
		}),
	}).View()
	...
	tests := []struct {
		...
		},
		{
			...
			stringPolicies: map[syspolicy.Key]string{
				...
				syspolicy.EnableRspscaleDNS:        "always",
				syspolicy.EnableRspscaleSubnets:    "always",
			},
		},
		{
			...
			stringPolicies: map[syspolicy.Key]string{
				...
				syspolicy.EnableRspscaleDNS:        "never",
				syspolicy.EnableRspscaleSubnets:    "never",
			},
		},
		{
		...
		},
		{
			...
			stringPolicies: map[syspolicy.Key]string{
				...
				syspolicy.EnableRspscaleDNS:        "user-decides",
				syspolicy.EnableRspscaleSubnets:    "user-decides",
			},
		},
		{
			name: "ControlURL",
...
func TestValidPopBrowserURL(t *testing.T) {
	...
	tests := []struct {
		...
		{"saas_login", "https://login.scale.ropsoft.cloud", "https://login.scale.ropsoft.cloud/a/foo", true},
		{"saas_controlplane", "https://controlplane.scale.ropsoft.cloud", "https://controlplane.scale.ropsoft.cloud/a/foo", true},
		{"saas_root", "https://login.scale.ropsoft.cloud", "https://scale.ropsoft.cloud/", true},
		{"saas_bad_hostname", "https://login.scale.ropsoft.cloud", "https://example.com/a/foo", false},
...
func TestUpdatePrefsOnSysPolicyChange(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			stringSettings: []source.TestSetting[string]{
				source.TestSettingOf(syspolicy.EnableRspscaleDNS, "always"),
