...
func (pm *profileManager) loadSavedPrefs(key ipn.StateKey) (ipn.PrefsView, error) {
	...
	// Ignore any old stored preferences for https://login.scale.ropsoft.cloud
	// as the control server that would override the new default of
	// controlplane.scale.ropsoft.cloud.
	...
	// Before
	// https://github.com/ropsoft7/rspscale/pull/11814/commits/1613b18f8280c2bce786980532d012c9f0454fa2#diff-314ba0d799f70c8998940903efb541e511f352b39a9eeeae8d475c921d66c2ac
...
// caller's identity in rspscale/corp#18342.
...
func readAutoStartKey(store ipn.StateStore, goos string) (ipn.StateKey, error) {
	...
	if goos == "windows" {
		// When rspscaled runs on Windows it is not typically run unattended.
