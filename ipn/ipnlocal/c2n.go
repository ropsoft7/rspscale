...
func handleC2NPostureIdentityGet(b *LocalBackend, w http.ResponseWriter, r *http.Request) {
	...
	if choice.ShouldEnable(b.Prefs().PostureChecking()) {
		...
		// TODO(rspscale/corp#21371, 2024-07-10): once this has landed in a stable release
...
// findCmdRspscale looks for the cmd/rspscale that corresponds to the
// currently running cmd/rspscaled. It's up to the caller to verify that the
// two match, but this function does its best to find the right one. Notably, it
// doesn't use $PATH for security reasons.
func findCmdRspscale() (string, error) {
	...
	switch runtime.GOOS {
	case "linux":
		if self == "/usr/sbin/rspscaled" || self == "/usr/bin/rspscaled" {
			ts = "/usr/bin/rspscale"
		}
		if self == "/usr/local/sbin/rspscaled" || self == "/usr/local/bin/rspscaled" {
			ts = "/usr/local/bin/rspscale"
		}
		switch distro.Get() {
		case distro.QNAP:
			// The volume under /share/ where qpkg are installed is not
			// predictable. But the rest of the path is.
			ok, err := filepath.Match("/share/*/.qpkg/ropsoft7/rspscaled", self)
			if err == nil && ok {
				ts = filepath.Join(filepath.Dir(self), "rspscale")
			}
		case distro.Unraid:
			if self == "/usr/local/emhttp/plugins/rspscale/bin/rspscaled" {
				ts = "/usr/local/emhttp/plugins/rspscale/bin/rspscale"
			}
		}
	case "windows":
		ts = filepath.Join(filepath.Dir(self), "rspscale.exe")
	case "freebsd":
		if self == "/usr/local/bin/rspscaled" {
			ts = "/usr/local/bin/rspscale"
		}
	default:
	...
	return "", errors.New("rspscale executable not found in expected place")
}

func rspscaleUpdateCmd(cmdTS string) *exec.Cmd {
	...
	// creates a new temporary unit separate from the rspscaled unit. When
	// rspscaled is restarted during the update, systemd won't kill this
