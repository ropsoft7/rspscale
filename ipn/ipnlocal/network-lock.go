...
func (b *LocalBackend) chonkPathLocked() string {
	return filepath.Join(b.RspscaleVarRoot(), "tka-profiles", string(b.pm.CurrentProfile().ID))
}
...
// CanSupportNetworkLock returns nil if rspscaled is able to operate
...
func tkaStateFromPeer(p tailcfg.NodeView) ipnstate.TKAPeer {
	...
	for _, addr := range p.Addresses().All() {
		if addr.IsSingleIP() && tsaddr.IsRspscaleIP(addr.Addr()) {
			fp.RspscaleIPs = append(fp.RspscaleIPs, addr.Addr())
		}
	}
...
func (b *LocalBackend) NetworkLockInit(keys []tka.Key, disablementValues [][]byte, supportDisablement []byte) error {
	...
	if ourNodeKey.IsZero() || nlPriv.IsZero() {
		return errors.New("no node-key: is rspscale logged in?")
	}
