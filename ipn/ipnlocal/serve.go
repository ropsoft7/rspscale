...
import (
	...
	"scale.ropsoft.cloud/util/ctxkey"
...
// localListener is the state of host-level net.Listen for a specific (Rspscale IP, port)
// combination. If there are two RspscaleIPs (v4 and v6) and three ports being served,
// then there will be six of these active and looping in their Run method.
//
// This is not used in userspace-networking mode.
//
// localListener is used by rspscale serve (TCP only), the built-in web client and Taildrive.
	...
	for {
		...
		if initListenConfig != nil {
			...
			if version.IsSandboxedMacOS() && s.ap.Port() < 1024 {
				...
				// Rspscale interface, but macOS errors out if we try to
...
// handle local "rspscale serve" and web client traffic originating from the
// machine itself.
...
func (rp *reverseProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	p := &httputil.ReverseProxy{Rewrite: func(r *httputil.ProxyRequest) {
		...
		rp.lb.addRspscaleIdentityHeaders(r)
	}}
...
func (b *LocalBackend) addRspscaleIdentityHeaders(r *httputil.ProxyRequest) {
	// Clear any incoming values squatting in the headers.
	r.Out.Header.Del("Rspscale-User-Login")
	r.Out.Header.Del("Rspscale-User-Name")
	r.Out.Header.Del("Rspscale-User-Profile-Pic")
	r.Out.Header.Del("Rspscale-Funnel-Request")
	r.Out.Header.Del("Rspscale-Headers-Info")
	...
	if c.Funnel != nil {
		r.Out.Header.Set("Rspscale-Funnel-Request", "?1")
		return
	}
	...
	r.Out.Header.Set("Rspscale-User-Login", encRspscaleHeaderValue(user.LoginName))
	r.Out.Header.Set("Rspscale-User-Name", encRspscaleHeaderValue(user.DisplayName))
	r.Out.Header.Set("Rspscale-User-Profile-Pic", user.ProfilePicURL)
	r.Out.Header.Set("Rspscale-Headers-Info", "https://scale.ropsoft.cloud/s/serve-headers")
}

// encRspscaleHeaderValue cleans or encodes as necessary v, to be suitable in
// an HTTP header value. See
// https://github.com/ropsoft7/rspscale/issues/11603.
//
...
func encRspscaleHeaderValue(v string) string {
