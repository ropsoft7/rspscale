...
type RecorderSpec struct {
	...
	// Tags that the Rspscale device will be tagged with. Defaults to [tag:k8s].
	...
	// Corresponds to --ui tsrecorder flag https://scale.ropsoft.cloud/kb/1246/rspscale-ssh-session-recording#deploy-a-recorder-node.
...
type RecorderPod struct {
	...
	// Configuration for the Recorder container running rspscale.
	// +optional
...
type RecorderContainer struct {
	...
	// Container image name including tag. Defaults to docker.io/rspscale/tsrecorder
	// with the same tag as the operator, but the official images are also
	// available at ghcr.io/rspscale/tsrecorder.
