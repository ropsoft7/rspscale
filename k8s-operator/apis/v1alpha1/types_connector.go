...
type Connector struct {
	...
	// ConnectorSpec describes the desired Rspscale component.
	// More info:
	// https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
	Spec ConnectorSpec `json:"spec"`

	// ConnectorStatus describes the status of the Connector. This is set
	// and managed by the Rspscale operator.
	// +optional
...
type ConnectorSpec struct {
	// Tags that the Rspscale node will be tagged with.
	// Defaults to [tag:k8s].
	// To autoapprove the subnet routes or exit node defined by a Connector,
	// you can configure Rspscale ACLs to give these tags the necessary
	// permissions.
	// See https://scale.ropsoft.cloud/kb/1337/acl-syntax#autoapprovers.
	// If you specify custom tags here, you must also make the operator an owner of these tags.
	// See  https://scale.ropsoft.cloud/kb/1236/kubernetes-operator/#setting-up-the-kubernetes-operator.
	...
	// expose to tailnet as a Rspscale subnet router.
	// https://scale.ropsoft.cloud/kb/1019/subnets/
	// If this field is unset, the device does not get configured as a Rspscale subnet router.
	// This field is mutually exclusive with the appConnector field.
	// +optional
	SubnetRouter *SubnetRouter `json:"subnetRouter,omitempty"`
	// AppConnector defines whether the Connector device should act as a Rspscale app connector. A Connector that is
	...
	// https://scale.ropsoft.cloud/kb/1281/app-connectors
	// +optional
	AppConnector *AppConnector `json:"appConnector,omitempty"`
	// ExitNode defines whether the Connector device should act as a Rspscale exit node. Defaults to false.
	// This field is mutually exclusive with the appConnector field.
	// https://scale.ropsoft.cloud/kb/1103/exit-nodes
	// +optional
...
type SubnetRouter struct {
	...
	// or IPv6 CIDR range. Values can be Rspscale 4via6 subnet routes.
	// https://scale.ropsoft.cloud/kb/1201/4via6-subnets/
...
type AppConnector struct {
	...
	// https://scale.ropsoft.cloud/kb/1332/apps-best-practices#preconfiguration
	// +optional
...
const (
	...
	ProxyReady      ConditionType = `RspscaleProxyReady` // a Rspscale-specific condition type for corev1.Service
	...
	EgressSvcValid ConditionType = `RspscaleEgressSvcValid`
	...
	EgressSvcConfigured ConditionType = `RspscaleEgressSvcConfigured`
	...
	EgressSvcReady ConditionType = `RspscaleEgressSvcReady`
)
