...
type DNSConfigSpec struct {
	...
	// associated with in-cluster proxies for Rspscale egress Services and
	// Rspscale Ingresses. The operator will always deploy this nameserver
	// when a DNSConfig is applied.
	Nameserver *Nameserver `json:"nameserver"`
}

type Nameserver struct {
	// Nameserver image. Defaults to rspscale/k8s-nameserver:unstable.
	// +optional
	Image *NameserverImage `json:"image,omitempty"`
}

type NameserverImage struct {
	// Repo defaults to rspscale/k8s-nameserver.
	// +optional
